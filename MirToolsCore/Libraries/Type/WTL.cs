        ...
        public void ToMLibrary()
        {
            ...
            try
            {
                Parallel.For(0, Images.Length, options, i =>
                {
                    ...
                    if (image.HasMask)
                        library.Images[i] = new Crystal_2.MImage(image.Image, image.MaskImage) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.Shadow, MaskX = image.MaskX, MaskY = image.MaskY };
                    else
                        library.Images[i] = new Crystal_2.MImage(image.Image) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.Shadow };
                });
            }
