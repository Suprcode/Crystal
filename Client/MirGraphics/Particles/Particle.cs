...
namespace Client.MirGraphics.Particles {
    ...
    public class Particle {
        ...
        public void Draw() {
            ...
            if(Blend) {
                ImageInfo.Library.DrawBlend(ImageInfo.BaseIndex + ImageInfo.CurrentFrame, new Point(drawx, drawy),
                    Color, true, BlendRate);
            } else {
                ImageInfo.Library.Draw(ImageInfo.BaseIndex + ImageInfo.CurrentFrame, new Point(drawx, drawy), Color,
                    true, BlendRate);
            }
        }

        public void ProcessImage() {
            if(CMain.Time <= ImageInfo.NextFrame) {
                return;
            }
            ...
            if(++ImageInfo.CurrentFrame >= ImageInfo.Count) {
                ...
                ImageInfo.NextFrame =
                    ImageInfo.Start + (ImageInfo.Duration / ImageInfo.Count * (ImageInfo.CurrentFrame + 1));
            } else {
                ImageInfo.NextFrame =
                    ImageInfo.Start + (ImageInfo.Duration / ImageInfo.Count * (ImageInfo.CurrentFrame + 1));
            }
        ...
        protected virtual void OnPositionChanged() {
            try {
                ...
                int xwidth = (int)(ImageInfo.Size.Width *
                                   (Math.Ceiling(Settings.ScreenWidth / (decimal)ImageInfo.Size.Width) + 2));
                int ywidth = (int)(ImageInfo.Size.Height *
                                   (Math.Ceiling(Settings.ScreenHeight / (decimal)ImageInfo.Size.Height) + 2));
                Vector2 xreset = new(xwidth, 0);
                Vector2 yreset = new(0, ywidth);
