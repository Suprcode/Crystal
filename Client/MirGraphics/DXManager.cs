...
namespace Client.MirGraphics {
    internal class DXManager {
        public static List<MImage> TextureList = new();
        public static List<MirControl> ControlList = new();
        ...
        public static List<Texture> Lights = new();
        ...
        public static void Create() {
            ...
            Device = new Device(d3d, d3d.Adapters.DefaultAdapter.Adapter, devType, Program.Form.Handle, devFlags,
                Parameters);
        ...
        private static unsafe void LoadPixelsShaders() {
            string shaderNormalPath = Settings.ShadersPath + "normal.ps";
            string shaderGrayScalePath = Settings.ShadersPath + "grayscale.ps";
            string shaderMagicPath = Settings.ShadersPath + "magic.ps";

            if(File.Exists(shaderNormalPath)) {
                using(ShaderBytecode gs = ShaderBytecode.AssembleFromFile(shaderNormalPath, ShaderFlags.None)) {
                    NormalPixelShader = new PixelShader(Device, gs);
                }
            }

            if(File.Exists(shaderGrayScalePath)) {
                using(ShaderBytecode gs = ShaderBytecode.AssembleFromFile(shaderGrayScalePath, ShaderFlags.None)) {
                    GrayScalePixelShader = new PixelShader(Device, gs);
                }
            }

            if(File.Exists(shaderMagicPath)) {
                using(ShaderBytecode gs = ShaderBytecode.AssembleFromFile(shaderMagicPath, ShaderFlags.None)) {
        ...
        private static unsafe void LoadTextures() {
            ...
            if(RadarTexture == null || RadarTexture.Disposed) {
                ...
                using(Bitmap image = new(2, 2, 8, PixelFormat.Format32bppArgb, stream.Data.DataPointer))
            ...
            if(PoisonDotBackground == null || PoisonDotBackground.Disposed) {
                ...
                using(Bitmap image = new(5, 5, 20, PixelFormat.Format32bppArgb, stream.Data.DataPointer))
                using(Graphics graphics = Graphics.FromImage(image)) {
                    graphics.Clear(Color.White);
                }
        ...
        private static unsafe void CreateLights() {
            ...
            for (int i = 1; i < LightSizes.Length; i++) {
                ...
                Texture light = new(Device, width, height, 1, Usage.None, Format.A8R8G8B8, Pool.Managed);

                DataRectangle stream = light.LockRectangle(0, LockFlags.Discard);
                using(Bitmap image = new(width, height, width * 4, PixelFormat.Format32bppArgb,
        ...
        public static void DrawOpaque(Texture texture, Rectangle? sourceRect, Vector3? position, Color4 color,
                                      float opacity) {
            color.Alpha = opacity;
        ...
        public static void SetOpacity(float opacity) {
            ...
            if(opacity >= 1 || opacity < 0) {
                ...
                Device.SetRenderState(RenderState.BlendFactor,
                    Color.FromArgb((byte)(255 * opacity), (byte)(255 * opacity), (byte)(255 * opacity),
        ...
        public static void SetBlend(bool value, float rate = 1F, BlendMode mode = BlendMode.NORMAL) {
            ...
            if(Blending) {
                ...
                Device.SetRenderState(RenderState.BlendFactor, Color.FromArgb((byte)(255 * BlendingRate),
        ...
        public static void SetNormal(float blend, Color tintcolor) {
            ...
            Device.SetPixelShaderConstant(0, new Vector4[] { new(1.0F, 1.0F, 1.0F, blend) });
            Device.SetPixelShaderConstant(1,
                new Vector4[] { new(tintcolor.R / 255, tintcolor.G / 255, tintcolor.B / 255, 1.0F) });
            Sprite.Flush();
        }
