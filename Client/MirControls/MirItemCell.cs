...
namespace Client.MirControls
{
    public sealed class MirItemCell : MirImageControl
    {
                ...
                case MouseButtons.Right:
                    ...
                    if (CMain.Shift)
                    {
                        if (Item != null)
                        {
                            ...
                            {
                                GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.UnableLinkItemMessageTooLong)], ChatType.System);
                                return;
                            }
                    ...
                    if (CMain.Shift)
                    {
                        if (GridType == MirGridType.Inventory || GridType == MirGridType.Storage)
                        {
                            if (GameScene.SelectedCell == null && Item != null)
                            {
                                if (FreeSpace() == 0)
                                {
                                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.NoRoomToSplitStack)], ChatType.System);
                                    return;
                                }

                                if (Item.Count > 1)
                                {
                                    MirAmountBox amountBox = new MirAmountBox(GameLanguage.ClientTextMap[nameof(ClientTextKeys.SplitAmount)], Item.Image, (uint)(Item.Count - 1));
        ...
        private void BuyItem()
        {
            ...
            if (Item.Count > 1)
            {
                amountBox = new MirAmountBox(GameLanguage.ClientTextMap[nameof(ClientTextKeys.PurchaseAmount)], Item.Image, Item.Count);
            ...
            {
                amountBox = new MirAmountBox(GameLanguage.ClientTextMap[nameof(ClientTextKeys.Purchase)], Item.Image, string.Format(GameLanguage.ClientTextMap[nameof(ClientTextKeys.ValueGold)], Item.Price()));
            ...
            switch (Item.Info.Type)
            {
                ...
                case ItemType.SealedHero:
                    if (CanUseItem() && (GridType == MirGridType.Inventory || GridType == MirGridType.HeroInventory))
                    {
                        ...
                        {
                            MirMessageBox messageBox = new MirMessageBox(GameLanguage.ClientTextMap[nameof(ClientTextKeys.AreYouWantUsePotion)], MirMessageBoxButtons.YesNo);
                ...
                switch (GridType)
                {
                        ...
                        switch (GameScene.SelectedCell.GridType)
                        {
                            ...
                            case MirGridType.Inventory: //From Invenotry
                                if (Item != null)
                                {
                                    if (CMain.Ctrl)
                                    {
                                        MirMessageBox messageBox = new MirMessageBox(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouWantCombineItems)], MirMessageBoxButtons.YesNo);
                                        messageBox.YesButton.Click += (o, e) =>
                                        {
                                            //Combine
                            ...
                            case MirGridType.GuildStorage:
                                if (Item != null)
                                {
                                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouCannotSwapItems)], ChatType.System);
                                    return;
                                }
                                if (!GuildDialog.MyOptions.HasFlag(GuildRankOptions.CanRetrieveItem))
                                {
                                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.InsufficientRightsRetrieveItems)], ChatType.System);
                                    return;
                                }
                            ...
                            case MirGridType.Renting:
                                ...
                                {
                                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.UnableRemoveLockedItem)], ChatType.System);
                                    GameScene.SelectedCell = null;
                                    return;
                                }
                        ...
                        switch (GameScene.SelectedCell.GridType)
                        {
                            case MirGridType.GuildStorage: //From Guild Storage
                                if (GameScene.SelectedCell.GridType == MirGridType.GuildStorage)
                                {
                                    ...
                                    {
                                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.InsufficientRightsStoreItems)], ChatType.System);
                                        return;
                                    }
                    ...
                    case MirGridType.Mail: //To Mail
                        if (GameScene.SelectedCell.GridType == MirGridType.Inventory)
                        {
                            ...
                            {
                                GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouCannotMailItem)], ChatType.System);
                                return;
                            }
                    ...
                    case MirGridType.HeroInventory:
                        ...
                        switch (GameScene.SelectedCell.GridType)
                        {
                            ...
                            case MirGridType.Inventory:
                                ...
                                {
                                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.TooHeavyToTransfer)], ChatType.System);
                                    GameScene.SelectedCell = null;
                                    return;
                                }
        ...
        private bool CanUseItem()
        {
            ...
            switch (actor.Gender)
            {
                case MirGender.Male:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.NotFemale)], ChatType.System);
                        return false;
                    }
                    break;
                case MirGender.Female:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.NotMale)], ChatType.System);
                        return false;
                    }
                    break;
            }
            ...
            switch (actor.Class)
            {
                case MirClass.Warrior:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.WarriorsCannotUseItem)], ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Wizard:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.WizardsCannotUseItem)], ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Taoist:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.TaoistsCannotUseItem)], ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Assassin:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.AssassinsCannotUseItem)], ChatType.System);
                        return false;
                    }
                    break;
                case MirClass.Archer:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.ArchersCannotUseItem)], ChatType.System);
                        return false;
                    }
                    break;
            }
            ...
            switch (Item.Info.RequiredType)
            {
                case RequiredType.Level:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.LowLevel)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxAC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.LowAC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxMAC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.LowMAC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxDC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.LowDC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxMC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.LowMC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxSC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.LowSC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MaxLevel:
                    if (actor.Level > Item.Info.RequiredAmount)
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouExceededMaxLevel)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinAC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouDoNotHaveEnoughBaseAC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinMAC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouDoNotHaveEnoughBaseMac)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinDC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouLackBaseDC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinMC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouLackBaseMC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinSC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouLackBaseSC)], ChatType.System);
                        return false;
                    }
                    break;
            }
            ...
            switch (Item.Info.Type)
            {
                ...
                case ItemType.Reins:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouDoNotHaveMountEquipped)], ChatType.System);
                        return false;
                    }
                    break;
                case ItemType.Hook:
                ...
                case ItemType.Reel:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouDoNotHaveFishingRodEquipped)], ChatType.System);
                        return false;
                    }
                    break;
            }
        ...
        private bool CanWearItem(UserObject actor, UserItem i)
        {
            ...
            switch (i.Info.RequiredType)
            {
                ...
                case RequiredType.MinDC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouNotEnoughBaseDC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinMC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouNotEnoughBaseMC)], ChatType.System);
                        return false;
                    }
                    break;
                case RequiredType.MinSC:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.YouDoNotHaveEnoughBaseSC)], ChatType.System);
                        return false;
                    }
                    break;
            }
            ...
            if (i.Info.Type == ItemType.Weapon || i.Info.Type == ItemType.Torch)
            {
                ...
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.TooHeavyToHold)], ChatType.System);
                    return false;
                }
            }
            else
            {
                ...
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.ClientTextMap[nameof(ClientTextKeys.ItIsTooHeavyToWear)], ChatType.System);
                    return false;
                }
            }
