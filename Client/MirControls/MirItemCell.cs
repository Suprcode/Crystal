...
namespace Client.MirControls {
    public sealed class MirItemCell : MirImageControl {
        ...
        public override bool Border => (GameScene.SelectedCell == this || MouseControl == this || Locked) &&
                                       !(GridType == MirGridType.DropPanel || GridType == MirGridType.Craft);
        ...
        public override void OnMouseClick(MouseEventArgs e) {
            ...
            if(GameScene.SelectedCell == null && GridType == MirGridType.Mail) {
                return;
            }
            ...
            switch (e.Button) {
                case MouseButtons.Right:
                    ...
                    if(CMain.Shift) {
                        if(Item != null) {
                            ...
                            if(GameScene.Scene.ChatDialog.ChatTextBox.Text.Length + text.Length >
                               Globals.MaxChatLength) {
                                GameScene.Scene.ChatDialog.ReceiveChat(
                                    "Unable to link item, message exceeds allowed length", ChatType.System);
                                return;
                            }

                            GameScene.Scene.ChatDialog.LinkedItems.Add(new ChatItem
                                { UniqueID = Item.UniqueID, Title = Item.FriendlyName, Grid = GridType });
                ...
                case MouseButtons.Left:
                    ...
                    if(CMain.Shift) {
                        if(GridType == MirGridType.Inventory || GridType == MirGridType.Storage) {
                            if(GameScene.SelectedCell == null && Item != null) {
                                ...
                                if(Item.Count > 1) {
                                    MirAmountBox amountBox = new("Split Amount:", Item.Image, (uint)(Item.Count - 1));

                                    amountBox.OKButton.Click += (o, a) => {
                                        if(amountBox.Amount == 0 || amountBox.Amount >= Item.Count) {
                                            return;
                                        }

                                        Network.Enqueue(new ClientPacket.SplitItem {
                                            Grid = GridType, UniqueID = Item.UniqueID, Count = (ushort)amountBox.Amount
                                        });
                                        Locked = true;
                                    };
                    ...
                    else if(CMain.Alt && GameScene.Scene.NpcDropDialog.Visible &&
                            GridType == MirGridType.Inventory) // alt sell/repair
                    {
                    ...
                    else if((GridType == MirGridType.HeroHPItem || GridType == MirGridType.HeroMPItem) &&
                            GameScene.SelectedCell == null && Item != null) {
                        Network.Enqueue(new ClientPacket.SetAutoPotItem { Grid = GridType, ItemIndex = 0 });
                    } else {
                        MoveItem();
                    }
        ...
        public override void OnMouseDoubleClick(MouseEventArgs e) {
            ...
            if(GameScene.PickedUpGold || GridType == MirGridType.Inspect || GridType == MirGridType.TrustMerchant ||
        ...
        private void BuyItem() {
            if(Item == null || Item.Price() * GameScene.NpcRate > GameScene.Gold) {
                return;
            }
            ...
            if(Item.Count > 1) {
                ...
                amountBox.OKButton.Click += (o, e) => {
                    Network.Enqueue(new ClientPacket.BuyItemBack
                        { UniqueID = Item.UniqueID, Count = (ushort)amountBox.Amount });
                    Locked = true;
                };
            } else {
                amountBox = new MirAmountBox("Purchase", Item.Image,
                    string.Format("Value: {0:#,##0} Gold", Item.Price()));

                amountBox.OKButton.Click += (o, e) => {
                    Network.Enqueue(new ClientPacket.BuyItemBack { UniqueID = Item.UniqueID, Count = 1 });
                    Locked = true;
                };
            }
        ...
        public void OpenItem() {
            if((GridType != MirGridType.Equipment && GridType != MirGridType.Inventory) || Item == null ||
        ...
        public void UseItem() {
            if(Locked || GridType == MirGridType.Inspect || GridType == MirGridType.TrustMerchant ||
               GridType == MirGridType.GuildStorage || GridType == MirGridType.Craft) {
                return;
            }

            if(!HeroGridType && MapObject.User.Fishing) {
                return;
            }

            if(MapObject.User.RidingMount && Item.Info.Type != ItemType.Scroll && Item.Info.Type != ItemType.Potion &&
            ...
            if((GridType != MirGridType.Inventory && GridType != MirGridType.Storage &&
                GridType != MirGridType.HeroInventory) || Item == null || !CanUseItem() ||
               GameScene.SelectedCell == this) {
                return;
            }
            ...
            switch (Item.Info.Type) {
                case ItemType.Weapon:
                    if(dialog.Grid[(int)EquipmentSlot.Weapon].CanWearItem(actor, Item)) {
                        Network.Enqueue(new ClientPacket.EquipItem
                ...
                case ItemType.Bracelet:
                    ...
                    break;
                case ItemType.Ring:
                    if(dialog.Grid[(int)EquipmentSlot.RingR].Item == null &&
                       dialog.Grid[(int)EquipmentSlot.RingR].CanWearItem(actor, Item)) {
                        Network.Enqueue(new ClientPacket.EquipItem
                ...
                case ItemType.Amulet:
                    ...
                    if(dialog.Grid[(int)EquipmentSlot.Amulet].Item != null && Item.Info.Type == ItemType.Amulet) {
                        if(dialog.Grid[(int)EquipmentSlot.Amulet].Item.Info == Item.Info &&
                           dialog.Grid[(int)EquipmentSlot.Amulet].Item.Count <
                           dialog.Grid[(int)EquipmentSlot.Amulet].Item.Info.StackSize) {
                            Network.Enqueue(new ClientPacket.MergeItem {
                                GridFrom = GridType,
                                GridTo = GridType == MirGridType.HeroInventory
                                    ? MirGridType.HeroEquipment
                                    : MirGridType.Equipment,
                                IDFrom = Item.UniqueID, IDTo = dialog.Grid[(int)EquipmentSlot.Amulet].Item.UniqueID
                            });
                ...
                case ItemType.SealedHero:
                    if(CanUseItem() && (GridType == MirGridType.Inventory || GridType == MirGridType.HeroInventory)) {
                        ...
                        if(Item.Info.Type == ItemType.Potion && Item.Info.Shape == 4) {
                            MirMessageBox messageBox = new("Are you use you want to use this Potion?",
                                MirMessageBoxButtons.YesNo);
                            messageBox.YesButton.Click += (o, e) => {
                                Network.Enqueue(new ClientPacket.UseItem { UniqueID = Item.UniqueID, Grid = GridType });
                                ...
                                if(Item.Count == 1 && ItemSlot < GameScene.User.BeltIdx) {
                                    for (int i = GameScene.User.BeltIdx; i < GameScene.User.Inventory.Length; i++) {
                                        if(ItemArray[i] != null && ItemArray[i].Info == Item.Info) {
                                            Network.Enqueue(new ClientPacket.MoveItem
                                                { Grid = MirGridType.Inventory, From = i, To = ItemSlot });
                                            GameScene.Scene.InventoryDialog.Grid[i - GameScene.User.BeltIdx].Locked =
                                                true;
                                            break;
                                        }
                                    }
                        ...
                        if(HeroGridType) {
                            if(Item.Count == 1 && ItemSlot < GameScene.User.HeroBeltIdx) {
                                for (int i = GameScene.User.HeroBeltIdx; i < GameScene.Hero.Inventory.Length; i++) {
                                    if(ItemArray[i] != null && ItemArray[i].Info == Item.Info) {
                                        Network.Enqueue(new ClientPacket.MoveItem
                                            { Grid = MirGridType.HeroInventory, From = i, To = ItemSlot });
                                        GameScene.Scene.HeroInventoryDialog.Grid[i - GameScene.User.HeroBeltIdx]
                                            .Locked = true;
                                        break;
                                    }
                                }
            ...
            switch (Item.Info.Type) {
                case ItemType.Socket:
                    if(GameScene.SelectedItem != null && !GameScene.SelectedItem.Info.IsFishingRod &&
                       GameScene.SelectedItem.Info.Type != ItemType.Mount) {
                        ...
                        for (int i = 0; i < GameScene.Scene.SocketDialog.Grid.Length; i++) {
                            if(!GameScene.Scene.SocketDialog.Grid[i].Visible ||
                               GameScene.Scene.SocketDialog.Grid[i].Item != null) {
                                continue;
                            }
                        ...
                        if(cell != null && cell.CanWearItem(GameScene.User, Item)) {
                            Network.Enqueue(new ClientPacket.EquipSlotItem {
                                Grid = GridType, UniqueID = Item.UniqueID, To = cell.ItemSlot,
                                GridTo = MirGridType.Socket, ToUniqueID = GameScene.SelectedItem.UniqueID
                            });
                            cell.Locked = true;
                            Locked = true;
                        }
                ...
                case ItemType.Reins:
                    ...
                    if(mountDialog.Grid[(int)MountSlot.Reins].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Mount];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)MountSlot.Reins,
                            GridTo = MirGridType.Mount, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Bells:
                    ...
                    if(mountDialog.Grid[(int)MountSlot.Bells].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Mount];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)MountSlot.Bells,
                            GridTo = MirGridType.Mount, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Ribbon:
                    ...
                    if(mountDialog.Grid[(int)MountSlot.Ribbon].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Mount];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)MountSlot.Ribbon,
                            GridTo = MirGridType.Mount, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Saddle:
                    ...
                    if(mountDialog.Grid[(int)MountSlot.Saddle].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Mount];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)MountSlot.Saddle,
                            GridTo = MirGridType.Mount, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Mask:
                    ...
                    if(mountDialog.Grid[(int)MountSlot.Mask].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Mount];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)MountSlot.Mask,
                            GridTo = MirGridType.Mount, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Hook:
                    ...
                    if(fishingDialog.Grid[(int)FishingSlot.Hook].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Weapon];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)FishingSlot.Hook,
                            GridTo = MirGridType.Fishing, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Float:
                    ...
                    if(fishingDialog.Grid[(int)FishingSlot.Float].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Weapon];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)FishingSlot.Float,
                            GridTo = MirGridType.Fishing, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Bait:
                    ...
                    if(fishingDialog.Grid[(int)FishingSlot.Bait].Item != null && Item.Info.Type == ItemType.Bait) {
                        if(fishingDialog.Grid[(int)FishingSlot.Bait].Item.Info == Item.Info &&
                           fishingDialog.Grid[(int)FishingSlot.Bait].Item.Count <
                           fishingDialog.Grid[(int)FishingSlot.Bait].Item.Info.StackSize) {
                            Network.Enqueue(new ClientPacket.MergeItem {
                                GridFrom = GridType, GridTo = MirGridType.Fishing, IDFrom = Item.UniqueID,
                                IDTo = fishingDialog.Grid[(int)FishingSlot.Bait].Item.UniqueID
                            });
                    ...
                    if(fishingDialog.Grid[(int)FishingSlot.Bait].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Weapon];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)FishingSlot.Bait,
                            GridTo = MirGridType.Fishing, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Finder:
                    ...
                    if(fishingDialog.Grid[(int)FishingSlot.Finder].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Weapon];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)FishingSlot.Finder,
                            GridTo = MirGridType.Fishing, ToUniqueID = toItem.UniqueID
                        });
                ...
                case ItemType.Reel:
                    ...
                    if(fishingDialog.Grid[(int)FishingSlot.Reel].CanWearItem(GameScene.User, Item)) {
                        UserItem toItem = MapObject.User.Equipment[(byte)EquipmentSlot.Weapon];
                        Network.Enqueue(new ClientPacket.EquipSlotItem {
                            Grid = GridType, UniqueID = Item.UniqueID, To = (int)FishingSlot.Reel,
                            GridTo = MirGridType.Fishing, ToUniqueID = toItem.UniqueID
                        });
        ...
        public void RemoveItem() {
            ...
            for (int i = 0; i < GameScene.User.Inventory.Length; i++) {
                MirItemCell itemCell = i < GameScene.User.BeltIdx
                    ? GameScene.Scene.BeltDialog.Grid[i]
                    : GameScene.Scene.InventoryDialog.Grid[i - GameScene.User.BeltIdx];
            ...
            if(Item == null || count < 1 || (MapObject.User.RidingMount && Item.Info.Type != ItemType.Torch)) {
                return;
            }
            ...
            if(Item.Info.StackSize > 1) {
                ...
                if(item != null && item.Count + Item.Count <= item.Info.StackSize) {
                    //Merge.
                    Network.Enqueue(new ClientPacket.MergeItem {
                        GridFrom = GridType, GridTo = MirGridType.Inventory, IDFrom = Item.UniqueID,
                        IDTo = item.UniqueID
                    });
            ...
            for (int i = 0; i < GameScene.User.Inventory.Length; i++) {
                ...
                if(Item.Info.Type == ItemType.Amulet) {
                    itemCell = i < GameScene.User.BeltIdx
                        ? GameScene.Scene.BeltDialog.Grid[i]
                        : GameScene.Scene.InventoryDialog.Grid[i - GameScene.User.BeltIdx];
                } else {
                    itemCell = i < GameScene.User.Inventory.Length - GameScene.User.BeltIdx
                        ? GameScene.Scene.InventoryDialog.Grid[i]
                        : GameScene.Scene.BeltDialog.Grid[i - GameScene.User.Inventory.Length];
                }
                ...
                if(GridType != MirGridType.Equipment) {
                    ...
                    if(GridType == MirGridType.Fishing) {
                        if(GameScene.Scene.CharacterDialog.Grid[(byte)EquipmentSlot.Weapon].Item == null) {
                            return;
                        }

                        fromID = GameScene.Scene.CharacterDialog.Grid[(byte)EquipmentSlot.Weapon].Item.UniqueID;
                    } else if(GridType == MirGridType.Mount) {
                        if(GameScene.Scene.CharacterDialog.Grid[(byte)EquipmentSlot.Mount].Item == null) {
                            return;
                        }
                    ...
                    Network.Enqueue(new ClientPacket.RemoveSlotItem {
                        Grid = GridType, UniqueID = Item.UniqueID, To = itemCell.ItemSlot,
                        GridTo = MirGridType.Inventory, FromUniqueID = fromID
                    });
                } else {
                    Network.Enqueue(new ClientPacket.RemoveItem
        ...
        public void RemoveHeroItem() {
            ...
            for (int i = 0; i < GameScene.Hero.Inventory.Length; i++) {
                MirItemCell itemCell = i < GameScene.User.HeroBeltIdx
                    ? GameScene.Scene.HeroBeltDialog.Grid[i]
                    : GameScene.Scene.HeroInventoryDialog.Grid[i - GameScene.User.HeroBeltIdx];
            ...
            if(Item == null || count < 1 || (MapObject.Hero.RidingMount && Item.Info.Type != ItemType.Torch)) {
                return;
            }
            ...
            for (int i = 0; i < GameScene.Hero.Inventory.Length; i++) {
                ...
                if(Item.Info.Type == ItemType.Amulet) {
                    itemCell = i < GameScene.User.HeroBeltIdx
                        ? GameScene.Scene.HeroBeltDialog.Grid[i]
                        : GameScene.Scene.HeroInventoryDialog.Grid[i - GameScene.User.HeroBeltIdx];
                } else {
                    itemCell = i < GameScene.Hero.Inventory.Length - GameScene.User.HeroBeltIdx
                        ? GameScene.Scene.HeroInventoryDialog.Grid[i]
                        : GameScene.Scene.HeroBeltDialog.Grid[i - GameScene.Hero.Inventory.Length];
                }
                ...
                Network.Enqueue(new ClientPacket.RemoveItem
                    { Grid = MirGridType.HeroInventory, UniqueID = Item.UniqueID, To = itemCell.ItemSlot });
        ...
        private void MoveItem() {
            if(GridType == MirGridType.BuyBack || GridType == MirGridType.DropPanel ||
               GridType == MirGridType.Inspect || GridType == MirGridType.TrustMerchant ||
               GridType == MirGridType.Craft) {
                return;
            }
                ...
                switch (GridType) {
                    ...
                    case MirGridType.Inventory: // To Inventory
                        switch (GameScene.SelectedCell.GridType) {
                            ...
                            case MirGridType.Inventory: //From Invenotry
                                if(Item != null) {
                                    if(CMain.Ctrl) {
                                        MirMessageBox messageBox = new("Do you want to try and combine these items?",
                                            MirMessageBoxButtons.YesNo);
                                        messageBox.YesButton.Click += (o, e) => {
                                            //Combine
                                            Network.Enqueue(new ClientPacket.CombineItem {
                                                Grid = GameScene.SelectedCell.GridType,
                                                IDFrom = GameScene.SelectedCell.Item.UniqueID, IDTo = Item.UniqueID
                                            });
                                            Locked = true;
                                            GameScene.SelectedCell.Locked = true;
                                            GameScene.SelectedCell = null;
                                    ...
                                    if(GameScene.SelectedCell.Item.Info == Item.Info &&
                                       Item.Count < Item.Info.StackSize) {
                                        //Merge
                                        Network.Enqueue(new ClientPacket.MergeItem {
                                            GridFrom = GameScene.SelectedCell.GridType, GridTo = GridType,
                                            IDFrom = GameScene.SelectedCell.Item.UniqueID, IDTo = Item.UniqueID
                                        });
                            ...
                            case MirGridType.Equipment: //From Equipment
                                ...
                                for (int x = 6; x < ItemArray.Length; x++) {
                                    if(ItemArray[x] == null) {
                                        ...
                                        MirItemCell temp = x < GameScene.User.BeltIdx
                                            ? GameScene.Scene.BeltDialog.Grid[x]
                                            : GameScene.Scene.InventoryDialog.Grid[x - GameScene.User.BeltIdx];
                            ...
                            case MirGridType.Storage: //From Storage
                                ...
                                if(Item == null) {
                                    Network.Enqueue(new ClientPacket.TakeBackItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                ...
                                for (int x = 6; x < ItemArray.Length; x++) {
                                    if(ItemArray[x] == null) {
                                        Network.Enqueue(new ClientPacket.TakeBackItem
                                            { From = GameScene.SelectedCell.ItemSlot, To = x });
                            ...
                            case MirGridType.GuildStorage:
                                ...
                                if(!GuildDialog.MyOptions.HasFlag(GuildRankOptions.CanRetrieveItem)) {
                                    GameScene.Scene.ChatDialog.ReceiveChat("Insufficient rights to retrieve items.",
                                        ChatType.System);
                                    return;
                                }

                                Network.Enqueue(new ClientPacket.GuildStorageItemChange
                                    { Type = 1, From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                Locked = true;
                                GameScene.SelectedCell.Locked = true;
                                GameScene.SelectedCell = null;
                                break;
                            ...
                            case MirGridType.Trade: //From Trade
                                ...
                                if(Item == null) {
                                    Network.Enqueue(new ClientPacket.RetrieveTradeItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                            ...
                            case MirGridType.AwakenItem: //From AwakenItem
                                Network.Enqueue(new ClientPacket.MoveItem {
                                    Grid = GridType, From = NpcAwakeDialog.ItemsIdx[GameScene.SelectedCell.ItemSlot],
                                    To = NpcAwakeDialog.ItemsIdx[GameScene.SelectedCell.ItemSlot]
                                });
                            ...
                            case MirGridType.Refine: //From AwakenItem
                                ...
                                if(Item == null) {
                                    Network.Enqueue(new ClientPacket.RetrieveRefineItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                            ...
                            case MirGridType.Renting:
                                if(GameScene.User.RentalItemLocked) {
                                    GameScene.Scene.ChatDialog.ReceiveChat(
                                        "Unable to remove locked item, cancel item rental and try again.",
                                        ChatType.System);
                                    GameScene.SelectedCell = null;
                                    return;
                                }
                                ...
                                if(Item == null) {
                                    Network.Enqueue(new ClientPacket.RetrieveRentalItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                            ...
                            case MirGridType.HeroInventory:
                                ...
                                if(Item == null) {
                                    Network.Enqueue(new ClientPacket.TakeBackHeroItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                    ...
                    case MirGridType.Equipment: //To Equipment
                        ...
                        if(GameScene.SelectedCell.GridType != MirGridType.Inventory &&
                           GameScene.SelectedCell.GridType != MirGridType.Storage) {
                            return;
                        }
                        ...
                        if(CorrectSlot(GameScene.SelectedCell.Item, GameScene.SelectedCell.GridType)) {
                            if(CanWearItem(GameScene.User, GameScene.SelectedCell.Item)) {
                                Network.Enqueue(new ClientPacket.EquipItem {
                                    Grid = GameScene.SelectedCell.GridType,
                                    UniqueID = GameScene.SelectedCell.Item.UniqueID, To = ItemSlot
                                });
                                Locked = true;
                                GameScene.SelectedCell.Locked = true;
                            }
                        ...
                        switch (GameScene.SelectedCell.GridType) {
                            ...
                            case MirGridType.Inventory: //From Invenotry
                                ...
                                if(ItemArray[ItemSlot] == null) {
                                    Network.Enqueue(new ClientPacket.StoreItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                    Locked = true;
                                    GameScene.SelectedCell.Locked = true;
                                    GameScene.SelectedCell = null;
                    ...
                    case MirGridType.GuildStorage: //To Guild Storage
                        switch (GameScene.SelectedCell.GridType) {
                            case MirGridType.GuildStorage: //From Guild Storage
                                if(GameScene.SelectedCell.GridType == MirGridType.GuildStorage) {
                                    if(!GuildDialog.MyOptions.HasFlag(GuildRankOptions.CanStoreItem)) {
                                        GameScene.Scene.ChatDialog.ReceiveChat("Insufficient rights to store items.",
                                            ChatType.System);
                                        return;
                                    }
                                    ...
                                    //{
                                    Network.Enqueue(new ClientPacket.GuildStorageItemChange
                                        { Type = 2, From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                    Locked = true;
                                    GameScene.SelectedCell.Locked = true;
                                    GameScene.SelectedCell = null;
                                    return;
                            ...
                            case MirGridType.Inventory:
                                ...
                                if(GameScene.SelectedCell.GridType == MirGridType.Inventory) {
                                    if(Item != null) {
                                        GameScene.Scene.ChatDialog.ReceiveChat("You cannot swap items.",
                                            ChatType.System);
                                        return;
                                    }
                                    ...
                                    if(ItemArray[ItemSlot] == null) {
                                        Network.Enqueue(new ClientPacket.GuildStorageItemChange
                                            { Type = 0, From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                        Locked = true;
                                        GameScene.SelectedCell.Locked = true;
                                        GameScene.SelectedCell = null;
                                        return;
                    ...
                    case MirGridType.Trade:
                        if(Item != null && Item.Info.Bind.HasFlag(BindMode.DontTrade)) {
                            return;
                        }
                        ...
                        switch (GameScene.SelectedCell.GridType) {
                            ...
                            case MirGridType.Inventory: //From Inventory
                                ...
                                if(ItemArray[ItemSlot] == null) {
                                    Network.Enqueue(new ClientPacket.DepositTradeItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                    Locked = true;
                                    GameScene.SelectedCell.Locked = true;
                                    GameScene.SelectedCell = null;
                    ...
                    case MirGridType.Refine:
                        ...
                        switch (GameScene.SelectedCell.GridType) {
                            ...
                            case MirGridType.Inventory: //From Inventory
                                ...
                                Network.Enqueue(new ClientPacket.DepositRefineItem
                                    { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                Locked = true;
                                GameScene.SelectedCell.Locked = true;
                                GameScene.SelectedCell = null;
                    ...
                    case MirGridType.Renting:
                        switch (GameScene.SelectedCell.GridType) {
                            case MirGridType.Inventory:
                                ...
                                if(Item == null) {
                                    Network.Enqueue(new ClientPacket.DepositRentalItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                                    Locked = true;
                                    GameScene.SelectedCell.Locked = true;
                                    GameScene.SelectedCell = null;
                    ...
                    case MirGridType.AwakenItem: {
                        ...
                        if(GameScene.SelectedCell.GridType != MirGridType.Inventory && _itemSlot < 1) {
                            return;
                        }
                                ...
                                if((GameScene.SelectedCell.Item.Info.Type == ItemType.Weapon ||
                                   ...
                                   _itemSlot == 0) {
                                    if(Item == null) {
                                        ...
                                        Network.Enqueue(new ClientPacket.AwakeningLockedItem
                                            { UniqueID = Item.UniqueID, Locked = false });
                                ...
                                switch (GameScene.SelectedCell.GridType) {
                                    case MirGridType.Inventory: {
                                        if(GameScene.SelectedCell.Item.Info.Type == ItemType.Awakening &&
                                           ...
                                           NpcAwakeDialog.ItemsIdx[_itemSlot] == 0) {
                                            Item = GameScene.SelectedCell.Item;
                                            GameScene.SelectedCell.Locked = true;
                                            NpcAwakeDialog.ItemsIdx[_itemSlot] = GameScene.SelectedCell._itemSlot;
                                        } else {
                                        ...
                                        break;
                                    case MirGridType.AwakenItem: {
                                        if(GameScene.SelectedCell.ItemSlot == ItemSlot ||
                                           GameScene.SelectedCell.ItemSlot == 0) {
                                            Locked = false;
                                            GameScene.SelectedCell = null;
                                        } else {
                                            GameScene.SelectedCell.Locked = false;
                                            ...
                                            NpcAwakeDialog.ItemsIdx[GameScene.SelectedCell._itemSlot] =
                                                NpcAwakeDialog.ItemsIdx[_itemSlot];
                    ...
                    case MirGridType.HeroInventory:
                        ...
                        switch (GameScene.SelectedCell.GridType) {
                            ...
                            case MirGridType.HeroEquipment:
                                ...
                                for (int x = 2; x < ItemArray.Length; x++) {
                                    if(ItemArray[x] == null) {
                                        ...
                                        MirItemCell temp = x < GameScene.User.HeroBeltIdx
                                            ? GameScene.Scene.HeroBeltDialog.Grid[x]
                                            : GameScene.Scene.HeroInventoryDialog.Grid[x - GameScene.User.HeroBeltIdx];
                            ...
                            case MirGridType.Inventory:
                                ...
                                if(GameScene.SelectedCell.Item.Weight + MapObject.User.CurrentBagWeight >
                                   MapObject.User.Stats[Stat.BagWeight]) {
                                ...
                                if(Item == null) {
                                    Network.Enqueue(new ClientPacket.TransferHeroItem
                                        { From = GameScene.SelectedCell.ItemSlot, To = ItemSlot });
                    ...
                    case MirGridType.HeroEquipment:
                        ...
                        if(GameScene.SelectedCell.GridType != MirGridType.HeroInventory) {
                            return;
                        }
                    ...
                    case MirGridType.HeroMPItem:
                        ...
                        if(GameScene.SelectedCell.Item.Info.Type != ItemType.Potion ||
                           GameScene.SelectedCell.Item.Info.Shape > 1) {
                            return;
                        }

                        Network.Enqueue(new ClientPacket.SetAutoPotItem
                            { Grid = GridType, ItemIndex = GameScene.SelectedCell.Item.Info.Index });
                        GameScene.SelectedCell = null;
        ...
        private bool CanUseItem() {
            ...
            switch (Item.Info.Type) {
                ...
                case ItemType.Finder:
                case ItemType.Reel:
                    if(actor.Equipment[(int)EquipmentSlot.Weapon] == null ||
                       !actor.Equipment[(int)EquipmentSlot.Weapon].Info.IsFishingRod) {
                        GameScene.Scene.ChatDialog.ReceiveChat("You do not have a fishing rod equipped.",
                            ChatType.System);
                        return false;
                    }
        ...
        private bool CanWearItem(UserObject actor, UserItem i) {
            ...
            if(i.Info.Type == ItemType.Weapon || i.Info.Type == ItemType.Torch) {
                if(i.Weight - (Item != null ? Item.Weight : 0) + actor.CurrentHandWeight >
                   actor.Stats[Stat.HandWeight]) {
                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.TooHeavyToHold, ChatType.System);
                    return false;
                }
            } else {
                if(i.Weight - (Item != null ? Item.Weight : 0) + actor.CurrentWearWeight >
                   actor.Stats[Stat.WearWeight]) {
            ...
            switch (i.Info.Type) {
                ...
                case ItemType.Saddle:
                    ...
                    break;
                case ItemType.Socket:
                    if(GameScene.SelectedItem == null || GameScene.SelectedItem.Info.Type == ItemType.Mount ||
                       (GameScene.SelectedItem.Info.Type == ItemType.Weapon &&
                        GameScene.SelectedItem.Info.IsFishingRod)) {
                        return false;
                    }
        ...
        protected internal override void DrawControl() {
            if(Item != null && GameScene.SelectedCell != this && Locked != true) {
                ...
                if(Library != null) {
                    ...
                    Point offSet = new((Size.Width - imgSize.Width) / 2, (Size.Height - imgSize.Height) / 2);

                    if(GridType == MirGridType.Craft) {
                        Libraries.Prguse.Draw(1121, DisplayLocation.Add(new Point(-2, -1)), Color.White, UseOffSet,
                            0.8F);
                    }
                    ...
                    if(Item.SealedInfo != null && Item.SealedInfo.ExpiryDate > CMain.Now) {
                        Libraries.StateItems.Draw(3590, DisplayLocation.Add(new Point(2, 2)), Color.White, UseOffSet,
                            1F);
                    }
                }
