...
namespace Client.MirControls {
    public sealed class MirGoodsCell : MirControl {
        ...
        private void Update() {
            ...
            if(UsePearls) {
                PriceLabel.Text = string.Format("Price: {0} pearl{1}", (uint)(Item.Price() * GameScene.NpcRate),
                    Item.Price() > 1 ? "s" : "");
            } else if(Recipe) {
                ClientRecipeInfo recipe =
                    GameScene.RecipeInfoList.SingleOrDefault(x => x.Item.ItemIndex == Item.ItemIndex);
