...
namespace Client.MirControls {
    public sealed class GameShopCell : MirImageControl {
        ...
        private Rectangle ItemDisplayArea;
        ...
        public GameShopCell() {
            ...
            PreviewItem.Click += (o, e) => {
                ...
                GameScene.Scene.GameShopDialog.Viewer = new GameShopViewer {
                    ...
                    Visible = true,
                    Location = Location.X < 350 ? new Point(416, 115) : new Point(151, 115)
                };
            ...
            quantityUp.Click += (o, e) => {
                ...
                if((decimal)(Quantity * Item.Count) / Item.Info.StackSize > 5) {
                    Quantity = 5 * Item.Info.StackSize / Item.Count > 99
                        ? Quantity = 99
                        : Quantity = (byte)(5 * Item.Info.StackSize / Item.Count);
                }
            ...
            switch (pType) {
                case 0: //  Credit
                    if(Item.CreditPrice * Quantity <= GameScene.Credit) {
                        ...
                        messageBox =
                            new MirMessageBox(
                                string.Format("Are you sure would you like to buy {1} x \n{0}({3}) for {2} Credits?",
                                    Item.Info.FriendlyName, Quantity, CreditCost, Item.Count),
                                MirMessageBoxButtons.YesNo);
                        messageBox.YesButton.Click += (o, e) => Network.Enqueue(new ClientPacket.GameshopBuy
                            { GIndex = Item.GIndex, Quantity = Quantity, PType = pType });
                ...
                case 1: //  Gold
                    if(Item.GoldPrice * Quantity <= GameScene.Gold) {
                        ...
                        messageBox =
                            new MirMessageBox(
                                string.Format("Are you sure would you like to buy{1} x \n{0}({3}) for {2} Gold?",
                                    Item.Info.FriendlyName, Quantity, GoldCost, Item.Count),
                                MirMessageBoxButtons.YesNo);
                        messageBox.YesButton.Click += (o, e) => Network.Enqueue(new ClientPacket.GameshopBuy
                            { GIndex = Item.GIndex, Quantity = Quantity, PType = pType });
                        messageBox.NoButton.Click += (o, e) => { };
        ...
        public override void OnMouseMove(MouseEventArgs e) {
            ...
            if(ShowItem == null && ItemDisplayArea.Contains(CMain.MPoint)) {
                ShowItem = new UserItem(Item.Info)
                    { MaxDura = Item.Info.Durability, CurrentDura = Item.Info.Durability, Count = Item.Count };
        ...
        public void UpdateText() {
            ...
            if(Item.Info.Type == ItemType.Mount || Item.Info.Type == ItemType.Weapon ||
               Item.Info.Type == ItemType.Armour || Item.Info.Type == ItemType.Transform) {
        ...
        protected internal override void DrawControl() {
            ...
            Point offSet = new((32 - size.Width) / 2, (32 - size.Height) / 2);

            Libraries.Items.Draw(Item.Info.Image, offSet.X + DisplayLocation.X + 12, offSet.Y + DisplayLocation.Y + 40);
            ItemDisplayArea =
                new Rectangle(new Point(offSet.X + DisplayLocation.X + 12, offSet.Y + DisplayLocation.Y + 40), size);
        }
    ...
    public sealed class GameShopViewer : MirImageControl {
        ...
        private void DrawMount() {
            ...
            if(GameScene.User.Equipment[(int)EquipmentSlot.Armour] != null) {
                PreviewImage.Library =
                    Libraries.CArmours[GameScene.User.Equipment[(int)EquipmentSlot.Armour].Info.Shape];
            } else {
