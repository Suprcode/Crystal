...
namespace Client.MirScenes.Dialogs {
    public sealed class TrustMerchantDialog : MirImageControl {
        ...
        public uint Amount = 0,
            MinConsignAmount = Globals.MinConsignment,
            MaxConsignAmount = Globals.MaxConsignment,
            MinBidAmount = Globals.MinStartingBid,
            MaxBidAmount = Globals.MaxStartingBid;

        public static long SearchTime, MarketTime;

        public MirTextBox SearchTextBox, PriceTextBox;

        public MirButton FindButton,
            RefreshButton,
            MailButton,
            BuyButton,
            SellNowButton,
        ...
        public List<ClientAuction> GameShopListings = new();
        ...
        private readonly string consignmentText =
        ...
        private readonly string auctionText =
            $"1. Auction cost is {Globals.AuctionCost} gold, max starting bid is {Globals.MaxStartingBid} gold per item \r\n\r\n2. 1% of final bid price is paid to Trust Merchant " +
        ...
        private List<MirButton> FilterButtons = new();
        ...
        public TrustMerchantDialog() {
            ...
            MailButton.Click += (o, e) => {
                ...
                string message =
                    $"I am interested in purchasing {Selected.Listing.Item.FriendlyName} for {Selected.Listing.Price}.";
            ...
            RefreshButton.Click += (o, e) => {
                if(CMain.Time < SearchTime) {
                    GameScene.Scene.ChatDialog.ReceiveChat(
                        string.Format("You can search again after {0} seconds.",
                            Math.Ceiling((SearchTime - CMain.Time) / 1000D)), ChatType.System);
                    return;
                }
                ...
                if(UserMode) {
                    if(Selected.Listing.ItemType == MarketItemType.Consign) {
                        if(Selected.Listing.Seller == "For Sale") {
                            MirMessageBox box =
                                new(
                                    string.Format("{0} has not sold, Are you sure you want to get it back?",
                                        Selected.Listing.Item.FriendlyName), MirMessageBoxButtons.YesNo);
                            box.YesButton.Click += (o1, e2) => {
                                MarketTime = CMain.Time + 3000;
                                Network.Enqueue(new ClientPacket.MarketGetBack
                    ...
                    switch (Selected.Listing.ItemType) {
                        ...
                        case MarketItemType.GameShop: {
                            MirMessageBox box =
                                new(
                                    string.Format("Are you sure you want to buy {0} for {1:#,##0} {2}?",
                                        Selected.Listing.Item.FriendlyName, Selected.Listing.Price,
                                        MarketType == MarketPanelType.GameShop ? "Credits" : "Gold"),
                        ...
                        case MarketItemType.Auction: {
                            MirAmountBox bidAmount = new("Bid Amount:", Selected.Listing.Item.Info.Image, uint.MaxValue,
                                Selected.Listing.Price + 1, Selected.Listing.Price + 1);

                            bidAmount.OKButton.Click += (o1, e1) => {
                                MirMessageBox box =
                                    new(
                                        string.Format("Are you sure you want to bid {0:#,##0} Gold for {1}?",
                                            bidAmount.Amount, Selected.Listing.Item.FriendlyName),
                                        MirMessageBoxButtons.YesNo);
                                box.YesButton.Click += (o2, e2) => {
                                    MarketTime = CMain.Time + 3000;
                                    Network.Enqueue(new ClientPacket.MarketBuy
                                        { AuctionID = Selected.Listing.AuctionID, BidPrice = bidAmount.Amount });
                                };
            ...
            SellItemButton.Click += (o, e) => {
                Network.Enqueue(new ClientPacket.ConsignItem
                    { UniqueID = SellItemSlot.UniqueID, Price = Amount, Type = MarketType });
                SellItemSlot = null;
        ...
        private void SetupFilters() {
            Filter all = new Filter { Index = 0, Title = "Show All Items", Type = ItemType.Nothing };
            Filter weapon = new Filter { Index = 1, Title = "Weapon Items", Type = ItemType.Weapon };
            Filter drapery = new Filter { Index = 2, Title = "Drapery Items", Type = null };
            Filter accessory = new Filter { Index = 3, Title = "Accessory Items", Type = null };
            Filter consumable = new Filter { Index = 4, Title = "Consumable Items", Type = null };
            Filter enhancement = new Filter { Index = 5, Title = "Enhancement", Type = null };
            Filter book = new Filter { Index = 6, Title = "Books", Type = null };
            Filter crafting = new Filter { Index = 7, Title = "Craft Items", Type = null };
            ...
            consumable.SubFilters.Add(new Filter
                { Index = 401, Title = "Recovery Pots", Type = ItemType.Potion, MaxShape = 2 });
            consumable.SubFilters.Add(new Filter
                { Index = 402, Title = "Buff Pots", Type = ItemType.Potion, MinShape = 3, MaxShape = 4 });
            ...
            enhancement.SubFilters.Add(new Filter
                { Index = 501, Title = "Gems", Type = ItemType.Potion, MinShape = 3, MaxShape = 3 });
            enhancement.SubFilters.Add(new Filter
                { Index = 502, Title = "Orbs", Type = ItemType.Potion, MinShape = 4, MaxShape = 4 });

            book.SubFilters.Add(new Filter { Index = 601, Title = "Warrior", Type = ItemType.Book, MaxShape = 30 });
            book.SubFilters.Add(new Filter
                { Index = 602, Title = "Wizard", Type = ItemType.Book, MinShape = 31, MaxShape = 60 });
            book.SubFilters.Add(new Filter
                { Index = 603, Title = "Taoist", Type = ItemType.Book, MinShape = 61, MaxShape = 90 });
            book.SubFilters.Add(new Filter
                { Index = 604, Title = "Assassin", Type = ItemType.Book, MinShape = 91, MaxShape = 120 });
            book.SubFilters.Add(new Filter
                { Index = 605, Title = "Archer", Type = ItemType.Book, MinShape = 121, MaxShape = 150 });
        ...
        private void DrawFilters(int index, int subIndex) {
            ...
            foreach(MirButton item in FilterButtons) {
                item.Dispose();
            }

            foreach(MirLabel item in FilterLabels) {
                item.Dispose();
            }
            ...
            foreach(Filter item in Filters) {
                ...
                btn.Click += (o, e) => {
                    if(item.SubFilters.Any()) {
                        ...
                        if(item.Type.HasValue) {
                            Network.Enqueue(new ClientPacket.MarketSearch {
                                Match = SearchTextBox.Text, Type = item.Type.Value, Usermode = false,
                                MinShape = item.MinShape, MaxShape = item.MaxShape, MarketType = MarketType
                            });
                        }
                    }
                };

                MirLabel lbl = new MirLabel {
                    Size = new Size(99, 18),
                ...
                if(item.Index == index) {
                    ...
                    foreach(Filter subItem in item.SubFilters) {
                        if(skipped > 0) {
                            skipped--;
                            continue;
                        }
                        ...
                        MirButton subBtn = new MirButton {
                            Index = 922,
                            PressedIndex = 923,
                            HoverIndex = 923,
                            Library = Libraries.Prguse2,
                        ...
                        subBtn.Click += (o, e) => {
                            ...
                            if(subItem.Type.HasValue) {
                                Network.Enqueue(new ClientPacket.MarketSearch {
                                    Match = SearchTextBox.Text, Type = subItem.Type.Value, Usermode = false,
                                    MinShape = subItem.MinShape, MaxShape = subItem.MaxShape, MarketType = MarketType
                                });
                            }
                        };


                        MirLabel sublbl = new MirLabel {
                            Size = new Size(99, 18),
        ...
        public void UpdateInterface() {
            ...
            TotalGold.Text = MarketType == MarketPanelType.GameShop
                ? GameScene.Credit.ToString("###,###,##0")
                : GameScene.Gold.ToString("###,###,##0");
        ...
        public sealed class AuctionRow : MirControl {
            ...
            public void UpdateInterface() {
                ...
                SellerLabel.Visible = Listing.ItemType == MarketItemType.Consign ||
                                      Listing.ItemType == MarketItemType.Auction;
                ExpireLabel.Visible = Listing != null && (Listing.ItemType == MarketItemType.Consign ||
                                                          Listing.ItemType == MarketItemType.Auction);
                ...
                ExpireLabel.Text = string.Format("{0:dd/MM/yy HH:mm:ss}",
                    Listing.ConsignmentDate.AddDays(Globals.ConsignmentLength));
            }
            ...
            public void Update(ClientAuction listing) {
                ...
                PriceLabel.Text = String.Format("{0:###,###,##0} {1}", Listing.Price,
                    listing.ItemType == MarketItemType.Auction ? "Bid" : "");
        ...
        public class Filter {
            ...
            public List<Filter> SubFilters = new();
        }
    }
}
