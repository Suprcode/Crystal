...
namespace Client.MirScenes.Dialogs {
    public class RankingDialog : MirImageControl {
        ...
        private int RowOffset {
            ...
            set {
                rowOffset = value;
                ScrollBar.Location = new Point(PrevButton.Location.X,
                    (int)(PrevButton.Location.Y + 13 + (rowOffset * GapPerRow)));
            }
        }
        ...
        public RankingDialog() {
            ...
            ScrollBar.OnMoving += (o, e) => {
                ...
                float row = Math.Max(0, Math.Min(RankCount - 20, (y - PrevButton.Location.Y - 13) / GapPerRow));
                RowOffset = (int)row;
                NextRequestTime = CMain.Now + TimeSpan.FromSeconds(0.5);
            };

            OnlineOnlyButton = new MirCheckBox {
                Index = 2086, UnTickedIndex = 2086, TickedIndex = 2087, Parent = this,
                Location = new Point(190, Size.Height - 20), Library = Libraries.Prguse
            };
        ...
        public void RequestRanks(byte RankType) {
            ...
            Network.Enqueue(new ClientPacket.GetRanking
                { RankType = RankType, RankIndex = RowOffset, OnlineOnly = OnlineOnly });
        }
        ...
        public sealed class RankingRow : MirControl {
            ...
            public void Inspect() {
                if(CMain.Time <= GameScene.InspectTime /* && Index == InspectDialog.InspectID*/) {
                    return;
                }

                GameScene.InspectTime = CMain.Time + 500;
                InspectDialog.InspectID = (uint)Index;
                Network.Enqueue(new ClientPacket.Inspect { ObjectID = (uint)Index, Ranking = true });
            }
