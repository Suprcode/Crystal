...
namespace Client.MirScenes.Dialogs {
    public class CompassDialog : MirControl {
        ...
        public void Process() {
            if(Destination == Point.Empty || (Destination.X == GameScene.User.CurrentLocation.X &&
                                              Destination.Y == GameScene.User.CurrentLocation.Y)) {
                Visible = false;
                return;
            }
            ...
            double angle = Math.Atan2(xDiff * -1, yDiff) * 180 / Math.PI;

            double degree = (angle + 360) % 360;

            double offset = (double)40 / 360 * degree;
