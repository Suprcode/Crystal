    ...
    public sealed class IntelligentCreatureDialog : MirImageControl
    {
        ...
        private void Control_MouseEnter(object sender, EventArgs e)
        {
            ...
            if (sender == FullnessMin)
            {
                ...
                HoverLabel.Text = string.Format(GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.NeededBy), GameScene.User.IntelligentCreatures[selectedCreature].CreatureRules.MinimalFullness.ToString());
        ...
        private void ButtonClick(object sender, EventArgs e)
        {
            ...
            if (sender == CreatureRenameButton)
            {
                ...
                inputBox.OKButton.Click += (o1, e1) =>
                {
                    ...
                    {
                        MirMessageBox failedMessage = new MirMessageBox(string.Format(GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.CreatureNameLengthRange), Globals.MinCharacterNameLength, Globals.MaxCharacterNameLength), MirMessageBoxButtons.OK);
                        failedMessage.Show();
                    }
                    else
                    {
        ...
        private void DrawCreatureAnimation()
        {
            ...
            CreatureInfo.Text = string.Format(GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.CanPickupItems), semi, mouse);
            ...
            {
                CreatureDeadline.Text = string.Format(GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.ExpireNever));
            }
            else
            {
                var seconds = (GameScene.User.IntelligentCreatures[selectedCreature].Expire - CMain.Now).TotalSeconds;

                CreatureDeadline.Text = string.Format(GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.Expire), Functions.PrintTimeSpanFromSeconds(seconds));
            }

            if (GameScene.User.IntelligentCreatures[selectedCreature].MaintainFoodTime == 0)
                CreatureMaintainFoodBuff.Text = "0";
            else
                CreatureMaintainFoodBuff.Text = string.Format(GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.FoodBuff), Functions.PrintTimeSpanFromSeconds(GameScene.User.IntelligentCreatures[selectedCreature].MaintainFoodTime));
