...
namespace Client.MirScenes.Dialogs {
    public sealed class GuildDialog : MirImageControl {
        ...
        public List<GuildBuffInfo> GuildBuffInfos = new();
        public List<GuildBuff> EnabledBuffs = new();
        ...
        public GuildDialog() {
            ...
            NoticeDownButton.Click += (o, e) => {
                if(NoticeScrollIndex == Notice.MultiText.Length - 1) {
                    return;
                }

                if(NoticeScrollIndex >= Notice.MultiText.Length - 25) {
                    NoticeScrollIndex--;
                }
            ...
            StatusPage.BeforeDraw += (o, e) => {
                if(MapControl.User.GuildName == "") {
                    ...
                    StatusMembers.Text = string.Format("{0}{1}", MemberCount,
                        MaxMembers == 0 ? "" : "/" + MaxMembers.ToString());
                }
            };
            ...
            String[] Options = {
                "Edit ranks", "Recruit member", "Kick member", "Store item", "Retrieve item", "Alter alliance",
                "Change notice", "Activate Buff"
            };
            ...
            for (int i = 0; i < RanksOptionsTexts.Length; i++) {
                RanksOptionsTexts[i] = new MirLabel() {
                    ...
                    NotControl = true,
                    Parent = RankPage,
                    Location = new Point(17 + (i % 2 == 0 ? 42 : 202),
                        i % 2 == 0 ? 118 + (i * 20) : 118 + ((i - 1) * 20)),
                    AutoSize = true,
                    Text = Options[i]
                };
            }
            ...
            for (byte i = 0; i < Buffs.Length; i++) {
                byte Id = i;
                Buffs[i] = new GuildBuffButton
                    { Parent = BuffPage, Visible = true, Location = new Point(4, 27 + (i * 38)), Id = Id };
            ...
            DownButton.Click += (o, e) => {
                if(GuildBuffInfos.Count < 8) {
                    return;
                }

                if(StartIndex == GuildBuffInfos.Count - 8) {
                    return;
                }
        ...
        public void RequestBuff(byte Id) {
            if(Id + StartIndex > GuildBuffInfos.Count) {
                return;
            }
            ...
            if(Buff == null) {
                ...
                if(Error != "") {
                    MirMessageBox messageBox = new(Error);
                    messageBox.Show();
                    return;
                }
        ...
        private void BuffsPanel_MouseWheel(object sender, MouseEventArgs e) {
            ...
            if(StartIndex == GuildBuffInfos.Count - 8 && count <= 0) {
                return;
            }
        ...
        public void CreateHintLabel(byte Id) {
            ...
            GameScene.Scene.GuildBuffLabel.Size = new Size(
                Math.Max(GameScene.Scene.GuildBuffLabel.Size.Width, HintName.DisplayRectangle.Right + 4),
            ...
            GameScene.Scene.GuildBuffLabel.Size = new Size(
                Math.Max(GameScene.Scene.GuildBuffLabel.Size.Width, RequiredLabel.DisplayRectangle.Right + 4),
            ...
            GameScene.Scene.GuildBuffLabel.Size = new Size(
                Math.Max(GameScene.Scene.GuildBuffLabel.Size.Width, InfoLabel.DisplayRectangle.Right + 4),
        ...
        public void ResetButtonStats() {
            if(MyOptions.HasFlag(GuildRankOptions.CanRetrieveItem) ||
               MyOptions.HasFlag(GuildRankOptions.CanStoreItem)) {
        ...
        public void UpdateNotice(bool forward = false) {
            ...
            if(NoticeScrollerIndex >= Notice.MultiText.Length) {
                NoticeScrollerIndex = Notice.MultiText.Length - 24;
            }
        ...
        private void UpdateNoticeScrollPosition() {
            ...
            if(y >= NoticeDownButton.Location.Y - 20) {
                y = NoticeDownButton.Location.Y - 20;
            }
        ...
        private void NoticePositionBar_OnMoving(object sender, MouseEventArgs e) {
            ...
            if(NoticeScrollIndex >= Notice.MultiText.Length - 25) {
                NoticeScrollIndex = Notice.MultiText.Length - 25;
            }
        ...
        private void NoticePanel_MouseWheel(object sender, MouseEventArgs e) {
            ...
            if(NoticeScrollIndex == Notice.MultiText.Length - 25 && count <= 0) {
                return;
            }
        ...
        public void OnNewRank(int Index, int SelectedIndex) {
            ...
            MirMessageBox messageBox =
                new(
                    string.Format("Are you sure you want to change the rank of {0} to {1}?", MembersName[Index].Text,
                        Ranks[SelectedIndex].Name), MirMessageBoxButtons.YesNo);

            messageBox.YesButton.Click += (o, a) => {
                Network.Enqueue(new ClientPacket.EditGuildMember {
                    ChangeType = 2, Name = MembersName[Index].Text, RankIndex = (byte)Ranks[SelectedIndex].Index,
                    RankName = Ranks[SelectedIndex].Name
                });
        ...
        public void DeleteMember(int Index) {
            if(MembersName[Index].Text == MapControl.User.Name) {
                return;
            }
            ...
            MirMessageBox messageBox = new(string.Format("Are you sure you want to kick {0}?", MembersName[Index].Text),
        ...
        public void UpdateMembers() {
            ...
            for (int j = 0; j < Ranks[i].Members.Count; j++) {
                if(Offset < MemberScrollIndex) {
                    ...
                    if(MyOptions.HasFlag(GuildRankOptions.CanKick) && Ranks[i].Index >= MyRankId &&
                       Ranks[i].Members[j].Name != MapControl.User.Name /* && (Ranks[i].Index != 0)*/) {
        ...
        public void MembersPositionBar_OnMoving(object sender, MouseEventArgs e) {
            ...
            if(y >= MembersDownButton.Location.Y - 20) {
                y = MembersDownButton.Location.Y - 20;
            }
        ...
        public void OnRankSelect(int Index) {
            if(Index < Ranks.Count) {
                ...
                MirMessageBox messageBox =
                    new("Are you sure you want to create a new rank?", MirMessageBoxButtons.YesNo);
                messageBox.YesButton.Click += (o, a) => {
                    Network.Enqueue(new ClientPacket.EditGuildMember
                        { ChangeType = 4, RankName = String.Format("Rank-{0}", Ranks.Count - 1) });
                    LastGuildMsg = CMain.Time + 5000;
                };
                messageBox.Show();
            }
        ...
        public void SwitchRankOption(int OptionIndex) {
            if(RanksSelectBox.SelectedIndex == -1 || RanksSelectBox.SelectedIndex >= Ranks.Count) {
                return;
            }
            ...
            Network.Enqueue(new ClientPacket.EditGuildMember {
                ChangeType = 5, RankIndex = (byte)Ranks[RanksSelectBox.SelectedIndex].Index,
                RankName = OptionIndex.ToString(), Name = RanksOptionsStatus[OptionIndex].Visible ? "false" : "true"
            });
        ...
        public void RanksChangeName() {
            if(!string.IsNullOrEmpty(RanksName.Text)) {
                Network.Enqueue(new ClientPacket.EditGuildMember {
                    ChangeType = 3, RankIndex = (byte)Ranks[RanksSelectBox.SelectedIndex].Index,
                    RankName = RanksName.Text
                });
        ...
        public void StoragePositionBar_OnMoving(object sender, MouseEventArgs e) {
            ...
            if(y >= StorageDownButton.Location.Y - 20) {
                y = StorageDownButton.Location.Y - 20;
            }
            ...
            for (int x = 0; x < 8; x++) {
                for (int y = 0; y < 14; y++) {
                    ...
                    if(StorageIndex <= y && y < StorageIndex + 8) {
                        StorageGrid[idx].Location = new Point((x * 35) + 31 + x,
                            ((y - StorageIndex) * 35) + 20 + (y - StorageIndex));
        ...
        public void StorageAddGold() {
            ...
            MirAmountBox amountBox = new("Deposit", 116, GameScene.Gold);
        ...
        public void StorageRemoveGold() {
            ...
            MirAmountBox amountBox = new("Gold to retrieve:", 116, Gold);
        ...
        public void StatusChanged(GuildRankOptions status) {
            ...
            if(MyOptions.HasFlag(GuildRankOptions.CanStoreItem) ||
               MyOptions.HasFlag(GuildRankOptions.CanRetrieveItem)) {
                StorageButton.Visible = true;
            } else {
                StorageButton.Visible = false;
            }
        ...
        public override void Show() {
            ...
            if(MapControl.User.GuildName == "") {
                MirMessageBox messageBox = new(GameLanguage.NotInGuild, MirMessageBoxButtons.OK);
                messageBox.Show();
                return;
            }
