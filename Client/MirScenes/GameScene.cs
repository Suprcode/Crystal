        ...
        private void DrawLights(LightSetting setting)
        {
            ...
            if (Settings.Effect)
            {
                for (int e = 0; e < Effects.Count; e++)
                {
                    ...
                    {
                        p.Offset(-(DXManager.LightSizes[light].X / 2) - (CellWidth / 2), -(DXManager.LightSizes[light].Y / 2) - (CellHeight / 2) - 5);
                        DXManager.Sprite.Draw(DXManager.Lights[light], SharpDX.Color.White, new RawRectangle(0, 0, DXManager.LightSizes[light].X, DXManager.LightSizes[light].Y), Vector3.Zero, new Vector3((float)p.X, (float)p.Y, 0.0F));
                    }
                }
            }
            #endregion
