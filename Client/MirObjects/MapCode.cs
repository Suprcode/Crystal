...
namespace Client.MirObjects {
    public class CellInfo {
        ...
        public void AddObject(MapObject ob) {
            if(CellObjects == null) {
                CellObjects = new List<MapObject>();
            }
        ...
        private void LoadMapType5() {
            try {
                ...
                for (int y = 0; y < Height / 2; y++) {
                    for (int i = 0; i < 4; i++) {
                        MapCells[(x * 2) + (i % 2), (y * 2) + (i / 2)].BackIndex =
                            (short)(Bytes[offset] != 255 ? Bytes[offset] + 200 : -1);
                        MapCells[(x * 2) + (i % 2), (y * 2) + (i / 2)].BackImage =
                            (int)(BitConverter.ToUInt16(Bytes, offset + 1) + 1);
                    }
                ...
                for (int y = 0; y < Height; y++) {
                    ...
                    if(MapCells[x, y].Light >= 100 && MapCells[x, y].Light <= 119) {
                        ...
                        MapCells[x, y].Light *=
                            2; //expand general mir3 lighting as default range is small. Might break new colour lights.
                    }
                }
            } catch(Exception ex) {
                if(Settings.LogErrors) {
        ...
        private void LoadMapType6() {
            try {
                ...
                for (int y = 0; y < Height; y++) {
                    ...
                    {
                        MapCells[x, y].FrontAnimationFrame =
                            (byte)( /*0x80 ^*/ MapCells[x, y].FrontAnimationFrame & 0x0F);
                    }
