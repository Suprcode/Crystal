    ...
    public class NpcObject : MapObject {
        ...
        public virtual void SetAction(bool randomStartFrame = false) {
            if(ActionFeed.Count == 0) {
                ...
                if(randomStartFrame) {
                    int frameIndex = new Random().Next(Frame.Count);
        ...
        public override void Draw() {
            ...
            Point offSet = BodyLibrary.GetOffSet(BaseIndex);
            Size size = BodyLibrary.GetSize(BaseIndex);

            int imageIndex = 981 + ((int)QuestIcon * 2) + QuestIndex;

            Libraries.Prguse.Draw(imageIndex, DrawLocation.Add(offSet).Add((size.Width / 2) - 28, -40), Color.White,
                false);
        }
        ...
        public void CreateNpcLabel(string word, int wordOrder) {
            ...
            for (int i = 0; i < LabelList.Count; i++) {
                if(LabelList[i].Text != word ||
                   LabelList[i].ForeColour != (wordOrder == 0 ? NameColour : Color.White)) {
                    continue;
                }
        ...
        public List<ClientQuestProgress> GetAvailableQuests(bool returnFirst = false) {
            List<ClientQuestProgress> quests = new();

            foreach(ClientQuestProgress q in User.CurrentQuests.Where(q =>
                        !User.CompletedQuests.Contains(q.QuestInfo.Index))) {
