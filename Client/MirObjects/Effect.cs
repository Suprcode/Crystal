...
namespace Client.MirObjects {
        ...
        public virtual void Draw() {
            ...
            if(Owner != null) {
                ...
                DrawLocation = new Point(
                    (Source.X - MapObject.User.Movement.X + MapControl.OffSetX) * MapControl.CellWidth,
    ...
    public class Missile : Effect {
        ...
        public Missile(MLibrary library, int baseIndex, int count, int duration, MapObject owner, Point target,
                       bool direction16 = true)
            : base(library, baseIndex, count, duration, owner) {
            Missiles.Add(this);
            Source = Owner.CurrentLocation;
            Destination = target;
            Direction = direction16
                ? MapControl.Direction16(Source, Destination)
                : (int)Functions.DirectionFromPoint(Source, Destination);
        }
    ...
    public class InterruptionEffect : Effect {
        public static List<InterruptionEffect> effectlist = new();
        private bool noProcess = false;

        public InterruptionEffect(MLibrary library, int baseIndex, int count, int duration, MapObject owner, bool blend,
                                  long starttime = 0)
            : base(library, baseIndex, count, duration, owner) {
            Repeat = true;
            Blend = blend;
    ...
    public class ElementsEffect : Effect {
        private int myType; //1 = green orb, 2 = blue orb, 3 = red orb, 4 = mixed orbs
        private long killAt; //holds the exp value for 4 orbs : kills all orbs when myType 4 is reached
        private bool loopit = false; //soundloop

        public ElementsEffect(MLibrary library, int baseIndex, int count, int duration, MapObject owner, bool blend,
                              int elementType, int killtime, bool loopon = false)
            : base(library, baseIndex, count, duration, owner) {
            Repeat = true;
            Blend = blend;
            myType = elementType;
    ...
    public class DelayedExplosionEffect : Effect {
        public static List<DelayedExplosionEffect> effectlist = new();
        public int stage;

        public DelayedExplosionEffect(MLibrary library, int baseIndex, int count, int duration, MapObject owner,
                                      bool blend, int Stage, long until)
            : base(library, baseIndex, count, duration, owner) {
    ...
    public class SpecialEffect : Effect {
        ...
        public SpecialEffect(MLibrary library, int baseIndex, int count, int duration, MapObject owner, bool blend,
                             bool drawBehind, uint type)
    ...
    public class BuffEffect : Effect {
        ...
        public BuffEffect(MLibrary library, int baseIndex, int count, int duration, MapObject owner, bool blend,
                          BuffType buffType)
    ...
    public class TrackableEffect : Effect {
        public static List<TrackableEffect> effectlist = new();
        public string EffectName = "default";

        public TrackableEffect(Effect baseEffect, string effName = "null")
            : base(baseEffect.Library, baseEffect.BaseIndex, baseEffect.Count, baseEffect.Duration, baseEffect.Owner,
                baseEffect.Start) {
    ...
    public class LightEffect : Effect {
        public LightEffect(int duration, MapObject owner, long starttime = 0, int lightDistance = 6,
        ...
        public LightEffect(int duration, Point source, long starttime = 0, int lightDistance = 6,
                           Color? lightColour = null)
