...
namespace Client.Forms {
    public partial class AMain : Form {
        ...
        public Queue<FileInformation> DownloadList = new();
        public List<Download> ActiveDownloads = new();
        ...
        private void CleanUp() {
            ...
            for (int i = 0; i < fileNames.Length; i++) {
                if(fileNames[i].StartsWith(".\\Screenshots\\")) {
                    continue;
                }

                fileName = Path.GetFileName(fileNames[i]);

                if(fileName == "Mir2Config.ini" || fileName == AppDomain.CurrentDomain.FriendlyName) {
                    continue;
                }
        ...
        public void DownloadFile(Download dl) {
            ...
            try {
                ...
                using(HttpClient client = new(progressMessageHandler)) {
                    ...
                    Task<HttpResponseMessage> task = Task.Run(() => client
                    ...
                    if(File.Exists(fileNameOut) && specialfiles.Contains(Path.GetExtension(fileNameOut).ToLower())) {
                        string oldFilename = Path.Combine(Path.GetDirectoryName(fileNameOut),
                            "Old__" + Path.GetFileName(fileNameOut));
        ...
        public byte[] Download(string fileName) {
            using(HttpClient client = new()) {
                ...
                Task<HttpResponseMessage> task = Task.Run(() => client
                    .GetAsync(new Uri(Settings.P_Host + Path.ChangeExtension(fileName, ".gz")),
        ...
        public static byte[] Decompress(byte[] raw) {
            using(GZipStream gStream = new(new MemoryStream(raw), CompressionMode.Decompress)) {
        ...
        private void AMain_Load(object sender, EventArgs e) {
            CoreWebView2Environment envir = CoreWebView2Environment.CreateAsync(null, Settings.ResourcePath).Result;
            ...
            Version_label.Text = string.Format("Build: {0}.{1}.{2}", Globals.ProductCodename,
        ...
        private void Main_browser_NavigationCompleted(object sender, CoreWebView2NavigationCompletedEventArgs e) {
            if(Main_browser.Source.AbsolutePath != "blank") {
        ...
        private void Launch() {
            ...
            Program.Form.Closed += (s, args) => Close();
            Program.Form.Show();
            Program.PForm.Hide();
        }
        ...
        private void Close_pb_MouseDown(object sender, MouseEventArgs e) {
            Close_pb.Image = Images.Cross_Pressed;
        }
        ...
        private void ProgressCurrent_pb_SizeChanged(object sender, EventArgs e) {
            ProgEnd_pb.Location = new Point(ProgressCurrent_pb.Location.X + ProgressCurrent_pb.Width,
                ProgressCurrent_pb.Location.Y);
        ...
        private void InterfaceTimer_Tick(object sender, EventArgs e) {
            try {
                if(Completed && ActiveDownloads.Count == 0) {
                    ...
                    if(ErrorFound) {
                        MessageBox.Show("One or more files failed to download, check Error.txt for details.",
                            "Failed to Download.");
                    }
                ...
                if(LabelSwitch) {
                    ActionLabel.Text = string.Format("{0} Files Remaining", _fileCount - _currentCount);
                } else {
                    ActionLabel.Text = string.Format("{0:#,##0}MB Remaining",
                        (_totalBytes - (_completedBytes + currentBytes)) / 1024 / 1024);
                }
                ...
                if(!(_completedBytes is 0 && currentBytes is 0 && _totalBytes is 0)) {
                    ...
                    TotalPercent_label.Text =
                        ((int)(100 * (_completedBytes + currentBytes) / _totalBytes)).ToString() + "%";
                }
            } catch {
                //to-do 
            }
        ...
        private void RepairOldFiles() {
            IEnumerable<string> files = Directory.GetFiles(Settings.P_Client, "*", SearchOption.AllDirectories)
                .Where(x => Path.GetFileName(x).StartsWith("Old__"));
        ...
        private void MoveOldFilesToCurrent() {
            ...
            foreach(string oldFilename in files) {
                string originalFilename = Path.Combine(Path.GetDirectoryName(oldFilename),
                    Path.GetFileName(oldFilename).Replace("Old__", ""));
