...
namespace Client.MirSounds {
    public static class SoundManager {
        ...
        private static List<KeyValuePair<long, int>> _delayList = new();
        private static Dictionary<int, CachedSound> _cachedOneShots = new();

        private static Dictionary<int, LoopProvider> _loopingSounds = new();
        ...
        public static void PlaySound(int index, bool loop = false, int delay = 0) {
            ...
            if(!loop) {
                ...
                LoopProvider sound = LoopProvider.TryCreate(index, _indexList[index], MusicVol, loop);
                if(sound != null) {
        ...
        public static void ProcessDelayedSounds() {
            ...
            List<KeyValuePair<long, int>> sounds = _delayList.Where(x => x.Key <= CMain.Time).ToList();

            foreach(KeyValuePair<long, int> sound in sounds) {
        ...
        private static void CheckSoundTimeOut() {
            if(CMain.Time >= _checkSoundTime) {
                ...
                foreach(int key in _cachedOneShots.Keys) {
