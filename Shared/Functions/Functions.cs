    ...
    public static class Functions {
        ...
        public static string PrintTimeSpanFromSeconds(double secs, bool accurate = true) {
            ...
            if(t.TotalMinutes < 1.0) {
                ...
                answer = accurate
                    ? string.Format("{0}m {1:D2}s", t.Minutes, t.Seconds)
                    : string.Format("{0}m", t.Minutes);
            } else if(t.TotalDays < 1.0) {
                answer = accurate
                    ? string.Format("{0}h {1:D2}m {2:D2}s", (int)t.Hours, t.Minutes, t.Seconds)
                    : string.Format("{0}h {1:D2}m", (int)t.TotalHours, t.Minutes);
            } else // more than 1 day
            {
                answer = accurate
                    ? string.Format("{0}d {1:D2}h {2:D2}m {3:D2}s", (int)t.Days, (int)t.Hours, t.Minutes, t.Seconds)
                    : string.Format("{0}d {1}h {2:D2}m", (int)t.TotalDays, (int)t.Hours, t.Minutes);
            }
        ...
        public static ItemInfo GetLevelBasedItem(ItemInfo Origin, ushort level, List<ItemInfo> ItemList) {
            ...
            for (int i = 0; i < ItemList.Count; i++) {
                ...
                if(info.Name.StartsWith(Origin.Name)) {
                    if(info.RequiredType == RequiredType.Level && info.RequiredAmount <= level &&
                       output.RequiredAmount < info.RequiredAmount && Origin.RequiredGender == info.RequiredGender) {
                        output = info;
                    }
                }
            }
        ...
        public static ItemInfo GetClassAndLevelBasedItem(ItemInfo Origin, MirClass job, ushort level,
...
#pragma warning restore SYSLIB0011
        ...
        public static List<List<T>> SplitList<T>(int width, List<T> originalList) {
            List<List<T>> _tempChunks = new List<List<T>>();
            ...
            if(width == 0) {
                ...
                for (int i = 0; i <= numberOfLists; i++) {
                    List<T> newChunk = originalList.Skip(i * width).Take(width).ToList();
