...
namespace AutoPatcherAdmin {
    public partial class AMain : Form {
        ...
        private void CleanUp() {
            ...
            string? rootPath = new Uri(Settings.Host).AbsolutePath;
            ...
            for (int i = 0; i < OldList.Count; i++) {
                ...
                bool compressed = OldList[i].Length != OldList[i].Compressed;

                try {
                    string? filename = OldList[i].FileName + (compressed ? ".gz" : "");
            ...
            for (int i = 0; i < OldList.Count; i++) {
                ...
                try {
                    ...
                    if(old.Compressed == old.Length) {
                        ...
                        string? compressedFilename = OldList[i].FileName + ".gz";
                        string? compressedFilePath = Path.Combine(rootPath, compressedFilename).Replace(@"\", "/");

                        string? uncompressedFilename = OldList[i].FileName;
                        string? uncompressedFilePath = Path.Combine(rootPath, compressedFilename).Replace(@"\", "/");
        ...
        private void OpenSession(Session session) {
            ...
            if(!Enum.TryParse(Settings.Protocol, true, out Protocol protocol)) {
        ...
        private void CreateTempUploadFiles(FileInformation info, byte[] raw) {
            ...
            string? tempSourceDir = Path.Combine(TempUploadDirectory, sourceDir);

            string? tempFilePath = Path.Combine(TempUploadDirectory, fileName).Replace(@"\", "/");
        ...
        private void UploadFiles(int uploadCount = 0) {
            ...
            TransferOperationResult? result =
                session.PutFilesToDirectory(TempUploadDirectory, rootPath, options: transferOptions);
            result.Check();
        ...
        private byte[] DownloadFile(string fileName) {
            ...
            try {
                ...
                TransferOperationResult? result = session.GetFiles(Path.Combine(rootPath, fileName),
                    Path.Combine(TempDownloadDirectory, fileName), options: transferOptions);
                result.Check();

                MemoryStream ms = new();
                using(FileStream fs = new(Path.Combine(TempDownloadDirectory, fileName), FileMode.Open,
                          FileAccess.Read)) {
                    fs.CopyTo(ms);
                }
        ...
        private void DownloadFiles() {
            ...
            TransferOperationResult? result =
                session.GetFilesToDirectory(rootPath, TempDownloadDirectory, options: transferOptions);
            result.Check();
        ...
        private void MoveTempDownloadedFiles() {
            for (int i = 0; i < OldList.Count; i++) {
                FileInformation? info = OldList[i];
                bool compressed = OldList[i].Length != OldList[i].Compressed;
                string? filename = OldList[i].FileName + (compressed ? ".gz" : "");

                string? currentPath = Path.Combine(TempDownloadDirectory, filename);

                string? relativeDestDir = Path.GetDirectoryName(info.FileName);
                string? destDir = Path.Combine(Settings.Client, relativeDestDir);
                string? destFilename = Path.Combine(Settings.Client, info.FileName);
