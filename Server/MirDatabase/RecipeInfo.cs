...
namespace Server.Library.MirDatabase {
    public class RecipeInfo {
        ...
        public List<int> RequiredFlag = new();
        public ushort? RequiredLevel = null;
        public List<int> RequiredQuest = new();
        public List<MirClass> RequiredClass = new();
            ...
            for (int i = 0; i < lines.Count; i++) {
                ...
                switch (mode) {
                    ...
                    case "tools": {
                        ...
                        if(info == null) {
                            MessageQueue.Enqueue(string.Format("Could not find Tool: {0}, Recipe: {1}", lines[i],
                                recipe));
                            continue;
                        }
                    ...
                    case "ingredients": {
                        ...
                        if(info == null) {
                            MessageQueue.Enqueue(string.Format("Could not find Ingredient: {0}, Recipe: {1}", lines[i],
                                recipe));
                            continue;
                        }
                    ...
                    case "criteria": {
                        ...
                        try {
                            ...
                            MessageQueue.Enqueue(string.Format("Could not parse option: {0}, Value: {1}", data[0],
                                data[1]));
                            continue;
                        }
                    }
        ...
        public bool CanCraft(PlayerObject player) {
            ...
            if(RequiredFlag.Count > 0) {
                foreach(int flag in RequiredFlag) {
            ...
            if(RequiredQuest.Count > 0) {
                foreach(int quest in RequiredQuest) {
        ...
        public ClientRecipeInfo CreateClientRecipeInfo() {
            ClientRecipeInfo clientInfo = new() {
                Gold = Gold,
                Chance = Chance,
                Item = Item.Clone(),
