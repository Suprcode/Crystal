...
namespace Server.Library.MirEnvir {
    public class Dragon {
                ...
                if(LinkedMonster.Spawn(map, Info.Location)) {
                    ...
                    if(bodyinfo != null) {
                        ...
                        for (int i = 0; i <= BodyLocations.Length - 1; i++) {
                            ...
                            spawnlocation = new Point(LinkedMonster.CurrentLocation.X + BodyLocations[i].X,
                                LinkedMonster.CurrentLocation.Y + BodyLocations[i].Y);
                            if(bodymob != null) {
                    ...
                    DropArea = new Rectangle(Info.DropAreaTop.X, Info.DropAreaTop.Y,
                        Info.DropAreaBottom.X - Info.DropAreaTop.X, Info.DropAreaBottom.Y - Info.DropAreaTop.Y);
                    Loaded = true;
                    return true;
                }
            } catch(Exception ex) {
        ...
        public void Drop(byte level) {
            if(level > Info.Drops.Length) {
                return;
            }

            if(Info.Drops[level - 1] == null) {
                return;
            }
            ...
            List<DragonInfo.DropInfo> droplist = new(Info.Drops[level - 1]);
        ...
        protected bool DropItem(UserItem item) {
            Point droplocation = new(DropArea.Left + (DropArea.Width / 2), DropArea.Top);
            ItemObject ob = new(LinkedMonster, item, droplocation) {
        ...
        protected bool DropGold(uint gold) {
            ...
            Point droplocation = new(DropArea.Left + (DropArea.Width / 2), DropArea.Top);
            ItemObject ob = new(LinkedMonster, gold, droplocation) {
                Owner = LinkedMonster.EXPOwner,
                OwnerTime = Envir.Time + Settings.Minute
            };
