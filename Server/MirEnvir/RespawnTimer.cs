namespace Server.Library.MirEnvir {
    ...
    public class RespawnTimer {
        ...
        public List<RespawnTickOption> Respawn = new();

        public RespawnTimer() {
            RespawnTickOption Option = new() { UserCount = 0, DelayLoss = 1.0 };
            Respawn.Add(Option);
            //LastTick = Envir.Time;
        }
        ...
        public RespawnTimer(BinaryReader reader) {
            ...
            for (int i = 0; i < Optioncount; i++) {
                RespawnTickOption Option = new(reader);
                Respawn.Add(Option);
            }
        ...
        public void Process() {
            ...
            if(Envir.Time >= LastTick + CurrentDelay) {
                CurrentTickcounter++;
                if(CurrentTickcounter ==
                   long.MaxValue) //by using long instead of ulong here you basicaly have a huge safe zone on the respawn ticks of mobs
                {
        ...
        public void GetTickSpeed() {
            if(LastUsercount == Envir.PlayerCount) {
                return;
            }
