    ...
    public class SpellObject : MapObject {
        ...
        public override void Process() {
            ...
            if(Envir.Time > ExpireTime ||
               ((Spell == Spell.FireWall || Spell == Spell.Portal || Spell == Spell.ExplosiveTrap ||
                 Spell == Spell.Reincarnation || Spell == Spell.HealingCircle) && Caster == null) ||
               (Spell == Spell.TrapHexagon && Target != null) || (Spell == Spell.Trap && Target != null)) {
            ...
            if(Spell == Spell.ExplosiveTrap && Caster != null &&
               !Functions.InRange(CurrentLocation, Caster.CurrentLocation, 15)) {
                CurrentMap.RemoveObject(this);
                Despawn();
                return;
            }
        ...
        public void ProcessSpell(MapObject ob) {
            ...
            switch (Spell) {
                ...
                {
                    if(ob.Race != ObjectType.Player && (ob.Race != ObjectType.Monster || ob.Master == null ||
                                                        ob.Master.Race != ObjectType.Player)) {
                        return;
                    }
                ...
                case Spell.Blizzard: {
                    ...
                    if(Caster != null && ((HumanObject)Caster).ActiveBlizzard == false) {
                        return;
                    }
                ...
                case Spell.TreeQueenGroundRoots: {
                    ...
                    if(Envir.Random.Next(3) > 0) {
                        ob.ApplyPoison(new Poison { PType = PoisonType.Paralysis, Duration = 5, TickSpeed = 1000 },
                            this);
                    }
                }
                    break;
                ...
                case Spell.Portal: {
                    ...
                    if(Caster != ob && (Caster == null || Caster.GroupMembers == null ||
                                        !Caster.GroupMembers.Contains((HumanObject)ob))) {
                        return;
                    }

                    SpellObject portal = Envir.Spells.SingleOrDefault(ob => ob != this && ob.Node != null
                ...
                case Spell.HealingCircle:
                    ...
                    if(Caster == ob || (Caster.GroupMembers != null && Caster.GroupMembers.Contains(ob)) ||
                       (ob.Master != null && Caster.GroupMembers != null && Caster.GroupMembers.Contains(ob.Master))) {
                        if(ob.Dead || ob.HealAmount != 0 || ob.PercentHealth == 100) {
                            return;
                        }
        ...
        public override void Despawn() {
            ...
            if(Spell == Spell.ExplosiveTrap && Caster != null) {
                IEnumerable<SpellObject> linkedTraps = CurrentMap.GetSpellObjects(Spell.ExplosiveTrap, Owner)
                    .Where(x => x.ExplosiveTrapID == ExplosiveTrapID);

                foreach(SpellObject trap in linkedTraps) {
                    trap.DetonateTrapNow();
                }
            }

            if(Spell == Spell.Portal && Caster != null) {
                SpellObject portal = Envir.Spells.SingleOrDefault(ob => ob.Node != null && ob != this
                    && ob.Spell == Spell.Portal
                    && ob.Caster == Caster);
