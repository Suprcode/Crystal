...
namespace Server.MirObjects
{
    public class ConquestObject
    {
        ...
        public List<int> WarOnlineList = new List<int>();
            ...
            for (int i = 0; i < tempGuild.Ranks.Count; i++)
                for (int j = 0; j < tempGuild.Ranks[i].Members.Count; j++)
                {
                    ...
                    if (player != null)
                    {
                        ...
                        player.RemoveConquestItems(Info.Index);
                    }
                }
        }
        ...
        public void StartWar(ConquestGame type)
        {
            ...
            for (int i = 0; i < Envir.Connections.Count; i++)
                if (Envir.Connections[i].Player != null)
                {
                    PlayerObject Player = Envir.Connections[i].Player;
                    Player.ReceiveChat("Wall Started !!! ", ChatType.System);

                    WarOnlineList.Add(Player.Info.Index);
                }
        }
        ...
        public void EndWar(ConquestGame type)
        {
            ...
            for (int i = 0; i < Envir.CharacterList.Count; i++)
            {
                ...
                if (character.Player != null)
                {
                    ...
                    Player.ReceiveChat("Wall End !!! ", ChatType.System);

                    if (Player.MyGuild == null) continue;

                    Player.Enqueue(new ServerPackets.ConquestChanged { Conquest = (Player.MyGuild.Conquest != null ? Player.MyGuild.Conquest.Info.Index : -1) });

                    if (Guild != null && Guild.NewMembersSinceConquest.Contains(Player))
                        continue;
                }

                if (Owner > -1 && character.GuildIndex == Owner)
                {
            ...
            if (Guild != null)
                Guild.NewMembersSinceConquest.Clear();
        ...
        private void GiveReward(CharacterInfo character)
        {
            MessageQueue.Enqueue($"Conquest - {Info.Index} - {character.Name} - Giving out reward");
            //MessageQueue.EnqueueDebugging(string.Format("{0}: Giving out reward. {1}", Info.Index, character.Name));

            foreach (DropInfo drop in Envir.ConquestDrops.Where(x => x.Type == Info.Index))
            {
                ...
                if (drop.Gold > 0)
                {
                    ...
                    if (character.Player != null)
                    {
                        character.Player.GainGold((uint)gold);
                    }
                    else
                    {
                        var mail = new MailInfo(character.Index, true)
                        {
                            Sender = character.Name,
                            Message = "You have received a reward from the Conquest outcome",
                            Gold = (uint)gold,
                            CanReply = false
                        };
                ...
                else
                {
                    ...
                    if (character.Player != null)
                    {
                        if (character.Player.CheckGroupQuestItem(item))
                        {
                            continue;
                        }
                    }
                    ...
                    if (character.Player != null)
                    {
                        character.Player.GainItem(item);
                    }
                    else
                    {
                        var mail = new MailInfo(character.Index, true)
