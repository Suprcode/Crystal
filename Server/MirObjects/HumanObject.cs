...
namespace Server.Library.MirObjects {
    public class HumanObject : MapObject {
        ...
        public MountInfo Mount => Info.Mount;

        public Reporting Report;

        public virtual bool CanMove => !Dead && Envir.Time >= ActionTime &&
        ...
        public virtual bool CanWalk => !Dead && Envir.Time >= ActionTime && !InTrapRock &&
                                       !CurrentPoison.HasFlag(PoisonType.Paralysis) &&
                                       !CurrentPoison.HasFlag(PoisonType.LRParalysis) &&
                                       !CurrentPoison.HasFlag(PoisonType.Frozen);

        public virtual bool CanRun => !Dead && Envir.Time >= ActionTime && (_stepCounter > 0 || FastRun) &&
                                      (!Sneaking || ActiveSwiftFeet) && CurrentBagWeight <= Stats[Stat.BagWeight] &&
                                      !CurrentPoison.HasFlag(PoisonType.Paralysis) &&
                                      !CurrentPoison.HasFlag(PoisonType.LRParalysis) &&
                                      !CurrentPoison.HasFlag(PoisonType.Frozen);

        public virtual bool CanAttack => !Dead && Envir.Time >= ActionTime && Envir.Time >= AttackTime &&
                                         !CurrentPoison.HasFlag(PoisonType.Paralysis) &&
                                         !CurrentPoison.HasFlag(PoisonType.LRParalysis) &&
                                         !CurrentPoison.HasFlag(PoisonType.Frozen) &&
                                         !CurrentPoison.HasFlag(PoisonType.Dazed) && Mount.CanAttack;

        public bool CanRegen => Envir.Time >= RegenTime;

        protected virtual bool CanCast =>
            !Dead && Envir.Time >= ActionTime && Envir.Time >= SpellTime && !CurrentPoison.HasFlag(PoisonType.Stun) &&
        ...
        public List<ItemSets> ItemSets = new();
        public List<EquipmentSlot> MirSet = new();
        ...
        public override bool Blocking => !Dead && !Observer;
        public HumanObject() { }
        ...
        public override void Process() {
            if((Race == ObjectType.Player && Connection == null) || Node == null || Info == null) {
                return;
            }

            if(CellTime + 700 < Envir.Time) {
                _stepCounter = 0;
            }
            ...
            if(FlamingSword && Envir.Time >= FlamingSwordTime) {
                ...
                Enqueue(new ServerPacket.SpellToggle
                    { ObjectID = ObjectID, Spell = Spell.FlamingSword, CanUse = false });
            }
            ...
            if((ReincarnationReady || ActiveReincarnation) &&
               (ReincarnationTarget == null || !ReincarnationTarget.Dead)) {
                ReincarnationReady = false;
                ActiveReincarnation = false;
                ReincarnationTarget = null;
            }
            ...
            if(Stacking && Envir.Time > StackingTime) {
                ...
                for (int i = 0; i < 8; i++) {
                    if(Pushed(this, (MirDirection)i, 1) == 1) {
                        break;
                    }
                }
            }
            ...
            if(Envir.Time > DuraTime) {
                ...
                for (int i = 0; i < Info.Equipment.Length; i++) {
                    ...
                    if(item == null || !item.DuraChanged) {
                        continue; // || item.Info.Type == ItemType.Mount
                    }
        ...
        public override void OnSafeZoneChanged() {
            ...
            for (int i = 0; i < Buffs.Count; i++) {
                if(Buffs[i].ObjectID == 0) {
                    continue;
                }

                if(!Buffs[i].Properties.HasFlag(BuffProperty.PauseInSafeZone)) {
                    continue;
                }
        ...
        protected virtual void ProcessBuffs() {
            ...
            for (int i = Buffs.Count - 1; i >= 0; i--) {
                ...
                switch (buff.Type) {
                    ...
                    case BuffType.ClearRing:
                        clearRing = true;
                        if(!SpecialMode.HasFlag(SpecialItemMode.ClearRing)) {
                ...
                if(buff.NextTime > Envir.Time) {
                    continue;
                }

                if(!buff.Paused && buff.StackType != BuffStackType.Infinite) {
                    long change = Envir.Time - buff.LastTime;
                ...
                if((buff.ExpireTime > 0 || buff.StackType == BuffStackType.Infinite) && !buff.FlagForRemoval) {
                    continue;
                }
                ...
                switch (buff.Type) {
                    ...
                    case BuffType.ClearRing:
                        if(!HasAnyBuffs(buff.Type, BuffType.ClearRing, BuffType.Hiding, BuffType.MoonLight,
                               BuffType.DarkBody)) {
                            Hidden = false;
                        }
                    ...
                    case BuffType.MagicShield:
                        CurrentMap.Broadcast(
                            new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.MagicShieldDown },
                            CurrentLocation);
                        break;
                    case BuffType.ElementalBarrier:
                        CurrentMap.Broadcast(
                            new ServerPacket.ObjectEffect
                                { ObjectID = ObjectID, Effect = SpellEffect.ElementalBarrierDown }, CurrentLocation);
                        break;
                }
            ...
            if(Info.Mentor != 0 && !mentor) {
                ...
                if(partnerP != null) {
                    if(Info.IsMentor) {
                        AddBuff(BuffType.Mentor, partnerP, 0,
                            new Stats { [Stat.MentorDamageRatePercent] = Settings.MentorDamageBoost });
                    } else {
                        AddBuff(BuffType.Mentee, partnerP, 0,
                            new Stats { [Stat.MentorExpRatePercent] = Settings.MentorExpBoost });
                    }
                }
            }
            ...
            if(Info.Married != 0 && !lover) {
                ...
                if(loverP != null) {
                    AddBuff(BuffType.Lover, loverP, 0,
                        new Stats { [Stat.LoverExpRatePercent] = Settings.LoverEXPBonus });
                }
            }
        ...
        private void ProcessRegen() {
            ...
            if(CanRegen) {
                ...
                if(HP < Stats[Stat.HP]) {
                    ...
                    healthRegen +=
                        (int)(healthRegen * ((double)Stats[Stat.HealthRecovery] / Settings.HealthRegenWeight));
                }
                    ...
                    if(poison.PType == PoisonType.Green || poison.PType == PoisonType.Bleeding) {
                        ...
                        if(poison.PType == PoisonType.Bleeding) {
                            Broadcast(new ServerPacket.ObjectEffect
                                { ObjectID = ObjectID, Effect = SpellEffect.Bleeding, EffectType = 0 });
                        }
        ...
        private bool ProcessDelayedExplosion(Poison poison) {
            ...
            if(ExplosionInflictedStage == 0) {
                Enqueue(new ServerPacket.ObjectEffect
                    { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion, EffectType = 0 });
                Broadcast(new ServerPacket.ObjectEffect
                    { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion, EffectType = 0 });
                return true;
            }
            ...
            if(ExplosionInflictedStage == 1) {
                ...
                Enqueue(new ServerPacket.ObjectEffect
                    { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion, EffectType = 1 });
                Broadcast(new ServerPacket.ObjectEffect
                    { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion, EffectType = 1 });
                return true;
            }
            ...
            if(ExplosionInflictedStage == 2) {
                Enqueue(new ServerPacket.ObjectEffect
                    { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion, EffectType = 2 });
                Broadcast(new ServerPacket.ObjectEffect
                    { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion, EffectType = 2 });
                if(poison.Owner != null) {
                    switch (poison.Owner.Race) {
                        case ObjectType.Player:
                            ...
                            DelayedAction action = new(DelayedType.Magic, Envir.Time, poison.Owner,
                                caster.GetMagic(Spell.DelayedExplosion), poison.Value, CurrentLocation);
                            CurrentMap.ActionList.Add(action);
                            break;
                        case ObjectType.Monster
                            : //this is in place so it could be used by mobs if one day someone chooses to
        ...
        private void ProcessItems() {
            ...
            for (int i = 0; i < Info.AccountInfo.Storage.Length; i++) {
                UserItem item = Info.AccountInfo.Storage[i];
                if(item?.ExpireInfo?.ExpiryDate <= Envir.Now) {
        ...
        protected void UpdateGMBuff() {
            ...
            if(GMGameMaster) {
                options |= GMOptions.GameMaster;
            }

            if(GMNeverDie) {
                options |= GMOptions.Superman;
            }

            if(Observer) {
                options |= GMOptions.Observer;
            }
        ...
        public void GetMinePayout(MineSet Mine) {
            if(Mine.Drops == null || Mine.Drops.Count == 0) {
                return;
            }
            ...
            for (int i = 0; i < Mine.Drops.Count; i++) {
                ...
                if(Drop.MinSlot <= Slot && Drop.MaxSlot >= Slot && Drop.Item != null) {
                    ItemInfo info = Envir.GetItemInfo(Drop.Item.Index);

                    UserItem item = Envir.CreateDropItem(info);
                    if(item.Info.Type == ItemType.Ore) {
                        item.CurrentDura = (ushort)Math.Min(ushort.MaxValue,
                            (Drop.MinDura + Envir.Random.Next(Math.Max(0, Drop.MaxDura - Drop.MinDura))) * 1000);
                        if(Drop.BonusChance > 0 && Envir.Random.Next(100) <= Drop.BonusChance) {
                            item.CurrentDura = (ushort)Math.Min(ushort.MaxValue,
                                item.CurrentDura + (Envir.Random.Next(Drop.MaxBonusDura) * 1000));
                        }
                    }

                    if(CheckGroupQuestItem(item)) {
                        continue;
                    }
            ...
            for (int i = 0; i < Info.Equipment.Length; i++) {
                if(Info.Equipment[i] != null && Info.Equipment[i].Slots.Length > 0) {
                    for (int j = 0; j < Info.Equipment[i].Slots.Length; j++) {
                        if(Info.Equipment[i].Slots[j] != item) {
                            continue;
                        }
        ...
        protected bool DropItem(UserItem item, int range = 1, bool DeathDrop = false) {
            ItemObject ob = new(this, item, DeathDrop);
            ...
            for (int i = 0; i < Info.Inventory.Length; i++) {
                ...
                if(item.Count > 1) {
                    ...
                    if(Envir.ReturnRentalItem(item, item.RentalInformation?.OwnerName, Info)) {
                        ...
                        ReceiveChat(
                            $"You died and {item.Info.FriendlyName} has been returned to has been returned to it's owner.",
                            ChatType.Hint);
                        Report?.ItemMailed(item, 1, 1);
        ...
        protected bool CorrectStartItem(ItemInfo info) {
            switch (Class) {
                ...
                case MirClass.Taoist:
                    ...
                    break;
                case MirClass.Assassin:
                    if(!info.RequiredClass.HasFlag(RequiredClass.Assassin)) {
                        return false;
                    }

                    break;
                case MirClass.Archer:
                    if(!info.RequiredClass.HasFlag(RequiredClass.Archer)) {
                        return false;
                    }
            ...
            switch (Gender) {
                case MirGender.Male:
                    if(!info.RequiredGender.HasFlag(RequiredGender.Male)) {
                        return false;
                    }

                    break;
                case MirGender.Female:
                    if(!info.RequiredGender.HasFlag(RequiredGender.Female)) {
                        return false;
                    }
        ...
        public void SetLevelEffects() {
            ...
            if(Info.Flags[990]) {
                LevelEffects |= LevelEffects.Mist;
            }

            if(Info.Flags[991]) {
                LevelEffects |= LevelEffects.RedDragon;
            }

            if(Info.Flags[992]) {
                LevelEffects |= LevelEffects.BlueDragon;
            }

            if(Info.Flags[993]) {
                LevelEffects |= LevelEffects.Rebirth1;
            }

            if(Info.Flags[994]) {
                LevelEffects |= LevelEffects.Rebirth2;
            }

            if(Info.Flags[995]) {
                LevelEffects |= LevelEffects.Rebirth3;
            }

            if(Info.Flags[996]) {
                LevelEffects |= LevelEffects.NewBlue;
            }

            if(Info.Flags[997]) {
                LevelEffects |= LevelEffects.YellowDragon;
            }

            if(Info.Flags[998]) {
                LevelEffects |= LevelEffects.Phoenix;
            }
        }
        ...
        protected void RefreshLevelStats() {
            ...
            foreach(BaseStat stat in Settings.ClassBaseStats[(byte)Class].Stats) {
        ...
        private void RefreshEquipmentStats() {
            ...
            List<string> skillsToRemove = new List<string>
                { Settings.HealRing, Settings.FireRing, Settings.BlinkSkill };
            ...
            for (int i = 0; i < Info.Equipment.Length; i++) {
                ...
                if(temp.CurrentDura == 0 && temp.Info.Durability > 0) {
                    continue;
                }
                ...
                ItemSets itemSet = ItemSets.Where(set =>
                    set.Set == realItem.Set && !set.Type.Contains(realItem.Type) && !set.SetComplete).FirstOrDefault();
                ...
                if(itemSet != null) {
                    ...
                    ItemSets.Add(new ItemSets
                        { Count = 1, Set = realItem.Set, Type = new List<ItemType> { realItem.Type } });
                }
            ...
            if(OldLooks_Armour != Looks_Armour || OldLooks_Weapon != Looks_Weapon ||
               OldLooks_WeaponEffect != Looks_WeaponEffect || OldLooks_Wings != Looks_Wings || OldLight != Light) {
            ...
            for (int j = 0; j < equipItem.Slots.Length; j++) {
                ...
                if(RealItem.Unique != SpecialItemMode.None) {
                    ...
                    if(RealItem.Unique.HasFlag(SpecialItemMode.Skill)) {
                        Stats[Stat.SkillGainMultiplier] = 3;
                    }
        ...
        private void RefreshItemSetStats() {
            foreach(ItemSets s in ItemSets) {
                if(s.Set == ItemSet.Smash &&
                   ((s.Type.Contains(ItemType.Ring) && s.Type.Contains(ItemType.Bracelet)) ||
                    (s.Type.Contains(ItemType.Ring) && s.Type.Contains(ItemType.Necklace)) ||
                    (s.Type.Contains(ItemType.Bracelet) && s.Type.Contains(ItemType.Necklace)))) {
                ...
                if(s.Set == ItemSet.DarkGhost && s.Type.Contains(ItemType.Necklace) &&
                   s.Type.Contains(ItemType.Bracelet)) {
                    Stats[Stat.HP] += 25;
                }
        ...
        private void RefreshMirSetStats() {
            ...
            if(MirSet.Contains(EquipmentSlot.RingL) | MirSet.Contains(EquipmentSlot.RingR) &&
               MirSet.Contains(EquipmentSlot.BraceletL) | MirSet.Contains(EquipmentSlot.BraceletR) &&
               MirSet.Contains(EquipmentSlot.Necklace)) {
                Stats[Stat.MaxMAC] += 1;
                Stats[Stat.MaxAC] += 1;
            ...
            if(MirSet.Contains(EquipmentSlot.Armour) && MirSet.Contains(EquipmentSlot.Helmet) &&
               MirSet.Contains(EquipmentSlot.Weapon)) {
                Stats[Stat.MaxDC] += 2;
                Stats[Stat.MaxMC] += 1;
                Stats[Stat.MaxSC] += 1;
                Stats[Stat.Agility] += 1;
            ...
            if(MirSet.Contains(EquipmentSlot.Armour) && MirSet.Contains(EquipmentSlot.Boots) &&
            ...
            if(MirSet.Contains(EquipmentSlot.Armour) && MirSet.Contains(EquipmentSlot.Boots) &&
               MirSet.Contains(EquipmentSlot.Belt) && MirSet.Contains(EquipmentSlot.Helmet) &&
               MirSet.Contains(EquipmentSlot.Weapon)) {
                Stats[Stat.MinDC] += 1;
                Stats[Stat.MaxDC] += 1;
                Stats[Stat.MinMC] += 1;
        ...
        public void RefreshStatCaps() {
            foreach(KeyValuePair<Stat, int> cap in Settings.ClassBaseStats[(byte)Class].Caps.Values) {
        ...
        private void AddTempSkills(IEnumerable<string> skillsToAdd) {
            foreach(string skill in skillsToAdd) {
                ...
                if(!Enum.TryParse(skill, out spelltype)) {
                    return;
                }

                for (int i = Info.Magics.Count - 1; i >= 0; i--) {
                    if(Info.Magics[i].Spell == spelltype) {
                        hasSkill = true;
                    }
                }
                ...
                UserMagic magic = new UserMagic(spelltype) { IsTempSpell = true };
        ...
        private void RemoveTempSkills(IEnumerable<string> skillsToRemove) {
            foreach(string skill in skillsToRemove) {
                if(!Enum.TryParse(skill, out Spell spelltype)) {
                    return;
                }

                for (int i = Info.Magics.Count - 1; i >= 0; i--) {
                    if(!Info.Magics[i].IsTempSpell || Info.Magics[i].Spell != spelltype) {
                        continue;
                    }
        ...
        public void BroadcastColourChange() {
            ...
            for (int i = CurrentMap.Players.Count - 1; i >= 0; i--) {
                ...
                if(Functions.InRange(CurrentLocation, player.CurrentLocation, Globals.DataRange)) {
                    player.Enqueue(new ServerPacket.ObjectColourChanged
                        { ObjectID = ObjectID, NameColour = player.GetNameColour(this) });
                }
            }
        }
        ...
        public int ReduceExp(uint amount, uint targetLevel) {
            ...
            if(Level < targetLevel + 10 || !Settings.ExpMobLevelDifference) {
                ...
                expPoint = (int)amount -
                           (int)Math.Round(Math.Max(amount / 15, 1) * ((double)Level - (targetLevel + 10)));
            }
            ...
            if(cell.Objects != null) {
                for (int i = 0; i < cell.Objects.Count; i++) {
                    ...
                    if(ob.Race == ObjectType.Merchant && Race == ObjectType.Player) {
                        ...
                        if(!Npc.Visible || !Npc.VisibleLog[Info.Index]) {
                            continue;
                        }
                    } else if(!ob.Blocking || (CheckCellTime && ob.CellTime >= Envir.Time)) {
                        continue;
                    }
        ...
        public bool Run(MirDirection dir) {
            ...
            int steps = RidingMount || (ActiveSwiftFeet && !Sneaking) ? 3 : 2;
        ...
        public override int Pushed(MapObject pusher, MirDirection dir, int distance) {
            ...
            for (int i = 0; i < distance; i++) {
                ...
                Broadcast(new ServerPacket.ObjectPushed
                    { ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation });
                GetPlayerLocation();
                result++;
            }
        ...
        public void GetPlayerLocation() {
            ...
            for (int i = 0; i < GroupMembers.Count; i++) {
                ...
                if(member.CurrentMap.Info.BigMap <= 0) {
                    continue;
                }

                member.Enqueue(new ServerPacket.SendMemberLocation
                    { MemberName = Name, MemberLocation = CurrentLocation });
                Enqueue(new ServerPacket.SendMemberLocation
                    { MemberName = member.Name, MemberLocation = member.CurrentLocation });
            }
        ...
        public void RangeAttack(MirDirection dir, Point location, uint targetID) {
            ...
            ItemInfo RealItem = Functions.GetRealItem(Info.Equipment[(int)EquipmentSlot.Weapon].Info, Info.Level,
                Info.Class, Envir.ItemInfoList);

            if(RealItem.Shape / Globals.ClassWeaponCount != 2) {
                return;
            }

            if(Functions.InRange(CurrentLocation, location, Globals.MaxAttackRange) == false) {
                return;
            }
            ...
            if(target != null) {
                ...
                int chanceToHit = (100 + Settings.RangeAccuracyBonus - (100 / Globals.MaxAttackRange * distance)) *
                                  (focus ? 2 : 1);
                ...
                if(Envir.Random.Next(100) < chanceToHit) {
                    ...
                    DelayedAction action = new(DelayedType.Damage, Envir.Time + delay, target, damage,
                        DefenceType.ACAgility, true);
                    ActionList.Add(action);
                } else {
                    DelayedAction action = new(DelayedType.DamageIndicator, Envir.Time + delay, target,
                        DamageType.Miss);
                    ActionList.Add(action);
                }
            } else {
            ...
            Broadcast(new ServerPacket.ObjectRangeAttack {
                ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, TargetID = targetID,
                Target = location, Spell = spell
            });
        ...
        public void Attack(MirDirection dir, Spell spell) {
            ...
            switch (spell) {
                ...
                case Spell.TwinDrakeBlade:
                    magic = GetMagic(spell);
                    if(!TwinDrakeBlade || magic == null ||
                       magic.Info.BaseCost + (magic.Level * magic.Info.LevelCost) > MP) {
                        spell = Spell.None;
                        break;
                    }
            ...
            if(!Slaying) {
                ...
                if(magic != null && Envir.Random.Next(12) <= magic.Level) {
                    Slaying = true;
                    Enqueue(new ServerPacket.SpellToggle
                        { ObjectID = ObjectID, Spell = Spell.Slaying, CanUse = Slaying });
                }
            }
            ...
            if(RidingMount) {
                DecreaseMountLoyalty(3);
            }

            Enqueue(new ServerPacket.UserLocation { Direction = Direction, Location = CurrentLocation });
            Broadcast(new ServerPacket.ObjectAttack {
                ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Spell = spell, Level = level
            });
            ...
            for (int i = 0; i < cell.Objects.Count; i++) {
                ...
                if(magic != null) {
                    ...
                    if(MPEater) {
                        ...
                        ServerPacket.ObjectEffect p = new ServerPacket.ObjectEffect
                            { ObjectID = ob.ObjectID, Effect = SpellEffect.MPEater, EffectType = ObjectID };
                        ...
                        MPEaterCount = 0;
                        MPEater = false;
                    } else if(!MPEater && 100 <= MPEaterCount) {
                        MPEater = true;
                    }
                }
                ...
                if(magic != null) {
                    ...
                    if(Hemorrhage) {
                        damageFinal = magic.GetDamage(damageBase);
                        LevelMagic(magic);
                        ServerPacket.ObjectEffect ef = new ServerPacket.ObjectEffect
                            { ObjectID = ob.ObjectID, Effect = SpellEffect.Hemorrhage };

                        CurrentMap.Broadcast(ef, ob.CurrentLocation);

                        if(ob == null || ob.Node == null) {
                            continue;
                        }
                        ...
                        Hemorrhage = false;
                    } else if(!Hemorrhage && 55 <= HemorrhageAttackCount) {
                        Hemorrhage = true;
                    }
                }
                ...
                switch (spell) {
                    ...
                    case Spell.DoubleSlash:
                        ...
                        if(defence == DefenceType.ACAgility) {
                            defence = DefenceType.MACAgility;
                        }

                        action = new DelayedAction(DelayedType.Damage, Envir.Time + 400, ob, damageFinal,
                            DefenceType.Agility, false);
                        ActionList.Add(action);
                        LevelMagic(magic);
                        break;
                    case Spell.Thrusting:
            ...
            if(Mined) {
                ...
                if(!Info.Equipment[(int)EquipmentSlot.Weapon].Info.CanMine) {
                    return;
                }

                if(Info.Equipment[(int)EquipmentSlot.Weapon].CurrentDura <=
                   0) //Stop dura 0 working. use below if you wish to break the item.
                    /*{
                ...
                /*}*/
                if(CurrentMap.Mine == null) {
                    return;
                }

                MineSpot Mine = CurrentMap.Mine[target.X, target.Y];
                if(Mine == null || Mine.Mine == null) {
                    return;
                }

                if(Mine.StonesLeft > 0) {
                    Mine.StonesLeft--;
                    if(Envir.Random.Next(100) < Mine.Mine.HitRate +
                       (Info.Equipment[(int)EquipmentSlot.Weapon].GetTotal(Stat.Accuracy) * 10)) {
                        ...
                        for (int i = 0; i < minecell.Objects.Count; i++) {
                            if(minecell.Objects[i].Race != ObjectType.Spell) {
                                continue;
                            }

                            SpellObject ob = (SpellObject)minecell.Objects[i];

                            if(ob.Spell != Spell.Rubble) {
                                continue;
                            }
        ...
        public void Magic(Spell spell, MirDirection dir, uint targetID, Point location, bool spellTargetLock = false) {
            ...
            if(location.X != 0 && location.Y != 0 && magic.Info.Range != 0 &&
               Functions.InRange(CurrentLocation, location, magic.Info.Range) == false) {
                return;
            }
            ...
            switch (spell) {
                ...
                case Spell.SoulFireBall:
                    if(!SoulFireball(target, magic, out cast)) {
                        targetID = 0;
                    }
                ...
                case Spell.MassHiding:
                    MassHiding(magic, spellTargetLock ? target != null ? target.CurrentLocation : location : location,
                        out cast);
                    break;
                case Spell.SoulShield:
                case Spell.BlessedArmour:
                    SoulShield(magic, spellTargetLock ? target != null ? target.CurrentLocation : location : location,
                        out cast);
                    break;
                case Spell.FireWall:
                ...
                case Spell.MagicShield:
                    ActionList.Add(new DelayedAction(DelayedType.Magic, Envir.Time + 500, magic,
                        magic.GetPower(GetAttackPower(Stats[Stat.MinMC], Stats[Stat.MaxMC]) + 15)));
                    break;
                ...
                case Spell.BattleCry:
                    CurrentMap.ActionList.Add(new DelayedAction(DelayedType.Magic, Envir.Time + 500, this, magic,
                        CurrentLocation));
                    break;
                ...
                case Spell.Mirroring:
                    ...
                    break;
                case Spell.Blizzard:
                    Blizzard(magic, spellTargetLock ? target != null ? target.CurrentLocation : location : location,
                        out cast);
                    break;
                case Spell.MeteorStrike:
                    MeteorStrike(magic, spellTargetLock ? target != null ? target.CurrentLocation : location : location,
                        out cast);
                    break;
                ...
                case Spell.TrapHexagon:
                    TrapHexagon(magic, spellTargetLock ? target != null ? target.CurrentLocation : location : location,
                        out cast);
                    break;
                case Spell.Reincarnation:
                    ...
                    break;
                case Spell.Curse:
                    Curse(magic, spellTargetLock ? target != null ? target.CurrentLocation : location : location,
                        out cast);
                    break;
                ...
                case Spell.UltimateEnhancer:
                    ...
                    break;
                case Spell.Plague:
                    Plague(magic, spellTargetLock ? target != null ? target.CurrentLocation : location : location,
                        out cast);
                    break;
                ...
                case Spell.StraightShot:
                    if(!StraightShot(target, magic)) {
                        targetID = 0;
                    }

                    break;
                case Spell.DoubleShot:
                    if(!DoubleShot(target, magic)) {
                        targetID = 0;
                    }
                ...
                case Spell.DelayedExplosion:
                    if(!DelayedExplosion(target, magic)) {
                        targetID = 0;
                    }
                ...
                case Spell.ElementalBarrier:
                    ActionList.Add(new DelayedAction(DelayedType.Magic, Envir.Time + 500, magic,
                        magic.GetPower(GetAttackPower(Stats[Stat.MinMC], Stats[Stat.MaxMC]))));
                    break;
                ...
                case Spell.Stonetrap:
                    ArcherSummonStone(magic,
                        spellTargetLock ? target != null ? target.CurrentLocation : location : location, out cast);
                    break;
                ...
                case Spell.HealingCircle:
                    HealingCircle(magic,
                        spellTargetLock ? target != null ? target.CurrentLocation : location : location);
                    break;
                ...
                case Spell.MeteorShower:
                    if(!MeteorShower(target, magic)) {
                        targetID = 0;
                    }

                    return;
                case Spell.FireBounce:
                    if(!FireBounce(target, magic, this)) {
                        targetID = 0;
                    }
            ...
            Enqueue(new ServerPacket.Magic
                { Spell = spell, TargetID = targetID, Target = location, Cast = cast, Level = level });
            Broadcast(new ServerPacket.ObjectMagic {
                ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Spell = spell,
                TargetID = targetID, Target = location, Cast = cast, Level = level
            });
        }
        ...
        private void Concentration(UserMagic magic) {
            ...
            Buff buff = AddBuff(BuffType.Concentration, this, Settings.Second * duration, new Stats());
        ...
        public void UpdateConcentration(bool concentrating, bool interrupted) {
            Enqueue(new ServerPacket.SetConcentration
                { ObjectID = ObjectID, Enabled = concentrating, Interrupted = interrupted });
            Broadcast(new ServerPacket.SetConcentration
                { ObjectID = ObjectID, Enabled = concentrating, Interrupted = interrupted });
        }
        ...
        private bool ElementalShot(MapObject target, UserMagic magic) {
            if(HasElemental) {
                ...
                if(Info.MentalState != 1 && !CanFly(target.CurrentLocation)) {
                    return false;
                }
                ...
                DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, magic, damage, target,
        ...
        public void ObtainElement(bool cast) {
            ...
            if(cast) {
                ...
                {
                    if(meditateLevel == 3) {
                        Enqueue(new ServerPacket.SetElemental {
                            ObjectID = ObjectID, Enabled = true, Value = (uint)Settings.OrbsExpList[0], ElementType = 1,
                            ExpLast = (uint)maxOrbs
                        });
                        Broadcast(new ServerPacket.SetElemental {
                            ObjectID = ObjectID, Enabled = true, Casted = true, Value = (uint)Settings.OrbsExpList[0],
                            ElementType = 1, ExpLast = (uint)maxOrbs
                        });
                ...
                if(ElementsLevel >= Settings.OrbsExpList[0]) {
                    HasElemental = true;
                }

                for (int i = 0; i <= Settings.OrbsExpList.Count - 1; i++) {
                    if(Settings.OrbsExpList[i] != ElementsLevel) {
                        continue;
                    }
            ...
            Enqueue(new ServerPacket.SetElemental {
                ObjectID = ObjectID, Enabled = HasElemental, Value = (uint)ElementsLevel, ElementType = (uint)orbType,
                ExpLast = (uint)maxOrbs
            });
            Broadcast(new ServerPacket.SetElemental {
                ObjectID = ObjectID, Enabled = HasElemental, Casted = cast, Value = (uint)ElementsLevel,
                ElementType = (uint)orbType, ExpLast = (uint)maxOrbs
            });
        }
        ...
        private bool Fireball(MapObject target, UserMagic magic) {
            if(target == null || !target.IsAttackTarget(this) || !CanFly(target.CurrentLocation)) {
                return false;
            }
                ...
                for (int y = CurrentLocation.Y - d; y <= CurrentLocation.Y + d; y++) {
                    ...
                    for (int x = CurrentLocation.X - d;
                        ...
                        for (int i = 0; cell.Objects != null && i < cell.Objects.Count; i++) {
                            ...
                            if(!ob.IsAttackTarget(this) || ob.Level >= Level) {
                                continue;
                            }
                            ...
                            if(ob.Pushed(this, dir, distance) == 0) {
                                continue;
                            }
        ...
        private void ElectricShock(MonsterObject target, UserMagic magic) {
            ...
            if(target.Level > Level + 2 || !target.Info.CanTame) {
                return;
            }
            ...
            int petBonus = Globals.MaxPets - 3;

            if(Pets.Count(t => !t.Dead && t.Race != ObjectType.Creature) >= magic.Level + petBonus) {
                return;
            }
        ...
        private void HellFire(UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, this, magic, damage, CurrentLocation,
                Direction, 4);
            CurrentMap.ActionList.Add(action);
            ...
            action = new DelayedAction(DelayedType.Magic, Envir.Time + 500, this, magic, damage, CurrentLocation, dir,
                4);
            CurrentMap.ActionList.Add(action);
        ...
        private void ThunderBolt(MapObject target, UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, magic, damage, target,
                target.CurrentLocation);
        ...
        private void FireBang(UserMagic magic, Point location) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, this, magic, damage, location);
            CurrentMap.ActionList.Add(action);
        }
        ...
        private void TurnUndead(MapObject target, UserMagic magic) {
            if(target != null &&
               ...
               target.IsAttackTarget(this)) {
                ...
                DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, magic, target);
                ActionList.Add(action);
            }
        }
        ...
        private void Mirroring(UserMagic magic) {
            ...
            for (int i = 0; i < Pets.Count; i++) {
                monster = Pets[i];
                if(monster.Info.Name != Settings.CloneName || monster.Dead) {
                    continue;
                }
        ...
        private void IceThrust(UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 1500, this, magic, location, Direction,
                damageFinish, (int)(damageFinish * 0.6));
        ...
        private void Healing(MapObject target, UserMagic magic) {
            if(target == null || !target.IsFriendlyTarget(this)) {
                return;
            }

            int health = magic.GetDamage(GetAttackPower(Stats[Stat.MinSC], Stats[Stat.MaxSC]) * 2) + Level;

            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, magic, health, target);
        ...
        private bool Poisoning(MapObject target, UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, magic, power, target, item);
            ActionList.Add(action);
            ConsumeItem(item, 1);
            return true;
        }
        ...
        private void SummonSkeleton(UserMagic magic) {
            ...
            for (int i = 0; i < Pets.Count; i++) {
                monster = Pets[i];
                if(monster.Info.Name != Settings.SkeletonName || monster.Dead) {
                    continue;
                }
            ...
            if(Pets.Count(x => x.Race == ObjectType.Monster) >= 2) {
                return;
            }
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, this, magic, monster, Front);
            CurrentMap.ActionList.Add(action);
        }
        ...
        private void SummonShinsu(UserMagic magic) {
            ...
            for (int i = 0; i < Pets.Count; i++) {
                monster = Pets[i];
                if(monster.Info.Name != Settings.ShinsuName || monster.Dead) {
                    continue;
                }
        ...
        private void Hiding(UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, magic,
                GetAttackPower(Stats[Stat.MinSC], Stats[Stat.MaxSC]) + ((magic.Level + 1) * 5));
            ActionList.Add(action);
        }
        ...
        private void MassHiding(UserMagic magic, Point location, out bool cast) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, this, magic,
                (GetAttackPower(Stats[Stat.MinSC], Stats[Stat.MaxSC]) / 2) + ((magic.Level + 1) * 2), location);
            CurrentMap.ActionList.Add(action);
        }
        ...
        private void SoulShield(UserMagic magic, Point location, out bool cast) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, this, magic,
                (GetAttackPower(Stats[Stat.MinSC], Stats[Stat.MaxSC]) * 4) + ((magic.Level + 1) * 50), location);
            CurrentMap.ActionList.Add(action);
        ...
        private void MassHealing(UserMagic magic, Point location) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, this, magic, value, location);
            CurrentMap.ActionList.Add(action);
        }
        ...
        private void Revelation(MapObject target, UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, magic, value, target);
        ...
        private void PoisonCloud(UserMagic magic, Point location, out bool cast) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, this, magic, damage, location,
                (byte)Envir.Random.Next(Stats[Stat.PoisonAttack]));
        ...
        private void MoonMist(UserMagic magic) {
            for (int i = 0; i < Buffs.Count; i++) {
                if(Buffs[i].Type == BuffType.MoonLight) {
                    return;
                }
            }

            int time = GetAttackPower(Stats[Stat.MinAC], Stats[Stat.MaxAC]);

            AddBuff(BuffType.MoonLight, this, (time + ((magic.Level + 1) * 5)) * 500, new Stats());

            CurrentMap.Broadcast(new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.MoonMist },
                CurrentLocation);
        ...
        private void Reincarnation(UserMagic magic, PlayerObject target, out bool cast) {
            ...
            if(!ActiveReincarnation && !ReincarnationReady) {
                ...
                ServerPacket.Chat p = new ServerPacket.Chat {
                    Message = string.Format("{0} is attempting to revive {1}", Name, target.Name), Type = ChatType.Shout
                };
                ...
                DelayedAction action = new(DelayedType.Magic, ExpireTime, magic);
        ...
        private void SummonHolyDeva(UserMagic magic) {
            ...
            for (int i = 0; i < Pets.Count; i++) {
                monster = Pets[i];
                if(monster.Info.Name != Settings.AngelName || monster.Dead) {
                    continue;
                }
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 1500, this, magic, monster, Front);
            CurrentMap.ActionList.Add(action);
        }

        private void Hallucination(MapObject target, UserMagic magic) {
            if(target == null || target.Race != ObjectType.Monster || !target.IsAttackTarget(this)) {
                return;
            }
            ...
            DelayedAction action = new(DelayedType.Magic, delay, magic, damage, target);
        ...
        private void EnergyShield(MapObject target, UserMagic magic, out bool cast) {
            ...
            if(target == null || target.Node == null || !target.IsFriendlyTarget(this)) {
                target = this; //offical is only party target
            }
                ...
                case ObjectType.Hero:
                    ...
                    if(target.IsFriendlyTarget(this)) {
                        ...
                        if(target.Race == ObjectType.Monster || ((HumanObject)target).Class == MirClass.Warrior ||
                           ((HumanObject)target).Class == MirClass.Assassin) {
                            stats[Stat.MaxDC] = value;
                        } else if(((HumanObject)target).Class == MirClass.Wizard ||
                                  ((HumanObject)target).Class == MirClass.Archer) {
        ...
        private void Plague(UserMagic magic, Point location, out bool cast) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, this, magic,
                magic.GetDamage(GetAttackPower(Stats[Stat.MinSC], Stats[Stat.MaxSC])), location, pType);
            CurrentMap.ActionList.Add(action);
        ...
        private void Curse(UserMagic magic, Point location, out bool cast) {
            ...
            if(Envir.Random.Next(10 - ((magic.Level + 1) * 2)) > 2) {
                return;
            }
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, this, magic,
                magic.GetDamage(GetAttackPower(Stats[Stat.MinSC], Stats[Stat.MaxSC])), location,
                1 + ((magic.Level + 1) * 2));
            CurrentMap.ActionList.Add(action);
        }
        ...
        private void PetEnhancer(MapObject target, UserMagic magic, out bool cast) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, magic, duration, target);
        ...
        private void ProtectionField(UserMagic magic) {
            ...
            AddBuff(BuffType.ProtectionField, this, Settings.Second * duration,
                new Stats { [Stat.MaxAC] = addValue, [Stat.MinAC] = addValue });
            OperateTime = 0;
            LevelMagic(magic);
        }
        ...
        private void Rage(UserMagic magic) {
            ...
            AddBuff(BuffType.Rage, this, Settings.Second * duration,
                new Stats { [Stat.MaxDC] = addValue, [Stat.MinDC] = addValue });
            OperateTime = 0;
            LevelMagic(magic);
        }
        ...
        private void ShoulderDash(UserMagic magic) {
            ...
            for (int i = 0; i < dist; i++) {
                ...
                if(_canDash) {
                    ...
                    Broadcast(new ServerPacket.ObjectDash
                        { ObjectID = ObjectID, Direction = Direction, Location = _nextLocation });
            ...
            if(_cellsTravelled == 0) {
                ...
                Broadcast(new ServerPacket.ObjectDashFail
                    { ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation });
        ...
        private void SlashingBurst(UserMagic magic, out bool cast) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, this, magic, damageFinal, CurrentLocation,
                Direction, 1);
            CurrentMap.ActionList.Add(action);
        ...
        private void CounterAttackCast(UserMagic magic, MapObject target) {
            ...
            if(Functions.InRange(CurrentLocation, target.CurrentLocation, 1) == false) {
                return;
            }

            if(Envir.Random.Next(10) > magic.Level + 6) {
                return;
            }

            Enqueue(new ServerPacket.ObjectMagic {
                ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Spell = Spell.CounterAttack,
                TargetID = target.ObjectID, Target = target.CurrentLocation, Cast = true,
                Level = GetMagic(Spell.CounterAttack).Level, SelfBroadcast = true
            });
            DelayedAction action = new(DelayedType.Damage, AttackTime, target, damageFinal, DefenceType.AC, true);
            ActionList.Add(action);
            LevelMagic(magic);
            CounterAttack = false;
        }
        ...
        private void Trap(UserMagic magic, MapObject target, out bool cast) {
            ...
            if(target == null || !target.IsAttackTarget(this) || !(target is MonsterObject)) {
                return;
            }
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, this, magic, value, target);
            CurrentMap.ActionList.Add(action);
            cast = true;
        }
        ...
        private bool PoisonSword(UserMagic magic) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                for (int o = 0; o < cell.Objects.Count; o++) {
                    ...
                    if(target.Race != ObjectType.Player && target.Race != ObjectType.Monster) {
                        continue;
                    }

                    if(target == null || !target.IsAttackTarget(this) || target.Node == null) {
                        continue;
                    }
        ...
        private void DarkBody(MapObject target, UserMagic magic) {
            ...
            for (int i = 0; i < Pets.Count; i++) {
                monster = Pets[i];
                if(monster.Info.Name != Settings.AssassinCloneName || monster.Dead) {
                    continue;
                }
            ...
            int duration = (GetAttackPower(Stats[Stat.MinAC], Stats[Stat.MaxAC]) + ((magic.Level + 1) * 5)) * 500;
                    ...
                    for (int j = 0; j < cell.Objects.Count; j++) {
                        ...
                        switch (target.Race) {
                            ...
                            case ObjectType.Player:
                                //Only targets
                                if(target.IsAttackTarget(this)) {
                                    DelayedAction action = new(DelayedType.Damage, Envir.Time + AttackSpeed, target,
                                        damageFinal, DefenceType.AC, true);
                                    ActionList.Add(action);
                                }
        ...
        private void FlashDash(UserMagic magic) {
            ...
            if(jumpDistance > 0) {
                ...
                Broadcast(new ServerPacket.ObjectDashAttack
                    { ObjectID = ObjectID, Direction = Direction, Location = location, Distance = jumpDistance });
            } else {
                Broadcast(new ServerPacket.ObjectAttack
                    { ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation });
            }
                    ...
                    for (int c = 0; c < cInfo.Objects.Count; c++) {
                        ...
                        switch (ob.Race) {
                            ...
                            case ObjectType.Player:
                                //Only targets
                                if(ob.IsAttackTarget(this)) {
                                    DelayedAction action = new(DelayedType.Damage, AttackTime, ob,
                                        magic.GetDamage(GetAttackPower(Stats[Stat.MinDC], Stats[Stat.MaxDC])),
                                        DefenceType.AC, true);
                                    ActionList.Add(action);
                                    success = true;
                                    if((ob.Race != ObjectType.Player || Settings.PvpCanResistPoison) &&
                                       Envir.Random.Next(Settings.PoisonAttackWeight) >= ob.Stats[Stat.PoisonResist] &&
                                       Envir.Random.Next(15) <= magic.Level + 1) {
                                        DelayedAction pa = new(DelayedType.Poison, AttackTime, ob, PoisonType.Stun,
                                            SpellEffect.TwinDrakeBlade, magic.Level + 1, 1000);
                                        ActionList.Add(pa);
                                    }
                                }
        ...
        private void BackStep(UserMagic magic) {
            ...
            if(jumpDistance > 0) {
                ...
                Broadcast(new ServerPacket.ObjectBackStep
                    { ObjectID = ObjectID, Direction = Direction, Location = location, Distance = jumpDistance });
                LevelMagic(magic);
            } else {
                Broadcast(new ServerPacket.ObjectBackStep {
                    ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Distance = jumpDistance
                });
        ...
        private bool DelayedExplosion(MapObject target, UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, magic, power, target,
                target.CurrentLocation);
            ActionList.Add(action);
            return true;
        }
        ...
        private void ExplosiveTrap(UserMagic magic, Point location) {
            ...
            IEnumerable<int> trapIDs = CurrentMap.GetSpellObjects(Spell.ExplosiveTrap, this)
                .Select(x => x.ExplosiveTrapID).Distinct();

            int max = magic.Level + 1;

            if(trapIDs.Count() >= max) {
                return;
            }
        ...
        public void DoKnockback(MapObject target, UserMagic magic) //ElementalShot - knockback
        {
            ...
            if(target.CurrentLocation.Y < 0 || target.CurrentLocation.Y >= CurrentMap.Height ||
               target.CurrentLocation.X < 0 || target.CurrentLocation.X >= CurrentMap.Height) {
                return;
            }
            ...
            if(!target.IsAttackTarget(this) || target.Level >= Level) {
                return;
            }

            if(Envir.Random.Next(20) >= 6 + (magic.Level * 3) + ElementsLevel + Level - target.Level) {
                return;
            }
        ...
        public void BindingShot(UserMagic magic, MapObject target, out bool cast) {
            ...
            if(((MonsterObject)target).ShockTime >= Envir.Time) {
                return; //Already shocked
            }
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, magic, value, target,
                target.CurrentLocation);
            ActionList.Add(action);
        ...
        public void NapalmShot(MapObject target, UserMagic magic) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, this, magic, damage,
                target.CurrentLocation);
            CurrentMap.ActionList.Add(action);
        }
        ...
        public void ArcherSummon(UserMagic magic, MapObject target, Point location) {
            ...
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, magic, value, location, target);
            ActionList.Add(action);
        }
        ...
        public void ArcherSummonStone(UserMagic magic, Point location, out bool cast) {
            ...
            int delay = (Functions.MaxDistance(CurrentLocation, location) * 50) + 500; //50 MS per Step
            //
            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, magic, duration, location);
            ActionList.Add(action);
        ...
        public void HealingCircle(UserMagic magic, Point location) {
            int damage = magic.GetDamage(GetAttackPower(Stats[Stat.MinSC], Stats[Stat.MaxSC]));
            DelayedAction action = new(DelayedType.Magic, Envir.Time + 500 + 1200, this, magic, damage, location);
            CurrentMap.ActionList.Add(action);
        }
        ...
        private void Portal(UserMagic magic, Point location, out bool cast) {
            ...
            int portalCount = Envir.Spells.Count(x => x.Spell == Spell.Portal && x.Caster == this);
        ...
        private bool FireBounce(MapObject target, UserMagic magic, MapObject source, int bounce = -1) {
            if(target == null || !target.IsAttackTarget(this) || !source.CanFly(target.CurrentLocation) ||
               bounce == 0) {
                return false;
            }
            ...
            if(bounce == -1) {
                ...
                CurrentMap.Broadcast(
                    new ServerPacket.ObjectProjectile
                        { Spell = magic.Info.Spell, Source = source.ObjectID, Destination = target.ObjectID },
                    source.CurrentLocation);
            }

            DelayedAction action = new(DelayedType.Magic, Envir.Time + delay, magic, damage, target,
                target.CurrentLocation, bounce);
            ActionList.Add(action);
        ...
        private bool MeteorShower(MapObject target, UserMagic magic) {
            ...
            if(target.Race == ObjectType.Monster) {
                ...
                for (int i = 0; i < secondaryTargetCount; i++) {
                    if(targets[i] == target || !targets[i].IsAttackTarget(this)) {
                        continue;
                    }

                    DelayedAction action2 = new(DelayedType.Magic, Envir.Time + delay, magic, damage / 2, targets[i],
                        targets[i].CurrentLocation);
            ...
            Broadcast(new ServerPacket.ObjectMagic {
                ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Spell = magic.Info.Spell,
                TargetID = target.ObjectID, Target = target.CurrentLocation, Cast = true, Level = magic.Level,
                SecondaryTargetIDs = targetIDs
            });
            Enqueue(new ServerPacket.Magic {
                Spell = Spell.MeteorShower, TargetID = target.ObjectID, Target = target.CurrentLocation, Cast = true,
                Level = magic.Level, SecondaryTargetIDs = targetIDs
            });
        ...
        private void CheckSneakRadius() {
            ...
            for (int y = CurrentLocation.Y - 3; y <= CurrentLocation.Y + 3; y++) {
                ...
                for (int x = CurrentLocation.X - 3; x <= CurrentLocation.X + 3; x++) {
                    ...
                    for (int i = 0; cell.Objects != null && i < cell.Objects.Count; i++) {
                        MapObject ob = cell.Objects[i];
                        if(ob.Race != ObjectType.Player || ob == this) {
                            continue;
                        }
            ...
            switch (magic.Spell) {
                ...
                case Spell.MeteorShower:
                    ...
                    if(target == null || !target.IsAttackTarget(this) || target.CurrentMap != CurrentMap ||
                       target.Node == null || !Functions.InRange(target.CurrentLocation, targetLocation, 2)) {
                        return;
                    }
                ...
                case Spell.FireBounce:
                    ...
                    if(target.Race == ObjectType.Monster) {
                        List<MapObject> targets = ((MonsterObject)target).FindAllNearby(3, target.CurrentLocation)
                            .Where(x => x != target && x.IsAttackTarget(this) && target.CanFly(x.CurrentLocation))
                            .ToList();

                        if(targets.Count > 0) {
                            MapObject nextTarget = targets[Envir.Random.Next(targets.Count)];
                ...
                case Spell.FrostCrunch:
                    ...
                    if(target.Attacked(this, value, DefenceType.MAC, false) > 0) {
                        if(Level + (target.Race == ObjectType.Player ? 2 : 10) >= target.Level &&
                           Envir.Random.Next(target.Race == ObjectType.Player ? 100 : 20) <= magic.Level) {
                            target.ApplyPoison(new Poison {
                                Owner = this,
                        ...
                        if(Level + (target.Race == ObjectType.Player ? 2 : 10) >= target.Level &&
                           Envir.Random.Next(target.Race == ObjectType.Player ? 100 : 40) <= magic.Level) {
                            target.ApplyPoison(new Poison {
                                Owner = this,
                                Duration = target.Race == ObjectType.Player
                                    ? 2
                                    : 5 + Envir.Random.Next(Stats[Stat.Freezing]),
                ...
                case Spell.Healing:
                    ...
                    if(target == null || !target.IsFriendlyTarget(this) || target.CurrentMap != CurrentMap ||
                       target.Node == null) {
                        return;
                    }
                ...
                case Spell.ElectricShock:
                    ...
                    if(monster == null || !monster.IsAttackTarget(this) || monster.CurrentMap != CurrentMap ||
                       monster.Node == null) {
                        return;
                    }
                ...
                case Spell.StormEscape:
                    ...
                    if(!CurrentMap.ValidPoint(location) || Envir.Random.Next(4) >= magic.Level + 1 ||
                       !Teleport(CurrentMap, location, false)) {
                        return;
                    }

                    CurrentMap.Broadcast(
                        new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.StormEscape },
                        CurrentLocation);

                    AddBuff(BuffType.TemporalFlux, this, Settings.Second * 30,
                ...
                case Spell.Blink: {
                    ...
                    if(Functions.InRange(CurrentLocation, location, magic.Info.Range) == false) {
                        return;
                    }
                    ...
                    CurrentMap.Broadcast(
                        new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.Teleport },
                        CurrentLocation);
                    LevelMagic(magic);
                ...
                case Spell.Haste: {
                    AddBuff(BuffType.Haste, this, (Settings.Second * 25) + (Settings.Second * magic.Level * 15),
                ...
                case Spell.Fury: {
                    AddBuff(BuffType.Fury, this, (Settings.Second * 60) + (magic.Level * 10000),
                        new Stats { [Stat.AttackSpeed] = 4 });
                    LevelMagic(magic);
                }
                    break;
                ...
                case Spell.LightBody: {
                    AddBuff(BuffType.LightBody, this, (magic.Level + 1) * Settings.Second * 30,
                ...
                case Spell.MagicShield: {
                    if(HasBuff(BuffType.MagicShield, out _)) {
                        return;
                    }

                    LevelMagic(magic);
                    AddBuff(BuffType.MagicShield, this, Settings.Second * (int)data[1],
                        new Stats { [Stat.DamageReductionPercent] = (magic.Level + 2) * 10 });
                }
                    break;
                ...
                case Spell.Revelation:
                    ...
                    if(Envir.Random.Next(4) > magic.Level || Envir.Time < target.RevTime) {
                        return;
                    }
                ...
                case Spell.Entrapment:
                    ...
                    if(target == null || !target.IsAttackTarget(this) || target.CurrentMap != CurrentMap ||
                       target.Node == null || target.Race != ObjectType.Monster ||
                       Functions.MaxDistance(CurrentLocation, target.CurrentLocation) > 7 ||
                       target.Level >= Level + 5 + Envir.Random.Next(8)) {
                        return;
                    }
                    ...
                    if((byte)pulldirection % 2 > 0) {
                        pulldistance = Math.Max(0,
                            Math.Min(Math.Abs(CurrentLocation.X - target.CurrentLocation.X),
                                Math.Abs(CurrentLocation.Y - target.CurrentLocation.Y)));
                    } else {
                        pulldistance = pulldirection == MirDirection.Up || pulldirection == MirDirection.Down
                            ? Math.Abs(CurrentLocation.Y - target.CurrentLocation.Y) - 2
                            : Math.Abs(CurrentLocation.X - target.CurrentLocation.X) - 2;
                    }

                    int levelgap = target.Race == ObjectType.Player
                        ? Level - target.Level + 4
                        : Level - target.Level + 9;
                    if(Envir.Random.Next(30) >= ((magic.Level + 1) * 3) + levelgap) {
                        return;
                    }

                    int duration = target.Race == ObjectType.Player
                        ? (int)Math.Round((magic.Level + 1) * 1.6)
                        : (int)Math.Round((magic.Level + 1) * 0.8);
                    if(duration > 0) {
                        target.ApplyPoison(
                            new Poison { PType = PoisonType.Paralysis, Duration = duration, TickSpeed = 1000 }, this);
                    }

                    CurrentMap.Broadcast(
                        new ServerPacket.ObjectEffect { ObjectID = target.ObjectID, Effect = SpellEffect.Entrapment },
                        target.CurrentLocation);
                    if(target.Pushed(this, pulldirection, pulldistance) > 0) {
                        LevelMagic(magic);
                    }
                ...
                case Spell.Hallucination:
                    ...
                    if(target == null || !target.IsAttackTarget(this) || target.CurrentMap != CurrentMap ||
                       target.Node == null ||
                       Functions.MaxDistance(CurrentLocation, target.CurrentLocation) > 7 ||
                       Envir.Random.Next(Level + 20 + (magic.Level * 5)) <= target.Level + 10) {
                        return;
                    }
                    ...
                    if(target.Attacked(this, value, DefenceType.AC, false) > 0) {
                        ...
                        if(rnd >= 8) {
                            if(target.Race == ObjectType.Player && Settings.PvpCanFreeze) {
                                target.ApplyPoison(
                                    new Poison {
                                        PType = PoisonType.Frozen, Duration = (magic.Level + 1) * 3, TickSpeed = 1000
                                    }, this);
                            } else {
                                rnd -= 4;
                            }
                        ...
                        if(rnd <= 4) {
                            target.ApplyPoison(
                                new Poison {
                                    PType = PoisonType.Stun, Duration = (magic.Level + 1) * 3, TickSpeed = 1000
                                }, this);
                        } else if(rnd <= 7) {
                            target.ApplyPoison(
                                new Poison {
                                    PType = PoisonType.Slow, Duration = (magic.Level + 1) * 3, TickSpeed = 1000
                                }, this);
                        }
                ...
                case Spell.ElementalBarrier: {
                    if(HasBuff(BuffType.ElementalBarrier, out _)) {
                        return;
                    }
                    ...
                    AddBuff(BuffType.ElementalBarrier, this, Settings.Second * ((int)data[1] + barrierPower),
                        new Stats { [Stat.DamageReductionPercent] = (magic.Level + 1) * 10 });
                    CurrentMap.Broadcast(
                        new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.ElementalBarrierUp },
                        CurrentLocation);
                }
                    break;
                ...
                case Spell.BindingShot:
                        ...
                        for (int x = place.X - 1; x <= place.X + 1; x++) {
                            ...
                            for (int i = 0; i < cell.Objects.Count; i++) {
                                ...
                                switch (targetob.Race) {
                                    case ObjectType.Monster:
                                        if(targetob == null || !targetob.IsAttackTarget(this) ||
                                           targetob.Node == null || targetob.Level > Level + 2) {
                                            continue;
                                        }
                    ...
                    centerTarget.Broadcast(new ServerPacket.SetBindingShot
                        { ObjectID = centerTarget.ObjectID, Enabled = true, Value = value });
                        ...
                        if(hasVampBuff || hasPoisonBuff) {
                            ...
                            for (int y = place.Y - 1; y <= place.Y + 1; y++) {
                                ...
                                for (int x = place.X - 1; x <= place.X + 1; x++) {
                                    ...
                                    for (int i = 0; i < cell.Objects.Count; i++) {
                                        MapObject targetob = cell.Objects[i];
                                        if(targetob.Race != ObjectType.Monster && targetob.Race != ObjectType.Player) {
                                            continue;
                                        }
                                        ...
                                        if(hasVampBuff) //Vampire Effect
                                        {
                                            ...
                                            if(VampAmount == 0) {
                                                VampTime = Envir.Time + Settings.Second;
                                            }
                                        ...
                                        if(hasPoisonBuff) //Poison Effect
                                        {
                                            ...
                                            targetob.ApplyPoison(new Poison {
                                                ...
                                                PType = PoisonType.Green,
                                                TickSpeed = 2000,
                                                Value = (value / 25) + magic.Level + 1 +
                                                        Envir.Random.Next(Stats[Stat.PoisonAttack])
                                            }, this);
                ...
                case Spell.SummonSnakes:
                    ...
                    for (int i = 0; i < Pets.Count; i++) {
                        monster = Pets[i];
                        if(monster.Info.Name != (SummonType == 1 ? Settings.VampireName :
                               SummonType == 2 ? Settings.ToadName : Settings.SnakeTotemName) || monster.Dead) {
                            continue;
                        }
                    ...
                    MonsterInfo info = Envir.GetMonsterInfo(SummonType == 1 ? Settings.VampireName :
                        SummonType == 2 ? Settings.ToadName : Settings.SnakeTotemName);
                    if(info == null) {
                        return;
                    }
                    ...
                    DelayedAction action = new(DelayedType.Magic, Envir.Time + 500, this, magic, monster, location);
                    CurrentMap.ActionList.Add(action);
                    break;
                case Spell.Stonetrap: {
                    duration = (int)data[1];
                    ...
                    if(Pets.Where(x => x.Race == ObjectType.Monster).Count() >= magic.Level + 1) {
                        return;
                    }
                    ...
                    DelayedAction act = new(DelayedType.Magic, Envir.Time + 500, this, magic, monster, location);
        ...
        protected void CompleteMine(IList<object> data) {
            ...
            target.Broadcast(new ServerPacket.MapEffect
                { Effect = SpellEffect.Mine, Location = target.CurrentLocation, Value = (byte)Direction });
        ...
        protected void CompleteAttack(IList<object> data) {
            ...
            if(FatalSword) {
                ServerPacket.ObjectEffect p = new ServerPacket.ObjectEffect
                    { ObjectID = target.ObjectID, Effect = SpellEffect.FatalSword };
            ...
            if(userMagic != null && finalHit) {
                if(userMagic.Spell == Spell.TwinDrakeBlade) {
                    if((target.Race != ObjectType.Player || Settings.PvpCanResistPoison) &&
                       ...
                       target.Level < Level + 10 && Envir.Random.Next(target.Race == ObjectType.Player ? 40 : 20) <=
                       userMagic.Level + 1) {
                        target.ApplyPoison(
                            new Poison {
                                PType = PoisonType.Stun,
                                Duration = target.Race == ObjectType.Player ? 2 : 2 + userMagic.Level, TickSpeed = 1000
                            }, this);
                        target.Broadcast(new ServerPacket.ObjectEffect
                            { ObjectID = target.ObjectID, Effect = SpellEffect.TwinDrakeBlade });
                    }
                }
            }
        ...
        protected UserItem GetAmulet(int count, int shape = 0) {
            for (int i = 0; i < Info.Equipment.Length; i++) {
                UserItem item = Info.Equipment[i];
                if(item != null && item.Info.Type == ItemType.Amulet && item.Info.Shape == shape &&
                   item.Count >= count) {
                    return item;
                }
            }
        ...
        public void LevelMagic(UserMagic magic) {
            ...
            Enqueue(new ServerPacket.MagicLeveled
                { ObjectID = ObjectID, Spell = magic.Spell, Level = magic.Level, Experience = magic.Experience });
        }
        ...
        public override bool Teleport(Map temp, Point location, bool effects = true, byte effectnumber = 0) {
            ...
            if(effects) {
                Enqueue(new ServerPacket.ObjectTeleportIn { ObjectID = ObjectID, Type = effectnumber });
            }
        ...
        public Packet GetInfoEx(HumanObject player) {
            ServerPacket.ObjectPlayer p = (ServerPacket.ObjectPlayer)GetInfo();
        ...
        public override int Attacked(HumanObject attacker, int damage, DefenceType type = DefenceType.ACAgility,
            ...
            int armour = GetArmour(type, attacker, out bool hit);
            ...
            if(Envir.Random.Next(100) < Stats[Stat.Reflect]) {
                if(attacker.IsAttackTarget(this)) {
                    ...
                    CurrentMap.Broadcast(
                        new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.Reflect },
                        CurrentLocation);
                }
            ...
            if(Envir.Random.Next(100) < attacker.Stats[Stat.CriticalRate] * Settings.CriticalRateWeight) {
                CurrentMap.Broadcast(
                    new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.Critical },
                    CurrentLocation);
                damage = Math.Min(int.MaxValue,
                    damage + (int)Math.Floor(damage * ((double)attacker.Stats[Stat.CriticalDamage] /
                        (double)Settings.CriticalDamageWeight * 10)));
                BroadcastDamageIndicator(DamageType.Critical);
            }

            if(HasBuff(BuffType.MagicShield, out Buff magicShield)) {
                int duration = (int)Math.Min(int.MaxValue, magicShield.ExpireTime - ((damage - armour) * 60));
                AddBuff(BuffType.MagicShield, this, duration, null);
            }

            if(HasBuff(BuffType.ElementalBarrier, out Buff elementalBarrier)) {
                int duration = (int)Math.Min(int.MaxValue, elementalBarrier.ExpireTime - ((damage - armour) * 60));
                AddBuff(BuffType.ElementalBarrier, this, duration, null);
            }

            if(attacker.Stats[Stat.HPDrainRatePercent] > 0 && damageWeapon) {
                attacker.HpDrain += Math.Max(0,
                    (float)(damage - armour) / 100 * attacker.Stats[Stat.HPDrainRatePercent]);
            ...
            Broadcast(new ServerPacket.ObjectStruck {
        ...
        public override int Attacked(MonsterObject attacker, int damage, DefenceType type = DefenceType.ACAgility) {
            ...
            if(StruckTime < Envir.Time) {
                Enqueue(new ServerPacket.Struck { AttackerID = attacker.ObjectID });
                Broadcast(new ServerPacket.ObjectStruck {
                    ObjectID = ObjectID, AttackerID = attacker.ObjectID, Direction = Direction,
                    Location = CurrentLocation
                });
        ...
        public override int Struck(int damage, DefenceType type = DefenceType.ACAgility) {
            ...
            Broadcast(new ServerPacket.ObjectStruck
                { ObjectID = ObjectID, AttackerID = 0, Direction = Direction, Location = CurrentLocation });
        ...
        public override void ApplyPoison(Poison p, MapObject Caster = null, bool NoResist = false,
            ...
            if(Caster != null && !NoResist) {
                if((Caster.Race != ObjectType.Player || Settings.PvpCanResistPoison) &&
                   Envir.Random.Next(Settings.PoisonResistWeight) < Stats[Stat.PoisonResist]) {
                    return;
                }
            }
            ...
            if(p.PType == PoisonType.Green || p.PType == PoisonType.Red) {
                p.Duration = Math.Max(0, p.Duration - Stats[Stat.PoisonRecovery]);
            }
            ...
            for (int i = 0; i < PoisonList.Count; i++) {
                ...
                if(PoisonList[i].PType != PoisonType.Green &&
                   PoisonList[i].Duration - PoisonList[i].Time > p.Duration) {
                    return; //cant cast 1 second poison to make a 1minute poison go away!
                }

                if(PoisonList[i].PType == PoisonType.Frozen || PoisonList[i].PType == PoisonType.Slow ||
                   PoisonList[i].PType == PoisonType.Paralysis || PoisonList[i].PType == PoisonType.LRParalysis) {
            ...
            switch (p.PType) {
                case PoisonType.DelayedExplosion: {
                    ...
                    Enqueue(new ServerPacket.ObjectEffect
                        { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion });
                    Broadcast(new ServerPacket.ObjectEffect
                        { ObjectID = ObjectID, Effect = SpellEffect.DelayedExplosion });
                ...
                case PoisonType.Dazed: {
                    Enqueue(new ServerPacket.ObjectEffect
                        { ObjectID = ObjectID, Effect = SpellEffect.Stunned, Time = (uint)(p.Duration * p.TickSpeed) });
                    Broadcast(new ServerPacket.ObjectEffect
                        { ObjectID = ObjectID, Effect = SpellEffect.Stunned, Time = (uint)(p.Duration * p.TickSpeed) });
                    ReceiveChat(GameLanguage.BeenPoisoned, ChatType.System2);
                }
                    break;
                case PoisonType.Blindness: {
                    AddBuff(BuffType.Blindness, Caster, (int)(p.Duration * p.TickSpeed),
                        new Stats { [Stat.Accuracy] = p.Value * -1 });
                    ReceiveChat(GameLanguage.BeenPoisoned, ChatType.System2);
                }
                    break;
                default: {
        ...
        public override Buff AddBuff(BuffType type, MapObject owner, int duration, Stats stats,
            ...
            switch (b.Type) {
                case BuffType.MagicShield:
                    CurrentMap.Broadcast(
                        new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = SpellEffect.MagicShieldUp },
                        CurrentLocation);
                    break;
                case BuffType.ElementalBarrier:
                    CurrentMap.Broadcast(
            ...
            ServerPacket.AddBuff packet = new ServerPacket.AddBuff { Buff = b.ToClientBuff() };
        ...
        public bool CanGainItems(UserItem[] items) {
            ...
            if(FreeSpace(Info.Inventory) < itemCount + stackOffset) {
                return false;
            }
        ...
        public bool CanEquipItem(UserItem item, int slot) {
            ...
            if(item.Info.Type == ItemType.Weapon || item.Info.Type == ItemType.Torch) {
                if(item.Weight - (Info.Equipment[slot] != null ? Info.Equipment[slot].Weight : 0) + CurrentHandWeight >
                   Stats[Stat.HandWeight]) {
                    return false;
                }
            } else if(item.Weight - (Info.Equipment[slot] != null ? Info.Equipment[slot].Weight : 0) +
                      CurrentWearWeight > Stats[Stat.WearWeight]) {
                return false;
            }
        ...
        public void GainItem(UserItem item) {
            ...
            Enqueue(new ServerPacket.GainedItem
                { Item = clonedItem }); //Cloned because we are probably going to change the amount.
        ...
        public void DamageItem(UserItem item, int amount, bool isChanged = false) {
            ...
            if(item.WeddingRing == Info.Married && Info.Equipment[(int)EquipmentSlot.RingL].UniqueID == item.UniqueID) {
                return;
            }

            if(item.GetTotal(Stat.Strong) > 0) {
                amount = Math.Max(1, amount - item.GetTotal(Stat.Strong));
            }
        ...
        public override void SendHealth(HumanObject player) {
            if(!player.IsMember(this) && Envir.Time > RevTime) {
                return;
            }

            byte time = Math.Min(byte.MaxValue, (byte)Math.Max(5, (RevTime - Envir.Time) / 1000));
            player.Enqueue(
                new ServerPacket.ObjectHealth { ObjectID = ObjectID, Percent = PercentHealth, Expire = time });
        }
        ...
        public void SpellToggle(Spell spell, SpellToggleState state) {
            ...
            switch (spell) {
                ...
                case Spell.FlamingSword:
                    ...
                    Enqueue(new ServerPacket.SpellToggle
                        { ObjectID = ObjectID, Spell = Spell.FlamingSword, CanUse = true });
                    ChangeMP(-cost);
                    break;
        ...
        public void IncreaseMountLoyalty(int amount) {
            ...
            if(item != null && item.CurrentDura < item.MaxDura) {
                ...
                Enqueue(new ServerPacket.ItemRepaired
                    { UniqueID = item.UniqueID, MaxDura = item.MaxDura, CurrentDura = item.CurrentDura });
            }
        }
