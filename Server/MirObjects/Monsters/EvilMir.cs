...
namespace Server.Library.MirObjects.Monsters {
    public class EvilMir : MonsterObject {
        ...
        public bool DragonLink {
            get => _dragonlink && Envir.DragonSystem != null;
            set {
        ...
        protected override bool CanAttack => !Sleeping && base.CanAttack;
        ...
        protected override void CompleteAttack(IList<object> data) {
            ...
            if(!Target.IsAttackTarget(this) || Target.CurrentMap != CurrentMap || Target.Node == null) {
                return;
            }

            List<MapObject> targets = MassAttack
                ? FindAllTargets(17 /*huge range so it even hits ppl with bigger resolutions*/, CurrentLocation, false)
                : FindAllTargets(2, Target.CurrentLocation, false);
            if(targets.Count == 0) {
                return;
            }
        ...
        protected override void ProcessTarget() {
            ...
            byte random = DragonLink
                ? (byte)(Envir.DragonSystem.MaxLevel + 3 - Envir.DragonSystem.Info.Level)
                : (byte)8;

            if(Envir.Random.Next(random) > 0 /*&& Target.CurrentLocation.Y >= CurrentLocation.Y - 1*/
              ) //in theory it shouldnt fire 'behind' it, but it should shoot at stuff in it's top left corner (and this code made it only hit below him not 'infront' of him)
            {
                MassAttack = false;
        ...
        protected override void Attack() {
            int damage = GetAttackPower(Stats[Stat.MinDC],
                DragonLink ? Stats[Stat.MaxDC] + (Envir.DragonSystem.Info.Level - (1 * 10)) : Stats[Stat.MaxDC]);
            if(!MassAttack) {
        ...
        public override void Die() {
            ...
            if(!DragonLink) {
                ...
                Envir.DragonSystem
                    .GainExp(250); //why would hitting em give you so little 'points', while hitting them gives so much
                Sleeping = true;
