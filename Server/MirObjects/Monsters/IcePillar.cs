...
namespace Server.Library.MirObjects.Monsters {
    public class IcePillar : MonsterObject {
        ...
        private void CloseAttack(int damage) {
            ...
            for (int i = 0; i < targets.Count; i++) {
                Broadcast(new ServerPacket.ObjectEffect
                    { ObjectID = targets[i].ObjectID, Effect = SpellEffect.IcePillar });

                if(targets[i].Attacked(this, damage, DefenceType.MACAgility) <= 0) {
                    continue;
                }

                PoisonTarget(targets[i], 5, GetAttackPower(Stats[Stat.MinMC], Stats[Stat.MaxMC]), PoisonType.Frozen,
                    1000);
            }
        }
        ...
        public override void Die() {
            ...
            for (int i = 0; i < targets.Count; i++) {
                int delay = (Functions.MaxDistance(CurrentLocation, targets[i].CurrentLocation) * 50) +
                            500; //50 MS per Step

                DelayedAction action = new(DelayedType.Die, Envir.Time + delay, targets[i], damage,
