...
namespace Server.Library.MirObjects.Monsters {
    public class KingGuard : MonsterObject {
        protected virtual byte AttackRange => 10;
                ...
                for (int i = 0; i < targets.Count; i++) {
                    ...
                    if(Envir.Random.Next(3) >= 0) {
                        Broadcast(new ServerPacket.ObjectEffect
                            { ObjectID = targets[i].ObjectID, Effect = SpellEffect.KingGuard, EffectType = 0 });
                        PoisonTarget(targets[i], 5, 10, PoisonType.Slow, 1000);
                    } else {
                        Broadcast(new ServerPacket.ObjectEffect
                            { ObjectID = targets[i].ObjectID, Effect = SpellEffect.KingGuard, EffectType = 1 });
