    ...
    public class VampireSpider : MonsterObject {
        ...
        private void MasterVampire(int value, MapObject ob) {
            ...
            if(Master.VampAmount == 0) {
                ((PlayerObject)Master).VampTime = Envir.Time + 1000;
            }

            Master.VampAmount += (ushort)(value * (PetLevel + 1) * 0.25F);
            ob.Broadcast(new ServerPacket.ObjectEffect
                { ObjectID = ob.ObjectID, Effect = SpellEffect.Bleeding, EffectType = 0 });
        }
