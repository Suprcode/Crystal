    ...
    public class SnowWolfKing : MonsterObject {
        ...
        private void FindWeakerTarget() {
            ...
            for (int i = 0; i < targets.Count; i++) {
                if(targets[i].Stats[Stat.MinDC] >= Target.Stats[Stat.MinDC]) {
                    continue;
                }
        ...
        private bool TeleportToTarget(MapObject target) {
            ...
            MirDirection reverse = Functions.ReverseDirection(Direction);

            Point point = Functions.PointMove(target.CurrentLocation, reverse, 1);

            if(point != CurrentLocation) {
                if(Teleport(CurrentMap, point, true, 11)) {
                    return true;
                }
            }
        ...
        protected override void Attack() {
            ...
            if(Envir.Random.Next(3) > 0) {
                ...
                if(HealthPercent >= 60) {
                    ...
                    DelayedAction action = new(DelayedType.Damage, Envir.Time + 500, Target, damage,
                        DefenceType.ACAgility, false, true);
                    ActionList.Add(action);
                } else {
                    Broadcast(new ServerPacket.ObjectAttack
