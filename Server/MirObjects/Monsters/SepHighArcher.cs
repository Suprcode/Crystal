        ...
        protected override void Attack() {
            ...
            if(Envir.Random.Next(3) == 0 && Functions.InRange(CurrentLocation, Target.CurrentLocation, 2)) {
                Broadcast(new ServerPacket.ObjectMagic {
                    ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Spell = Spell.BackStep,
            ...
            if(hasPoisonBuff && Envir.Random.Next(2) == 0) {
                Broadcast(new ServerPacket.ObjectMagic {
                    ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Spell = Spell.CrippleShot,
                    TargetID = Target.ObjectID, Target = Target.CurrentLocation, Cast = true, Level = 3
                });
                        ...
                        for (int x = place.X - 1; x <= place.X + 1; x++) {
                            ...
                            for (int i = 0; i < cell.Objects.Count; i++) {
                                ...
                                if(targetob.Race != ObjectType.Monster && targetob.Race != ObjectType.Player &&
                                   targetob.Race != ObjectType.Hero) {
                                    continue;
                                }

                                if(targetob == null || !targetob.IsAttackTarget(this) || targetob.Node == null) {
                                    continue;
                                }
            ...
            Broadcast(new ServerPacket.ObjectMagic {
                ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation, Spell = Spell.PoisonShot,
                TargetID = Target.ObjectID, Target = Target.CurrentLocation, Cast = true, Level = 3
            });
