...
namespace Server.Library.MirObjects.Monsters {
    public class Trainer : MonsterObject {
        ...
        public override void PoisonDamage(int damage, MapObject attacker) {
            ...
            _currentAttacker.ReceiveChat(
                string.Format("{1} inflicted {0} Damage, Dps: {2:#.00}.", damage,
                    attacker is MonsterObject ? "Your pets poison" : "Your poison", Dps), ChatType.Trainer);
            Poisoned = true;
        }
        ...
        public override void ChangeHP(int amount) {
            ...
            double Dps = _totalDamage / (timespend * 0.001);
            _currentAttacker.ReceiveChat(string.Format("Your poison stopped {0} regen, Dps: {1:#.00}.", amount, Dps),
                ChatType.Trainer);
        }
        ...
        private void ReportDamage(int damage, DefenceType type, bool Pet) {
            ...
            double Dps = _totalDamage / (timespend * 0.001);
            _currentAttacker.ReceiveChat(
                string.Format("{3} inflicted {0} {1} Damage, Dps: {2:#.00}.", damage, output, Dps,
                    Pet ? "Your pet" : "You"), ChatType.Trainer);
        }
        ...
        private void OutputAverage() {
            ...
            double Dps = _totalDamage / (timespend * 0.001);
            _currentAttacker.ReceiveChat(
                string.Format("{0} Average Damage inflicted on the trainer, Dps: {1:#.00}.",
                    (int)(_totalDamage / _hitCount), Dps), ChatType.Trainer);
        }
    }
}
