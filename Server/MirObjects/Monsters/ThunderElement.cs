        ...
        public override int Pushed(MapObject pusher, MirDirection dir, int distance) {
            ...
            if(result > 0) {
                if(pusher is PlayerObject) {
                    Attacked((PlayerObject)pusher, Math.Max(50, Envir.Random.Next(Stats[Stat.HP])),
                        DefenceType.Repulsion);
                } else if(pusher is MonsterObject) {
                    Attacked((MonsterObject)pusher, Math.Max(50, Envir.Random.Next(Stats[Stat.HP])),
                        DefenceType.Repulsion);
                }
            }
