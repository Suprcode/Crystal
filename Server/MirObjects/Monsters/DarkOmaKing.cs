...
namespace Server.Library.MirObjects.Monsters {
    public class DarkOmaKing : MonsterObject {
        ...
        protected override void Attack() {
            ...
            if(Envir.Time > _OrbTime) {
                ...
                for (int j = 0; j < count; j++) {
                    for (int i = 0; i < attempts; i++) {
                        ...
                        if(location == CurrentLocation || location == Target.CurrentLocation) {
                            continue;
                        }

                        if(PowerBead.SpawnRandom(this, location)) {
                            break;
                        }
                    }
                }
            ...
            if(!ranged) {
                if(Envir.Random.Next(4) > 0) {
                    ...
                    DelayedAction action2 = new(DelayedType.Spawn, Envir.Time + start, ob);
        ...
        protected override void CompleteAttack(IList<object> data) {
            ...
            if(aoe) {
                List<MapObject> targets = FindAllTargets(5, CurrentLocation, false);
