...
namespace Server.Library.MirObjects.Monsters {
    ...
    public class HornedCommander : MonsterObject {
        ...
        private readonly List<SpellObject> _RockSpikeEffects = new();
        ...
        protected override void Attack() {
            ...
            if(_StartAdvanced && Envir.Random.Next(20) == 0) {
                ...
                Point front = Functions.PointMove(CurrentLocation, Direction, 2);

                MassSpawnRockFall(front, rockFallDuration);

                int damage = GetAttackPower(Stats[Stat.MinDC], Stats[Stat.MaxDC]) * rockFallLoops;

                DelayedAction action = new(DelayedType.RangeDamage, Envir.Time + rockFallDuration + 500, Target, damage,
                    DefenceType.AC, 5);
            ...
            if(_StartAdvanced && Envir.Random.Next(15) == 0) {
                ...
                DelayedAction action = new(DelayedType.Damage, Envir.Time + spinDuration + 500, Target, damage,
                    DefenceType.AC, true);
                ActionList.Add(action);

                action = new DelayedAction(DelayedType.Damage, Envir.Time + spinDuration + 1000, Target, damage,
                    DefenceType.AC, true);
                ActionList.Add(action);
            ...
            if(_StartAdvanced && Envir.Random.Next(10) == 0) {
                ...
                DelayedAction action = new(DelayedType.RangeDamage, Envir.Time + 300, Target, damage, DefenceType.AC,
                    3);
                ActionList.Add(action);
                return;
            }
            ...
            if(_StartAdvanced && Envir.Random.Next(10) == 0) {
                ...
                DelayedAction action = new(DelayedType.RangeDamage, Envir.Time + 300);
                ActionList.Add(action);
        ...
        protected override void CompleteRangeAttack(IList<object> data) {
            ...
            if(data.Count > 0) {
                ...
                Point front = Functions.PointMove(CurrentLocation, Direction, 2);
                List<MapObject> targets = FindAllTargets(aoeSize, front, false);
        ...
        public override bool TeleportRandom(int attempts, int distance, Map temp = null) {
            for (int i = 0; i < attempts; i++) {
                ...
                if(Teleport(CurrentMap, location, true, 10)) {
                    return true;
                }
            }
        ...
        private void MassSpawnRockFall(Point centerPoint, int duration) {
            ...
            for (int x = -range; x < range; x += 10) {
                for (int y = -range; y < range; y += 10) {
                    Point location = new Point(centerPoint.X + x, centerPoint.Y + y);
        ...
        private void SetupRockSpike() {
            int xLength = _RockSpikeArea.GetLength(0);
            int yLength = _RockSpikeArea.GetLength(1);

            int midX = ((int)Math.Ceiling((decimal)xLength / 2) - 1) * -1;
            int midY = ((int)Math.Ceiling((decimal)yLength / 2) - 1) * -1;
            ...
            for (int x = 0; x < xLength; x++) {
                for (int y = 0; y < yLength; y++) {
                    Point point = new Point(_MapCentre.X + (actualX * 5), _MapCentre.Y + (actualY * 5));
        ...
        private bool SpawnRockSpikes() {
            ...
            for (int x = 0; x < xLength; x++) {
                for (int y = 0; y < yLength; y++) {
                    Point point = _RockSpikeArea[x, y];

                    bool existing = _RockSpikeEffects.Any(x => x.CastLocation == point);
        ...
        private void SpawnBoulder() {
            ...
            for (int i = 0; i < 8; i++) {
                MonsterObject mob = GetMonster(Envir.GetMonsterInfo(Settings.HornedCommanderBombMob));

                bool odd = i % 2 != 0;

                Point point = Functions.PointMove(CurrentLocation, (MirDirection)i, odd ? 7 : 9);
        ...
        private void SpawnSlave() {
            ...
            MonsterObject mob = GetMonster(Envir.GetMonsterInfo(Settings.HornedCommanderMob));
        ...
        private void KillRockSpikes() {
            ...
            foreach(SpellObject effect in _RockSpikeEffects) {
