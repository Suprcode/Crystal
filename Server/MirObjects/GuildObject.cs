...
namespace Server.Library.MirObjects {
    public sealed class GuildObject {
        ...
        public List<GuildObject> WarringGuilds = new();

        public ConquestObject Conquest;

        public List<GuildObject> AllyGuilds = new();
        ...
        public void PlayerLogged(PlayerObject member, bool online, bool New = false) {
            ...
            SendServerPacket(new ServerPacket.GuildMemberChange()
                { Name = member.Name, Status = (byte)(New ? 2 : online ? 1 : 0) });
        ...
        public void NewMember(PlayerObject newMember) {
            ...
            GuildMember member = new() {
                Name = newMember.Info.Name, Player = newMember, Id = newMember.Info.Index, LastLogin = Envir.Now,
                Online = true
            };
        ...
        public bool ChangeRank(PlayerObject self, string memberName, byte rankIndex, string rankName = "Members") {
            ...
            CharacterInfo Character = Envir.GetCharacterInfo(memberName);
            if(Character == null) {
                return false;
            }
            ...
            if(rankIndex == 0 && Character.Level < Settings.Guild_RequiredLevel) {
                self.ReceiveChat(
                    String.Format("A guild leader needs to be at least level {0}", Settings.Guild_RequiredLevel),
                    ChatType.System);
                return false;
            }
            ...
            List<GuildRank> NewRankList = new() {
                Ranks[rankIndex]
            };
            NeedSave = true;
            ...
            if(player != null) {
                ...
                player.Enqueue(new ServerPacket.GuildMemberChange()
        ...
        public bool ChangeRankOption(PlayerObject Self, byte RankIndex, int Option, string Enabled) {
            ...
            Ranks[RankIndex].Options = Enabled == "true"
                ? Ranks[RankIndex].Options |= (GuildRankOptions)(1 << Option)
                : Ranks[RankIndex].Options ^= (GuildRankOptions)(1 << Option);
        ...
        public bool DeleteMember(PlayerObject Kicker, string membername) {
            ...
            if(MemberRank.Index == 0) {
                ...
                Kicker.ReceiveChat("You need to be the last leading member of the guild to disband the guild.",
                    ChatType.System);
                return false;
            }
        ...
        public void DeleteMember(string name) { //carefull this can lead to guild with no ranks or members(or no leader)
        ...
        public void MemberDeleted(string name, PlayerObject formerMember, bool kickSelf) {
            ...
            if(formerMember != null) {
                ...
                formerMember.ReceiveChat(
                    kickSelf ? "You have left your guild." : "You have been removed from your guild.", ChatType.Guild);
                formerMember.RefreshStats();
                formerMember.Enqueue(new ServerPacket.GuildStatus()
                    { GuildName = "", GuildRankName = "", MyOptions = (GuildRankOptions)0 });
        ...
        public bool HasRoom() {
            if(Info.Level < Settings.Guild_MembercapList.Count) {
                if(Settings.Guild_MembercapList[Info.Level] != 0 &&
                   Info.Membercount >= Settings.Guild_MembercapList[Info.Level]) {
                    return false;
                }
            }
        ...
        public void GainExp(uint amount) {
            ...
            long experience = Info.Experience;
        ...
        public bool GoToWar(GuildObject enemyGuild) {
            ...
            if(Envir.GuildsAtWar.Where(e => e.GuildA == this && e.GuildB == enemyGuild).Any() ||
               Envir.GuildsAtWar.Where(e => e.GuildA == enemyGuild && e.GuildB == this).Any()) {
                return false;
            }
        ...
        public void Process() {
            ...
            List<GuildBuff> UpdatedBuffs = new();
            for (int k = 0; k < BuffList.Count; k++) {
                if(BuffList[k].Info == null || BuffList[k].Info.TimeLimit == 0) {
        ...
        public void ActivateBuff(int Id) {
            ...
            if(Info.Gold < buff.Info.ActivationCost) {
                return;
            }
            ...
            SendServerPacket(new ServerPacket.GuildBuffList { ActiveBuffs = NewBuff });
            SendServerPacket(new ServerPacket.GuildStorageGoldChange()
