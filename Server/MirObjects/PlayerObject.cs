...
namespace Server.Library.MirObjects {
    public class PlayerObject : HumanObject {
            ...
            set {
                ...
                if(currentHero != null) {
                    ...
                    for (int i = 0; i < Info.Heroes.Length; i++) {
                        if(Info.Heroes[i].Index != currentHero.Index) {
                            continue;
                        }
        ...
        public override bool CanWalk => base.CanMove && !Fishing;
        public override bool CanAttack => base.CanAttack && !Fishing;

        protected override bool CanCast => base.CanCast && !Fishing;
        ...
        public const long TurnDelay = 350,
            HarvestDelay = 350,
            FishingCastDelay = 750,
            FishingDelay = 200,
            MovementDelay = 2000;
        ...
        public Dictionary<NpcSegment, bool> NpcSuccess = new();
        public bool NpcDelayed;
        public List<string> NpcSpeech = new();
        ...
        public int PageSent;
        public List<AuctionInfo> Search = new();
        ...
        public bool
            GuildNoticeChanged =
                true; //set to false first time client requests notice list, set to true each time someone in guild edits notice
        ...
        public List<QuestProgressInfo> CurrentQuests => Info.CurrentQuests;

        public List<int> CompletedQuests => Info.CompletedQuests;
        ...
        protected override void Load(CharacterInfo info, MirConnection connection) {
            ...
            if(HP == 0) {
                ...
                if(Info.PKPoints >= 200) {
                    ...
                    Point tempLocation = new(Settings.PKTownPositionX, Settings.PKTownPositionY);
        ...
        public override void Process() {
            if(Connection == null || Node == null || Info == null) {
                return;
            }
            ...
            if(Account.HasExpandedStorage && Envir.Now > Account.ExpandedStorageExpiryDate) {
                ...
                Enqueue(new ServerPacket.ResizeStorage {
                    Size = Account.Storage.Length, HasExpandedStorage = Account.HasExpandedStorage,
                    ExpiryTime = Account.ExpandedStorageExpiryDate
                });
            }
        ...
        public override void Die() {
            if(SpecialMode.HasFlag(SpecialItemMode.Revival) && Envir.Time > LastRevivalTime) {
                ...
                for (int i = (int)EquipmentSlot.RingL; i <= (int)EquipmentSlot.RingR; i++) {
            ...
            Enqueue(new ServerPacket.Death { Direction = Direction, Location = CurrentLocation });
            ...
            if(killer == null || killer.Race != ObjectType.Player) {
                for (int i = 0; i < Info.Equipment.Length; i++) {
                    ...
                    if(item.Count > 1) {
                        ...
                        ushort count = (ushort)Math.Ceiling(item.Count / 10F * percent);

                        if(count > item.Count) {
                            throw new ArgumentOutOfRangeException();
                        }

                        UserItem temp2 = Envir.CreateFreshItem(item.Info);
                        temp2.Count = count;
                        ...
                        if(Envir.ReturnRentalItem(item, item.RentalInformation?.OwnerName, Info)) {
                            ...
                            ReceiveChat($"You died and {item.Info.FriendlyName} has been returned to it's owner.",
                                ChatType.Hint);
        ...
        public override void WinExp(uint amount, uint targetLevel = 0) {
            ...
            if(GroupMembers != null) {
                ...
                if(nearCount > partyExpRate.Length) {
                    nearCount = partyExpRate.Length;
                }
                ...
                for (int i = 0; i < GroupMembers.Count; i++) {
                    ...
                    if(player.CurrentMap == CurrentMap &&
                       Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) && !player.Dead) {
                        player.GainExp((uint)((float)expPoint * partyExpRate[nearCount - 1] * (float)player.Level /
                                              (float)sumLevel));
                    }
                }
            } else {
        ...
        public override void GainExp(uint amount) {
            ...
            if(Info.Married != 0) {
                if(HasBuff(BuffType.Lover, out Buff buff)) {
                    ...
                    if(loverPlayer != null && loverPlayer.CurrentMap == CurrentMap &&
                       Functions.InRange(loverPlayer.CurrentLocation, CurrentLocation, Globals.DataRange) &&
                       !loverPlayer.Dead) {
            ...
            if(Info.Mentor != 0 && !Info.IsMentor) {
                if(HasBuff(BuffType.Mentee, out _)) {
                    ...
                    if(mentorPlayer != null && mentorPlayer.CurrentMap == CurrentMap &&
                       Functions.InRange(mentorPlayer.CurrentLocation, CurrentLocation, Globals.DataRange) &&
                       !mentorPlayer.Dead) {
            ...
            Enqueue(new ServerPacket.GainExperience { Amount = amount });

            for (int i = 0; i < Pets.Count; i++) {
                MonsterObject monster = Pets[i];
                if(monster.CurrentMap == CurrentMap &&
                   Functions.InRange(monster.CurrentLocation, CurrentLocation, Globals.DataRange) && !monster.Dead) {
        ...
        public override void LevelUp() {
            ...
            Enqueue(new ServerPacket.LevelChanged
                { Level = Level, Experience = Experience, MaxExperience = MaxExperience });
        ...
        public void CheckQuestInfo(QuestInfo info) {
            if(Connection.SentQuestInfo.Contains(info)) {
                return;
            }

            Enqueue(new ServerPacket.NewQuestInfo { Info = info.CreateClientQuestInfo() });
            Connection.SentQuestInfo.Add(info);
        }

        public void CheckRecipeInfo(RecipeInfo info) {
            if(Connection.SentRecipeInfo.Contains(info)) {
                return;
            }

            CheckItemInfo(info.Item.Info);

            foreach(UserItem tool in info.Tools) {
                CheckItemInfo(tool.Info);
            }

            foreach(UserItem ingredient in info.Ingredients) {
                CheckItemInfo(ingredient.Info);
            }

            Enqueue(new ServerPacket.NewRecipeInfo { Info = info.CreateClientRecipeInfo() });
            Connection.SentRecipeInfo.Add(info);
        }

        public void CheckMapInfo(MapInfo mapInfo) {
            if(!Connection.WorldMapSetupSent) {
                Enqueue(new ServerPacket.WorldMapSetupInfo
                    { Setup = Settings.WorldMapSetup, TeleportToNpcCost = Settings.TeleportToNpcCost });
                Connection.WorldMapSetupSent = true;
            }

            if(Connection.SentMapInfo.Contains(mapInfo)) {
                return;
            }

            Map map = Envir.GetMap(mapInfo.Index);
            if(map == null) {
                return;
            }
            ...
            foreach(MovementInfo mInfo in mapInfo.Movements.Where(x => x.ShowOnBigMap)) {
                ...
                ClientMovementInfo cmInfo = new ClientMovementInfo() {
            ...
            Enqueue(new ServerPacket.NewMapInfo { MapIndex = mapInfo.Index, Info = info });
        ...
        private void StartGameSuccess() {
            ...
            Enqueue(new ServerPacket.StartGame { Result = 4, Resolution = Settings.AllowedResolution });
            ...
            if(!string.IsNullOrWhiteSpace(Settings.Notice.Message) &&
               Settings.Notice.LastUpdate > Info.LastLogoutDate) {
                Enqueue(new ServerPacket.UpdateNotice { Notice = Settings.Notice });
            }
            ...
            SendBaseStats();
            GetObjectsPassive();
            Enqueue(new ServerPacket.TimeOfDay { Lights = Envir.Lights });
            Enqueue(new ServerPacket.ChangeAMode { Mode = AMode });
            Enqueue(new ServerPacket.ChangePMode { Mode = PMode });
            Enqueue(new ServerPacket.SwitchGroup { AllowGroup = AllowGroup });

            Enqueue(new ServerPacket.DefaultNpc { ObjectID = Envir.DefaultNpc.LoadedObjectID });

            Enqueue(new ServerPacket.GuildBuffList() { GuildBuffs = Settings.Guild_BuffList });
            RequestedGuildBuffInfo = true;

            if(Info.Thrusting) {
                Enqueue(new ServerPacket.SpellToggle { ObjectID = ObjectID, Spell = Spell.Thrusting, CanUse = true });
            }

            if(Info.HalfMoon) {
                Enqueue(new ServerPacket.SpellToggle { ObjectID = ObjectID, Spell = Spell.HalfMoon, CanUse = true });
            }

            if(Info.CrossHalfMoon) {
                Enqueue(new ServerPacket.SpellToggle
                    { ObjectID = ObjectID, Spell = Spell.CrossHalfMoon, CanUse = true });
            }

            if(Info.DoubleSlash) {
                Enqueue(new ServerPacket.SpellToggle { ObjectID = ObjectID, Spell = Spell.DoubleSlash, CanUse = true });
            }
            ...
            for (int i = 0; i < Info.Pets.Count; i++) {
                ...
                MonsterInfo monsterInfo = Envir.GetMonsterInfo(info.MonsterIndex);
                if(monsterInfo == null) {
                    continue;
                }
            ...
            if(MyGuild != null) {
                ...
                if(MyGuild.BuffList.Count > 0) {
                    Enqueue(new ServerPacket.GuildBuffList() { ActiveBuffs = MyGuild.BuffList });
                }
            }
        ...
        private void StartGameFailed() {
            Enqueue(new ServerPacket.StartGame { Result = 3 });
            CleanUp();
        }
        ...
        public void GiveRestedBonus(int count) {
            if(count > 0) {
                ...
                int duration = (int)Math.Min(int.MaxValue,
                    (Settings.RestedBuffLength * Settings.Minute * count) + existingTime);
        ...
        private static readonly ServerPacketIds[] BroadcastObservePackets = new ServerPacketIds[] {
        ...
        private void GetUserInfo(MirConnection c) {
            ...
            ServerPacket.UserInformation packet = new() {
                ObjectID = ObjectID,
        ...
        private void GetMapInfo(MirConnection c) {
            Enqueue(new ServerPacket.MapInformation {
                MapIndex = CurrentMap.Info.Index,
                FileName = CurrentMap.Info.FileName,
                Title = CurrentMap.Info.Title,
                MiniMap = CurrentMap.Info.MiniMap,
                ...
                for (int x = CurrentLocation.X - Globals.DataRange; x <= CurrentLocation.X + Globals.DataRange; x++) {
                    ...
                    for (int i = 0; i < cell.Objects.Count; i++) {
                        ...
                        if(ob.Race == ObjectType.Player) {
                            ...
                            if(obSpell.Spell != Spell.ExplosiveTrap ||
                               (obSpell.Caster != null && IsFriendlyTarget(obSpell.Caster))) {
        ...
        public override void RefreshNameColour() {
            Color prevColor = NameColour;
            NameColour = GetNameColour(this);

            if(prevColor == NameColour) {
                return;
            }
        ...
        public void Chat(string message, List<ChatItem> linkedItems = null) {
            ...
            if(message.StartsWith("/")) {
                ...
                player.ReceiveChat(string.Format("{0}=>{1}", Name, message.Remove(0, parts[0].Length)),
                    ChatType.WhisperIn);
                ...
                if(Envir.Time < ShoutTime) {
                    ReceiveChat(
                        string.Format("You cannot shout for another {0} seconds.",
                            Math.Ceiling((ShoutTime - Envir.Time) / 1000D)), ChatType.System);
                    return;
                }
                ...
                if(HasMapShout) {
                    ...
                    List<PlayerObject> playersInRange = new();

                    for (int i = 0; i < CurrentMap.Players.Count; i++) {
                        if(!Functions.InRange(CurrentLocation, CurrentMap.Players[i].CurrentLocation,
                               Globals.DataRange * 2)) {
                            continue;
                        }
                ...
                switch (parts[0].ToUpper()) {
                    ...
                    case "KILL":
                        ...
                        if(parts.Length >= 2) {
                            ...
                            if(!player.GMNeverDie) {
                                ...
                                ChatSystem.SystemMessage($"{player} was totally killed by GM: {Name}");
                            }
                        } else {
                    ...
                    case "CHANGEGENDER":
                        ...
                        ReceiveChat(string.Format("Player {0} has been changed to {1}", data.Name, data.Gender),
                            ChatType.System);
                        MessageQueue.Enqueue(string.Format("Player {0} has been changed to {1} by {2}", data.Name,
                            data.Gender, Name));

                        ChatSystem.SystemMessage(
                            $"{data.Player.Name} had gender change to {data.Gender.ToString()} by GM: {Name}");
                    ...
                    case "LEVEL":
                        if((!IsGM && !Settings.TestServer) || parts.Length < 2) {
                            return;
                        }
                        ...
                        if(parts.Length >= 3) {
                            ...
                            if(ushort.TryParse(parts[2], out level)) {
                                ...
                                ReceiveChat(
                                    string.Format("Player {0} has been Levelled {1} -> {2}.", player.Name, old,
                                        player.Level), ChatType.System);
                                MessageQueue.Enqueue(string.Format("Player {0} has been Levelled {1} -> {2} by {3}",
                                    player.Name, old, player.Level, Name));
                                ChatSystem.SystemMessage(
                                    $"Player {player.Name} has been Levelled: {old} -> {player.Level} by GM: {Name}");
                            ...
                            if(ushort.TryParse(parts[1], out level)) {
                                ...
                                ReceiveChat(string.Format("{0} {1} -> {2}.", GameLanguage.LevelUp, old, Level),
                                    ChatType.System);
                                MessageQueue.Enqueue(string.Format("Player {0} has been Leveled {1} -> {2} by {3}",
                                    Name, old, Level, Name));
                                return;
                            }
                        }
                    ...
                    case "LEVELHERO":
                        ...
                        if(parts.Length >= 3) {
                            ...
                            if(ushort.TryParse(parts[2], out level)) {
                                ...
                                ReceiveChat(
                                    string.Format("Player {0}'s hero has been Levelled {1} -> {2}.", player.Name, old,
                                        hero.Level), ChatType.System);
                                MessageQueue.Enqueue(string.Format(
                                    "Player {0}'s hero has been Levelled {1} -> {2} by {3}", player.Name, old,
                                    hero.Level, Name));
                                ChatSystem.SystemMessage(
                                    $"Player {player.Name}'s hero has been Levelled: {old} -> {hero.Level} by GM: {Name}");
                                return;
                            }
                        } else {
                            ...
                            if(ushort.TryParse(parts[1], out level)) {
                                ...
                                ReceiveChat(string.Format("{0} {1} -> {2}.", GameLanguage.LevelUp, old, hero.Level),
                                    ChatType.System);
                                MessageQueue.Enqueue(string.Format(
                                    "Player {0}'s hero has been Leveled {1} -> {2} by {3}", Name, old, hero.Level,
                                    Name));
                                return;
                            }
                        }
                    ...
                    case "MAKE": {
                        ...
                        ReceiveChat(string.Format("{0} x{1} has been created.", iInfo.FriendlyName, tempCount),
                            ChatType.System);
                        MessageQueue.Enqueue(string.Format("Player {0} has attempted to Create {1} x{2}", Name,
                            iInfo.Name, tempCount));
                    }
                        break;
                    case "CLEARBUFFS":
                    ...
                    case "CLEARBAG":
                        ...
                        for (int i = 0; i < player.Info.Inventory.Length; i++) {
                            ...
                            player.Enqueue(new ServerPacket.DeleteItem
                    ...
                    case "OBSERVE":
                        ...
                        if((!player.AllowObserve || !Settings.AllowObserve) && !IsGM) {
                            return;
                        }
                    ...
                    case "GROUPRECALL":
                        ...
                        if(Envir.Time < LastRecallTime) {
                            ReceiveChat(
                                string.Format("You cannot recall for another {0} seconds",
                                    (LastRecallTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                    ...
                    case "RECALLLOVER":
                        ...
                        if(Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing == Info.Married) {
                            ...
                            if(player.Info.Equipment[(int)EquipmentSlot.RingL] == null) {
                                player.ReceiveChat(
                                    string.Format("You need to wear a Wedding Ring for recall.", Lover.Name),
                                    ChatType.System);
                                ReceiveChat(string.Format("{0} Isn't wearing a Wedding Ring.", Lover.Name),
                                    ChatType.System);
                                return;
                            }
                            ...
                            if(player.Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing != player.Info.Married) {
                                player.ReceiveChat(
                                    string.Format("You need to wear a Wedding Ring on your left finger for recall.",
                                        Lover.Name), ChatType.System);
                                ReceiveChat(string.Format("{0} Isn't wearing a Wedding Ring.", Lover.Name),
                                    ChatType.System);
                                return;
                            }
                            ...
                            if(!player.AllowLoverRecall) {
                                player.ReceiveChat("A recall was attempted without your permission",
                                    ChatType.System);
                                ReceiveChat(string.Format("{0} is blocking Lover Recall.", player.Name),
                                    ChatType.System);
                                return;
                            }
                        ...
                        break;
                    case "TIME":
                        ReceiveChat(string.Format("The time is : {0}", Envir.Now.ToString("hh:mm tt")),
                            ChatType.System);
                        break;
                    ...
                    case "ROLL":
                        ...
                        for (int i = 0; i < GroupMembers.Count; i++) {
                            ...
                            playerSend.ReceiveChat(string.Format("{0} has rolled a {1}", Name, diceNum),
                                ChatType.Group);
                        }
                    ...
                    case "MAP":
                        string mapName = CurrentMap.Info.FileName;
                        string mapTitle = CurrentMap.Info.Title;
                        ReceiveChat(string.Format("You are currently in {0}. Map ID: {1}", mapTitle, mapName),
                            ChatType.System);
                        break;

                    case "BACKUPPLAYER": {
                        if(!IsGM || parts.Length < 2) {
                            return;
                        }
                    ...
                    case "ARCHIVEPLAYER": {
                        ...
                        AccountInfo account = Envir.GetAccountByCharacter(parts[1]);

                        if(account == null) {
                            ReceiveChat(string.Format("Player {0} was not found in any account", parts[1]),
                                ChatType.System);
                            return;
                        }
                    ...
                    case "LOADPLAYER": {
                        ...
                        CharacterInfo bak = Envir.GetArchivedCharacter(parts[1]);

                        if(bak == null) {
                            ReceiveChat(
                                string.Format(
                                    "Player {0} could not be loaded. Try specifying the full archive filename",
                                    parts[1]), ChatType.System);
                            return;
                        }

                        CharacterInfo info = Envir.GetCharacterInfo(bak.Name);
                    ...
                    case "RESTOREPLAYER": {
                        ...
                        if(parts.Length > 2) {
                            ...
                            if(account.Characters.Count >= Globals.MaxCharacterCount) {
                                ReceiveChat(
                                    string.Format("Account {0} already has {1} characters", parts[2],
                                        Globals.MaxCharacterCount), ChatType.System);
                                return;
                            }
                        }
                        ...
                        if(data == null) {
                            if(account != null) {
                                ...
                                if(data == null) {
                                    ReceiveChat(
                                        string.Format(
                                            "Player {0} could not be restored. Try specifying the full archive filename",
                                            parts[1]), ChatType.System);
                                    return;
                                }
                    ...
                    case "MOVE":
                        if(!IsGM && !SpecialMode.HasFlag(SpecialItemMode.Teleport) && !Settings.TestServer) {
                            return;
                        }
                        ...
                        if(Envir.Time < LastTeleportTime) {
                            ReceiveChat(
                                string.Format("You cannot teleport for another {0} seconds",
                                    (LastTeleportTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                    ...
                    case "MAPMOVE":
                        ...
                        Map map = Envir.GetMapByNameAndInstance(parts[1], instanceID);
                        if(map == null) {
                            ReceiveChat(string.Format("Map {0}:[{1}] could not be found", parts[1], instanceID),
                                ChatType.System);
                            return;
                        }
                        ...
                        switch (parts.Length) {
                            case 2:
                                ReceiveChat(
                                    TeleportRandom(200, 0, map)
                                        ? string.Format("Moved to Map {0}", map.Info.FileName)
                                        : string.Format("Failed movement to Map {0}", map.Info.FileName),
                                    ChatType.System);
                                break;
                            case 3:
                                ReceiveChat(
                                    TeleportRandom(200, 0, map)
                                        ? string.Format("Moved to Map {0}:[{1}]", map.Info.FileName, instanceID)
                                        : string.Format("Failed movement to Map {0}:[{1}]", map.Info.FileName,
                                            instanceID), ChatType.System);
                                break;
                            case 4:
                                ReceiveChat(
                                    Teleport(map, new Point(x, y))
                                        ? string.Format("Moved to Map {0} at {1}:{2}", map.Info.FileName, x, y)
                                        : string.Format("Failed movement to Map {0} at {1}:{2}", map.Info.FileName, x,
                                            y), ChatType.System);
                                break;
                            case 5:
                                ReceiveChat(
                                    Teleport(map, new Point(x, y))
                                        ? string.Format("Moved to Map {0}:[{1}] at {2}:{3}", map.Info.FileName,
                                            instanceID, x, y)
                                        : string.Format("Failed movement to Map {0}:[{1}] at {2}:{3}",
                                            map.Info.FileName, instanceID, x, y), ChatType.System);
                                break;
                        }
                    ...
                    case "MOB":
                        ...
                        if(parts.Length >= 3 && IsGM) {
                            if(!uint.TryParse(parts[2], out count)) {
                                count = 1;
                            }
                        }
                        ...
                        for (int i = 0; i < count; i++) {
                            ...
                            if(spread == 0) {
                                ...
                                for (int _ = 0; _ < 20; _++) {
                                    if(monster.Spawn(CurrentMap,
                                           CurrentLocation.Add(Envir.Random.Next(-spread, spread + 1),
                                               Envir.Random.Next(-spread, spread + 1)))) {
                                        break;
                                    }
                                }
                            }
                        ...
                        ReceiveChat(string.Format("Monster {0} x{1} has been spawned.", mInfo.Name, count),
                            ChatType.System);
                        break;
                    ...
                    case "RECALLMOB":
                        ...
                        if(parts.Length > 2) {
                            if(!uint.TryParse(parts[2], out count) || count > 50) {
                                count = 1;
                            }
                        }

                        if(parts.Length > 3) {
                            if(!byte.TryParse(parts[3], out petlevel) || petlevel > 7) {
                                petlevel = 0;
                            }
                        }

                        if(!IsGM && Pets.Count(t => !t.Dead && t.Race != ObjectType.Creature) >= Globals.MaxPets) {
                            return;
                        }
                        ...
                        ReceiveChat(string.Format("Pet {0} x{1} has been recalled.", mInfo2.Name, count),
                            ChatType.System);
                        break;
                    ...
                    case "GIVEGOLD":
                        ...
                        } else if(!uint.TryParse(parts[1], out count)) {
                            return;
                        }
                        ...
                        ChatSystem.SystemMessage(goldMsg);
                    ...
                    case "GIVEPEARLS":
                        ...
                        string pearlMsg = count == 1
                            ? $"Player {player.Name} has been given 1 pearl by GM: {Name}"
                            : $"Player {player.Name} has been given {count} pearls by GM: {Name}";

                        MessageQueue.Enqueue(pearlMsg);
                        ChatSystem.SystemMessage(pearlMsg);
                    ...
                    case "GIVECREDIT":
                        ...
                        ChatSystem.SystemMessage(creditMsg);

                        break;
                    case "GIVESKILL":
                        if((!IsGM && !Settings.TestServer) || parts.Length < 3) {
                            return;
                        }
                        ...
                        if(!Enum.TryParse(parts.Length > 3 ? parts[2] : parts[1], true, out skill)) {
                            return;
                        }
                        ...
                        spellLevel = byte.TryParse(parts.Length > 3 ? parts[3] : parts[2], out spellLevel)
                            ? Math.Min((byte)3, spellLevel)
                            : (byte)0;
                        ...
                        UserMagic magic = new UserMagic(skill) { Level = spellLevel };

                        if(player.Info.Magics.Any(e => e.Spell == skill)) {
                            player.Info.Magics.FirstOrDefault(e => e.Spell == skill).Level = spellLevel;

                            string skillChangeMsg =
                                $"{player} Spell {skill.ToString()} changed to level {spellLevel} by GM: {Name}";

                            player.ReceiveChat(
                                string.Format("Spell {0} changed to level {1}", skill.ToString(), spellLevel),
                                ChatType.Hint);
                            ChatSystem.SystemMessage(skillChangeMsg);

                            return;
                        } else {
                            player.ReceiveChat(
                                string.Format("You have learned {0} at level {1}", skill.ToString(), spellLevel),
                                ChatType.Hint);

                            if(player != this) {
                                ReceiveChat(
                                    string.Format("{0} has learned {1} at level {2}", player.Name, skill.ToString(),
                                        spellLevel), ChatType.Hint);
                            }

                            string skillLearnedMg =
                                $"{player} Spell {skill.ToString()} learnt and set to level {spellLevel} by GM: {Name}";
                            ChatSystem.SystemMessage(skillLearnedMg);
                    ...
                    case "FIND":
                        if(!IsGM && !SpecialMode.HasFlag(SpecialItemMode.Probe)) {
                            return;
                        }

                        if(Envir.Time < LastProbeTime) {
                            ReceiveChat(
                                string.Format("You cannot search for another {0} seconds",
                                    (LastProbeTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                        ...
                        ReceiveChat(
                            string.Format("{0} is located at {1} ({2},{3})", player.Name, player.CurrentMap.Info.Title,
                                player.CurrentLocation.X, player.CurrentLocation.Y), ChatType.System);
                        break;
                    ...
                    case "CREATEGUILD":
                        ...
                        if(player.MyGuild != null) {
                            ReceiveChat(string.Format("Player {0} is already in a guild.", player.Name),
                                ChatType.System);
                            return;
                        }
                    ...
                    case "SETFLAG":
                        ...
                        if(!int.TryParse(parts[1], out tempInt)) {
                            return;
                        }

                        if(tempInt > Info.Flags.Length - 1) {
                            return;
                        }
                        ...
                        for (int f = CurrentMap.Npcs.Count - 1; f >= 0; f--) {
                            if(Functions.InRange(CurrentMap.Npcs[f].CurrentLocation, CurrentLocation,
                                   Globals.DataRange)) {
                    ...
                    case "CHANGECLASS": //@changeclass [Player] [Class]
                        ...
                        if(!Enum.TryParse(parts[parts.Length - 1], true, out mirClass) || data.Class == mirClass) {
                            return;
                        }

                        data.Class = mirClass;

                        ReceiveChat(string.Format("Player {0} has been changed to {1}", data.Name, data.Class),
                            ChatType.System);
                        MessageQueue.Enqueue(string.Format("Player {0} has been changed to {1} by {2}", data.Name,
                            data.Class, Name));

                        ChatSystem.SystemMessage(
                            $"{data.Player.Name} class changed to {data.Class.ToString()} by GM: {Name}");
                    ...
                    case "AWAKENING": {
                        ...
                        if(!Enum.TryParse(parts[2], true, out awakeType)) {
                            return;
                        }
                        ...
                        foreach(UserItem temp in Info.Equipment) {
                            ...
                            ItemInfo realItem =
                                Functions.GetRealItem(temp.Info, Info.Level, Info.Class, Envir.ItemInfoList);
                            ...
                            if(realItem.Type == type) {
                                ...
                                switch (result) {
                                    case -1:
                                        ReceiveChat(string.Format("{0} : Condition Error.", temp.FriendlyName),
                                            ChatType.System);
                                        break;
                                    case 0:
                                        ReceiveChat(string.Format("{0} : Upgrade Failed.", temp.FriendlyName),
                                            ChatType.System);
                                        break;
                                    case 1:
                                        ReceiveChat(
                                            string.Format("{0} : AWAKE Level {1}, value {2}~{3}.", temp.FriendlyName,
                                                awake.GetAwakeLevel(), awake.GetAwakeValue(), awake.GetAwakeValue()),
                                            ChatType.System);
                                        p = new ServerPacket.RefreshItem { Item = temp };
                                        Enqueue(p);
                                        break;
                                    default:
                                        break;
                    ...
                    case "REMOVEAWAKENING": {
                        ...
                        foreach(UserItem temp in Info.Equipment) {
                            ...
                            if(realItem.Type == type) {
                                ...
                                switch (result) {
                                    case 0:
                                        ReceiveChat(string.Format("{0} : Remove failed Level 0", temp.FriendlyName),
                                            ChatType.System);
                                        break;
                                    case 1:
                                        ReceiveChat(
                                            string.Format("{0} : Remove success. Level {1}", temp.FriendlyName,
                                                temp.Awake.GetAwakeLevel()), ChatType.System);
                                        p = new ServerPacket.RefreshItem { Item = temp };
                                        Enqueue(p);
                                        break;
                                    default:
                    ...
                    case "STARTWAR":
                        ...
                        if(MyGuild.GoToWar(enemyGuild)) {
                            ...
                            enemyGuild.SendMessage(string.Format("{0} has started a war", MyGuild.Name),
                                ChatType.System);
                        }
                    ...
                    case "ADDINVENTORY": {
                        ...
                        if(Account.Gold >= openGold) {
                            ...
                            Enqueue(new ServerPacket.LoseGold { Gold = openGold });
                            Enqueue(new ServerPacket.ResizeInventory { Size = Info.ResizeInventory() });
                    ...
                    case "ADDSTORAGE": {
                        TimeSpan addedTime = new(10, 0, 0, 0);
                        uint cost = 1000000;
                        ...
                        if(Account.Gold >= cost) {
                            ...
                            if(Account.ExpandedStorageExpiryDate > Envir.Now) {
                                ...
                                ReceiveChat(
                                    GameLanguage.ExpandedStorageExpiresOn +
                                    Account.ExpandedStorageExpiryDate.ToString(), ChatType.System);
                            } else {
                            ...
                            Enqueue(new ServerPacket.LoseGold { Gold = cost });
                            Enqueue(new ServerPacket.ResizeStorage {
                                Size = Account.ExpandStorage(), HasExpandedStorage = Account.HasExpandedStorage,
                                ExpiryTime = Account.ExpandedStorageExpiryDate
                            });
                        } else {
                            ReceiveChat(GameLanguage.LowGold, ChatType.System);
                    ...
                    case "ALLOWOBSERVE":
                        AllowObserve = !AllowObserve;
                        Enqueue(new ServerPacket.AllowObserve { Allow = AllowObserve });
                        break;
                    ...
                    case "INFO": {
                        ...
                        switch (ob.Race) {
                            case ObjectType.Player:
                                ...
                                ReceiveChat(
                                    string.Format("Name : {0}, Level : {1}, X : {2}, Y : {3}", plOb.Name, plOb.Level,
                                        plOb.CurrentLocation.X, plOb.CurrentLocation.Y), ChatType.System2);
                                break;
                            case ObjectType.Monster:
                                MonsterObject monOb = (MonsterObject)ob;
                                ReceiveChat("--Monster Info--", ChatType.System2);
                                ReceiveChat(string.Format("ID : {0}, Name : {1}", monOb.Info.Index, monOb.Name),
                                    ChatType.System2);
                                ReceiveChat(
                                    string.Format("Level : {0}, X : {1}, Y : {2}, Dir: {3}", monOb.Level,
                                        monOb.CurrentLocation.X, monOb.CurrentLocation.Y, monOb.Direction),
                                    ChatType.System2);
                                ReceiveChat(
                                    string.Format("HP : {0}, MinDC : {1}, MaxDC : {2}", monOb.Info.Stats[Stat.HP],
                                        monOb.Stats[Stat.MinDC], monOb.Stats[Stat.MaxDC]), ChatType.System2);
                                break;
                            case ObjectType.Merchant:
                                NpcObject npcOb = (NpcObject)ob;
                                ReceiveChat("--Npc Info--", ChatType.System2);
                                ReceiveChat(string.Format("ID : {0}, Name : {1}", npcOb.Info.Index, npcOb.Name),
                                    ChatType.System2);
                                ReceiveChat(
                                    string.Format("X : {0}, Y : {1}", ob.CurrentLocation.X, ob.CurrentLocation.Y),
                                    ChatType.System2);
                    ...
                    case "SETQUEST":
                        ...
                        QuestProgressInfo activeQuest = player.CurrentQuests.FirstOrDefault(e => e.Index == questID);
                    ...
                    case "STARTCONQUEST": {
                        ...
                        if(MyGuild == null) {
                            ...
                            return;
                        } else if(!int.TryParse(parts[1], out conquestID)) {
                            return;
                        }
                    ...
                    case "GATES":
                        if(MyGuild == null || MyGuild.Conquest == null ||
                           !MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeRank) || MyGuild.Conquest.WarIsOn) {
                            ReceiveChat(string.Format("You don't have access to control any gates at the moment."),
                                ChatType.System);
                            return;
                        }
                        ...
                        if(parts.Length > 1) {
                            ...
                            if(openclose.ToUpper() == "CLOSE") {
                                ...
                                ReceiveChat(string.Format("You must type /Gates Open or /Gates Close."),
                                    ChatType.System);
                                return;
                            }

                            for (int i = 0; i < MyGuild.Conquest.GateList.Count; i++) {
                                if(MyGuild.Conquest.GateList[i].Gate != null &&
                        ...
                        if(openClose) {
                            ReceiveChat(string.Format("The gates at {0} have been closed.", MyGuild.Conquest.Info.Name),
                                ChatType.System);
                        } else {
                            ReceiveChat(string.Format("The gates at {0} have been opened.", MyGuild.Conquest.Info.Name),
                                ChatType.System);
                        }
                    ...
                    case "CHANGEFLAG":
                        if(MyGuild == null || MyGuild.Conquest == null ||
                           !MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeRank) || MyGuild.Conquest.WarIsOn) {
                            ReceiveChat(string.Format("You don't have access to change any flags at the moment."),
                                ChatType.System);
                            return;
                        }
                    ...
                    case "REVIVE":
                        ...
                        if(parts.Length < 2) {
                            ...
                            ChatSystem.SystemMessage($"{player} was revived to full health by GM: {Name}");
                        }
                    ...
                    case "DELETESKILL":
                        ...
                        if(!Enum.TryParse(parts.Length > 2 ? parts[2] : parts[1], true, out skill1)) {
                            return;
                        }

                        if(skill1 == Spell.None) {
                            return;
                        }
                        ...
                        UserMagic magics = new UserMagic(skill1);
                        bool removed = false;

                        for (int i = player.Info.Magics.Count - 1; i >= 0; i--) {
                            if(player.Info.Magics[i].Spell != skill1) {
                                continue;
                            }

                            player.Info.Magics.RemoveAt(i);
                            player.Enqueue(new ServerPacket.RemoveMagic { PlaceId = i });
                            removed = true;
                        }

                        if(removed) {
                            ReceiveChat(
                                string.Format("You have deleted skill {0} from player {1}", skill1.ToString(),
                                    player.Name), ChatType.Hint);
                            player.ReceiveChat(string.Format("{0} has been removed from you.", skill1), ChatType.Hint);

                            ChatSystem.SystemMessage($"{player} Skill {skill1.ToString()} was removed by GM: {Name}");
                        } else {
                    ...
                    case "SETTIMER":
                        ...
                        if(!byte.TryParse(parts[3], out byte timerType)) {
                            return;
                        }
                    ...
                    case "SETLIGHT":
                        ...
                        if(!byte.TryParse(parts[1], out byte light)) {
                            return;
                        }
                ...
                foreach(string command in Envir.CustomCommands) {
                    if(string.Compare(parts[0], command, true) != 0) {
                        continue;
                    }
        ...
        private string ProcessChatItems(string text, List<PlayerObject> recipients, List<ChatItem> chatItems) {
            ...
            foreach(ChatItem chatItem in chatItems) {
                Regex r = new(chatItem.RegexInternalName, RegexOptions.IgnoreCase);
                ...
                for (int i = 0; i < array.Length; i++) {
                    item = array[i];
                    if(item == null || item.UniqueID != chatItem.UniqueID) {
                        continue;
                    }
                ...
                if(item != null) {
                    if(recipients == null) {
                        for (int i = CurrentMap.Players.Count - 1; i >= 0; i--) {
                            ...
                            if(player == null || player.Info == null || player.Node == null) {
                                continue;
                            }
                            ...
                            if(Functions.InRange(CurrentLocation, player.CurrentLocation, Globals.DataRange)) {
                                ...
                                if(!player.Connection.SentChatItem.Contains(item)) {
                                    player.Enqueue(new ServerPacket.NewChatItem { Item = item });
                    ...
                    if(!Connection.SentChatItem.Contains(item)) {
                        Enqueue(new ServerPacket.NewChatItem { Item = item });
        ...
        public void Turn(MirDirection dir) {
            ...
            if(CanMove) {
                ...
                Direction = dir;
                if(CheckMovement(CurrentLocation)) {
                    return;
                }
        ...
        public void Harvest(MirDirection dir) {
            ...
            Enqueue(new ServerPacket.UserLocation { Direction = Direction, Location = CurrentLocation });
            Broadcast(new ServerPacket.ObjectHarvest
                { ObjectID = ObjectID, Direction = Direction, Location = CurrentLocation });
            ...
            for (int d = 0; d <= 1; d++) {
                for (int y = front.Y - d; y <= front.Y + d; y++) {
                    ...
                    for (int x = front.X - d; x <= front.X + d; x += Math.Abs(y - front.Y) == d ? 1 : d * 2) {
                        ...
                        for (int i = 0; i < cell.Objects.Count; i++) {
                            MapObject ob = cell.Objects[i];
                            if(ob.Race != ObjectType.Monster || !ob.Dead || ob.Harvested) {
                                continue;
                            }
        ...
        private UserItem GetBait(int count) {
            ...
            if(item == null || item.Info.Type != ItemType.Weapon || !item.Info.IsFishingRod) {
                return null;
            }

            UserItem bait = item.Slots[(int)FishingSlot.Bait];

            if(bait == null || bait.Count < count) {
                return null;
            }
        ...
        private void DeleteFishingItem(FishingSlot type) {
            ...
            Enqueue(new ServerPacket.DeleteItem { UniqueID = slotItem.UniqueID, Count = 1 });
        ...
        public override bool CheckMovement(Point location) {
            ...
            for (int i = 0; i < CurrentMap.Info.Movements.Count; i++) {
                ...
                if(info.NeedHole) {
                    ...
                    if(cell.Objects == null ||
                       cell.Objects.Where(ob => ob.Race == ObjectType.Spell).All(ob =>
                           ((SpellObject)ob).Spell != Spell.DigOutZombie &&
                           ((SpellObject)ob).Spell != Spell.DigOutArmadillo)) {
                        continue;
                    }
                }
                ...
                if(info.ConquestIndex > 0) {
                    ...
                    if(MyGuild.Conquest.Info.Index != info.ConquestIndex) {
                        continue;
                    }
                }
                ...
                if(temp == null || !temp.ValidPoint(info.Destination)) {
                    continue;
                }
        ...
        private void CompleteMapMovement(params object[] data) {
            ...
            if(CurrentMap != checkmap || CurrentLocation != checklocation) {
                return;
            }
        ...
        public bool TeleportEscape(int attempts) {
            ...
            for (int i = 0; i < attempts; i++) {
                Point location = new(BindLocation.X + Envir.Random.Next(-100, 100),
                    BindLocation.Y + Envir.Random.Next(-100, 100));

                if(Teleport(temp, location)) {
                    return true;
                }
            }
        ...
        public override bool MagicTeleport(UserMagic magic) {
            ...
            for (int i = 0; i < 200; i++) {
                Point location = new(BindLocation.X + Envir.Random.Next(-mapSizeX, mapSizeX),
        ...
        public override bool IsAttackTarget(HumanObject attacker) {
            ...
            if(Dead || InSafeZone || attacker.InSafeZone || attacker == this || GMGameMaster) {
                return false;
            }

            if(CurrentMap.Info.NoFight) {
                return false;
            }
        ...
        public override bool IsAttackTarget(MonsterObject attacker) {
            ...
            if(Dead || attacker.Master == this || GMGameMaster) {
                return false;
            }
            ...
            if(attacker.Master == null) {
                return true;
            }

            if(InSafeZone || attacker.InSafeZone || attacker.Master.InSafeZone) {
                return false;
            }
            ...
            if(LastHitter != attacker.Master && attacker.Master.LastHitter != this) {
                ...
                for (int i = 0; i < attacker.Master.Pets.Count; i++) {
                    if(attacker.Master.Pets[i].Target != this) {
                        continue;
                    }
        ...
        public override bool IsFriendlyTarget(MonsterObject ally) {
            if(ally.Race != ObjectType.Monster) {
                return false;
            }
            ...
            switch (ally.Master.Race) {
                case ObjectType.Player:
                    if(!ally.Master.IsFriendlyTarget(this)) {
                        return false;
                    }
        ...
        public override Packet GetInfo() {
            ...
            return new ServerPacket.ObjectPlayer {
                ...
                Dead = Dead,
                Hidden = Hidden,
                Effect = HasBuff(BuffType.MagicShield, out _) ? SpellEffect.MagicShieldUp :
                    HasBuff(BuffType.ElementalBarrier, out _) ? SpellEffect.ElementalBarrierUp : SpellEffect.None,
        ...
        public void EquipSlotItem(MirGridType grid, ulong id, int to, MirGridType gridTo, ulong idTo) {
            ServerPacket.EquipSlotItem p = new()
                { Grid = grid, UniqueID = id, To = to, GridTo = gridTo, Success = false };
            ...
            switch (gridTo) {
                ...
                case MirGridType.Socket:
                    ...
                    for (int i = 0; i < Info.Equipment.Length; i++) {
                        ...
                        if(temp2 == null || temp2.UniqueID != idTo) {
                            continue;
                        }
            ...
            switch (grid) {
                case MirGridType.Inventory:
                    ...
                    break;
                case MirGridType.Storage:
                    if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.StorageKey,
                    ...
                    for (int i = 0; i < CurrentMap.Npcs.Count; i++) {
                        if(CurrentMap.Npcs[i].ObjectID != NpcObjectID) {
                            continue;
                        }
        ...
        public void RemoveItem(MirGridType grid, ulong id, int to) {
            ServerPacket.RemoveItem p = new() { Grid = grid, UniqueID = id, To = to, Success = false };
        ...
        public void RemoveSlotItem(MirGridType grid, ulong id, int to, MirGridType gridTo, ulong idFrom) {
            ServerPacket.RemoveSlotItem p = new()
                { Grid = grid, UniqueID = id, To = to, GridTo = gridTo, Success = false };
            UserItem[] array;
            switch (gridTo) {
                case MirGridType.Inventory:
            ...
            switch (grid) {
                ...
                case MirGridType.Socket:
                    ...
                    for (int i = 0; i < Info.Equipment.Length; i++) {
                        temp2 = Info.Equipment[i];
                        if(temp2 == null || temp2.UniqueID != idFrom) {
                            continue;
                        }
            ...
            for (int i = 0; i < temp.Slots.Length; i++) {
                ...
                if(slotTemp == null || slotTemp.UniqueID != id) {
                    continue;
                }
            ...
            if(!CanRemoveItem(gridTo, slotTemp)) {
                return;
            }
        ...
        public void MoveItem(MirGridType grid, int from, int to) {
            ServerPacket.MoveItem p = new() { Grid = grid, From = from, To = to, Success = false };
            UserItem[] array;
            switch (grid) {
                case MirGridType.Inventory:
                    array = Info.Inventory;
            ...
            if(from >= 0 && to >= 0 && from < array.Length && to < array.Length) {
                if(array[from] == null) {
                    ...
                    ReceiveChat("Item Move Error - Please report the item you tried to move and the time",
                        ChatType.System);
                    Enqueue(p);
                    return;
                }
        ...
        public void StoreItem(int from, int to) {
            ServerPacket.StoreItem p = new() { From = from, To = to, Success = false };

            if(NpcPage == null ||
               !String.Equals(NpcPage.Key, NpcScript.StorageKey, StringComparison.CurrentCultureIgnoreCase)) {
                Enqueue(p);
                return;
            }
        ...
        public void TakeBackItem(int from, int to) {
            ServerPacket.TakeBackItem p = new() { From = from, To = to, Success = false };
        ...
        public void EquipItem(MirGridType grid, ulong id, int to) {
            ServerPacket.EquipItem p = new() { Grid = grid, UniqueID = id, To = to, Success = false };
        ...
        public void TakeBackHeroItem(int from, int to) {
            ServerPacket.TakeBackHeroItem p = new() { From = from, To = to, Success = false };
        ...
        public void TransferHeroItem(int from, int to) {
            ServerPacket.TransferHeroItem p = new() { From = from, To = to, Success = false };
        ...
        public override void UseItem(ulong id) {
            ServerPacket.UseItem p = new() { UniqueID = id, Grid = MirGridType.Inventory, Success = false };
            ...
            switch (item.Info.Type) {
                case ItemType.Potion:
                    switch (item.Info.Shape) {
                        case 0: //NormalPotion
                            PotHealthAmount = (ushort)Math.Min(ushort.MaxValue,
                                PotHealthAmount + item.Info.Stats[Stat.HP]);
                        ...
                        case 3: //Buff
                        {
                            ...
                            if(item.GetTotal(Stat.MaxDC) > 0) {
                                AddBuff(BuffType.Impact, this, time * Settings.Minute,
                                    new Stats { [Stat.MaxDC] = item.GetTotal(Stat.MaxDC) });
                            }

                            if(item.GetTotal(Stat.MaxMC) > 0) {
                                AddBuff(BuffType.Magic, this, time * Settings.Minute,
                                    new Stats { [Stat.MaxMC] = item.GetTotal(Stat.MaxMC) });
                            }

                            if(item.GetTotal(Stat.MaxSC) > 0) {
                                AddBuff(BuffType.Taoist, this, time * Settings.Minute,
                                    new Stats { [Stat.MaxSC] = item.GetTotal(Stat.MaxSC) });
                            }

                            if(item.GetTotal(Stat.AttackSpeed) > 0) {
                                AddBuff(BuffType.Storm, this, time * Settings.Minute,
                                    new Stats { [Stat.AttackSpeed] = item.GetTotal(Stat.AttackSpeed) });
                            }

                            if(item.GetTotal(Stat.HP) > 0) {
                                AddBuff(BuffType.HealthAid, this, time * Settings.Minute,
                                    new Stats { [Stat.HP] = item.GetTotal(Stat.HP) });
                            }

                            if(item.GetTotal(Stat.MP) > 0) {
                                AddBuff(BuffType.ManaAid, this, time * Settings.Minute,
                                    new Stats { [Stat.MP] = item.GetTotal(Stat.MP) });
                            }

                            if(item.GetTotal(Stat.MaxAC) > 0) {
                                AddBuff(BuffType.Defence, this, time * Settings.Minute,
                                    new Stats { [Stat.MaxAC] = item.GetTotal(Stat.MaxAC) });
                            }

                            if(item.GetTotal(Stat.MaxMAC) > 0) {
                                AddBuff(BuffType.MagicDefence, this, time * Settings.Minute,
                                    new Stats { [Stat.MaxMAC] = item.GetTotal(Stat.MaxMAC) });
                            }

                            if(item.GetTotal(Stat.BagWeight) > 0) {
                                AddBuff(BuffType.BagWeight, this, time * Settings.Minute,
                                    new Stats { [Stat.BagWeight] = item.GetTotal(Stat.BagWeight) });
                            }
                        }
                            break;
                        case 4: //Exp
                        {
                            int time = item.Info.Durability;
                            AddBuff(BuffType.Exp, this, Settings.Minute * time,
                                new Stats { [Stat.ExpRatePercent] = item.GetTotal(Stat.Luck) });
                        }
                            break;
                        case 5: //Drop
                        {
                            int time = item.Info.Durability;
                            AddBuff(BuffType.Drop, this, Settings.Minute * time,
                                new Stats { [Stat.ItemDropRatePercent] = item.GetTotal(Stat.Luck) });
                        }
                            break;
                    }
                ...
                case ItemType.Scroll:
                    ...
                    switch (item.Info.Shape) {
                        ...
                        case 4: //RepairOil
                            ...
                            temp.MaxDura = (ushort)Math.Max(0,
                            ...
                            Enqueue(new ServerPacket.ItemRepaired
                        ...
                        case 7: //CreditScroll
                            if(item.Info.Price > 0) {
                                ...
                                ReceiveChat(
                                    String.Format("{0} Credits have been added to your Account", item.Info.Price),
                                    ChatType.Hint);
                            }
                        ...
                        case 13: //Hero unlock autopot
                            ...
                            Hero.AutoPot = true;
                            Enqueue(new ServerPacket.UnlockHeroAutoPot());
                    ...
                    break;
                case ItemType.Book:
                    UserMagic magic = new((Spell)item.Info.Shape);
                ...
                case ItemType.Food:
                    ...
                    switch (item.Info.Shape) {
                        case 0:
                            temp.MaxDura = (ushort)Math.Max(0,
                                temp.MaxDura - Math.Min(1000, temp.MaxDura - (temp.CurrentDura / 30)));
                            break;
                        case 1:
                            break;
                    }
                ...
                case ItemType.Pets:
                    if(item.Info.Shape >= 20) {
                        switch (item.Info.Shape) {
                            case 20: //Mirror
                            {
                                Enqueue(new ServerPacket.IntelligentCreatureEnableRename());
                            }
                                break;
                            case 21: //BlackStone
                            {
                            ...
                            case 22: //Nuts
                            {
                                if(CreatureSummoned) {
                                    for (int i = 0; i < Pets.Count; i++) {
                                        if(Pets[i].Race != ObjectType.Creature) {
                                            continue;
                                        }

                                        IntelligentCreatureObject pet = (IntelligentCreatureObject)Pets[i];
                                        if(pet.PetType != SummonedCreatureType) {
                                            continue;
                                        }
                            ...
                            case 26: //Wonderdrug
                            {
                                ...
                                ushort time = item.Info.Durability;
                            ...
                            case 28: //Knapsack
                            {
                                ...
                                AddBuff(BuffType.Knapsack, this, time * Settings.Minute,
                                    new Stats { [Stat.BagWeight] = item.GetTotal(Stat.Luck) });
                            }
                                break;
                        }
                    } else {
                        int slotIndex = Info.IntelligentCreatures.Count;
                        UserIntelligentCreature petInfo = new((IntelligentCreatureType)item.Info.Shape, slotIndex,
                            item.Info.Effect);
                ...
                {
                    AddBuff(BuffType.Transform, this, Settings.Second * item.Info.Durability, new Stats(),
                        values: item.Info.Shape);
                }
                    break;
                case ItemType.Deco:
                ...
                case ItemType.MonsterSpawn:
                    ...
                    int monsterID = item.Info.Stats[Stat.HP];
                    bool spawnAsPet = item.Info.Shape == 1;
                    bool conquestOnly = item.Info.Shape == 2;

                    MonsterInfo monsterInfo = Envir.GetMonsterInfo(monsterID);
                    if(monsterInfo == null) {
                        break;
                    }
                    ...
                    if(conquestOnly) {
                        ConquestObject con = CurrentMap.GetConquest(CurrentLocation);
                        if(con == null) {
                            ReceiveChat(
                                string.Format("{0} can only be spawned during a conquest.", monsterInfo.GameName),
                                ChatType.Hint);
                            Enqueue(p);
                            return;
                        }
        ...
        public void SplitItem(MirGridType grid, ulong id, ushort count) {
            ServerPacket.SplitItem1 p = new() { Grid = grid, UniqueID = id, Count = count, Success = false };
            UserItem[] array;
            switch (grid) {
                case MirGridType.Inventory:
                    array = Info.Inventory;
            ...
            for (int i = 0; i < array.Length; i++) {
                if(array[i] == null || array[i].UniqueID != id) {
                    continue;
                }
            ...
            p.Success = true;
            Enqueue(p);
            Enqueue(new ServerPacket.SplitItem { Item = temp, Grid = grid });
        ...
        public void MergeItem(MirGridType gridFrom, MirGridType gridTo, ulong fromID, ulong toID) {
            ServerPacket.MergeItem p = new()
                { GridFrom = gridFrom, GridTo = gridTo, IDFrom = fromID, IDTo = toID, Success = false };
            ...
            for (int i = 0; i < arrayFrom.Length; i++) {
                if(arrayFrom[i] == null || arrayFrom[i].UniqueID != fromID) {
                    continue;
                }
            ...
            for (int i = 0; i < arrayTo.Length; i++) {
                if(arrayTo[i] == null || arrayTo[i].UniqueID != toID) {
                    continue;
                }
        ...
        public void CombineItem(MirGridType grid, ulong fromID, ulong toID) {
            ServerPacket.CombineItem p = new() { Grid = grid, IDFrom = fromID, IDTo = toID, Success = false };
            ...
            for (int i = 0; i < array.Length; i++) {
                if(array[i] == null || array[i].UniqueID != fromID) {
                    continue;
                }
            ...
            for (int i = 0; i < array.Length; i++) {
                if(array[i] == null || array[i].UniqueID != toID) {
                    continue;
                }
            ...
            switch (tempFrom.Info.Shape) {
                ...
                case 7: //slots
                    ...
                    if(tempTo.RentalInformation != null &&
                       tempTo.RentalInformation.BindingFlags.HasFlag(BindMode.DontUpgrade)) {
                        Enqueue(p);
                        return;
                    }
                ...
                case 8: //Seal
                    ...
                    if(tempTo.SealedInfo != null && tempTo.SealedInfo.NextSealDate > Envir.Now) {
                        ...
                        ReceiveChat(
                            $"Item cannot be resealed for another {Functions.PrintTimeSpanFromSeconds(remainingSeconds, false)}.",
                            ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                ...
                case 4: //orbs
                    ...
                    if(tempTo.GemCount >= tempFrom.Info.Stats[Stat.CriticalDamage] ||
                       GetCurrentStatCount(tempFrom, tempTo) >= tempFrom.Info.Stats[Stat.HPDrainRatePercent]) {
                    ...
                    successchance = successchance >= tempFrom.Info.Stats[Stat.CriticalRate]
                        ? 0
                        : tempFrom.Info.Stats[Stat.CriticalRate] - successchance + Stats[Stat.GemRatePercent];
                    ...
                    if(tempFrom.GetTotal(Stat.MaxDC) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.MaxDC] += tempFrom.GetTotal(Stat.MaxDC);
                        }
                    } else if(tempFrom.GetTotal(Stat.MaxMC) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.MaxMC] += tempFrom.GetTotal(Stat.MaxMC);
                        }
                    } else if(tempFrom.GetTotal(Stat.MaxSC) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.MaxSC] += tempFrom.GetTotal(Stat.MaxSC);
                        }
                    } else if(tempFrom.GetTotal(Stat.MaxAC) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.MaxAC] += tempFrom.GetTotal(Stat.MaxAC);
                        }
                    } else if(tempFrom.GetTotal(Stat.MaxMAC) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.MaxMAC] += tempFrom.GetTotal(Stat.MaxMAC);
                        }
                        ...
                        if(succeeded) {
                            tempTo.AddedStats[Stat.AttackSpeed] += tempFrom.GetTotal(Stat.AttackSpeed);
                        }
                    } else if(tempFrom.GetTotal(Stat.Agility) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.Agility] += tempFrom.GetTotal(Stat.Agility);
                        }
                    } else if(tempFrom.GetTotal(Stat.Accuracy) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.Accuracy] += tempFrom.GetTotal(Stat.Accuracy);
                        }
                    } else if(tempFrom.GetTotal(Stat.PoisonAttack) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.PoisonAttack] += tempFrom.GetTotal(Stat.PoisonAttack);
                        }
                    } else if(tempFrom.GetTotal(Stat.Freezing) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.Freezing] += tempFrom.GetTotal(Stat.Freezing);
                        }
                    } else if(tempFrom.GetTotal(Stat.MagicResist) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.MagicResist] += tempFrom.GetTotal(Stat.MagicResist);
                        }
                    } else if(tempFrom.GetTotal(Stat.PoisonResist) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.PoisonResist] += tempFrom.GetTotal(Stat.PoisonResist);
                        }
                    } else if(tempFrom.GetTotal(Stat.Luck) > 0) {
                        if(succeeded) {
                            tempTo.AddedStats[Stat.Luck] += tempFrom.GetTotal(Stat.Luck);
                        }
                    } else {
            ...
            if(canRepair && array[indexTo] != null) {
                switch (tempTo.Info.Shape) {
                    case 1:
                    case 2: {
                        tempTo.MaxDura = (ushort)Math.Max(0,
                            Math.Min(tempTo.MaxDura, tempTo.MaxDura - (100 * Envir.Random.Next(10))));
                    }
                        break;
                    default:
                        break;
                ...
                Enqueue(new ServerPacket.ItemRepaired
                    { UniqueID = tempTo.UniqueID, MaxDura = tempTo.MaxDura, CurrentDura = tempTo.CurrentDura });
            }

            if(canUpgrade && array[indexTo] != null) {
                tempTo.GemCount++;
                ReceiveChat("Item has been upgraded.", ChatType.Hint);
                Enqueue(new ServerPacket.ItemUpgraded { Item = tempTo });
            }
            ...
            if(canSlotUpgrade && array[indexTo] != null) {
                ...
                Enqueue(new ServerPacket.ItemSlotSizeChanged
                    { UniqueID = tempTo.UniqueID, SlotSize = tempTo.Slots.Length });
            }

            if(canSeal && array[indexTo] != null) {
                ushort minutes = tempFrom.CurrentDura;
                ...
                Enqueue(new ServerPacket.ItemSealChanged
                    { UniqueID = tempTo.UniqueID, ExpiryDate = tempTo.SealedInfo.ExpiryDate });
            }
        ...
        public void DropItem(ulong id, ushort count, bool isHeroItem) {
            ServerPacket.DropItem p = new() { UniqueID = id, Count = count, HeroItem = isHeroItem, Success = false };
            if(Dead) {
                Enqueue(p);
                return;
            }
        ...
        public void DropGold(uint gold) {
            ...
            Account.Gold -= gold;
            Enqueue(new ServerPacket.LoseGold { Gold = gold });
        }
        ...
        public void RequestMapInfo(int mapIndex) {
            MapInfo info = Envir.GetMapInfo(mapIndex);
            CheckMapInfo(info);
        }
        ...
        public void SearchMap(string text) {
            ...
            ServerPacket.SearchMapResult p = new();
        ...
        public void GainGold(uint gold) {
            ...
            Enqueue(new ServerPacket.GainedGold { Gold = gold });
        }
        ...
        public void GainCredit(uint credit) {
            ...
            Enqueue(new ServerPacket.GainedCredit { Credit = credit });
        }

        public void GainItemMail(UserItem item, int reason) {
            Envir.MailCharacter(Info, item, reason: reason);
        }
        ...
        public bool CheckQuestItem(UserItem uItem, ushort count) {
            foreach(UserItem item in Info.QuestInventory.Where(item => item != null && item.Info == uItem.Info)) {
                if(count > item.Count) {
                    count -= item.Count;
                    continue;
                }
        ...
        public bool CanGainQuestItem(UserItem item) {
            if(FreeSpace(Info.QuestInventory) > 0) {
                return true;
            }
        ...
        public void GainQuestItem(UserItem item) {
            ...
            Enqueue(new ServerPacket.GainedQuestItem { Item = clonedItem });
        ...
        public void TakeQuestItem(ItemInfo uItem, ushort count) {
            for (int o = 0; o < Info.QuestInventory.Length; o++) {
                ...
                if(count > item.Count) {
                    Enqueue(new ServerPacket.DeleteQuestItem { UniqueID = item.UniqueID, Count = item.Count });
                ...
                Enqueue(new ServerPacket.DeleteQuestItem { UniqueID = item.UniqueID, Count = count });
        ...
        public override void ReceiveChat(string text, ChatType type) {
            Enqueue(new ServerPacket.Chat { Message = text, Type = type });
        }

        public void ReceiveOutputMessage(string text, OutputMessageType type) {
            Enqueue(new ServerPacket.SendOutputMessage { Message = text, Type = type });
        }

        public void Opendoor(byte Doorindex) {
            //todo: add check for sw doors
            if(CurrentMap.OpenDoor(Doorindex)) {
                Enqueue(new ServerPacket.Opendoor() { DoorIndex = Doorindex });
        ...
        public void CallDefaultNpc(DefaultNpcType type, params object[] value) {
            ...
            DelayedAction action = new(DelayedType.Npc, Envir.Time, Envir.DefaultNpc.LoadedObjectID,
                Envir.DefaultNpc.ScriptID, key);
            ActionList.Add(action);

            Enqueue(new ServerPacket.NpcUpdate { NpcID = Envir.DefaultNpc.LoadedObjectID });
        }

        public void CallDefaultNpc(string key) {
            if(NpcObjectID != Envir.DefaultNpc.LoadedObjectID) {
                return;
            }
        ...
        public void CallNpc(uint objectID, string key) {
            ...
            for (int i = 0; i < CurrentMap.Npcs.Count; i++) {
                ...
                if(!Functions.InRange(ob.CurrentLocation, CurrentLocation, Globals.DataRange)) {
                    return;
                }

                ob.CheckVisible(this);

                if(!ob.VisibleLog[Info.Index] || !ob.Visible) {
                    return;
                }

                int scriptID = NpcScriptID;
        ...
        private void CallNpcNextPage() {
            ...
            for (int i = 0; i < ActionList.Count; i++) {
                if(ActionList[i].Type != DelayedType.Npc || ActionList[i].Time != -1) {
                    continue;
                }
        ...
        public void BuyItem(ulong index, ushort count, PanelType type) {
            ...
            if(NpcPage == null ||
               !(String.Equals(NpcPage.Key, NpcScript.BuySellKey, StringComparison.CurrentCultureIgnoreCase) ||
                 ...
                 String.Equals(NpcPage.Key, NpcScript.BuySellNewKey, StringComparison.CurrentCultureIgnoreCase))) {
                return;
            }

            for (int i = 0; i < CurrentMap.Npcs.Count; i++) {
                NpcObject ob = CurrentMap.Npcs[i];
                if(ob.ObjectID != NpcObjectID) {
                    continue;
                }
        ...
        public void SellItem(ulong uniqueID, ushort count) {
            ServerPacket.SellItem p = new() { UniqueID = uniqueID, Count = count };
            ...
            if(NpcPage == null ||
               !(String.Equals(NpcPage.Key, NpcScript.BuySellKey, StringComparison.CurrentCultureIgnoreCase) ||
                 String.Equals(NpcPage.Key, NpcScript.SellKey, StringComparison.CurrentCultureIgnoreCase))) {
                Enqueue(p);
                return;
            }
            ...
            for (int n = 0; n < CurrentMap.Npcs.Count; n++) {
                ...
                for (int i = 0; i < Info.Inventory.Length; i++) {
                    temp = Info.Inventory[i];
                    if(temp == null || temp.UniqueID != uniqueID) {
                        continue;
                    }
                ...
                if(Settings.GoodsOn) {
                    NpcObject callingNpc = NpcObject.Get(NpcObjectID);

                    if(callingNpc != null) {
                        if(!callingNpc.BuyBack.ContainsKey(Name)) {
                            callingNpc.BuyBack[Name] = new List<UserItem>();
                        }

                        if(Settings.GoodsBuyBackMaxStored > 0 &&
                           callingNpc.BuyBack[Name].Count >= Settings.GoodsBuyBackMaxStored) {
        ...
        public void RepairItem(ulong uniqueID, bool special = false) {
            Enqueue(new ServerPacket.RepairItem { UniqueID = uniqueID });
            ...
            if(NpcPage == null ||
               (!String.Equals(NpcPage.Key, NpcScript.RepairKey, StringComparison.CurrentCultureIgnoreCase) &&
                !special) ||
               (!String.Equals(NpcPage.Key, NpcScript.SRepairKey, StringComparison.CurrentCultureIgnoreCase) &&
                special)) {
                return;
            }
            ...
            for (int n = 0; n < CurrentMap.Npcs.Count; n++) {
                ...
                if(temp.Info.Bind.HasFlag(BindMode.DontRepair) ||
                   (temp.Info.Bind.HasFlag(BindMode.NoSRepair) && special)) {
                ...
                Enqueue(new ServerPacket.LoseGold { Gold = cost });
                if(ob.Conq != null) {
                    ob.Conq.GuildInfo.GoldStorage += cost - baseCost;
                }
                ...
                Enqueue(new ServerPacket.ItemRepaired
                    { UniqueID = uniqueID, MaxDura = temp.MaxDura, CurrentDura = temp.CurrentDura });
                return;
            }
        }

        public void SendStorage() {
            if(Connection.StorageSent) {
                return;
            }
            ...
            Enqueue(new ServerPacket.UserStorage
                { Storage = Account.Storage }); // Should be no alter before being sent.
        }
        ...
        public void ConsignItem(ulong uniqueID, uint price, MarketPanelType panelType) {
            ServerPacket.ConsignItem p = new() { UniqueID = uniqueID };
            ...
            for (int n = 0; n < CurrentMap.Npcs.Count; n++) {
                ...
                MarketItemType type = panelType == MarketPanelType.Consign
                ...
                AuctionInfo auction = new(Info, temp, price, type);
        ...
        private bool Match(AuctionInfo info) {
            return (UserMatch || (!info.Expired && !info.Sold))
                   && (!UserMatch ||
                       (MarketPanelType == MarketPanelType.Auction && info.ItemType == MarketItemType.Auction) ||
                       (MarketPanelType != MarketPanelType.Auction && info.ItemType == MarketItemType.Consign))
                   && (MatchType == ItemType.Nothing || info.Item.Info.Type == MatchType)
                   && info.Item.Info.Shape >= MinShapes && info.Item.Info.Shape <= MaxShapes
                   && (string.IsNullOrWhiteSpace(MatchName) || info.Item.Info.Name.Replace(" ", "")
                       .IndexOf(MatchName, StringComparison.OrdinalIgnoreCase) >= 0);
        }

        public void MarketPage(int page) {
            if(Dead || Envir.Time < SearchTime) {
                return;
            }
            ...
            if(MarketPanelType != MarketPanelType.GameShop) {
                ...
                if(NpcPage == null ||
                   !String.Equals(NpcPage.Key, NpcScript.MarketKey, StringComparison.CurrentCultureIgnoreCase) ||
                   page <= PageSent) {
                    return;
                }
            ...
            List<AuctionInfo> listings = new();
            List<ClientAuction> clientListings = new();

            for (int i = 0; i < 10; i++) {
                if(i + (page * 10) >= Search.Count) {
                    break;
                }
            ...
            foreach(AuctionInfo listing in listings) {
            ...
            PageSent = page;
            Enqueue(new ServerPacket.NpcMarketPage { Listings = clientListings });
        }
        ...
        public void GetMarket(string name, ItemType type) {
            ...
            Enqueue(new ServerPacket.NpcMarket
                { Listings = clientListings, Pages = ((Search.Count - 1) / 10) + 1, UserMode = UserMatch });

            MessageQueue.EnqueueDebugging(string.Format("{0}ms to match {1} items",
                Envir.Stopwatch.ElapsedMilliseconds - start,
                MarketPanelType == MarketPanelType.GameShop ? Envir.GameShopList.Count :
                UserMatch ? Account.Auctions.Count : Envir.Auctions.Count));
        }
        ...
        public void MarketSearch(string match, ItemType type) {
            ...
            if(MarketPanelType == MarketPanelType.GameShop) {
                ...
                if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.MarketKey,
                       StringComparison.CurrentCultureIgnoreCase)) {
                    return;
                }
                ...
                for (int n = 0; n < CurrentMap.Npcs.Count; n++) {
                    ...
                    if(!Functions.InRange(CurrentLocation, ob.CurrentLocation, Globals.DataRange)) {
                        return;
                    }
        ...
        public void MarketBuy(ulong auctionID, uint bidPrice = 0) {
            if(Dead) {
                Enqueue(new ServerPacket.MarketFail { Reason = 0 });
                return;
            }
            ...
            if(MarketPanelType == MarketPanelType.GameShop) {
                foreach(AuctionInfo auction in Search) {
                    ...
                    if(auction.ItemType != MarketItemType.GameShop) {
                        continue;
                    }

                    if(auction.Price > Account.Credit) {
                        Enqueue(new ServerPacket.MarketFail { Reason = 4 });
                        return;
                    }

                    if(!CanGainItem(auction.Item)) {
                        Enqueue(new ServerPacket.MarketFail { Reason = 5 });
                        return;
                    }
                    ...
                    GainItem(item);
                    Enqueue(new ServerPacket.MarketSuccess {
                        Message = string.Format("You bought {0} for {1:#,##0} Credit", auction.Item.FriendlyName,
                            auction.Price)
                    });
                ...
                if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.MarketKey,
                       StringComparison.CurrentCultureIgnoreCase)) {
                    Enqueue(new ServerPacket.MarketFail { Reason = 1 });
                    return;
                }
                ...
                for (int n = 0; n < CurrentMap.Npcs.Count; n++) {
                    ...
                    foreach(AuctionInfo auction in Search) {
                        ...
                        if(auction.ItemType != MarketItemType.Consign && auction.ItemType != MarketItemType.Auction) {
                            continue;
                        }

                        if(auction.Sold) {
                            Enqueue(new ServerPacket.MarketFail { Reason = 2 });
                            return;
                        }

                        if(auction.Expired) {
                            Enqueue(new ServerPacket.MarketFail { Reason = 3 });
                            return;
                        }
                        ...
                        if(Account.Auctions.Contains(auction)) {
                            Enqueue(new ServerPacket.MarketFail { Reason = 6 });
                            return;
                        }
                        ...
                        if(auction.ItemType == MarketItemType.Consign) {
                            ...
                            Enqueue(new ServerPacket.LoseGold { Gold = auction.Price });
                            GainItem(auction.Item);

                            Envir.MessageAccount(auction.SellerInfo.AccountInfo,
                                string.Format("You sold {0} for {1:#,##0} Gold", auction.Item.FriendlyName,
                                    auction.Price), ChatType.Hint);
                            Enqueue(new ServerPacket.MarketSuccess {
                                Message = string.Format("You bought {0} for {1:#,##0} Gold", auction.Item.FriendlyName,
                                    auction.Price)
                            });
                            MarketSearch(MatchName, MatchType);
                        } else {
                            if(auction.CurrentBid > bidPrice) {
                                Enqueue(new ServerPacket.MarketFail { Reason = 9 });
                                return;
                            }

                            if(auction.CurrentBuyerInfo != null) {
                                string message = string.Format("You have been outbid on {0}. Refunded {1:#,##0} Gold.",
                                    auction.Item.FriendlyName, auction.CurrentBid);

                                Envir.MailCharacter(auction.CurrentBuyerInfo, gold: auction.CurrentBid,
                            ...
                            Enqueue(new ServerPacket.LoseGold { Gold = bidPrice });

                            Envir.MessageAccount(auction.SellerInfo.AccountInfo,
                                string.Format("Someone has bid {1:#,##0} Gold for {0}", auction.Item.FriendlyName,
                                    auction.CurrentBid), ChatType.Hint);
                            Enqueue(new ServerPacket.MarketSuccess {
                                Message = string.Format("You bid {1:#,##0} Gold for {0}", auction.Item.FriendlyName,
                                    auction.CurrentBid)
                            });
                            MarketSearch(MatchName, MatchType);
                        }
            ...
            Enqueue(new ServerPacket.MarketFail { Reason = 7 });
        }
            ...
            for (int n = 0; n < CurrentMap.Npcs.Count; n++) {
                ...
                foreach(AuctionInfo auction in Account.Auctions) {
                    ...
                    if(auction.Expired || auction.Sold ||
                       Envir.Now >= auction.ConsignmentDate.AddDays(Globals.ConsignmentLength)) {
                        Enqueue(new ServerPacket.MarketFail { Reason = 10 });
                        return;
                    }
                    ...
                    if(!CanGainGold(auction.CurrentBid)) {
                        Enqueue(new ServerPacket.MarketFail { Reason = 8 });
                        return;
                    }

                    auction.Sold = true;

                    string message = string.Format("You won {0} for {1:#,##0} Gold.", auction.Item.FriendlyName,
                        auction.CurrentBid);

                    Envir.MailCharacter(auction.CurrentBuyerInfo, auction.Item, customMessage: message);
                    Envir.MessageAccount(auction.CurrentBuyerInfo.AccountInfo,
                        string.Format("You bought {0} for {1:#,##0} Gold", auction.Item.FriendlyName,
                            auction.CurrentBid), ChatType.Hint);
                    ...
                    Enqueue(new ServerPacket.MarketSuccess {
                        Message = string.Format(
                            "You sold {0} for {1:#,##0} Gold. \nEarnings: {2:#,##0} Gold.\nCommision: {3:#,##0} Gold.‎",
                            auction.Item.FriendlyName, cost, gold, cost - gold)
                    });
                    MarketSearch(MatchName, MatchType);
                    return;
                }
        ...
        public void RequestUserName(uint id) {
            ...
            if(Character != null) {
                Enqueue(new ServerPacket.UserName { Id = (uint)Character.Index, Name = Character.Name });
            }
        }
        ...
        public void Awakening(ulong UniqueID, AwakeType type) {
            if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.AwakeningKey,
                   StringComparison.CurrentCultureIgnoreCase)) {
                return;
            }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++) {
                ...
                if(item.Info.Bind.HasFlag(BindMode.DontUpgrade)) {
                    Enqueue(new ServerPacket.Awakening { result = -1, removeID = -1 });
                    return;
                }
                ...
                if(awake.IsMaxLevel()) {
                    Enqueue(new ServerPacket.Awakening { result = -2, removeID = -1 });
                    return;
                }

                if(Info.AccountInfo.Gold < item.AwakeningPrice()) {
                    Enqueue(new ServerPacket.Awakening { result = -3, removeID = -1 });
                    return;
                }

                if(HasAwakeningNeedMaterials(item, type)) {
                    Info.AccountInfo.Gold -= item.AwakeningPrice();
                    Enqueue(new ServerPacket.LoseGold { Gold = item.AwakeningPrice() });
                    ...
                    switch (awake.UpgradeAwake(item, type, out isHit)) {
                        ...
                        case 0:
                            ...
                            Info.Inventory[i] = null;
                            Enqueue(new ServerPacket.Awakening { result = 0, removeID = (long)item.UniqueID });
                            break;
                        case 1:
        ...
        public void DowngradeAwakening(ulong UniqueID) {
            if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.DowngradeKey,
                   StringComparison.CurrentCultureIgnoreCase)) {
                return;
            }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++) {
                ...
                if(item != null) {
                    if(item.UniqueID == UniqueID) {
                        if(item.RentalInformation != null) {
                            ReceiveChat(
                                $"Unable to downgrade {item.FriendlyName} as it belongs to {item.RentalInformation.OwnerName}",
                                ChatType.System);
                            return;
                        }
                        ...
                        if(Info.AccountInfo.Gold >= item.DowngradePrice()) {
                            ...
                            Enqueue(new ServerPacket.LoseGold { Gold = item.DowngradePrice() });

                            Awake awake = item.Awake;
                            int result = awake.RemoveAwake();
                            switch (result) {
                                case 0:
                                    ReceiveChat(string.Format("{0} : Remove failed Level 0", item.FriendlyName),
                                        ChatType.System);
                                    break;
                                case 1:
                                    ushort maxDura = Envir.Random.Next(20) == 0
                                        ? (ushort)(item.MaxDura - 1000)
                                        : item.MaxDura;
                                    if(maxDura < 1000) {
                                        maxDura = 1000;
                                    }

                                    Info.Inventory[i].CurrentDura = Info.Inventory[i].CurrentDura >= maxDura
                                        ? maxDura
                                        : Info.Inventory[i].CurrentDura;
                                    Info.Inventory[i].MaxDura = maxDura;
                                    ReceiveChat(
                                        string.Format("{0} : Remove success. Level {1}", item.FriendlyName,
                                            item.Awake.GetAwakeLevel()), ChatType.System);
                                    Enqueue(new ServerPacket.RefreshItem { Item = item });
                                    break;
                                default:
                                    break;
        ...
        public void DisassembleItem(ulong UniqueID) {
            if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.DisassembleKey,
                   StringComparison.CurrentCultureIgnoreCase)) {
                return;
            }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++) {
                ...
                if(item.RentalInformation != null &&
                   item.RentalInformation.BindingFlags.HasFlag(BindMode.UnableToDisassemble)) {
                    ReceiveChat(
                        $"Unable to disassemble {item.FriendlyName} as it belongs to {item.RentalInformation.OwnerName}",
                        ChatType.System);
                    return;
                }

                if(Info.AccountInfo.Gold >= item.DisassemblePrice()) {
                    List<ItemInfo> dropList = new();
                    ...
                    gainItem.Count = (ushort)Envir.Random.Next(Math.Min(ushort.MaxValue,
                        (int)(((byte)item.Info.Grade * item.Info.RequiredAmount / 10) + item.Quality())));
                    if(gainItem.Count < 1) {
                        gainItem.Count = 1;
                    }
                    ...
                    Enqueue(new ServerPacket.LoseGold { Gold = item.DisassemblePrice() });
        ...
        public void ResetAddedItem(ulong UniqueID) {
            if(NpcPage == null ||
               !String.Equals(NpcPage.Key, NpcScript.ResetKey, StringComparison.CurrentCultureIgnoreCase)) {
                return;
            }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++) {
                ...
                if(item != null) {
                    if(item.UniqueID == UniqueID) {
                        if(item.RentalInformation != null) {
                            ReceiveChat(
                                $"Unable to reset {item.FriendlyName} as it belongs to {item.RentalInformation.OwnerName}",
                                ChatType.System);
                            return;
                        }
                        ...
                        if(Info.AccountInfo.Gold >= item.ResetPrice()) {
                            ...
                            Enqueue(new ServerPacket.LoseGold { Gold = item.ResetPrice() });

                            UserItem newItem = new(item.Info);
                            ...
                            Enqueue(new ServerPacket.RefreshItem { Item = Info.Inventory[i] });
                        }
                    }
                }
            }
                ...
                if(item != null) {
                    if(item.UniqueID == UniqueID) {
                        ...
                        foreach(List<byte> material in Awake.AwakeMaterials[(int)type - 1]) {
                            byte materialRate = (byte)(Awake.AwakeMaterialRate[(int)item.Info.Grade - 1] *
                                                       (float)awake.GetAwakeLevel());
                        ...
                        Enqueue(new ServerPacket.AwakeningNeedMaterials
                            { Materials = materials, MaterialsCount = materialCount });
                        break;
                    }
                }
            }
        ...
        public void AwakeningEffect(bool isSuccess, bool[] isHit) {
            for (int i = 0; i < 5; i++) {
                Enqueue(new ServerPacket.ObjectEffect {
                    ObjectID = ObjectID, Effect = isHit[i] ? SpellEffect.AwakeningHit : SpellEffect.AwakeningMiss,
                    EffectType = 0, DelayTime = (uint)(i * 500)
                });
            ...
            Enqueue(new ServerPacket.ObjectEffect {
                ObjectID = ObjectID, Effect = isSuccess ? SpellEffect.AwakeningSuccess : SpellEffect.AwakeningFail,
                EffectType = 0, DelayTime = 2500
            });
            Broadcast(new ServerPacket.ObjectEffect {
                ObjectID = ObjectID, Effect = isSuccess ? SpellEffect.AwakeningSuccess : SpellEffect.AwakeningFail,
                EffectType = 0, DelayTime = 2500
        ...
        public bool HasAwakeningNeedMaterials(UserItem item, AwakeType type) {
            ...
            foreach(List<byte> material in Awake.AwakeMaterials[(int)type - 1]) {
                byte materialRate =
                    (byte)(Awake.AwakeMaterialRate[(int)item.Info.Grade - 1] * (float)awake.GetAwakeLevel());
                materialCount[idx] = material[(int)item.Info.Grade - 1];
                materialCount[idx] += materialRate;
                idx++;
            }
            ...
            for (int i = 0; i < materialCount.Length; i++) {
                if(materialCount[i] != currentCount[i]) {
                    Enqueue(new ServerPacket.Awakening { result = -4, removeID = -1 });
                    return false;
                }
            }
                        ...
                        if(Info.Inventory[i].Info.Shape == (int)type - 1 &&
                           currentCount[0] > 0) {
                            if(Info.Inventory[i].Count <= currentCount[0]) {
                                Enqueue(new ServerPacket.DeleteItem
                                    { UniqueID = Info.Inventory[i].UniqueID, Count = Info.Inventory[i].Count });
                                currentCount[0] -= (byte)Info.Inventory[i].Count;
                                Info.Inventory[i] = null;
                            } else if(Info.Inventory[i].Count > currentCount[0]) {
                                Enqueue(new ServerPacket.DeleteItem
                                    { UniqueID = Info.Inventory[i].UniqueID, Count = currentCount[0] });
                            ...
                            if(Info.Inventory[i].Count <= currentCount[1]) {
                                ...
                                Enqueue(new ServerPacket.DeleteItem
                                    { UniqueID = Info.Inventory[i].UniqueID, Count = currentCount[1] });
        ...
        public void SwitchGroup(bool allow) {
            Enqueue(new ServerPacket.SwitchGroup { AllowGroup = allow });
        ...
        public void LeaveGroup() {
            if(GroupMembers != null) {
                ...
                if(GroupMembers.Count > 1) {
                    ...
                    GroupMembers[0].Enqueue(new ServerPacket.DeleteGroup());
        ...
        public void AddMember(string name) {
            if(Envir.Time < NextGroupInviteTime) {
                return;
            }
            ...
            SwitchGroup(true);
            player.Enqueue(new ServerPacket.GroupInvite { Name = Name });
        ...
        public void DelMember(string name) {
            ...
            for (int i = 0; i < GroupMembers.Count; i++) {
                if(String.Compare(GroupMembers[i].Name, name, StringComparison.OrdinalIgnoreCase) != 0) {
                    continue;
                }
            ...
            player.Enqueue(new ServerPacket.DeleteGroup());
            player.LeaveGroup();
        }
        ...
        public void GroupInvite(bool accept) {
            ...
            if(GroupInvitation.GroupMembers != null && GroupInvitation.GroupMembers.Count >= Globals.MaxGroup) {
                ReceiveChat(GroupInvitation.Name + "'s group already has the maximum number of members.",
                    ChatType.System);
                GroupInvitation = null;
                return;
            }
            ...
            if(GroupInvitation.GroupMembers == null) {
                ...
                GroupInvitation.Enqueue(new ServerPacket.AddMember { Name = GroupInvitation.Name });
                GroupInvitation.Enqueue(new ServerPacket.GroupMembersMap
                    { PlayerName = GroupInvitation.Name, PlayerMap = GroupInvitation.CurrentMap.Info.Title });
                GroupInvitation.Enqueue(new ServerPacket.SendMemberLocation
                    { MemberName = GroupInvitation.Name, MemberLocation = GroupInvitation.CurrentLocation });
            }
            ...
            for (int i = 0; i < GroupMembers.Count; i++) {
                ...
                member.Enqueue(p);
                Enqueue(new ServerPacket.AddMember { Name = member.Name });

                if(CurrentMap != member.CurrentMap ||
                   !Functions.InRange(CurrentLocation, member.CurrentLocation, Globals.DataRange)) {
                    continue;
                }

                byte time = Math.Min(byte.MaxValue, (byte)Math.Max(5, (RevTime - Envir.Time) / 1000));

                member.Enqueue(new ServerPacket.ObjectHealth
                    { ObjectID = ObjectID, Percent = PercentHealth, Expire = time });
                Enqueue(new ServerPacket.ObjectHealth
                    { ObjectID = member.ObjectID, Percent = member.PercentHealth, Expire = time });

                for (int j = 0; j < member.Pets.Count; j++) {
                    MonsterObject pet = member.Pets[j];

                    Enqueue(new ServerPacket.ObjectHealth
                        { ObjectID = pet.ObjectID, Percent = pet.PercentHealth, Expire = time });
                }
            }
        ...
        public void GroupMemberMapNameChanged() {
            ...
            for (int i = 0; i < GroupMembers.Count; i++) {
                PlayerObject member = GroupMembers[i];
                member.Enqueue(
                    new ServerPacket.GroupMembersMap { PlayerName = Name, PlayerMap = CurrentMap.Info.Title });
                Enqueue(new ServerPacket.GroupMembersMap
                    { PlayerName = member.Name, PlayerMap = member.CurrentMap.Info.Title });
            }

            Enqueue(new ServerPacket.GroupMembersMap { PlayerName = Name, PlayerMap = CurrentMap.Info.Title });
        }
        ...
        public void NewHero(ClientPacket.NewHero p) {
            ...
            if(heroCount >= Info.MaximumHeroCount) {
                Enqueue(new ServerPacket.NewHero { Result = 4 });
                return;
            }
            ...
            if(!passedItemCheck) {
                Enqueue(new ServerPacket.NewHero { Result = 6 });
                return;
            }

            HeroInfo info = new HeroInfo(p) { Index = ++Envir.NextHeroID };
            ...
            Enqueue(new ServerPacket.NewHero { Result = 10 });
        }
        ...
        public void SetAutoPotValue(Stat stat, uint value) {
            ...
            Enqueue(new ServerPacket.SetAutoPotValue() { Stat = stat, Value = value });
        }
        ...
        public void SetAutoPotItem(MirGridType Grid, int ItemIndex) {
            ...
            Enqueue(new ServerPacket.SetAutoPotItem() { Grid = Grid, ItemIndex = ItemIndex });
        }
        ...
        public void SetHeroBehaviour(HeroBehaviour behaviour) {
            ...
            if(Info.HeroBehaviour == behaviour) {
                return;
            }

            Info.HeroBehaviour = behaviour;
            Enqueue(new ServerPacket.SetHeroBehaviour() { Behaviour = behaviour });
        ...
        public void ChangeHero(int index) {
            ...
            if(Hero != null) {
                DespawnHero();
                Info.HeroSpawned = false;
                Enqueue(new ServerPacket.UpdateHeroSpawnState { State = HeroSpawnState.None });
            }
            ...
            Enqueue(new ServerPacket.ChangeHero() { FromIndex = index - 1 });

            if(Info.Heroes[0] == null || !respawn) {
                Enqueue(new ServerPacket.UpdateHeroSpawnState { State = HeroSpawnState.Unsummoned });
                return;
            }
        ...
        public bool CreateGuild(string guildName) {
            if(MyGuild != null || Info.GuildIndex != -1) {
                return false;
            }

            if(Envir.GetGuild(guildName) != null) {
                return false;
            }

            if(Info.Level < Settings.Guild_RequiredLevel) {
                ReceiveChat(
                    String.Format("Your level is not high enough to create a guild, required: {0}",
                        Settings.Guild_RequiredLevel), ChatType.System);
                return false;
            }

            if(!Info.AccountInfo.AdminAccount &&
               String.Equals(guildName, Settings.NewbieGuild, StringComparison.OrdinalIgnoreCase)) {
                ...
                for (int i = 0; i < Settings.Guild_CreationCostList.Count; i++) {
                    ...
                    if(Required.Item == null) {
                        if(Info.AccountInfo.Gold < Required.Amount) {
                            ReceiveChat(
                                String.Format("Insufficient gold. Creating a guild requires {0} gold.",
                                    Required.Amount), ChatType.System);
                            return false;
                        }
                    } else {
                        ushort count = (ushort)Math.Min(Required.Amount, ushort.MaxValue);

                        foreach(UserItem item in Info.Inventory.Where(item =>
                                    item != null && item.Info == Required.Item)) {
                        ...
                        if(count != 0) {
                            if(Required.Amount == 1) {
                                ReceiveChat(
                                    String.Format("{0} is required to create a guild.", Required.Item.FriendlyName),
                                    ChatType.System);
                            } else {
                                if(Required.Item.Type == ItemType.Ore) {
                                    ReceiveChat(
                                        string.Format("{0} with purity {1} is recuired to create a guild.",
                                            Required.Item.FriendlyName, Required.Amount / 1000), ChatType.System);
                                } else {
                                    ReceiveChat(
                                        string.Format("Insufficient {0}, you need {1} to create a guild.",
                                            Required.Item.FriendlyName, Required.Amount), ChatType.System);
                                }
                            }
                ...
                for (int i = 0; i < Settings.Guild_CreationCostList.Count; i++) {
                    ...
                    if(Required.Item == null) {
                        if(Info.AccountInfo.Gold >= Required.Amount) {
                            ...
                            Enqueue(new ServerPacket.LoseGold { Gold = Required.Amount });
                        }
                    } else {
                        ushort count = (ushort)Math.Min(Required.Amount, ushort.MaxValue);
                        ...
                        for (int o = 0; o < Info.Inventory.Length; o++) {
                            ...
                            Enqueue(new ServerPacket.DeleteItem { UniqueID = item.UniqueID, Count = (ushort)count });
                            if(count == item.Count) {
                                Info.Inventory[o] = null;
                            } else {
            ...
            //make the guild
            GuildInfo guildInfo = new GuildInfo(this, guildName) { GuildIndex = ++Envir.NextGuildID };
            Envir.GuildList.Add(guildInfo);

            GuildObject guild = new(guildInfo);
        ...
        public void EditGuildMember(string Name, string RankName, byte RankIndex, byte ChangeType) {
            ...
            switch (ChangeType) {
                case 0: //add member
                    ...
                    player.Enqueue(new ServerPacket.GuildInvite { Name = MyGuild.Name });
            ...
            switch (Type) {
                case 0: //notice
                    if(GuildNoticeChanged) {
                        Enqueue(new ServerPacket.GuildNoticeChange() { notice = MyGuild.Info.Notice });
                    }
                ...
                case 1: //memberlist
                    if(GuildMembersChanged) {
                        Enqueue(new ServerPacket.GuildMemberChange() { Status = 255, Ranks = MyGuild.Ranks });
                    }
        ...
        public void GuildStorageGoldChange(byte type, uint amount) {
            ...
            if(type == 0) //donate
            {
                ...
                Account.Gold -= amount;
                MyGuild.Gold += amount;
                Enqueue(new ServerPacket.LoseGold { Gold = amount });
                MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                    { Type = 0, Name = Info.Name, Amount = amount });
                ...
                GainGold(amount);
                MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                    { Type = 1, Name = Info.Name, Amount = amount });
                MyGuild.NeedSave = true;
            }
        }

        public void GuildStorageItemChange(byte type, int from, int to) {
            ServerPacket.GuildStorageItemChange p = new() { Type = (byte)(3 + type), From = from, To = to };
            if(MyGuild == null || MyGuildRank == null) {
                Enqueue(p);
                ReceiveChat("You are not part of a guild.", ChatType.System);
                return;
            ...
            switch (type) {
                case 0: //store
                    ...
                    if(Info.Inventory[from].RentalInformation != null && Info.Inventory[from].RentalInformation
                           .BindingFlags.HasFlag(BindMode.DontStore)) {
                        Enqueue(p);
                        return;
                    }
                    ...
                    MyGuild.StoredItems[to] = new GuildStorageItem()
                        { Item = Info.Inventory[from], UserId = Info.Index };
                    Info.Inventory[from] = null;
                    RefreshBagWeight();
                    MyGuild.SendItemInfo(MyGuild.StoredItems[to].Item);
                    MyGuild.SendServerPacket(new ServerPacket.GuildStorageItemChange()
                        { Type = 0, User = Info.Index, Item = MyGuild.StoredItems[to], To = to, From = from });
                    MyGuild.NeedSave = true;
                    break;
                case 1: //retrieve
                    if(!MyGuildRank.Options.HasFlag(GuildRankOptions.CanRetrieveItem)) {
                        ReceiveChat("You do not have permission to retrieve items from guild storage.",
                            ChatType.System);
                        return;
                    }
                    ...
                    MyGuild.SendServerPacket(new ServerPacket.GuildStorageItemChange()
                        { Type = 1, User = Info.Index, To = to, From = from });
                    RefreshBagWeight();
                    MyGuild.NeedSave = true;
                    break;
                case 2: // Move Item
                    ...
                    if(MyGuild.StoredItems[from] != null) {
                        MyGuild.SendItemInfo(MyGuild.StoredItems[from].Item);
                    }

                    MyGuild.SendServerPacket(new ServerPacket.GuildStorageItemChange()
                        { Type = 2, User = Info.Index, Item = MyGuild.StoredItems[to], To = to, From = from });
                    MyGuild.NeedSave = true;
                    break;
                case 3: //request list
                    if(!GuildCanRequestItems) {
                        return;
                    }

                    GuildCanRequestItems = false;
                    for (int i = 0; i < MyGuild.StoredItems.Length; i++) {
                        if(MyGuild.StoredItems[i] == null) {
                            continue;
                        }
                    ...
                    Enqueue(new ServerPacket.GuildStorageList() { Items = MyGuild.StoredItems });
                    break;
            }
        }

        public void GuildWarReturn(string Name) {
            if(MyGuild == null || MyGuildRank != MyGuild.Ranks[0]) {
                return;
            }
            ...
            if(MyGuild.GoToWar(enemyGuild)) {
                ...
                MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                    { Type = 2, Name = Info.Name, Amount = Settings.Guild_WarCost });
            }
        }
        ...
        public override bool AtWar(HumanObject attacker) {
            ...
            if(attacker is PlayerObject playerAttacker) {
                if(attacker == null || playerAttacker.MyGuild == null) {
                    return false;
                }

                if(!MyGuild.WarringGuilds.Contains(playerAttacker.MyGuild)) {
                    return false;
                }
            }
            ...
            switch (type) {
                ...
                case 1: //buy the buff
                    ...
                    if(MyGuild.Info.Level < BuffInfo.LevelRequirement ||
                       MyGuild.Info.SparePoints < BuffInfo.PointsRequirement) {
                        return; //client checks this so it shouldnt be possible without a moded client :p
                    }
                ...
                case 2: //activate the buff
                    ...
                    if(MyGuild.Gold < Buff.Info.ActivationCost || Buff.Active) {
                        return;
                    }
        ...
        public void DepositTradeItem(int from, int to) {
            ServerPacket.DepositTradeItem p = new() { From = from, To = to, Success = false };
        ...
        public void RetrieveTradeItem(int from, int to) {
            ServerPacket.RetrieveTradeItem p = new() { From = from, To = to, Success = false };
        ...
        public void TradeRequest() {
            ...
            if(player != null) {
                ...
                if(player.TradeInvitation != null) {
                    ReceiveChat(string.Format("Player {0} already has a trade invitation.", player.Info.Name),
                        ChatType.System);
                    return;
                }

                if(!player.AllowTrade) {
                    ReceiveChat(string.Format("Player {0} is not allowing trade at the moment.", player.Info.Name),
                        ChatType.System);
                    return;
                }
                ...
                if(!Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) ||
                   player.CurrentMap != CurrentMap) {
                    ReceiveChat(string.Format("Player {0} is not within trading range.", player.Info.Name),
                        ChatType.System);
                    return;
                }
                ...
                player.Enqueue(new ServerPacket.TradeRequest { Name = Info.Name });
            }
        }
        ...
        public void TradeReply(bool accept) {
            ...
            if(!accept) {
                TradeInvitation.ReceiveChat(string.Format("Player {0} has refused to trade.", Info.Name),
                    ChatType.System);
                TradeInvitation = null;
                return;
            }
            ...
            if(TradeInvitation.TradePartner != null) {
                ReceiveChat(string.Format("Player {0} is already trading.", TradeInvitation.Info.Name),
                    ChatType.System);
                TradeInvitation = null;
                return;
            }
            ...
            Enqueue(new ServerPacket.TradeAccept { Name = TradePartner.Info.Name });
            TradePartner.Enqueue(new ServerPacket.TradeAccept { Name = Info.Name });
        }
        ...
        public void TradeGold(uint amount) {
            ...
            Enqueue(new ServerPacket.LoseGold { Gold = amount });
            TradePartner.Enqueue(new ServerPacket.TradeGold { Amount = TradeGoldAmount });
        }
        ...
        public void TradeItem() {
            ...
            TradePartner.Enqueue(new ServerPacket.TradeItem { TradeItems = Info.Trade });
        }
        ...
        public void TradeConfirm(bool confirm) {
            ...
            if(!Functions.InRange(TradePartner.CurrentLocation, CurrentLocation, Globals.DataRange) ||
               TradePartner.CurrentMap != CurrentMap ||
               !Functions.FacingEachOther(Direction, CurrentLocation, TradePartner.Direction,
                   TradePartner.CurrentLocation)) {
                TradeCancel();
                return;
            }
            ...
            if(TradeLocked && !TradePartner.TradeLocked) {
                TradePartner.ReceiveChat(string.Format("Player {0} is waiting for you to confirm trade.", Info.Name),
                    ChatType.System);
            }
            ...
            for (int p = 0; p < 2; p++) {
                ...
                if(!TradePair[o].CanGainItems(TradePair[p].Info.Trade)) {
                    ...
                    TradePair[p].Enqueue(new ServerPacket.TradeCancel { Unlock = true });

                    TradePair[o].ReceiveChat("Unable to accept all items.", ChatType.System);
                    TradePair[o].Enqueue(new ServerPacket.TradeCancel { Unlock = true });
            ...
            if(CanTrade) {
                for (int p = 0; p < 2; p++) {
                    ...
                    for (int i = 0; i < TradePair[p].Info.Trade.Length; i++) {
                        ...
                        Report.ItemMoved(u, MirGridType.Trade, MirGridType.Inventory, i, -99,
                            string.Format("Trade from {0} to {1}", TradePair[p].Name, TradePair[o].Name));
                    }

                    if(TradePair[p].TradeGoldAmount > 0) {
                        Report.GoldChanged(TradePair[p].TradeGoldAmount, true,
                            string.Format("Trade from {0} to {1}", TradePair[p].Name, TradePair[o].Name));
                    ...
                    TradePair[p].Enqueue(new ServerPacket.TradeConfirm());
                ...
                if(TradePair[p] != null) {
                    for (int t = 0; t < TradePair[p].Info.Trade.Length; t++) {
                        ...
                        if(FreeSpace(TradePair[p].Info.Inventory) < 1) {
                            ...
                            TradePair[p].Enqueue(new ServerPacket.DeleteItem
                                { UniqueID = temp.UniqueID, Count = temp.Count });
                            TradePair[p].Info.Trade[t] = null;
                            continue;
                        }

                        for (int i = 0; i < TradePair[p].Info.Inventory.Length; i++) {
                            if(TradePair[p].Info.Inventory[i] != null) {
                                continue;
                            }
                    ...
                    TradePair[p].Enqueue(new ServerPacket.TradeCancel { Unlock = false });
                }
            }
        }
        ...
        public void FishingCast(bool cast, bool cancel = false) {
            ...
            if(fishingPoint.X < 0 || fishingPoint.Y < 0 || CurrentMap.Width < fishingPoint.X ||
               CurrentMap.Height < fishingPoint.Y) {
                Fishing = false;
                return;
            }
            ...
            flexibilityStat = (byte)Math.Max(byte.MinValue,
                Math.Min(byte.MaxValue, flexibilityStat + rod.Info.Stats[Stat.CriticalRate]));
            successStat = (sbyte)Math.Max(sbyte.MinValue,
                Math.Min(sbyte.MaxValue, successStat + rod.Info.Stats[Stat.MaxAC]));
                    ...
                    case ItemType.Hook: {
                        flexibilityStat = (byte)Math.Max(byte.MinValue,
                            Math.Min(byte.MaxValue,
                                flexibilityStat + temp.AddedStats[Stat.CriticalRate] +
                                realItem.Stats[Stat.CriticalRate]));
                    }
                        break;
                    case ItemType.Float: {
                        nibbleMin = (byte)Math.Max(byte.MinValue,
                            Math.Min(byte.MaxValue, nibbleMin + realItem.Stats[Stat.MinAC]));
                        nibbleMax = (byte)Math.Max(byte.MinValue,
                            Math.Min(byte.MaxValue, nibbleMax + realItem.Stats[Stat.MaxAC]));
                    }
                        break;
                    case ItemType.Bait: {
                        successStat = (sbyte)Math.Max(sbyte.MinValue,
                            Math.Min(sbyte.MaxValue, successStat + realItem.Stats[Stat.MaxAC]));
                    }
                        break;
                    case ItemType.Finder: {
                        failedAddSuccessMin = (byte)Math.Max(byte.MinValue,
                            Math.Min(byte.MaxValue, failedAddSuccessMin + realItem.Stats[Stat.MinAC]));
                        failedAddSuccessMax = (byte)Math.Max(byte.MinValue,
                            Math.Min(byte.MaxValue, failedAddSuccessMax + realItem.Stats[Stat.MaxAC]));
                    }
                        break;
                    case ItemType.Reel: {
                        FishingAutoReelChance = (sbyte)Math.Max(sbyte.MinValue,
                            Math.Min(sbyte.MaxValue, FishingAutoReelChance + realItem.Stats[Stat.MaxMAC]));
                        successStat = (sbyte)Math.Max(sbyte.MinValue,
                            Math.Min(sbyte.MaxValue, successStat + realItem.Stats[Stat.MaxAC]));
                    }
                        break;
            ...
            if(cast) {
                FishingChance = Settings.FishingSuccessStart + (int)successStat +
                                (FishingChanceCounter != 0
                                    ? Envir.Random.Next(failedAddSuccessMin, failedAddSuccessMax)
                                    : 0) + (FishingChanceCounter * Settings.FishingSuccessMultiplier); //10 //10
            }

            if(FishingChanceCounter != 0) {
                DamagedFishingItem(FishingSlot.Finder, 1);
            }
            ...
            if(cast) {
                ...
                if(FishFound) {
                    int getChance = FishingChance + Envir.Random.Next(10, 24) +
                                    (FishingProgress > 50 ? flexibilityStat / 2 : 0);
                    ...
                    if(Envir.Random.Next(0, 100) <= getChance) {
                        ...
                        foreach(DropInfo drop in
                                Envir.FishingDrops.Where(x => x.Type == fishingCell.FishingAttribute)) {
                            DropRewardInfo reward = drop.AttemptDrop(EXPOwner?.Stats[Stat.ItemDropRatePercent] ?? 0,
                                EXPOwner?.Stats[Stat.GoldDropRatePercent] ?? 0);

                            if(reward != null) {
                                foreach(ItemInfo dropitems in reward.Items) {
        ...
        public void FishingChangeAutocast(bool autoCast) {
            ...
            if(rod == null || !rod.Info.IsFishingRod) {
                return;
            }
        ...
        public void AcceptQuest(int index) {
            ...
            if(CurrentQuests.Exists(e => e.Index == index)) {
                return; //e.Info.NpcIndex == npcIndex && 
            }
            ...
            for (int i = CurrentMap.Npcs.Count - 1; i >= 0; i--) {
                if(CurrentMap.Npcs[i].ObjectID != info.NpcIndex) {
                    continue;
                }
            ...
            if(npc == null || !npc.VisibleLog[Info.Index] || !npc.Visible) {
                return;
            }
            ...
            QuestProgressInfo quest = new(index);
        ...
        public void FinishQuest(int questIndex, int selectedItemIndex = -1) {
            ...
            for (int i = CurrentMap.Npcs.Count - 1; i >= 0; i--) {
                if(CurrentMap.Npcs[i].ObjectID != quest.Info.FinishNpcIndex) {
                    continue;
                }
            ...
            List<UserItem> rewardItems = new();

            foreach(QuestItemReward reward in quest.Info.FixedRewards) {
        ...
        public void ShareQuest(int questIndex) {
            ...
            if(GroupMembers != null) {
                foreach(PlayerObject player in GroupMembers.Where(player => player.CurrentMap == CurrentMap &&
                                                                            Functions.InRange(player.CurrentLocation,
                                                                                CurrentLocation, Globals.DataRange) &&
                                                                            !player.Dead && player != this)) {
                    player.Enqueue(new ServerPacket.ShareQuest { QuestIndex = questIndex, SharerName = Name });
                    shared = true;
                }
            }
        ...
        public override bool CheckGroupQuestItem(UserItem item, bool gainItem = true) {
            ...
            if(GroupMembers != null) {
                foreach(PlayerObject player in GroupMembers.Where(player =>
                            player != null && player.Node != null && player.CurrentMap == CurrentMap &&
        ...
        public bool CheckNeedQuestItem(UserItem item, bool gainItem = true) {
            foreach(QuestProgressInfo quest in CurrentQuests.Where(e => e.ItemTaskCount.Count > 0)
                        .Where(e => e.NeedItem(item.Info)).Where(e => CanGainQuestItem(item))) {
                if(gainItem) {
                    GainQuestItem(item);
                    quest.ProcessItem(Info.QuestInventory);

                    Enqueue(new ServerPacket.SendOutputMessage {
                        Message = string.Format("You found {0}.", item.FriendlyName), Type = OutputMessageType.Quest
                    });
        ...
        public bool CheckNeedQuestFlag(int flagNumber) {
            foreach(QuestProgressInfo quest in CurrentQuests.Where(e => e.FlagTaskSet.Count > 0)
        ...
        public void CheckNeedQuestKill(MonsterInfo mInfo) {
            foreach(QuestProgressInfo quest in CurrentQuests.Where(e => e.KillTaskCount.Count > 0)
                        .Where(quest => quest.NeedKill(mInfo))) {
                quest.ProcessKill(mInfo);

                Enqueue(new ServerPacket.SendOutputMessage
                    { Message = string.Format("You killed {0}.", mInfo.GameName), Type = OutputMessageType.Quest });
        ...
        public void RecalculateQuestBag() {
            for (int i = Info.QuestInventory.Length - 1; i >= 0; i--) {
                ...
                if(!itemRequired && !isCarryItem) {
                    ...
                    Enqueue(new ServerPacket.DeleteQuestItem { UniqueID = itm.UniqueID, Count = itm.Count });
                }
            }
        }
        ...
        public void SendUpdateQuest(QuestProgressInfo quest, QuestState state, bool trackQuest = false) {
            ...
            Enqueue(new ServerPacket.ChangeQuest {
        ...
        public void GetCompletedQuests() {
            Enqueue(new ServerPacket.CompleteQuest {
        ...
        public void SendMail(string name, string message) {
            ...
            //sent from player
            MailInfo mail = new(player.Index, true) {
                Sender = Info.Name,
                Message = message,
                Gold = 0
            };
        ...
        public void SendMail(string name, string message, uint gold, ulong[] items, bool stamped) {
            ...
            if(Account.Gold < totalGold || Account.Gold < gold || gold > totalGold) {
                Enqueue(new ServerPacket.MailSent { Result = -1 });
                return;
            }
            ...
            if(stamped) {
                for (int i = 0; i < Info.Inventory.Length; i++) {
                    ...
                    if(item == null || item.Info.Type != ItemType.Nothing || item.Info.Shape != 1 || item.Count < 1) {
                        continue;
                    }
                    ...
                    Enqueue(new ServerPacket.DeleteItem { UniqueID = item.UniqueID, Count = 1 });
                    break;
                }
            }

            List<UserItem> giftItems = new();
            ...
            for (int j = 0; j < (hasStamp ? 5 : 1); j++) {
                ...
                for (int i = 0; i < Info.Inventory.Length; i++) {
                    ...
                    if(item == null || items[j] != item.UniqueID) {
                        continue;
                    }
                    ...
                    if(item.RentalInformation != null &&
                       item.RentalInformation.BindingFlags.HasFlag(BindMode.DontTrade)) {
                        ReceiveChat(string.Format("{0} cannot be mailed", item.FriendlyName), ChatType.System);
                        return;
                    }
            ...
            if(totalGold > 0) {
                ...
                Enqueue(new ServerPacket.LoseGold { Gold = totalGold });
            }
        ...
        public void CollectMail(ulong mailID) {
            ...
            Enqueue(new ServerPacket.ParcelCollected { Result = 1 });
        ...
        public void SummonIntelligentCreature(IntelligentCreatureType pType) {
            if(pType == IntelligentCreatureType.None) {
                return;
            }
            ...
            if(CreatureSummoned == true || SummonedCreatureType != IntelligentCreatureType.None) {
                return;
            }

            for (int i = 0; i < Info.IntelligentCreatures.Count; i++) {
                if(Info.IntelligentCreatures[i].PetType != pType) {
                    continue;
                }
                ...
                IntelligentCreatureObject pet = (IntelligentCreatureObject)monster;
                ...
                ReceiveChat(string.Format("Creature {0} has been summoned.", Info.IntelligentCreatures[i].CustomName),
                    ChatType.System);
                break;
            }
        ...
        public void UnSummonIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true) {
            ...
            for (int i = 0; i < Pets.Count; i++) {
                ...
                IntelligentCreatureObject pet = (IntelligentCreatureObject)Pets[i];
                if(pet.PetType != pType) {
                    continue;
                }

                if(doUpdate) {
                    ReceiveChat(string.Format("Creature {0} has been dismissed.", pet.CustomName), ChatType.System);
                }
        ...
        public void ReleaseIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true) {
            ...
            for (int i = 0; i < Info.IntelligentCreatures.Count; i++) {
                ...
                if(doUpdate) {
                    ReceiveChat(
                        string.Format("Creature {0} has been released.", Info.IntelligentCreatures[i].CustomName),
                        ChatType.System);
                }
        ...
        public void RefreshCreaturesTimeLeft() {
            ...
            if(Envir.Time > CreatureTimeLeftTicker) {
                //ExpireTime
                List<int> releasedPets = new();
                CreatureTimeLeftTicker = Envir.Time + Settings.Second;

                for (int i = 0; i < Info.IntelligentCreatures.Count; i++) {
                    if(Info.IntelligentCreatures[i].Expire == DateTime.MinValue) {
                        continue; //permanent
                    }
                ...
                for (int i = releasedPets.Count - 1; i >= 0; i--) {
                    ReceiveChat(
                        string.Format("Creature {0} has expired.",
                            Info.IntelligentCreatures[releasedPets[i]].CustomName), ChatType.System);
                    ReleaseIntelligentCreature(Info.IntelligentCreatures[releasedPets[i]].PetType, false);
                }

                if(SendIntelligentCreatureUpdates && CreatureSummoned &&
                   SummonedCreatureType != IntelligentCreatureType.None) {
                    //update client
                    GetCreaturesInfo();
                }
        ...
        public void RefreshCreatureSummoned() {
            ...
            if(!petFound) {
                MessageQueue.EnqueueDebugging(string.Format("{0}: SummonedCreature no longer exists?!?. {1}", Name,
                    SummonedCreatureType.ToString()));
                CreatureSummoned = false;
                SummonedCreatureType = IntelligentCreatureType.None;
            }
        }
        ...
        public void IntelligentCreatureSay(IntelligentCreatureType pType, string message) {
            if(!CreatureSummoned || message == "") {
                return;
            }

            if(pType != SummonedCreatureType) {
                return;
            }
            ...
            for (int i = 0; i < Pets.Count; i++) {
                ...
                Enqueue(new ServerPacket.ObjectChat
                    { ObjectID = Pets[i].ObjectID, Text = message, Type = ChatType.Normal });
                return;
            }
        }
        ...
        private UserItem CreateDynamicWonderDrug(int boxtype, UserItem dropitem) {
            ...
            switch ((int)dropitem.Info.Effect) {
                case 0: //exp low/med/high
                    ...
                    if(boxtype > 0) {
                        dropitem.AddedStats[Stat.ExpRatePercent] = 10;
                    }

                    if(boxtype > 1) {
                        dropitem.AddedStats[Stat.ExpRatePercent] = 20;
                    }
                ...
                case 1: //drop low/med/high
                    ...
                    if(boxtype > 0) {
                        dropitem.AddedStats[Stat.ItemDropRatePercent] = 20;
                    }

                    if(boxtype > 1) {
                        dropitem.AddedStats[Stat.ItemDropRatePercent] = 50;
                    }
                ...
                case 2: //hp low/med/high
                    ...
                    if(boxtype > 0) {
                        dropitem.AddedStats[Stat.HP] = 100;
                    }

                    if(boxtype > 1) {
                        dropitem.AddedStats[Stat.HP] = 200;
                    }
                ...
                case 3: //mp low/med/high
                    ...
                    if(boxtype > 0) {
                        dropitem.AddedStats[Stat.MP] = 100;
                    }

                    if(boxtype > 1) {
                        dropitem.AddedStats[Stat.MP] = 200;
                    }
                ...
                case 4: //ac low/med/high
                    ...
                    if(boxtype > 0) {
                        dropitem.AddedStats[Stat.MaxAC] = 3;
                    }

                    if(boxtype > 1) {
                        dropitem.AddedStats[Stat.MaxAC] = 5;
                    }
                ...
                case 5: //amc low/med/high
                    ...
                    if(boxtype > 0) {
                        dropitem.AddedStats[Stat.MaxMAC] = 3;
                    }

                    if(boxtype > 1) {
                        dropitem.AddedStats[Stat.MaxMAC] = 5;
                    }
                ...
                case 6: //speed low/med/high
                    ...
                    if(boxtype > 0) {
                        dropitem.AddedStats[Stat.AttackSpeed] = 3;
                    }

                    if(boxtype > 1) {
                        dropitem.AddedStats[Stat.AttackSpeed] = 4;
                    }
        ...
        private IntelligentCreatureObject GetCreatureByName(string creatureName) {
            if(!CreatureSummoned || creatureName == "") {
                return null;
            }

            if(SummonedCreatureType == IntelligentCreatureType.None) {
                return null;
            }
        ...
        private void GetCreaturesInfo() {
            ServerPacket.UpdateIntelligentCreatureList packet = new() {
        ...
        public void AddFriend(string name, bool blocked = false) {
            ...
            FriendInfo friend = new(info, blocked);
        ...
        public void GetFriends() {
            ...
            Enqueue(new ServerPacket.FriendUpdate { Friends = friends });
        }
        ...
        public void DepositRefineItem(int from, int to) {
            ServerPacket.DepositRefineItem p = new() { From = from, To = to, Success = false };

            if(NpcPage == null ||
               !String.Equals(NpcPage.Key, NpcScript.RefineKey, StringComparison.CurrentCultureIgnoreCase)) {
                Enqueue(p);
                return;
            }
        ...
        public void RetrieveRefineItem(int from, int to) {
            ServerPacket.RetrieveRefineItem p = new() { From = from, To = to, Success = false };
        ...
        public void RefineCancel() {
            for (int t = 0; t < Info.Refine.Length; t++) {
                ...
                for (int i = 0; i < Info.Inventory.Length; i++) {
                    ...
                    {
                        Enqueue(new ServerPacket.DeleteItem { UniqueID = temp.UniqueID, Count = temp.Count });

                        MailInfo mail = new(Info.Index) {
                            MailID = ++Envir.NextMailID,
                            Sender = "Refiner",
                            Message =
                                "Refining was cancelled with an item which couldn't be returned to your inventory.",
        ...
        public void RefineItem(ulong uniqueID) {
            Enqueue(new ServerPacket.RepairItem { UniqueID = uniqueID }); //CHECK THIServerPacket.
            ...
            for (int i = 0; i < Info.Inventory.Length; i++) {
                if(Info.Inventory[i] == null || Info.Inventory[i].UniqueID != uniqueID) {
                    continue;
                }
            ...
            if(Info.Inventory[index].RefineAdded != 0) {
                ReceiveChat(
                    String.Format("Your {0} needs to be checked before you can attempt to refine it again.",
                        Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }

            if(Info.Inventory[index].Info.Type != ItemType.Weapon && Settings.OnlyRefineWeapon) {
                ReceiveChat(String.Format("Your {0} can't be refined.", Info.Inventory[index].FriendlyName),
                    ChatType.System);
                return;
            }
            ...
            if(Info.Inventory[index].RentalInformation != null &&
               Info.Inventory[index].RentalInformation.BindingFlags.HasFlag(BindMode.DontUpgrade)) {
                ReceiveChat(String.Format("Your {0} can't be refined.", Info.Inventory[index].FriendlyName),
                    ChatType.System);
                return;
            }
            ...
            if(cost > Account.Gold) {
                ReceiveChat(
                    String.Format("You don't have enough gold to refine your {0}.", Info.Inventory[index].FriendlyName),
                    ChatType.System);
                return;
            }
            ...
            Enqueue(new ServerPacket.RefineItem { UniqueID = uniqueID });
            ...
            for (int i = 0; i < Info.Refine.Length; i++) {
                ...
                if(ingredient.Info.Stats[Stat.MaxDC] > 0 || ingredient.Info.Stats[Stat.MaxMC] > 0 ||
                   ingredient.Info.Stats[Stat.MaxSC] > 0) {
                    totalDC += (short)(ingredient.Info.Stats[Stat.MinDC] + ingredient.Info.Stats[Stat.MaxDC] +
                                       ingredient.AddedStats[Stat.MaxDC]);
                    totalMC += (short)(ingredient.Info.Stats[Stat.MinMC] + ingredient.Info.Stats[Stat.MaxMC] +
                                       ingredient.AddedStats[Stat.MaxMC]);
                    totalSC += (short)(ingredient.Info.Stats[Stat.MinSC] + ingredient.Info.Stats[Stat.MaxSC] +
                                       ingredient.AddedStats[Stat.MaxSC]);
                    requiredLevel += ingredient.Info.RequiredAmount;
                    if(Math.Floor(ingredient.MaxDura / 1000M) == Math.Floor(ingredient.Info.Durability / 1000M)) {
                        durability++;
                    }

                    if(Math.Floor(ingredient.CurrentDura / 1000M) == Math.Floor(ingredient.MaxDura / 1000M)) {
                        currentDura++;
                    }
            ...
            if(totalDC == 0 && totalMC == 0 && totalSC == 0) {
                ...
                if(Settings.RefineTime == 0) {
                    ...
                    ReceiveChat(
                        String.Format("Your {0} is now being refined, please check back in {1} minute(s).",
                            Info.CurrentRefine.FriendlyName, Settings.RefineTime), ChatType.System);
                }
            ...
            if(requiredLevel / itemAmount > Info.CurrentRefine.Info.RequiredAmount - 5) {
                itemSuccess += 10; //20%
            }

            if(durability == itemAmount) {
                itemSuccess += 10; //30%
            }

            if(currentDura == itemAmount) {
                itemSuccess += 5; //35%
            }

            int oreSuccess = 0; //Chance out of 35%

            if(oreAmount >= itemAmount) {
                oreSuccess += 15; //15%
            }

            if(orePurity / oreAmount >= refineStat / itemAmount) {
                oreSuccess += 15; //30%
            }

            if(orePurity == refineStat) {
                oreSuccess += 5; //35%
            }
            ...
            addedStats = (byte)(Info.CurrentRefine.AddedStats[Stat.MaxDC] + Info.CurrentRefine.AddedStats[Stat.MaxMC] +
                                Info.CurrentRefine.AddedStats[Stat.MaxSC]);
        ...
        public void CollectRefine() {
            ServerPacket.NpcCollectRefine p = new() { Success = false };
            ...
            if(Info.CollectTime > Envir.Time) {
                ReceiveChat(
                    string.Format("Your {0} will be ready to collect in {1} minute(s).",
                        Info.CurrentRefine.FriendlyName, (Info.CollectTime - Envir.Time) / Settings.Minute),
                    ChatType.System);
                Enqueue(p);
                return;
            }
            ...
            if(index == -1) {
                ReceiveChat(
                    String.Format("There isn't room in your bag for your {0}, make some space and try again.",
                        Info.CurrentRefine.FriendlyName), ChatType.System);
                Enqueue(p);
                return;
            }
        ...
        public void CheckRefine(ulong uniqueID) {
            ...
            if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.RefineCheckKey,
                   StringComparison.CurrentCultureIgnoreCase)) {
                return;
            }
            ...
            if(Info.Inventory[index].RefineAdded == 0) {
                ReceiveChat(
                    String.Format("{0} doesn't need to be checked as it hasn't been refined yet.",
                        Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }
            ...
            if(Envir.Random.Next(1, 100) < Settings.RefineCritChance) {
                Info.Inventory[index].RefineAdded =
                    (byte)(Info.Inventory[index].RefineAdded * Settings.RefineCritIncrease);
            }

            if(Info.Inventory[index].RefinedValue == RefinedValue.DC && Info.Inventory[index].RefineAdded > 0) {
                ReceiveChat(
                    String.Format("Congratulations, your {0} now has +{1} extra DC.",
                        Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
                Info.Inventory[index].AddedStats[Stat.MaxDC] = (int)Math.Min(int.MaxValue,
                    Info.Inventory[index].AddedStats[Stat.MaxDC] + Info.Inventory[index].RefineAdded);
                ...
                ReceiveChat(
                    String.Format("Congratulations, your {0} now has +{1} extra MC.",
                        Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
                Info.Inventory[index].AddedStats[Stat.MaxMC] = (int)Math.Min(int.MaxValue,
                    Info.Inventory[index].AddedStats[Stat.MaxMC] + Info.Inventory[index].RefineAdded);
                Info.Inventory[index].RefineAdded = 0;
                Info.Inventory[index].RefinedValue = RefinedValue.None;
                Info.Inventory[index].RefineSuccessChance = 0;
            } else if(Info.Inventory[index].RefinedValue == RefinedValue.SC && Info.Inventory[index].RefineAdded > 0) {
                ReceiveChat(
                    String.Format("Congratulations, your {0} now has +{1} extra SC.",
                        Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
                Info.Inventory[index].AddedStats[Stat.MaxSC] = (int)Math.Min(int.MaxValue,
                    Info.Inventory[index].AddedStats[Stat.MaxSC] + Info.Inventory[index].RefineAdded);
                Info.Inventory[index].RefineAdded = 0;
                Info.Inventory[index].RefinedValue = RefinedValue.None;
                Info.Inventory[index].RefineSuccessChance = 0;
            } else if(Info.Inventory[index].RefinedValue == RefinedValue.None &&
                      Info.Inventory[index].RefineAdded > 0) {
                ReceiveChat(
                    String.Format("Your {0} smashed into a thousand pieces upon testing.",
                        Info.Inventory[index].FriendlyName), ChatType.System);
                Enqueue(new ServerPacket.RefineItem { UniqueID = Info.Inventory[index].UniqueID });
                Info.Inventory[index].RefineSuccessChance = 0;
                Info.Inventory[index] = null;
                return;
            }

            Enqueue(new ServerPacket.ItemUpgraded { Item = Info.Inventory[index] });
            return;
        }
        ...
        public void NpcDivorce() {
            ...
            if(Info.Equipment[(int)EquipmentSlot.RingL] != null) {
                ...
                Enqueue(new ServerPacket.RefreshItem { Item = Info.Equipment[(int)EquipmentSlot.RingL] });
            }
            ...
            if(player != null) {
                ...
                if(player.Info.Equipment[(int)EquipmentSlot.RingL] != null) {
                    player.Enqueue(new ServerPacket.RefreshItem
                        { Item = player.Info.Equipment[(int)EquipmentSlot.RingL] });
                }
            }
        }
        ...
        public bool CheckMakeWeddingRing() {
            ...
            if(Info.Equipment[(int)EquipmentSlot.RingL] == null) {
                ReceiveChat(string.Format("You need to wear a ring on your left finger to make a Wedding Ring."),
                    ChatType.System);
                return false;
            }
        ...
        public void ReplaceWeddingRing(ulong uniqueID) {
            ...
            if(NpcPage == null || !String.Equals(NpcPage.Key, NpcScript.ReplaceWedRingKey,
                   StringComparison.CurrentCultureIgnoreCase)) {
                return;
            }
            ...
            Enqueue(new ServerPacket.EquipItem {
                Grid = MirGridType.Inventory, UniqueID = temp.UniqueID, To = (int)EquipmentSlot.RingL, Success = true
            });

            Enqueue(new ServerPacket.RefreshItem { Item = Info.Inventory[index] });
            Enqueue(new ServerPacket.RefreshItem { Item = Info.Equipment[(int)EquipmentSlot.RingL] });
        }
        ...
        public void MarriageRequest() {
            ...
            if(Info.MarriedDate.AddDays(Settings.MarriageCooldown) > Envir.Now) {
                ReceiveChat(
                    string.Format("You can't get married again yet, there is a {0} day cooldown after a divorce.",
                        Settings.MarriageCooldown), ChatType.System);
                return;
            }

            if(Info.Level < Settings.MarriageLevelRequired) {
                ReceiveChat(
                    string.Format("You need to be at least level {0} to get married.", Settings.MarriageLevelRequired),
                    ChatType.System);
                return;
            }
            ...
            if(player != null) {
                if(!Functions.FacingEachOther(Direction, CurrentLocation, player.Direction, player.CurrentLocation)) {
                    ReceiveChat(string.Format("You need to be facing each other to perform a marriage."),
                        ChatType.System);
                    return;
                }
                ...
                if(player.Level < Settings.MarriageLevelRequired) {
                    ReceiveChat(
                        string.Format("Your lover needs to be at least level {0} to get married.",
                            Settings.MarriageLevelRequired), ChatType.System);
                    return;
                }

                if(player.Info.MarriedDate.AddDays(Settings.MarriageCooldown) > Envir.Now) {
                    ReceiveChat(
                        string.Format("{0} can't get married again yet, there is a {1} day cooldown after divorce",
                            player.Name, Settings.MarriageCooldown), ChatType.System);
                    return;
                }

                if(!player.AllowMarriage) {
                    ReceiveChat("The person you're trying to propose to isn't allowing marriage requests.",
                        ChatType.System);
                    return;
                }
                ...
                if(player.MarriageProposal != null) {
                    ReceiveChat(string.Format("{0} already has a marriage invitation.", player.Info.Name),
                        ChatType.System);
                    return;
                }
                ...
                player.Enqueue(new ServerPacket.MarriageRequest { Name = Info.Name });
            } else {
        ...
        public void MarriageReply(bool accept) {
            ...
            if(!accept) {
                MarriageProposal.ReceiveChat(string.Format("{0} has refused to marry you.", Info.Name),
                    ChatType.System);
                MarriageProposal = null;
                return;
            }
            ...
            MarriageProposal.ReceiveChat(string.Format("Congratulations, you're now married to {0}.", Info.Name),
                ChatType.System);
            ReceiveChat(String.Format("Congratulations, you're now married to {0}.", MarriageProposal.Info.Name),
                ChatType.System);
        ...
        public void DivorceRequest() {
            ...
            if(player != null) {
                ...
                if(player.Dead || Dead) {
                    ReceiveChat("You can't divorce a dead player.",
                        ChatType.System); //GOT TO HERE, NEED TO KEEP WORKING ON IT.
                    return;
                }
                ...
                player.Enqueue(new ServerPacket.DivorceRequest { Name = Info.Name });
            } else {
                ReceiveChat(string.Format("You need to be facing your lover to divorce them."), ChatType.System);
                return;
            }
        ...
        public void DivorceReply(bool accept) {
            ...
            if(!accept) {
                DivorceProposal.ReceiveChat(string.Format("{0} has refused to divorce you.", Info.Name),
                    ChatType.System);
                DivorceProposal = null;
                return;
            }
            ...
            if(DivorceProposal.Info.Equipment[(int)EquipmentSlot.RingL] != null) {
                ...
                DivorceProposal.Enqueue(new ServerPacket.RefreshItem
                    { Item = DivorceProposal.Info.Equipment[(int)EquipmentSlot.RingL] });
            }
        ...
        public void GetRelationship(bool CheckOnline = true) {
            if(Info.Married == 0) {
                Enqueue(new ServerPacket.LoverUpdate
                    { Name = "", Date = Info.MarriedDate, MapName = "", MarriedDays = 0 });
            } else {
                CharacterInfo Lover = Envir.GetCharacterInfo(Info.Married);
                ...
                if(player == null) {
                    Enqueue(new ServerPacket.LoverUpdate {
                        Name = Lover.Name, Date = Info.MarriedDate, MapName = "",
                        MarriedDays = (short)(Envir.Now - Info.MarriedDate).TotalDays
                    });
                } else {
                    Enqueue(new ServerPacket.LoverUpdate {
                        Name = Lover.Name, Date = Info.MarriedDate, MapName = player.CurrentMap.Info.Title,
                        MarriedDays = (short)(Envir.Now - Info.MarriedDate).TotalDays
                    });
                    if(CheckOnline) {
                        player.GetRelationship(false);
        ...
        public void LogoutRelationship() {
            ...
            PlayerObject player = Envir.GetPlayer(lover.Name);
            if(player != null) {
                player.Enqueue(new ServerPacket.LoverUpdate {
                    Name = Info.Name, Date = player.Info.MarriedDate, MapName = "",
                    MarriedDays = (short)(Envir.Now - Info.MarriedDate).TotalDays
                });
        ...
        public void MentorBreak(bool force = false) {
            ...
            if(force) {
                ...
                ReceiveChat(
                    String.Format("You now have a {0} day cooldown on starting a new Mentorship.",
                        Settings.MentorLength), ChatType.System);
            } else {
        ...
        public void AddMentor(string Name) {
            ...
            if(mentor == null) {
                ...
                if(!mentor.AllowMentor) {
                    ReceiveChat(String.Format("{0} is not allowing Mentor requests.", mentor.Info.Name),
                        ChatType.System);
                    return;
                }

                if(mentor.Info.MentorDate > Envir.Now) {
                    ReceiveChat(String.Format("{0} can't start another Mentorship yet.", mentor.Info.Name),
                        ChatType.System);
                    return;
                }
                ...
                if(Info.Level + Settings.MentorLevelGap > mentor.Level) {
                    ReceiveChat(
                        String.Format("You can only be mentored by someone who at least {0} level(s) above you.",
                            Settings.MentorLevelGap), ChatType.System);
                    return;
                }

                mentor.MentorRequest = this;
                mentor.Enqueue(new ServerPacket.MentorRequest { Name = Info.Name, Level = Info.Level });
        ...
        public void MentorReply(bool accept) {
            ...
            if(student == null) {
                ...
                if(Info.Level - Settings.MentorLevelGap < student.Level) {
                    ReceiveChat(
                        String.Format("You can only mentor someone who at least {0} level(s) below you.",
                            Settings.MentorLevelGap), ChatType.System);
                    return;
                }
        ...
        public void GetMentor(bool CheckOnline = true) {
            if(Info.Mentor == 0) {
                Enqueue(new ServerPacket.MentorUpdate { Name = "", Level = 0, Online = false, MenteeEXP = 0 });
            } else {
                CharacterInfo mentor = Envir.GetCharacterInfo(Info.Mentor);

                PlayerObject player = Envir.GetPlayer(mentor.Name);

                Enqueue(new ServerPacket.MentorUpdate
                    { Name = mentor.Name, Level = mentor.Level, Online = player != null, MenteeEXP = Info.MentorExp });
        ...
        public void LogoutMentor() {
            ...
            if(player != null) {
                player.Enqueue(new ServerPacket.MentorUpdate
                    { Name = Info.Name, Level = Info.Level, Online = false, MenteeEXP = mentor.MentorExp });
                player.ReceiveChat(String.Format("{0} has gone offline.", Info.Name), ChatType.System);
            }
        }
        ...
        public void GameShopStock(GameShopItem item) {
            ...
            if(item.Stock - purchased >= 0) {
                ...
                Enqueue(new ServerPacket.GameShopStock { GIndex = item.Info.Index, StockLevel = StockLevel });
            }
        }
        ...
        public void GameshopBuy(int GIndex, byte Quantity) {
            ...
            if(Product.Stock != 0) {
                ...
                if(Product.Stock - purchased - Quantity >= 0) {
                    ...
                    MessageQueue.EnqueueDebugging(Info.Name + " is trying to buy " + Product.Info.FriendlyName + " x " +
                                                  Quantity + " - Stock isn't available.");
                    return;
                }
            } else {
                stockAvailable = true;
            ...
            if(stockAvailable) {
                MessageQueue.EnqueueDebugging(Info.Name + " is trying to buy " + Product.Info.FriendlyName + " x " +
                                              Quantity + " - Stock is available");

                uint cost = Product.CreditPrice * Quantity;
                if(cost < Account.Credit || cost == 0) {
                    canAfford = true;
                    CreditCost = cost;
                } else if(Product.CanBuyGold) {
                    //Needs to attempt to pay with gold and credits
                    uint totalCost = Product.GoldPrice * Quantity / cost * (cost - Account.Credit);
                    if(Account.Gold >= totalCost) {
                        ...
                        MessageQueue.EnqueueDebugging(Info.Name + " is trying to buy " + Product.Info.FriendlyName +
                                                      " x " + Quantity + " - not enough currency.");
                        return;
                    }
                }
            } else {
            ...
            if(canAfford) {
                ...
                if(GoldCost != 0) {
                    Enqueue(new ServerPacket.LoseGold { Gold = GoldCost });
                }

                if(CreditCost != 0) {
                    Enqueue(new ServerPacket.LoseCredit { Credit = CreditCost });
                }
        ...
        public void GetGameShop() {
            ...
            for (int i = 0; i < Envir.GameShopList.Count; i++) {
                ...
                if(item.Stock != 0) {
                    ...
                    if(item.Stock - purchased >= 0) {
                        stockLevel = item.Stock - purchased;
                        Enqueue(new ServerPacket.GameShopInfo { Item = item, StockLevel = stockLevel });
                    }
                } else {
                    Enqueue(new ServerPacket.GameShopInfo { Item = item, StockLevel = item.Stock });
                }
            }
        }
        ...
        public void CheckConquest(bool checkPalace = false) {
            if(CurrentMap.tempConquest == null && CurrentMap.Conquest != null) {
                ...
                if(checkPalace && CurrentMap.Info.Index == CurrentMap.tempConquest.PalaceMap.Info.Index &&
                   CurrentMap.tempConquest.GameType == ConquestGame.CapturePalace) {
        ...
        public void GetRentedItems() {
            Enqueue(new ServerPacket.GetRentedItems { RentedItems = Info.RentedItems });
        }
        ...
        public void ItemRentalRequest() {
            ...
            Point targetPosition = Functions.PointMove(CurrentLocation, Direction, 1);
            if(!CurrentMap.ValidPoint(targetPosition)) {
                return;
            }

            Cell targetCell = CurrentMap.GetCell(targetPosition);
            ...
            foreach(MapObject mapObject in targetCell.Objects) {
            ...
            Enqueue(new ServerPacket.ItemRentalRequest { Name = targetPlayer.Name, Renting = false });
            ItemRentalPartner.Enqueue(new ServerPacket.ItemRentalRequest { Name = Name, Renting = true });
        }
        ...
        public void SetItemRentalFee(uint amount) {
            ...
            Enqueue(new ServerPacket.LoseGold { Gold = amount });
            ItemRentalPartner.Enqueue(new ServerPacket.ItemRentalFee { Amount = amount });
        }
        ...
        public void SetItemRentalPeriodLength(uint days) {
            ...
            ItemRentalPartner.Enqueue(new ServerPacket.ItemRentalPeriod { Days = days });
        }

        public void DepositRentalItem(int from, int to) {
            ServerPacket.DepositRentalItem packet = new ServerPacket.DepositRentalItem
                { From = from, To = to, Success = false };
            ...
            UserItem item = Info.Inventory[from];
            ...
            if(item.RentalInformation?.RentalLocked == true) {
                ReceiveChat($"Unable to rent {item.FriendlyName} until {item.RentalInformation.ExpiryDate}",
                    ChatType.System);
                Enqueue(packet);
                return;
            }
            ...
            if(item.RentalInformation != null && item.RentalInformation.BindingFlags.HasFlag(BindMode.UnableToRent)) {
                ReceiveChat($"Unable to rent {item.FriendlyName} as it belongs to {item.RentalInformation.OwnerName}",
                    ChatType.System);
                Enqueue(packet);
                return;
            }
        ...
        public void RetrieveRentalItem(int from, int to) {
            ServerPacket.RetrieveRentalItem packet = new ServerPacket.RetrieveRentalItem
                { From = from, To = to, Success = false };
            ...
            UserItem item = ItemRentalDepositedItem;
        ...
        private void UpdateRentalItem() {
            ...
            ItemRentalPartner.Enqueue(new ServerPacket.UpdateRentalItem { LoanItem = ItemRentalDepositedItem });
        }
        ...
        public void CancelItemRental() {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if(rentalPair[i].ItemRentalDepositedItem != null) {
                    UserItem item = rentalPair[i].ItemRentalDepositedItem;

                    if(FreeSpace(rentalPair[i].Info.Inventory) < 1) {
                        rentalPair[i].GainItemMail(item, 1);
                        rentalPair[i].Enqueue(new ServerPacket.DeleteItem
                            { UniqueID = item.UniqueID, Count = item.Count });
                    ...
                    for (int j = 0; j < rentalPair[i].Info.Inventory.Length; j++) {
                ...
                rentalPair[i].Enqueue(new ServerPacket.CancelItemRental());
            }
        }

        public void ItemRentalLockFee() {
            ServerPacket.ItemRentalLock p = new() { Success = false, GoldLocked = false, ItemLocked = false };
            ...
            if(ItemRentalFeeAmount > 0) {
                ...
                ItemRentalPartner.Enqueue(new ServerPacket.ItemRentalPartnerLock { GoldLocked = ItemRentalFeeLocked });
            }

            if(ItemRentalFeeLocked && ItemRentalPartner.ItemRentalItemLocked) {
                ItemRentalPartner.Enqueue(new ServerPacket.CanConfirmItemRental());
        ...
        public void ItemRentalLockItem() {
            ...
            if(ItemRentalDepositedItem != null) {
                ...
                ItemRentalPartner.Enqueue(new ServerPacket.ItemRentalPartnerLock { ItemLocked = ItemRentalItemLocked });
            }

            if(ItemRentalItemLocked && ItemRentalPartner.ItemRentalFeeLocked) {
                Enqueue(new ServerPacket.CanConfirmItemRental());
        ...
        public void ConfirmItemRental() {
            ...
            if(!ItemRentalPartner.CanGainItem(ItemRentalDepositedItem)) {
                ...
                Enqueue(new ServerPacket.CancelItemRental());

                ItemRentalPartner.ReceiveChat("Unable to accept the rental item.", ChatType.System);
                ItemRentalPartner.Enqueue(new ServerPacket.CancelItemRental());
            ...
            item.RentalInformation = new RentalInformation {
                ...
                BindingFlags = BindMode.DontDrop | BindMode.DontStore | BindMode.DontSell | BindMode.DontTrade |
                               BindMode.UnableToRent | BindMode.DontUpgrade | BindMode.UnableToDisassemble
            };

            ItemRentalInformation itemRentalInformation = new ItemRentalInformation {
            ...
            ItemRentalPartner.ReceiveChat(
                $"You have rented {item.FriendlyName} from {Name} until {item.RentalInformation.ExpiryDate}",
                ChatType.System);
            ...
            Enqueue(new ServerPacket.ConfirmItemRental());
            ItemRentalPartner.Enqueue(new ServerPacket.ConfirmItemRental());
        ...
        public void SetTimer(string key, int seconds, byte type = 0) {
            ...
            Timer t = new(timerKey, seconds, type);

            Envir.Timers[timerKey] = t;

            Enqueue(new ServerPacket.SetTimer { Key = t.Key, Seconds = t.Seconds, Type = t.Type });
        }
        ...
        public void ExpireTimer(string key) {
            ...
            Enqueue(new ServerPacket.ExpireTimer { Key = timerKey });
        }

        public void SetCompass(Point location) {
            Enqueue(new ServerPacket.SetCompass { Location = location });
        }

        public bool HasHero => CurrentHero != null;

        public bool HeroSpawned => Hero != null;
        ...
        public void SummonHero() {
            ...
            Enqueue(new ServerPacket.UpdateHeroSpawnState
                { State = hero.Dead ? HeroSpawnState.Dead : HeroSpawnState.Summoned });
        }
        ...
        public void ReviveHero() {
            ...
            if(Hero != null) {
                ...
                Enqueue(new ServerPacket.UpdateHeroSpawnState { State = HeroSpawnState.Summoned });
            } else {
                CurrentHero.HP = -1;
            }
        }
        ...
        public void SealHero() {
            ...
            if(Settings.HeroSealItemName == string.Empty) {
                return;
            }
        ...
        private bool AddHero(HeroInfo hero) {
            ...
            for (int i = 0; i < Info.Heroes.Length; i++) {
                ...
                if(!HasHero) {
                    ...
                    Enqueue(new ServerPacket.NewHeroInfo { Info = hero.ClientInformation, StorageIndex = i - 1 });
                }
        ...
        public void ManageHeroes() {
            ServerPacket.ManageHeroes p = new()
                { MaximumCount = Info.MaximumHeroCount, CurrentHero = CurrentHero?.ClientInformation };
        ...
        public void SendNpcGoods(ServerPacket.NpcGoods goods) {
            List<List<UserItem>> chunks = Functions.SplitList(10, goods.List); // Split into chunks of 10..
            if(chunks.Count == 1) {
                goods.Progress = 3;
                Enqueue(goods);
                return;
            ...
            for (int i = 0; i < chunks.Count; i++) {
                ...
                Enqueue(new ServerPacket.NpcGoods {
                    Progress = prog, List = chunks[i], Rate = goods.Rate, Type = goods.Type,
                    HideAddedStats = goods.HideAddedStats
                });
            }
        }
    }
