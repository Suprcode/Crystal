...
namespace Server.MirObjects
{
    public class PlayerObject : HumanObject
    {
        ...
        private void RedDeathDrop(MapObject killer)
        {
            if (killer == null || killer.Race != ObjectType.Player)
            {
                for (var i = 0; i < Info.Equipment.Length; i++)
                {
                    ...
                    if (item.Info.Bind.HasFlag(BindMode.BreakOnDeath))
                    {
                        ...
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ShatteredOnDeath)], item.FriendlyName), ChatType.System2);
                    ...
                    else if (Envir.Random.Next(10) == 0)
                    {
                        if (Envir.ReturnRentalItem(item, item.RentalInformation?.OwnerName, Info))
                        {
                            ...
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerDeathItemReturn)], item.Info.FriendlyName), ChatType.Hint);
                        ...
                        if (item.Info.GlobalDropNotify)
                            ...
                            {
                                player.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerDroppedItem)], Name, item.FriendlyName), ChatType.System2);
                            }
            ...
            for (var i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (Envir.ReturnRentalItem(item, item.RentalInformation?.OwnerName, Info))
                {
                    ...
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ItemReturnedToOwnerOnDeath)], item.Info.FriendlyName), ChatType.Hint);
                    Report.ItemMailed(item, 1, 1, "Death Dropped Rental Item");
                ...
                if (item.Info.GlobalDropNotify)
                    foreach (var player in Envir.Players)
                    {
                        player.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerItemDropped)], Name, item.FriendlyName), ChatType.System2);
                    }
        ...
        public void Chat(string message, List<ChatItem> linkedItems = null)
        {
            ...
            if (GMLogin)
            {
                if (message == GMPassword)
                {
                    IsGM = true;
                    UpdateGMBuff();
                    MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.UserIsNowGM)], Name));
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouHaveBeenMadeGM)], ChatType.System);
                    Envir.RemoveRank(Info);//remove gm chars from ranking to avoid causing bugs in rank list
                }
                else
                {
                    MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.AttemptedGmLogin)], Name));
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.LoginPasswordInvalid)], ChatType.System);
                }
                GMLogin = false;
                return;
            }
            ...
            if (Info.ChatBanned)
            {
                if (Info.ChatBanExpiryDate > Envir.Now)
                {
                    ...
                    if (chatBanRemaining.Days > 0)
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ChatBanRemainingTimeByDay)], chatBanRemaining.Days, chatBanRemaining.Hours, chatBanRemaining.Minutes, chatBanRemaining.Seconds), ChatType.System);
                    else if (chatBanRemaining.Hours > 0)
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ChatBanRemainingTimeByHour)], chatBanRemaining.Hours, chatBanRemaining.Minutes, chatBanRemaining.Seconds), ChatType.System);
                    else if (chatBanRemaining.Minutes > 0)
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ChatBanRemainingTimeByMinutes)], chatBanRemaining.Minutes, chatBanRemaining.Seconds), ChatType.System);
                    else
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ChatBanRemainingTimeBySecond)], chatBanRemaining.Seconds), ChatType.System);
                    return;
                }
            ...
            else
            {
                if (ChatTime > Envir.Time)
                {
                    if (ChatTick >= 5 & !IsGM)
                    {
                        ...
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ChatBanDuration5Minutes)], ChatType.System);
                        return;
                    }
            ...
            if (message.StartsWith("/"))
            {
                ...
                if (player == null)
                {
                    ...
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CouldNotFind)], parts[0]), ChatType.System);
                    return;
                }

                if (player.Info.Friends.Any(e => e.Info == Info && e.Blocked))
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNotAcceptingMessages)]), ChatType.System);
                    return;
                }

                if (Info.Friends.Any(e => e.Info == player.Info && e.Blocked))
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotMessageBlacklistedPlayer)], ChatType.System);
                    return;
                }
            ...
            else if (message.StartsWith("!#"))
            {
                ...
                if (player == null)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.TargetIsNotOnline)], Mentor.Name), ChatType.System);
                    return;
                }
            ...
            else if (message.StartsWith("!"))
            {
                ...
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotShoutForSeconds)], Math.Ceiling((ShoutTime - Envir.Time) / 1000D)), ChatType.System);
                    return;
                }
                if (Level < 8 && (!HasMapShout && !HasServerShout))
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.RequiredLevel8ForShout)], ChatType.System);
                    return;
                }
            ...
            else if (message.StartsWith(":)"))
            {
                ...
                if (player == null)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.TargetIsNotOnline)], Lover.Name), ChatType.System);
                    return;
                }
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    case "LOGIN":
                        GMLogin = true;
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.EnterGmPassword)], ChatType.Hint);
                        return;
                    ...
                    case "KILL":
                        ...
                        if (parts.Length >= 2)
                        {
                            ...
                            if (!player.GMNeverDie)
                            {
                                ...
                                Helpers.ChatSystem.SystemMessage(chatMessage: string.Format(ServerTextMap[nameof(ServerTextKeys.KilledByGM)], player, Name));
                            }
                        }
                        else
                        {
                    ...
                    case "CHANGEGENDER":
                        ...
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerChangedTo)], data.Name, data.Gender), ChatType.System);
                        MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerChangedBy)], data.Name, data.Gender, Name));

                        Helpers.ChatSystem.SystemMessage(chatMessage: string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerGenderChangedByGM)], data.Player.Name, data.Gender.ToString(), Name));
                    ...
                    case "LEVEL":
                        ...
                        if (parts.Length >= 3)
                        {
                            ...
                            if (ushort.TryParse(parts[2], out level))
                            {
                                ...
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHasBeenLevelled)], player.Name, old, player.Level), ChatType.System);
                                MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerLevelChangedBy)], player.Name, old, player.Level, Name));
                                Helpers.ChatSystem.SystemMessage(chatMessage: string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerLevelledByGM)], player.Name, old, player.Level, Name));
                        ...
                        else
                        {
                            ...
                            if (ushort.TryParse(parts[1], out level))
                            {
                                ...
                                MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerLevelChangedBy)], Name, old, Level, Name));
                                return;
                            }
                        }

                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CouldNotLevelPlayer)], ChatType.System);
                        break;
                    ...
                    case "LEVELHERO":
                        ...
                        if (parts.Length >= 3)
                        {
                            ...
                            if (ushort.TryParse(parts[2], out level))
                            {
                                ...
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHeroLevelled)], player.Name, old, hero.Level), ChatType.System);
                                MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHeroLevelledBy)], player.Name, old, hero.Level, Name));
                                Helpers.ChatSystem.SystemMessage(chatMessage: string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHeroLevelledByGM)], player.Name, old, hero.Level, Name));
                                return;
                            }
                        }
                        else
                        {
                            ...
                            if (ushort.TryParse(parts[1], out level))
                            {
                                ...
                                MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHeroLeveledBy)], Name, old, hero.Level, Name));
                                return;
                            }
                        }
                    ...
                    case "MAKE":
                        {
                            ...
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ItemHasBeenCreated)], GameLanguage.DbLocalization(iInfo.FriendlyName), tempCount), ChatType.System);
                            MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAttemptCreateItem)], Name, GameLanguage.DbLocalization(iInfo.FriendlyName), tempCount));
                        }
                        break;
                    case "CLEARBUFFS":
                    ...
                    case "SUPERMAN":
                        ...
                        hintstring = GMNeverDie ? ServerTextMap[nameof(ServerTextKeys.InvincibleMode)] : ServerTextMap[nameof(ServerTextKeys.NormalMode)];
                    ...
                    case "GAMEMASTER":
                        ...
                        hintstring = GMGameMaster ? ServerTextMap[nameof(ServerTextKeys.GameMasterMode)] : ServerTextMap[nameof(ServerTextKeys.NormalMode)];
                        ReceiveChat(hintstring, ChatType.Hint);
                        UpdateGMBuff();
                        break;
                    ...
                    case "OBSERVER":
                        ...
                        hintstring = Observer ? ServerTextMap[nameof(ServerTextKeys.ObserverMode)] : ServerTextMap[nameof(ServerTextKeys.NormalMode)];
                        ReceiveChat(hintstring, ChatType.Hint);
                        UpdateGMBuff();
                        break;
                    case "ALLOWGUILD":
                        EnableGuildInvite = !EnableGuildInvite;
                        hintstring = EnableGuildInvite ? ServerTextMap[nameof(ServerTextKeys.GuildInvitesEnabled)] : ServerTextMap[nameof(ServerTextKeys.GuildInvitesDisabled)];
                        ReceiveChat(hintstring, ChatType.Hint);
                        break;
                    case "RECALL":
                        if (!IsGM) return;
                    ...
                    case "ENABLEGROUPRECALL":
                        ...
                        hintstring = EnableGroupRecall ? ServerTextMap[nameof(ServerTextKeys.GroupRecallEnabled)] : ServerTextMap[nameof(ServerTextKeys.GroupRecallDisabled)];
                        ReceiveChat(hintstring, ChatType.Hint);
                        break;
                    ...
                    case "GROUPRECALL":
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRecallOnMap)], ChatType.System);
                            return;
                        }

                        if (Envir.Time < LastRecallTime)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotRecallForSeconds)], (LastRecallTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                        ...
                        if (ItemSets.Any(set => set.Set == ItemSet.Recall && set.SetComplete))
                        {
                            ...
                            for (var i = 1; i < GroupMembers.Count(); i++)
                            {
                                if (GroupMembers[i].EnableGroupRecall)
                                    ...
                                    GroupMembers[i].ReceiveChat(ServerTextMap[nameof(ServerTextKeys.RecallAttemptWithoutPermission)],
                                        ChatType.System);
                            }
                        }
                        break;
                    case "RECALLMEMBER":
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotGroupLeader)], ChatType.System);
                            return;
                        }

                        if (Dead)
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRecallWhenDead)], ChatType.System);
                            return;
                        }
                        ...
                        if (Envir.Time < LastRecallTime)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotRecallSeconds)], (LastRecallTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                        if (ItemSets.Any(set => set.Set == ItemSet.Recall && set.SetComplete))
                        {
                            ...
                            {
                                ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerCouldNotFound)], parts[1])), ChatType.System);
                                return;
                            }
                            if (!player.EnableGroupRecall)
                            {
                                player.ReceiveChat(ServerTextMap[nameof(ServerTextKeys.RecallAttemptWithoutPermission)], ChatType.System);
                                ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.BlockingGroupRecall)], player.Name)), ChatType.System);
                                return;
                            }
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRecallWithoutRecallset)], ChatType.System);
                            return;
                        }
                        break;
                    ...
                    case "RECALLLOVER":
                        if (Info.Married == 0)
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouAreNotMarried)], ChatType.System);
                            return;
                        }
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NeedWeddingRingForRecall)], ChatType.System);
                            return;
                        }
                        ...
                        if (Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing == Info.Married)
                        {
                            ...
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.TeleportationViaWeddingRingDisabled)], ChatType.System);
                                return;
                            }

                            if (player == null)
                            {
                                ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerIsNotOnline)], Lover.Name)), ChatType.System);
                                return;
                            }

                            if (player.Dead)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRecallDeadPlayer)], ChatType.System);
                                return;
                            }

                            if (player.Info.Equipment[(int)EquipmentSlot.RingL] == null)
                            {
                                player.ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.NeedWeddingRingRecall)], Lover.Name)), ChatType.System);
                                ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.NotWearingWeddingRing)], Lover.Name)), ChatType.System);
                                return;
                            }

                            if (player.Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing != player.Info.Married)
                            {
                                player.ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.NeedWeddingRingForRecallOnLeft)], Lover.Name)), ChatType.System);
                                ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.NotWearingWeddingRing)], Lover.Name)), ChatType.System);
                                return;
                            }
                            ...
                            {
                                player.ReceiveChat(ServerTextMap[nameof(ServerTextKeys.RecallWithoutPermission)], ChatType.System);
                                ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.BlockingLoverRecall)], player.Name)), ChatType.System);
                                return;
                            }
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRecallLoverWithoutRing)], ChatType.System);
                            return;
                        }
                        break;
                    case "TIME":
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.TheTimeIs)], Envir.Now.ToString("hh:mm tt")), ChatType.System);
                        break;
                    ...
                    case "ROLL":
                        ...
                        for (int i = 0; i < GroupMembers.Count; i++)
                        {
                            ...
                            playerSend.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HasRolledNumber)], Name, diceNum), ChatType.Group);
                        }
                        break;
                    ...
                    case "MAP":
                        ...
                        ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.YouAreInMapId)], GameLanguage.DbLocalization(mapTitle), mapName)), ChatType.System);
                        break;
                    ...
                    case "BACKUPPLAYER":
                        {
                            ...
                            if (info == null)
                            {
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNotFound)], parts[1]), ChatType.System);
                                return;
                            }

                            Envir.SaveArchivedCharacter(info);

                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHasBeenBackedUp)], info.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerBackedUpBy)], info.Name, Name));
                        }
                        break;
                    ...
                    case "ARCHIVEPLAYER":
                        {
                            ...
                            if (data == Info)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotArchiveCurrentPlayer)], ChatType.System);
                                return;
                            }
                            ...
                            if (account == null)
                            {
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNotFoundInAccount)], parts[1]), ChatType.System);
                                return;
                            }
                            ...
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHasBeenArchived)], data.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerArchivedBy)], data.Name, Name));
                        }
                        break;
                    ...
                    case "LOADPLAYER":
                        {
                            ...
                            if (bak == null)
                            {
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerCouldNotBeLoaded)], parts[1]), ChatType.System);
                                return;
                            }
                            ...
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotLoadPlayerIdMismatch)], ChatType.System);
                                return;
                            }

                            info = bak;

                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHasBeenLoaded)], info.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerLoadedBy)], info.Name, Name));
                        }
                        break;
                    ...
                    case "RESTOREPLAYER":
                        {
                            ...
                            if (parts.Length > 2)
                            {
                                ...
                                {
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.AccountNotFound)], parts[2]), ChatType.System);
                                    return;
                                }
                                ...
                                {
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.AccountHasCharacters)], parts[2], Globals.MaxCharacterCount), ChatType.System);
                                    return;
                                }
                            }
                            ...
                            if (data == null)
                            {
                                if (account != null)
                                {
                                    ...
                                    if (data == null)
                                    {
                                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerRestoreFailFullFilename)], parts[1]), ChatType.System);
                                        return;
                                    }
                            ...
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerRestoredBy)], data.Name, Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerRestoredBy)], data.Name, Name));
                        }
                        break;
                    ...
                    case "MOVE":
                        ...
                        {
                            ReceiveChat((ServerTextMap[nameof(ServerTextKeys.CannotPositionMoveOnMap)]), ChatType.System);
                            return;
                        }
                        if (Envir.Time < LastTeleportTime)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotTeleportSecondsLeft)], (LastTeleportTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                    ...
                    case "MAPMOVE":
                        ...
                        if (map == null)
                        {
                            ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.MapCouldNotBeFound)], parts[1], instanceID)), ChatType.System);
                            return;
                        }
                        ...
                        {
                            case 2:
                                ReceiveChat(TeleportRandom(200, 0, map) ? (string.Format(ServerTextMap[nameof(ServerTextKeys.MovedToMap1)], map.Info.FileName)) :
                                    (string.Format(ServerTextMap[nameof(ServerTextKeys.FailedMovementToMap1)], map.Info.FileName)), ChatType.System);
                                break;
                            case 3:
                                ReceiveChat(TeleportRandom(200, 0, map) ? (string.Format(ServerTextMap[nameof(ServerTextKeys.MovedToMap2)], map.Info.FileName, instanceID)) :
                                    (string.Format(ServerTextMap[nameof(ServerTextKeys.FailedMovementToMap2)], map.Info.FileName, instanceID)), ChatType.System);
                                break;
                            case 4:
                                ReceiveChat(Teleport(map, new Point(x, y)) ? (string.Format(ServerTextMap[nameof(ServerTextKeys.MovedToMap3)], map.Info.FileName, x, y)) :
                                    (string.Format(ServerTextMap[nameof(ServerTextKeys.FailedMovementToMap3)], map.Info.FileName, x, y)), ChatType.System);
                                break;
                            case 5:
                                ReceiveChat(Teleport(map, new Point(x, y)) ? (string.Format(ServerTextMap[nameof(ServerTextKeys.MovedToMap4)], map.Info.FileName, instanceID, x, y)) :
                                    (string.Format(ServerTextMap[nameof(ServerTextKeys.FailedMovementToMap4)], map.Info.FileName, instanceID, x, y)), ChatType.System);
                                break;
                        }
                        break;
                    ...
                    case "MOB":
                        ...
                        if (parts.Length < 2)
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotEnoughParamsSpawnMonster)], ChatType.System);
                            return;
                        }
                        ...
                        if (mInfo == null)
                        {
                            ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.MonsterDoesNotExist)], parts[1])), ChatType.System);
                            return;
                        }

                        if (conquestAIs.Contains(mInfo.AI))
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotSpawnConquestItem)], GameLanguage.DbLocalization(mInfo.Name)), ChatType.System);
                            return;
                        }
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotSpawnIntelligentCreature)], ChatType.System);
                                return;
                            }
                        ...
                        ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.MonsterSpawned)], GameLanguage.DbLocalization(mInfo.Name), count)), ChatType.System);
                        break;
                    ...
                    case "RECALLMOB":
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            {
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotSpawnConquestItem)], GameLanguage.DbLocalization(monster.Name)), ChatType.System);
                                return;
                            }
                        ...
                        ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.PetHasBeenRecalled)], GameLanguage.DbLocalization(mInfo2.Name), count)), ChatType.System);
                        break;
                    ...
                    case "RELOADDROPS":
                        ...
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.DropsReloaded)], ChatType.Hint);
                        break;
                    ...
                    case "RELOADNPCS":
                        ...
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NpcScriptsReloaded)], ChatType.Hint);
                        break;
                    ...
                    case "GIVEGOLD":
                        ...
                        string goldMsg = string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerGivenGoldByGM)], player.Name, count, Name);
                    ...
                    case "GIVEPEARLS":
                        ...
                        string pearlMsg = string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerGivenPearlByGM)], player.Name, count, Name);
                    ...
                    case "GIVECREDIT":
                        ...
                        string creditMsg = string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerGivenCreditByGM)], player.Name, count, Name);

                        MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerGivenCredit)], player.Name, count));
                    ...
                    case "GIVESKILL":
                        ...
                        if (player.Info.Magics.Any(e => e.Spell == skill))
                        {
                            ...
                            string skillChangeMsg = string.Format(ServerTextMap[nameof(ServerTextKeys.SpellChangedByGM)], player.Name, GameLanguage.DbLocalization(skill.ToString()), spellLevel, Name);

                            player.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.SpellChangedLevel)], GameLanguage.DbLocalization(skill.ToString()), spellLevel), ChatType.Hint);
                        ...
                        {
                            player.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.LearnedSkillAtLevel)], GameLanguage.DbLocalization(skill.ToString()), spellLevel), ChatType.Hint);

                            if (player != this)
                            {
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HasLearnedSkillAtLevel)], player.Name, GameLanguage.DbLocalization(skill.ToString()), spellLevel), ChatType.Hint);
                            }

                            string skillLearnedMg = string.Format(ServerTextMap[nameof(ServerTextKeys.SpellLearntSetLevelByGM)], player.Name, GameLanguage.DbLocalization(skill.ToString()), spellLevel, Name);
                    ...
                    case "FIND":
                        ...
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotSearchSeconds)], (LastProbeTime - Envir.Time) / 1000), ChatType.System);
                            return;
                        }
                        ...
                        if (player == null)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerIsNotOnline)], parts[1]), ChatType.System);
                            return;
                        }
                        if (player.CurrentMap == null) return;
                        if (!IsGM)
                            LastProbeTime = Envir.Time + 180000;
                        ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerLocationInfo)], player.Name, player.CurrentMap.Info.Title, player.CurrentLocation.X, player.CurrentLocation.Y)), ChatType.System);
                        break;
                    ...
                    case "LEAVEGUILD":
                        ...
                        if (MyGuild.IsAtWar())
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotLeaveGuildAtWar)], ChatType.System);
                            return;
                        }
                    ...
                    case "CREATEGUILD":
                        ...
                        if (player.MyGuild != null)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyInGuild)], player.Name), ChatType.System);
                            return;
                        }
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildNameRestrictedLength)], ChatType.System);
                            return;
                        }
                        ...
                        if (guild != null)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.GuildAlreadyExists)], gName), ChatType.System);
                            return;
                        }
                        ...
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.SuccessfullyCreatedGuild)], gName), ChatType.System);
                        }
                        else
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.FailedToCreateGuild)], ChatType.System);
                        }
                    ...
                    case "ALLOWTRADE":
                        ...
                        if (AllowTrade)
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.AllowTradeNow)], ChatType.System);
                        else
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoLongerAllowTrade)], ChatType.System);
                        break;
                    ...
                    case "TRIGGER":
                        ...
                        if (parts.Length >= 3)
                        {
                            ...
                            if (player == null)
                            {
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNotFound)], parts[2]), ChatType.System);
                                return;
                            }
                    ...
                    case "LISTFLAGS":
                        ...
                        for (int i = 0; i < Info.Flags.Length; i++)
                        {
                            ...
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.Flag)], i), ChatType.Hint);
                        }
                        break;
                    ...
                    case "CHANGECLASS": //@changeclass [Player] [Class]
                        ...
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerChangedTo)], data.Name, data.Class), ChatType.System);
                        MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerChangedBy)], data.Name, data.Class, Name));

                        Helpers.ChatSystem.SystemMessage(chatMessage: string.Format(ServerTextMap[nameof(ServerTextKeys.ClassChangedByGM)], data.Player.Name, data.Class.ToString(), Name));
                    ...
                    case "AWAKENING":
                        {
                            ...
                            foreach (UserItem temp in Info.Equipment)
                            {
                                ...
                                if (realItem.Type == type)
                                {
                                    ...
                                    {
                                        case -1:
                                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ConditionError)], temp.FriendlyName), ChatType.System);
                                            break;
                                        case 0:
                                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UpgradeFailed)], temp.FriendlyName), ChatType.System);
                                            break;
                                        case 1:
                                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.AwakeLevelValue)], temp.FriendlyName, awake.GetAwakeLevel(), awake.GetAwakeValue(), awake.GetAwakeValue()), ChatType.System);
                    ...
                    case "INFO":
                        {
                            ...
                            switch (ob.Race)
                            {
                                case ObjectType.Player:
                                    ...
                                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.PlayerInfoTitle)], ChatType.System2);
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerInfo)], plOb.Name, plOb.Level, plOb.CurrentLocation.X, plOb.CurrentLocation.Y), ChatType.System2);
                                    break;
                                case ObjectType.Monster:
                                    MonsterObject monOb = (MonsterObject)ob;
                                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MonsterInfoTitle)], ChatType.System2);
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.MonsterInfo1)], monOb.Info.Index, GameLanguage.DbLocalization(monOb.Name)), ChatType.System2);
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.MonsterInfo2)], monOb.Level, monOb.CurrentLocation.X, monOb.CurrentLocation.Y, monOb.Direction), ChatType.System2);
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.MonsterInfo3)], monOb.Info.Stats[Stat.HP], monOb.Stats[Stat.MinDC], monOb.Stats[Stat.MaxDC]), ChatType.System2);
                                    break;
                                case ObjectType.Merchant:
                                    NPCObject npcOb = (NPCObject)ob;
                                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NpcInfoTitle)], ChatType.System2);
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NpcInfo1)], npcOb.Info.Index, GameLanguage.DbLocalization(npcOb.Name)), ChatType.System2);
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NpcInfo2)], ob.CurrentLocation.X, ob.CurrentLocation.Y), ChatType.System2);
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NpcInfo3)], npcOb.Info.FileName), ChatType.System2);
                                    break;
                            }
                        }
                        break;
                    ...
                    case "SETQUEST":
                        ...
                        if (player == null)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerIsNotOnline)], parts[3]), ChatType.System);
                            return;
                        }
                    ...
                    case "TOGGLETRANSFORM":
                        if (HasBuff(BuffType.Transform, out Buff transform))
                        {
                            ...
                            hintstring = transform.Paused ? ServerTextMap[nameof(ServerTextKeys.TransformDisabled)] : ServerTextMap[nameof(ServerTextKeys.TransformEnabled)];
                            ReceiveChat(hintstring, ChatType.Hint);
                        }
                        break;
                    ...
                    case "STARTCONQUEST":
                        {
                            ...
                            if (parts.Length < 1)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.SyntaxStartConquest)], ChatType.System);
                                return;
                            }

                            if (MyGuild == null)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NeedGuildToStartWar)], ChatType.System);
                                return;
                            }
                            ...
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.WarStarted)], tempConq.Info.Name), ChatType.System);
                            MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.WarStarted)], tempConq.Info.Name));

                            foreach (var pl in Envir.Players)
                            {
                                if (tempConq.WarIsOn)
                                {
                                    pl.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.WarStarted)], tempConq.Info.Name), ChatType.System);
                                }
                                else
                                {
                                    pl.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.WarStopped)], tempConq.Info.Name), ChatType.System);
                                }
                    ...
                    case "RESETCONQUEST":
                        {
                            ...
                            if (parts.Length < 1)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.SyntaxResetConquest)], ChatType.System);
                                return;
                            }
                            ...
                            {
                                resetConq.Reset();
                                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HasBeenReset)], resetConq.Info.Name), ChatType.System);
                            }
                            else
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ConquestNotFoundOrWarOn)], ChatType.System);
                            }
                        }
                        break;
                    case "GATES":
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoAccessControlGates)], ChatType.System);
                            return;
                        }
                        ...
                        if (parts.Length > 1)
                        {
                            ...
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MustTypeGatesCommand)], ChatType.System);
                                return;
                            }
                        ...
                        if (openClose)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.GatesClosed)], MyGuild.Conquest.Info.Name), ChatType.System);
                        }
                        else
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.GatesOpenedAt)], MyGuild.Conquest.Info.Name), ChatType.System);
                        }
                        break;
                    ...
                    case "CHANGEFLAG":
                        if (MyGuild == null || MyGuild.Conquest == null || !MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeRank) || MyGuild.Conquest.WarIsOn)
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoAccessChangeFlags)], ChatType.System);
                            return;
                        }
                    ...
                    case "REVIVE":
                        ...
                        else
                        {
                            ...
                            Helpers.ChatSystem.SystemMessage(chatMessage: string.Format(ServerTextMap[nameof(ServerTextKeys.RevivedToFullHealthByGM)], player, Name));
                        }
                        break;
                    case "DELETESKILL":
                        if ((!IsGM) || parts.Length < 2) return;
                        ...
                        if (removed)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.DeletedSkillFromPlayer)], GameLanguage.DbLocalization(skill1.ToString()), player.Name), ChatType.Hint);
                            player.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ItemRemovedFromYou)], skill1), ChatType.Hint);

                            Helpers.ChatSystem.SystemMessage(chatMessage: string.Format(ServerTextMap[nameof(ServerTextKeys.SkillRemovedByGM)], player, skill1.ToString(), Name));
                        }
                        else
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.UnableDeleteSkillNotFound)], ChatType.Hint);
                        }
        ...
        public void Harvest(MirDirection dir)
        {
            ...
            if (send)
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoNearbyOwnedCarcasses)], ChatType.System);
        }
        ...
        public void MoveItem(MirGridType grid, int from, int to)
        {
            ...
            if (from >= 0 && to >= 0 && from < array.Length && to < array.Length)
            {
                if (array[from] == null)
                {
                    ...
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemMoveErrorReport)], ChatType.System);
                    Enqueue(p);
                    return;
                }
        ...
        public void TransferHeroItem(int from, int to)
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.TooHeavyToTransfer)], ChatType.System);
                Enqueue(p);
                return;
            }
        ...
        public override void UseItem(ulong id)
        {
            ...
            switch (item.Info.Type)
            {
                case ItemType.Potion:
                    switch (item.Info.Shape)
                    {
                        ...
                        case 2: //MysteryWater
                            if (UnlockCurse)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CanAlreadyUnequipCursedItem)], ChatType.Hint);
                                Enqueue(p);
                                return;
                            }
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CanNowUnequipCursedItem)], ChatType.Hint);
                            UnlockCurse = true;
                            break;
                        case 3: //Buff
                            {
                ...
                case ItemType.Scroll:
                    ...
                    switch (item.Info.Shape)
                    {
                        ...
                        case 4: //RepairOil
                            ...
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WeaponPartiallyRepaired)], ChatType.Hint);
                        ...
                        case 5: //WarGodOil
                            ...
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WeaponCompletelyRepaired)], ChatType.Hint);
                            Enqueue(new S.ItemRepaired { UniqueID = temp.UniqueID, MaxDura = temp.MaxDura, CurrentDura = temp.CurrentDura });
                            break;
                        case 6: //ResurrectionScroll
                            if (CurrentMap.Info.NoReincarnation)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotUseOnMap)], ChatType.System);
                                Enqueue(p);
                                return;
                            }
                            if (Dead)
                        ...
                        case 7: //CreditScroll
                            if (item.Info.Price > 0)
                            {
                                ...
                                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.CreditsAddedToAccount)], item.Info.Price), ChatType.Hint);
                            }
                            break;
                        case 8: //MapShoutScroll
                            HasMapShout = true;
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.FreeMapShout)], ChatType.Hint);
                            break;
                        case 9://ServerShoutScroll
                            HasServerShout = true;
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.FreeServerShout)], ChatType.Hint);
                            break;
                        ...
                        case 12://LotteryTicket                                                                                    
                            ...
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.FirstPrizeGoldReward)], ChatType.Hint);
                                GainGold(1000000);
                            }
                            else if (Envir.Random.Next(item.Info.Effect * 16) == 1)  // 2nd prize : 200,000
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WonSecondPrizeGold)], ChatType.Hint);
                                GainGold(200000);
                            }
                            else if (Envir.Random.Next(item.Info.Effect * 8) == 1)  // 3rd prize : 100,000
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WonThirdPrizeGold)], ChatType.Hint);
                                GainGold(100000);
                            }
                            else if (Envir.Random.Next(item.Info.Effect * 4) == 1) // 4th prize : 10,000
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WonFourthPrizeGold)], ChatType.Hint);
                                GainGold(10000);
                            }
                            else if (Envir.Random.Next(item.Info.Effect * 2) == 1)  // 5th prize : 1,000
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WonFifthPrizeGold)], ChatType.Hint);
                                GainGold(1000);
                            }
                            else if (Envir.Random.Next(item.Info.Effect) == 1)  // 6th prize 500
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WonSixthPrizeGold)], ChatType.Hint);
                                GainGold(500);
                            }
                            else
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WonNothing)], ChatType.Hint);
                            }
                            break;
                        case 13://Hero unlock autopot
                            ...
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.HeroAutoPotUnlocked)], ChatType.Hint);
                            break;
                        case 14: //Increase maximum hero count
                            if (Info.MaximumHeroCount >= Settings.MaximumHeroCount)
                            {
                                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MaximumHeroCountReached)], ChatType.Hint);
                                Enqueue(p);
                                return;
                            }
                        ...
                        case 15: //Increase Hero Inventory
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MustBeUsedOnHero)], ChatType.Hint);
                            Enqueue(p);
                            break;
                    }
                    break;
                ...
                case ItemType.Food:
                    ...
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MountFed)], ChatType.Hint);
                    Enqueue(new S.ItemRepaired { UniqueID = temp.UniqueID, MaxDura = temp.MaxDura, CurrentDura = temp.CurrentDura });
                ...
                case ItemType.Pets:
                            ...
                            case 26://Wonderdrug
                                {
                                    ...
                                    {
                                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.WonderDrugActive)], ChatType.System);
                                        Enqueue(p);
                                        return;
                                    }
                    ...
                    else
                    {
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.AlreadyHaveCreature)], ChatType.Hint);
                            petInfo = null;
                        }
                        ...
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ObtainedNewCreature)], petInfo.CustomName), ChatType.Hint);
                ...
                case ItemType.MonsterSpawn:
                    ...
                    if (spawnAsPet)
                    {
                        ...
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MaximumPetsReached)], ChatType.Hint);
                            Enqueue(p);
                            return;
                        }
                    ...
                    if (conquestOnly)
                    {
                        ...
                        if (con == null)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.SpawnOnlyDuringConquest)], monsterInfo.GameName), ChatType.Hint);
                            Enqueue(p);
                            return;
                        }
                    }
        ...
        public void CombineItem(MirGridType grid, ulong fromID, ulong toID)
        {
            ...
            switch (tempFrom.Info.Shape)
            {
                ...
                case 6: //SpecialSewingSupplies
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemNoRepairNeeded)], ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    break;
                case 7: //slots
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.InvalidCombination)], ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    if (tempTo.Info.RandomStats == null)
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemMaxSockets)], ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                ...
                case 8: //Seal
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemAlreadySealed)], ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    if (tempTo.SealedInfo != null && tempTo.SealedInfo.NextSealDate > Envir.Now)
                    {
                        ...
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ItemCannotBeResealedFor)], Functions.PrintTimeSpanFromSeconds(remainingSeconds, false)), ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                ...
                case 4: //orbs
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemMaxAddedStats)], ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotCombineItems)], ChatType.Hint);
                        Enqueue(p);
                        return;
                    }
                    ...
                    if (!succeeded)
                    {
                        ...
                        {
                            //item destroyed
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemHasBeenDestroyed)], ChatType.Hint);
                        ...
                        else
                        {
                            ...
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.UpgradeNoEffect)], ChatType.Hint);
                        }
            ...
            if (canRepair && array[indexTo] != null)
            {
                ...
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemRepaired)], ChatType.Hint);
            ...
            {
                tempTo.GemCount++;
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemUpgraded)], ChatType.Hint);
            ...
            if (canSlotUpgrade && array[indexTo] != null)
            {
                ...
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ItemSocketsIncreased)], ChatType.Hint);
            ...
            if (canSeal && array[indexTo] != null)
            {
                ...
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ItemSealedFor)], Functions.PrintTimeSpanFromSeconds(minutes * 60)), ChatType.Hint);
        ...
        public void PickUp()
        {
                ...
                if (item.Item != null)
                {
                    ...
                    if (item.Item.Info.ShowGroupPickup && IsGroupMember(this))
                        for (int j = 0; j < GroupMembers.Count; j++)
                            GroupMembers[j].ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PickedUpItem)], Name, item.Item.FriendlyName), ChatType.System);
            ...
            if (sendFail)
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotPickupNotOwner)], ChatType.System);
        ...
        public bool CanGainQuestItem(UserItem item)
        {
            ...
            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotCarryMoreQuestItems)], ChatType.System);
        ...
        public void SellItem(ulong uniqueID, ushort count)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotSellItemHere)], ChatType.System);
                    Enqueue(p);
                    return;
                }
        ...
        public void RepairItem(ulong uniqueID, bool special = false)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRepairItem)], ChatType.System);
                    return;
                }
                ...
                if (script.Types.Count != 0 && !script.Types.Contains(temp.Info.Type))
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRepairItemHere)], ChatType.System);
                    return;
                }
        ...
        public void GetMarket(string name, ItemType type)
        {
            ...
            MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.MsToMatchItems)], Envir.Stopwatch.ElapsedMilliseconds - start, MarketPanelType == MarketPanelType.GameShop ? Envir.GameShopList.Count : (UserMatch ? Account.Auctions.Count : Envir.Auctions.Count)));
        }
        ...
        public void MarketBuy(ulong auctionID, uint bidPrice = 0)
        {
            ...
            if (MarketPanelType == MarketPanelType.GameShop)
            {
                foreach (AuctionInfo auction in Search)
                {
                    ...
                    GainItem(item);
                    Enqueue(new S.MarketSuccess { Message = string.Format(ServerTextMap[nameof(ServerTextKeys.BoughtItemForCredit)], auction.Item.FriendlyName, auction.Price) });
                ...
                for (int n = 0; n < CurrentMap.NPCs.Count; n++)
                {
                    ...
                    foreach (AuctionInfo auction in Search)
                    {
                        ...
                        if (auction.ItemType == MarketItemType.Consign)
                        {
                            ...
                            Envir.MessageAccount(auction.SellerInfo.AccountInfo, string.Format(ServerTextMap[nameof(ServerTextKeys.SoldItemForGold)], auction.Item.FriendlyName, auction.Price), ChatType.Hint);
                            Enqueue(new S.MarketSuccess { Message = string.Format(ServerTextMap[nameof(ServerTextKeys.BoughtItemForGold)], auction.Item.FriendlyName, auction.Price) });
                            MarketSearch(MatchName, MatchType);
                        }
                        else
                        {
                            ...
                            {
                                string message = string.Format(ServerTextMap[nameof(ServerTextKeys.OutbidRefundGold)], auction.Item.FriendlyName, auction.CurrentBid);
                            ...
                            Envir.MessageAccount(auction.SellerInfo.AccountInfo, string.Format(ServerTextMap[nameof(ServerTextKeys.SomeoneBidGoldForItem)], auction.Item.FriendlyName, auction.CurrentBid), ChatType.Hint);
                            Enqueue(new S.MarketSuccess { Message = string.Format(ServerTextMap[nameof(ServerTextKeys.YouBidGoldForItem)], auction.Item.FriendlyName, auction.CurrentBid) });
                            MarketSearch(MatchName, MatchType);
                        }
        ...
        public void MarketSellNow(ulong auctionID)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                foreach (AuctionInfo auction in Account.Auctions)
                {
                    ...
                    {
                        MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.AuctionSoldAndExpired)], Account.AccountID));
                        return;
                    }
                    ...
                    string message = string.Format(ServerTextMap[nameof(ServerTextKeys.YouWonItemForGold)], auction.Item.FriendlyName, auction.CurrentBid);

                    Envir.MailCharacter(auction.CurrentBuyerInfo, item: auction.Item, customMessage: message);
                    Envir.MessageAccount(auction.CurrentBuyerInfo.AccountInfo, string.Format(ServerTextMap[nameof(ServerTextKeys.YouBoughtItemForGold)], auction.Item.FriendlyName, auction.CurrentBid), ChatType.Hint);

                    Account.Auctions.Remove(auction);
                    Envir.Auctions.Remove(auction);
                    GainGold(gold);
                    Enqueue(new S.MarketSuccess { Message = string.Format(ServerTextMap[nameof(ServerTextKeys.YouSoldItemGoldEarningsCommission)], auction.Item.FriendlyName, cost, gold, cost - gold) });
                    MarketSearch(MatchName, MatchType);
                    return;
                }
        ...
        public void MarketGetBack(MarketCollectionMode mode, ulong auctionID)
        {
            ...
            bool TakeAuction(AuctionInfo auction)
            {
                if (auction.Sold && auction.Expired)
                {
                    MessageQueue.Enqueue(string.Format(ServerTextMap[nameof(ServerTextKeys.AuctionSoldExpired)], Account.AccountID));
                    return false;
                }
                ...
                if (mode == MarketCollectionMode.Any || (mode == MarketCollectionMode.Expired && auction.Expired))
                {
                    if (!auction.Sold || auction.Expired)
                    {
                        ...
                        if (auction.CurrentBuyerInfo != null)
                        {
                            string message = string.Format(ServerTextMap[nameof(ServerTextKeys.AuctionOutbidRefund)], auction.Item.FriendlyName, auction.CurrentBid);
                ...
                if (mode == MarketCollectionMode.Any || (mode == MarketCollectionMode.Sold && auction.Sold))
                {
                    ...
                    GainGold(gold);
                    Enqueue(new S.MarketSuccess { Message = string.Format(ServerTextMap[nameof(ServerTextKeys.SoldItemEarningsCommission)], auction.Item.FriendlyName, cost, gold, cost - gold) });
                    return true;
                }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (item != null)
                {
                    if (item.UniqueID == UniqueID)
                    {
                        ...
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableToDowngradeBelongsTo)], item.FriendlyName, item.RentalInformation.OwnerName), ChatType.System);
                            return;
                        }
                        ...
                        if (Info.AccountInfo.Gold >= item.DowngradePrice())
                        {
                            ...
                            {
                                case 0:
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.RemoveFailedLevel0)], item.FriendlyName), ChatType.System);
                                    break;
                                case 1:
                                    ...
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.RemoveSuccessLevel)], item.FriendlyName, item.Awake.GetAwakeLevel()), ChatType.System);
                                    Enqueue(new S.RefreshItem { Item = item });
                                    break;
                                default:
                                    break;
        ...
        public void DisassembleItem(ulong UniqueID)
        {
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableToDisassemble)], item.FriendlyName), ChatType.System);
                    return;
                }

                if (item.RentalInformation != null && item.RentalInformation.BindingFlags.HasFlag(BindMode.UnableToDisassemble))
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableToDisassembleBelongsTo)], item.FriendlyName, item.RentalInformation.OwnerName), ChatType.System);
                    return;
                }
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (item != null)
                {
                    if (item.UniqueID == UniqueID)
                    {
                        if (item.RentalInformation != null)
                        {
                            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableToResetBelongsTo)], item.FriendlyName, item.RentalInformation.OwnerName), ChatType.System);
                            return;
                        }
        ...
        public void AddMember(string name)
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouGroupMaxMembers)], ChatType.System);
                return;
            }
            ...
            if (player == null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CouldNotBeFound)], name), ChatType.System);
                return;
            }
            if (player == this)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotGroupSelf)], ChatType.System);
                return;
            }

            if (!player.AllowGroup)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NotAllowGroup)], name), ChatType.System);
                return;
            }

            if (player.GroupMembers != null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.AlreadyInAnotherGroup)], name), ChatType.System);
                return;
            }

            if (player.GroupInvitation != null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.AlreadyReceivingInviteFromOtherPlayer)], name), ChatType.System);
                return;
            }
        ...
        public void DelMember(string name)
        {
            if (GroupMembers == null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotInGroup)], ChatType.System);
                return;
            }
            ...
            if (player == null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NotInYourGroup)], name), ChatType.System);
                return;
            }
        ...
        public void GroupInvite(bool accept)
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotInvitedToGroup)], ChatType.System);
                return;
            }

            if (!accept)
            {
                GroupInvitation.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.DeclinedGroupInvite)], Name), ChatType.System);
                GroupInvitation = null;
                return;
            }
            ...
            if (GroupMembers != null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotJoinGroup)], GroupInvitation.Name), ChatType.System);
                GroupInvitation = null;
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NoLongerGroupLeader)], GroupInvitation.Name), ChatType.System);
                GroupInvitation = null;
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.GroupMaxMembers)], GroupInvitation.Name), ChatType.System);
                GroupInvitation = null;
                return;
            }
            if (!GroupInvitation.AllowGroup)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NotInAllowGroup)], GroupInvitation.Name), ChatType.System);
                GroupInvitation = null;
                return;
            }
            if (GroupInvitation.Node == null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNoLongerOnline)], GroupInvitation.Name), ChatType.System);
                GroupInvitation = null;
                return;
            }
        ...
        public bool CreateGuild(string guildName)
        {
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.LevelNotEnoughCreateGuild)], Settings.Guild_RequiredLevel), ChatType.System);
                return false;
            }

            if (!Info.AccountInfo.AdminAccount && String.Equals(guildName, Settings.NewbieGuild, StringComparison.OrdinalIgnoreCase))
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotCreateNewbieGuild)], ChatType.System);
                return false;
            }
            ...
            if (!Info.AccountInfo.AdminAccount)
            {
                ...
                for (int i = 0; i < Settings.Guild_CreationCostList.Count; i++)
                {
                    ...
                    if (Required.Item == null)
                    {
                        ...
                        {
                            ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.InsufficientGoldCreateGuild)], Required.Amount), ChatType.System);
                            return false;
                        }
                    }
                    else
                    {
                        ...
                        if (count != 0)
                        {
                            if (Required.Amount == 1)
                                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.RequiredToCreateGuild)], GameLanguage.DbLocalization(Required.Item.FriendlyName)), ChatType.System);
                            else
                            {
                                if (Required.Item.Type == ItemType.Ore)
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ItemPurityRequiredForGuildCreation)], GameLanguage.DbLocalization(Required.Item.FriendlyName), Required.Amount / 1000), ChatType.System);
                                else
                                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.InsufficientNeedCreateGuild)], GameLanguage.DbLocalization(Required.Item.FriendlyName), Required.Amount), ChatType.System);
                            }
                            return false;
                        }
                    }
        ...
        public void EditGuildMember(string Name, string RankName, byte RankIndex, byte ChangeType)
        {
            ...
            switch (ChangeType)
            {
                case 0: //add member
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotAllowedRecruitMembers)], ChatType.System);
                        return;
                    }
                    ...
                    if (player == null)
                    {
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerIsNotOnline)], Name), ChatType.System);
                        return;
                    }
                    if ((player.MyGuild != null) || (player.MyGuildRank != null) || (player.Info.GuildIndex != -1))
                    {
                        ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.AlreadyInGuild)], Name), ChatType.System);
                        return;
                    }
                    if (!player.EnableGuildInvite)
                    {
                        ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.DisablingGuildInvites)], Name), ChatType.System);
                        return;
                    }
                    if (player.PendingGuildInvite != null)
                    {
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.GuildInvitePending)], Name), ChatType.System);
                        return;
                    }

                    if (MyGuild.IsAtWar())
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRecruitDuringWar)], ChatType.System);
                        return;
                    }
                ...
                case 1: //delete member
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotRemoveMembers)], ChatType.System);
                        return;
                    }
                    if (Name == "") return;
                ...
                case 2: //promote member (and it'll auto create a new rank at bottom if the index > total ranks!)
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotAllowedChangeOtherRank)], ChatType.System);
                        return;
                    }
                    if (Name == "") return;
                ...
                case 3: //change rank name
                    if (!MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeRank))
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotAllowedChangeRank)], ChatType.System);
                        return;
                    }
                    if ((RankName == "") || (RankName.Length < 3))
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.RankNameTooShort)], ChatType.System);
                        return;
                    }
                ...
                case 4: //new rank
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoMoreRankSlotsAvailable)], ChatType.System);
                        return;
                    }
        ...
        public void EditGuildNotice(List<string> notice)
        {
            ...
            if (!MyGuildRank.Options.HasFlag(GuildRankOptions.CanChangeNotice))
            {
                ...
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildNoticeChangeNotAllowed)], ChatType.System);
                return;
            }
            if (notice.Count > 200)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildNoticeMaxLines)], ChatType.System);
                return;
            }
        ...
        public void GuildInvite(bool accept)
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildNotInvited)], ChatType.System);
                return;
            }
            if (!accept)
            {
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.GuildInviteFull)], PendingGuildInvite.Name), ChatType.System);
                return;
            }
        ...
        public void GuildNameReturn(string Name)
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildNameTooLong)], ChatType.System);
            ...
            if (MyGuild != null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouAlreadyInGuild)], ChatType.System);
                CanCreateGuild = false;
                return;
            }
            ...
            if (guild != null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.GuildAlreadyExists)], Name), ChatType.System);
                CanCreateGuild = false;
                return;
            }
        ...
        public void GuildStorageGoldChange(byte type, uint amount)
        {
            if ((MyGuild == null) || (MyGuildRank == null))
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotPartOfGuild)], ChatType.System);
                return;
            }

            if (!InSafeZone)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotUseGuildStorageOutsideSafezones)], ChatType.System);
                return;
            }
            ...
            if (type == 0)//donate
            {
                ...
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.InsufficientGold)], ChatType.System);
                    return;
                }

                if ((MyGuild.Gold + (ulong)amount) > uint.MaxValue)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildGoldLimitReached)], ChatType.System);
                    return;
                }
            ...
            else
            {
                ...
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GoldLimitReached)], ChatType.System);
                    return;
                }

                if (MyGuildRank.Index != 0)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.InsufficientRank)], ChatType.System);
                    return;
                }
        ...
        public void GuildStorageItemChange(byte type, int from, int to)
        {
            ...
            {
                Enqueue(p);
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildStorageOutsideSafezone)], ChatType.System);
                return;
            }
            ...
            switch (type)
            {
                case 0://store
                    ...
                    {
                        Enqueue(p);
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoPermissionGuildStorage)], ChatType.System);
                        return;
                    }
                    if (from < 0 || from >= Info.Inventory.Length)
                    {
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.TargetSlotNotEmpty)], ChatType.System);
                        Enqueue(p);
                        return;
                    }
                ...
                case 1://retrieve
                    if (!MyGuildRank.Options.HasFlag(GuildRankOptions.CanRetrieveItem))
                    {
                        ...
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoPermissionGuildStorageRetrieve)], ChatType.System);
                        return;
                    }
                ...
                case 2: // Move Item
                    ...
                    {
                        Enqueue(p);
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoGuildStorageMovePermission)], ChatType.System);
                        return;
                    }
                    if (from < 0 || from >= MyGuild.StoredItems.Length)
                    {
        ...
        public void GuildWarReturn(string Name)
        {
            ...
            if (enemyGuild == null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.GuildNotFound)], Name), ChatType.System);
                return;
            }

            if (MyGuild == enemyGuild)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotWarOwnGuild)], ChatType.System);
                return;
            }

            if (enemyGuild.Name == Settings.NewbieGuild)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotWarNewPlayersGuild)], ChatType.System);
                return;
            }

            if (MyGuild.WarringGuilds.Contains(enemyGuild))
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.AlreadyAtWarWithGuild)], ChatType.System);
                return;
            }

            if (MyGuild.Gold < Settings.Guild_WarCost)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildBankFundsInsufficient)], ChatType.System);
                return;
            }

            if (MyGuild.GoToWar(enemyGuild))
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.YouStartedWarWith)], Name), ChatType.System);
                enemyGuild.SendMessage(string.Format(ServerTextMap[nameof(ServerTextKeys.HasStartedWar)], MyGuild.Name), ChatType.System);
        ...
        public void GuildBuffUpdate(byte type, int id)
        {
            ...
            switch (type)
            {
                ...
                case 1://buy the buff
                    ...
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoCorrectGuildRank)], ChatType.System);
                        return;
                    }
                    GuildBuffInfo BuffInfo = Envir.FindGuildBuffInfo(id);
                    if (BuffInfo == null)
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.BuffNotExist)], ChatType.System);
                        return;
                    }
                    if (MyGuild.GetBuff(id) != null)
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.BuffAlreadyObtained)], ChatType.System);
                        return;
                    }
                ...
                case 2://activate the buff
                    if (!MyGuildRank.Options.HasFlag(GuildRankOptions.CanActivateBuff))
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.IncorrectGuildRank)], ChatType.System);
                        return;
                    }
                    GuildBuff Buff = MyGuild.GetBuff(id);
                    if (Buff == null)
                    {
                        ReceiveChat(ServerTextMap[nameof(ServerTextKeys.BuffNotObtained)], ChatType.System);
                        return;
                    }
        ...
        public void PurchaseGuildTerritory(string owner)
        {
            ...
            if (gt == null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.OwnerGuildNotFound)], ChatType.System);
                return;
            }

            if (gt.Price == 0)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.TerritoryNoLongerForSale)], ChatType.System);
                return;
            }

            if (MyGuild == null || MyGuildRank.Index != 0)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildLeaderToPurchaseTerritory)], ChatType.System);
                return;
            }

            if (gt.Owner == MyGuild.Name)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.AlreadyOwnTerritory)], ChatType.System);
                return;
            }

            if (MyGuild.HasGT)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.AlreadyOwnATerritory)], ChatType.System);
                return;
            }

            if (MyGuild.Gold < gt.Price)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.InsufficientFunds)], ChatType.System);
                return;
            }

            MyGuild.Gold -= (uint)gt.Price;
            MyGuild.SendServerPacket(new S.GuildStorageGoldChange { Type = 2, Amount = (uint)gt.Price });
            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.GuildTerritoryPurchaseProcess24Hrs)], ChatType.System);
            ...
            if (guild != null)
            {
                ...
                {
                    Message = ServerTextMap[nameof(ServerTextKeys.TerritorySold)],
        ...
        public void TradeRequest()
        {
            ...
            if (TradePartner != null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.AlreadyTrading)], ChatType.System);
                return;
            }
            ...
            if (player != null)
            {
                ...
                if (player == this)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotTradeWithSelf)], ChatType.System);
                    return;
                }

                if (player.Dead || Dead)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotTradeWhenDead)], ChatType.System);
                    return;
                }

                if (player.TradeInvitation != null)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyHasTradeInvitation)], player.Info.Name), ChatType.System);
                    return;
                }

                if (!player.AllowTrade)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNotAllowingTrade)], player.Info.Name), ChatType.System);
                    return;
                }

                if (!Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) || player.CurrentMap != CurrentMap)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNotInTradeRange)], player.Info.Name), ChatType.System);
                    return;
                }

                if (player.TradePartner != null)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyTrading)], player.Info.Name), ChatType.System);
                    return;
                }
        ...
        public void TradeReply(bool accept)
        {
            ...
            if (!accept)
            {
                TradeInvitation.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerRefusedTrade)], Info.Name), ChatType.System);
                TradeInvitation = null;
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyTrading)], TradeInvitation.Info.Name), ChatType.System);
                TradeInvitation = null;
                return;
            }
        ...
        public void TradeConfirm(bool confirm)
        {
            ...
            {
                TradePartner.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerWaitingConfirmTrade)], Info.Name), ChatType.System);
            }
            ...
            for (int p = 0; p < 2; p++)
            {
                ...
                {
                    CanTrade = false;
                    TradePair[p].ReceiveChat(ServerTextMap[nameof(ServerTextKeys.TradingPartnerCannotAcceptAllItems)], ChatType.System);
                    TradePair[p].Enqueue(new S.TradeCancel { Unlock = true });

                    TradePair[o].ReceiveChat(ServerTextMap[nameof(ServerTextKeys.UnableAcceptAllItems)], ChatType.System);
                ...
                {
                    CanTrade = false;
                    TradePair[p].ReceiveChat(ServerTextMap[nameof(ServerTextKeys.PartnerCannotAcceptMoreGold)], ChatType.System);
                    TradePair[p].Enqueue(new S.TradeCancel { Unlock = true });

                    TradePair[o].ReceiveChat(ServerTextMap[nameof(ServerTextKeys.UnableAcceptMoreGold)], ChatType.System);
                    TradePair[o].Enqueue(new S.TradeCancel { Unlock = true });
            ...
            if (CanTrade)
            {
                for (int p = 0; p < 2; p++)
                {
                    ...
                    for (int i = 0; i < TradePair[p].Info.Trade.Length; i++)
                    {
                        ...
                        Report.ItemMoved(u, MirGridType.Trade, MirGridType.Inventory, i, -99, string.Format(ServerTextMap[nameof(ServerTextKeys.TradeFromTo)], TradePair[p].Name, TradePair[o].Name));
                    }

                    if (TradePair[p].TradeGoldAmount > 0)
                    {
                        Report.GoldChanged(TradePair[p].TradeGoldAmount, true, string.Format(ServerTextMap[nameof(ServerTextKeys.TradeFromTo)], TradePair[p].Name, TradePair[o].Name));

                        TradePair[o].GainGold(TradePair[p].TradeGoldAmount);
                        TradePair[p].TradeGoldAmount = 0;
                    }

                    TradePair[p].ReceiveChat(ServerTextMap[nameof(ServerTextKeys.TradeSuccessful)], ChatType.System);
        ...
        public void FishingCast(bool cast, bool cancel = false)
        {
            ...
            if (hook == null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NeedHook)], ChatType.System);
                return;
            }
            else
            {
            ...
            if (cast)
            {
                ...
                if (item == null)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouNeedBait)], ChatType.System);
                    return;
                }
            ...
            else
            {
                ...
                if (FishFound)
                {
                    ...
                    if (Envir.Random.Next(0, 100) <= getChance)
                    {
                        ...
                        if (dropItem == null)
                        {
                            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.FishGotAway)], ChatType.System);
                        }
        ...
        public void AcceptQuest(int index)
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MaximumQuestsTaken)], ChatType.System);
                return;
            }

            if (CompletedQuests.Contains(index))
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.QuestAlreadyCompleted)], ChatType.System);
                return;
            }
            ...
            if (!canAccept)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CouldNotAcceptQuest)], ChatType.System);
                return;
            }
        ...
        public void FinishQuest(int questIndex, int selectedItemIndex = -1)
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotHandInQuestBagFull)], ChatType.System);
                return;
            }
        ...
        public void ShareQuest(int questIndex)
        {
            ...
            if (!shared)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.QuestNotShared)], ChatType.System);
            }
        }
        ...
        public bool CheckNeedQuestItem(UserItem item, bool gainItem = true)
        {
                ...
                Where(e => CanGainQuestItem(item)))
            {
                if (gainItem)
                {
                    ...
                    Enqueue(new S.SendOutputMessage { Message = string.Format(ServerTextMap[nameof(ServerTextKeys.YouFound)], item.FriendlyName), Type = OutputMessageType.Quest });
        ...
        public void CheckNeedQuestKill(MonsterInfo mInfo)
        {
                    ...
                    Where(quest => quest.NeedKill(mInfo)))
            {
                ...
                Enqueue(new S.SendOutputMessage { Message = string.Format(ServerTextMap[nameof(ServerTextKeys.YouKilled)], mInfo.GameName), Type = OutputMessageType.Quest });
        ...
        public void SendMail(string name, string message)
        {
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.TextExceedsMailLimit)]), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.RecipientsMailboxFull)], ChatType.System);
                return;
            }

            if (player.Friends.Any(e => e.Info == Info && e.Blocked))
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.PlayerNotAcceptingMail)], ChatType.System);
                return;
            }

            if (Info.Friends.Any(e => e.Info == player && e.Blocked))
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotMailPlayerOnBlacklist)], ChatType.System);
                return;
            }
        ...
        public void SendMail(string name, string message, uint gold, ulong[] items, bool stamped)
        {
            ...
            if (message.Length > 500)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.TextExceedsMailSizeLimit)]), ChatType.System);
                return;
            }
            ...
            for (int j = 0; j < (hasStamp ? 5 : 1); j++)
            {
                ...
                for (int i = 0; i < Info.Inventory.Length; i++)
                {
                    ...
                    {
                        ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotBeMailed)], item.FriendlyName), ChatType.System);
                        return;
                    }
        ...
        public void CollectMail(ulong mailID)
        {
            ...
            if (!mail.Collected)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MailCollectFromPostOffice)], ChatType.System);
                return;
            }
            ...
            if (mail.Items.Count > 0)
            {
                ...
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotCollectWhenBagFull)], ChatType.System);
                    return;
                }
        ...
        public void SummonIntelligentCreature(IntelligentCreatureType pType)
        {
            ...
            for (int i = 0; i < Info.IntelligentCreatures.Count; i++)
            {
                ...
                ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.CreatureSummoned)], Info.IntelligentCreatures[i].CustomName)), ChatType.System);
                break;
            }
        ...
        public void UnSummonIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true)
        {
            ...
            for (int i = 0; i < Pets.Count; i++)
            {
                ...
                if (doUpdate) ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CreatureDismissed)], pet.CustomName), ChatType.System);
        ...
        public void ReleaseIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true)
        {
            ...
            for (int i = 0; i < Info.IntelligentCreatures.Count; i++)
            {
                ...
                if (doUpdate) ReceiveChat((string.Format(ServerTextMap[nameof(ServerTextKeys.CreatureReleased)], Info.IntelligentCreatures[i].CustomName)), ChatType.System);
        ...
        public void RefreshCreaturesTimeLeft()
        {
            ...
            if (Envir.Time > CreatureTimeLeftTicker)
            {
                ...
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CreatureExpired)], Info.IntelligentCreatures[releasedPets[i]].CustomName), ChatType.System);
        ...
        public void RefreshCreatureSummoned()
        {
            ...
            if (!petFound)
            {
                MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.SummonedCreatureNotExist)], Name, SummonedCreatureType.ToString()));
                CreatureSummoned = false;
                SummonedCreatureType = IntelligentCreatureType.None;
            }
        }
        ...
        public bool IntelligentCreatureProduceBlackStone()
        {
            ...
            if (!CanGainItem(item))
            {
                MailInfo mail = new MailInfo(Info.Index)
                {
                    MailID = ++Envir.NextMailID,
                    Sender = "BlackStone",
                    Message = ServerTextMap[nameof(ServerTextKeys.PetProducedBlackStoneInventoryFull)],
        ...
        public void StrongboxRewardItem(int boxtype)
        {
            ...
            if (dropItem == null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NothingFound)], ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NoMoreSpace)], ChatType.System);
                return;
            }
        ...
        public void AddFriend(string name, bool blocked = false)
        {
            ...
            if (info == null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.PlayerDoesNotExist)], ChatType.System);
                return;
            }

            if (Name == name)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotAddYourself)], ChatType.System);
                return;
            }

            if (Info.Friends.Any(e => e.Index == info.Index))
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyAdded)], ChatType.System);
                return;
            }
                ...
                for (int i = 0; i < Info.Inventory.Length; i++)
                {
                    ...
                    else //Send item via mail if it can no longer be stored
                    {
                        ...
                        MailInfo mail = new MailInfo(Info.Index)
                        {
                            MailID = ++Envir.NextMailID,
                            Sender = "Refiner",
                            Message = ServerTextMap[nameof(ServerTextKeys.RefiningCancelledItemNotReturned)],
        ...
        public void RefineItem(ulong uniqueID)
        {
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.CheckBeforeRefine)], Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }

            if ((Info.Inventory[index].Info.Type != ItemType.Weapon) && (Settings.OnlyRefineWeapon))
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.ItemCannotBeRefined)], Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.NotEnoughGoldToRefine)], Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }
            ...
            if ((totalDC == 0) && (totalMC == 0) && (totalSC == 0))
            {
                ...
                {
                    ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.ItemRefiningCheckLater)], Info.CurrentRefine.FriendlyName, Settings.RefineTime), ChatType.System);
                }
        ...
        public void CollectRefine()
        {
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.NotRefiningItems)], ChatType.System);
                Enqueue(p);
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ItemReadyInMinutes)], Info.CurrentRefine.FriendlyName, ((Info.CollectTime - Envir.Time) / Settings.Minute)), ChatType.System);
                Enqueue(p);
                return;
            }
            ...
            if (index == -1)
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.BagNoRoomForItem)], Info.CurrentRefine.FriendlyName), ChatType.System);
                Enqueue(p);
                return;
            }

            ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.ItemReturned)]), ChatType.System);
            p.Success = true;
        ...
        public void CheckRefine(ulong uniqueID)
        {
            ...
            if (Info.Inventory[index].RefineAdded == 0)
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.NoCheckNotRefined)], Info.Inventory[index].FriendlyName), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.CongratulationsExtraDC)], Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.CongratulationsExtraMC)], Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.CongratulationsExtraSC)], Info.Inventory[index].FriendlyName, Info.Inventory[index].RefineAdded), ChatType.System);
            ...
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.ItemSmashedOnTest)], Info.Inventory[index].FriendlyName), ChatType.System);
        ...
        public void NPCDivorce()
        {
            if (Info.Married == 0)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouNotMarried)], ChatType.System);
                return;
            }
            ...
            if (player != null)
            {
                ...
                player.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ForcefullyDivorced)]), ChatType.System);
                if (player.Info.Equipment[(int)EquipmentSlot.RingL] != null)
        ...
        public bool CheckMakeWeddingRing()
        {
            if (Info.Married == 0)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NeedMarriedForWeddingRing)]), ChatType.System);
                return false;
            }

            if (Info.Equipment[(int)EquipmentSlot.RingL] == null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NeedRingOnLeftFinger)]), ChatType.System);
                return false;
            }

            if (Info.Equipment[(int)EquipmentSlot.RingL].WeddingRing != -1)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.AlreadyWearingWeddingRing)]), ChatType.System);
                return false;
            }

            if (Info.Equipment[(int)EquipmentSlot.RingL].Info.Bind.HasFlag(BindMode.NoWeddingRing))
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotUseRingType)]), ChatType.System);
                return false;
            }
        ...
        public void ReplaceWeddingRing(ulong uniqueID)
        {
            ...
            if (CurrentRing == null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NotWearingUpgradeRing)]), ChatType.System);
                return;
            }

            if (CurrentRing.WeddingRing == -1)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NotWearingWeddingRingUpgrade)]), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotReplaceWeddingRing)]), ChatType.System);
                return;
            }

            if (!CanEquipItem(temp, (int)EquipmentSlot.RingL))
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CannotEquipItemInUse)]), ChatType.System);
                return;
            }
            ...
            if (cost > Account.Gold)
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.NotEnoughGoldForWeddingRing)]), ChatType.System);
                return;
            }
        ...
        public void MarriageRequest()
        {
            ...
            if (Info.Married != 0)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.YouAlreadyMarried)]), ChatType.System);
                return;
            }

            if (Info.MarriedDate.AddDays(Settings.MarriageCooldown) > Envir.Now)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.MarriageCooldownAfterDivorce)], Settings.MarriageCooldown), ChatType.System);
                return;
            }

            if (Info.Level < Settings.MarriageLevelRequired)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NeedLevelToMarry)], Settings.MarriageLevelRequired), ChatType.System);
                return;
            }
            ...
            if (player != null)
            {
                ...
                if (!Functions.FacingEachOther(Direction, CurrentLocation, player.Direction, player.CurrentLocation))
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NeedFaceEachOtherForMarriage)]), ChatType.System);
                    return;
                }

                if (player.Level < Settings.MarriageLevelRequired)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.LoverMinLevelMarriage)], Settings.MarriageLevelRequired), ChatType.System);
                    return;
                }

                if (player.Info.MarriedDate.AddDays(Settings.MarriageCooldown) > Envir.Now)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.MarriageCooldownAfterDivorceOther)], player.Name, Settings.MarriageCooldown), ChatType.System);
                    return;
                }

                if (!player.AllowMarriage)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.ProposalNotAllowed)], ChatType.System);
                    return;
                }

                if (player == this)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CantMarryYourself)], ChatType.System);
                    return;
                }

                if (player.Dead || Dead)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CannotMarryDeadPlayer)], ChatType.System);
                    return;
                }

                if (player.MarriageProposal != null)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.AlreadyHasMarriageInvitation)], player.Info.Name), ChatType.System);
                    return;
                }

                if (!Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) || player.CurrentMap != CurrentMap)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NotWithinMarriageRange)], player.Info.Name), ChatType.System);
                    return;
                }

                if (player.Info.Married != 0)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyMarried)], player.Info.Name), ChatType.System);
                    return;
                }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.FacePlayerForMarriageRequest)]), ChatType.System);
                return;
            }
        }
        ...
        public void MarriageReply(bool accept)
        {
            ...
            if (!accept)
            {
                MarriageProposal.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HasRefusedToMarryYou)], Info.Name), ChatType.System);
                MarriageProposal = null;
                return;
            }
            ...
            if (Info.Married != 0)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouAlreadyMarried)], ChatType.System);
                MarriageProposal = null;
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyMarried)], MarriageProposal.Info.Name), ChatType.System);
                MarriageProposal = null;
                return;
            }
            ...
            MarriageProposal.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.CongratulationsMarriedTo)], Info.Name), ChatType.System);
            ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.CongratulationsMarriedTo)], MarriageProposal.Info.Name), ChatType.System);
        ...
        public void DivorceRequest()
        {
            ...
            if (player == null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.FaceLoverToDivorce)]), ChatType.System);
                return;
            }
            ...
            if (player != null)
            {
                ...
                if (player == this)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CantDivorceSelf)], ChatType.System);
                    return;
                }

                if (player.Dead || Dead)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.CantDivorceDeadPlayer)], ChatType.System); //GOT TO HERE, NEED TO KEEP WORKING ON IT.
                    return;
                }

                if (player.Info.Index != Info.Married)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.YouNotMarriedTo)], player.Info.Name), ChatType.System);
                    return;
                }

                if (!Functions.InRange(player.CurrentLocation, CurrentLocation, Globals.DataRange) || player.CurrentMap != CurrentMap)
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.NotInDivorceRange)], player.Info.Name), ChatType.System);
                    return;
                }
        ...
        public void DivorceReply(bool accept)
        {
            ...
            if (!accept)
            {
                DivorceProposal.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HasRefusedDivorceYou)], Info.Name), ChatType.System);
                DivorceProposal = null;
                return;
            }
            ...
            if (Info.Married == 0)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouNotRequireDivorce)], ChatType.System);
                DivorceProposal = null;
                return;
            }
            ...
            DivorceProposal.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.YouAreDivorced)], Info.Name), ChatType.System);
            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouAreDivorced)], ChatType.System);
        ...
        public void GetRelationship(bool CheckOnline = true)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        player.GetRelationship(false);
                        player.ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHasComeOnline)], Info.Name), ChatType.System);
                    }
                }
            }
        }
        public void LogoutRelationship()
        {
            ...
            if (lover == null)
            {
                MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerMarriedIdNotFound)], Name, Info.Married));
                return;
            }
            ...
            if (player != null)
            {
                ...
                player.ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.PlayerHasGoneOffline)], Info.Name), ChatType.System);
            }
        }
        ...
        public void MentorBreak(bool force = false)
        {
            ...
            if (force)
            {
                ...
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.YouHaveMentorshipCooldown)], Settings.MentorLength), ChatType.System);
            }
            else
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MentorshipExpired)], ChatType.System);
            }
            ...
            if (partnerP != null)
            {
                partnerP.ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MentorshipExpired)], ChatType.System);
        ...
        public void AddMentor(string Name)
        {
            if (Info.Mentor != 0)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouAlreadyHaveMentor)], ChatType.System);
                return;
            }

            if (Info.Name == Name)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouCantMentorYourself)], ChatType.System);
                return;
            }

            if (Info.MentorDate > Envir.Now)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouCantStartNewMentorship)], ChatType.System);
                return;
            }
            ...
            if (mentor == null)
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.CannotFindPlayerByName)], Name), ChatType.System);
            }
            else
            {
                ...
                {
                    ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNotAllowMentorRequests)], mentor.Info.Name), ChatType.System);
                    return;
                }

                if (mentor.Info.MentorDate > Envir.Now)
                {
                    ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.PlayerCantStartMentorshipYet)], mentor.Info.Name), ChatType.System);
                    return;
                }

                if (mentor.Info.Mentor != 0)
                {
                    ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.PlayerIsAlreadyMentor)], mentor.Info.Name), ChatType.System);
                    return;
                }

                if (Info.Class != mentor.Info.Class)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.MentoredBySameClass)], ChatType.System);
                    return;
                }
                if ((Info.Level + Settings.MentorLevelGap) > mentor.Level)
                {
                    ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.YouCanBeMentoredByHigherLevel)], Settings.MentorLevelGap), ChatType.System);
                    return;
                }

                mentor.MentorRequest = this;
                mentor.Enqueue(new S.MentorRequest { Name = Info.Name, Level = Info.Level });
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.RequestSent)], ChatType.System);
            }
        ...
        public void MentorReply(bool accept)
        {
            ...
            if (!accept)
            {
                MentorRequest.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerRefusedMentor)], Info.Name), ChatType.System);
                MentorRequest = null;
                return;
            }
            ...
            if (Info.Mentor != 0)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouAlreadyHaveStudent)], ChatType.System);
                return;
            }
            ...
            if (student == null)
            {
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.PlayerNoLongerOnline)], student.Name), ChatType.System);
                return;
            }
            else
            {
                ...
                {
                    ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.PlayerAlreadyHasMentor)], student.Info.Name), ChatType.System);
                    return;
                }
                if (Info.Class != student.Info.Class)
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouCanOnlyMentorSameClass)], ChatType.System);
                    return;
                }
                if ((Info.Level - Settings.MentorLevelGap) < student.Level)
                {
                    ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.YouCanMentorBelowLevel)], Settings.MentorLevelGap), ChatType.System);
                    return;
                }
                ...
                ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.YouAreMentorOf)], student.Info.Name), ChatType.System);
                student.ReceiveChat(String.Format(ServerTextMap[nameof(ServerTextKeys.YouAreMentoredBy)], Info.Name), ChatType.System);
                GetMentor(false);
        ...
        public void LogoutMentor()
        {
            ...
            if (mentor == null)
            {
                MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerMentorIdNotFound)], Name, Info.Mentor));
                return;
            }
        ...
        public void GameshopBuy(int GIndex, byte Quantity, int PType)
        {
            ...
            if (Product == null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouBuyItemNotInShop)], ChatType.System);
                MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerBuyNonexistentItem)], Info.Name));
                return;
            }
            ...
            if (Product.Stock != 0)
            {
                ...
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouBuyMoreThanAvailable)], ChatType.System);
                    GameShopStock(Product);
                    MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerBuyItemStockUnavailable)], Info.Name, Product.Info.FriendlyName, Quantity));
                    return;
                }
            }
            else
            ...
            if (stockAvailable)
            {
                MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerBuyItemStockAvailable)], Info.Name, Product.Info.FriendlyName, Quantity));
                ...
                {
                    ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouDontHaveEnoughCurrency)], ChatType.System);
                    MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerBuyItemNotEnoughCurrency)], Info.Name, Product.Info.FriendlyName, Quantity));
                    return;
                }
            }
            else
            ...
            if (canAfford)
            {
                MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerBuyItemEnoughCurrency)], Info.Name, Product.Info.FriendlyName, Quantity, PType));
                if (PType == 0)
                {
            ...
            MailInfo mail = new MailInfo(Info.Index)
            {
                MailID = ++Envir.NextMailID,
                Sender = "Gameshop",
                Message = ServerTextMap[nameof(ServerTextKeys.ThankYouPurchaseGameshop)],
                Items = mailItems,
            };
            mail.Send();

            MessageQueue.EnqueueDebugging(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerBuyingItemsSent)], Info.Name, Product.Info.FriendlyName, Quantity));
            ReceiveChat(ServerTextMap[nameof(ServerTextKeys.PurchasesSentMailbox)], ChatType.Hint);
        }
        ...
        public void ItemRentalRequest()
        {
            if (Dead)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.UnableRentItemsWhileDead)], ChatType.System);
                return;
            }

            if (ItemRentalPartner != null)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouAreRentingItem)], ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.FacePlayerToRentItem)], ChatType.System);
                return;
            }

            if (Info.RentedItems.Count >= 3)
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.UnableRentMoreThan3Items)], ChatType.System);
                return;
            }

            if (targetPlayer.Info.HasRentedItem)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerUnableToRentItems)], targetPlayer.Name), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouCannotRentItemsYourself)], ChatType.System);
                return;
            }

            if (targetPlayer.Dead)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableRentItemsToPlayerWhileDead)], targetPlayer.Name), ChatType.System);
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.TargetNotInRange)], targetPlayer.Name), ChatType.System);
                return;
            }

            if (targetPlayer.ItemRentalPartner != null)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerIsBusy)], targetPlayer.Name), ChatType.System);
                return;
            }
        ...
        public void DepositRentalItem(int from, int to)
        {
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableToRentUntil)], item.FriendlyName, item.RentalInformation.ExpiryDate), ChatType.System);
                Enqueue(packet);
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableToRent)], item.FriendlyName), ChatType.System);
                Enqueue(packet);
                return;
            }
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.UnableToRentBelongsTo)], item.FriendlyName, item.RentalInformation.OwnerName), ChatType.System);
                Enqueue(packet);
                return;
            }
        ...
        public void ItemRentalLockFee()
        {
            ...
            if (ItemRentalFeeLocked && ItemRentalPartner.ItemRentalItemLocked)
                ...
                ItemRentalPartner.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerLockedRentalFee)], Name), ChatType.System);
        ...
        public void ItemRentalLockItem()
        {
            ...
            if (ItemRentalItemLocked && ItemRentalPartner.ItemRentalFeeLocked)
                ...
                ItemRentalPartner.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerLockedRentalItem)], Name), ChatType.System);
        ...
        public void ConfirmItemRental()
        {
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerUnableReceiveItem)], ItemRentalPartner.Name), ChatType.System);
                Enqueue(new S.CancelItemRental());

                ItemRentalPartner.ReceiveChat(ServerTextMap[nameof(ServerTextKeys.UnableToAcceptRentalItem)], ChatType.System);
            ...
            {
                ReceiveChat(ServerTextMap[nameof(ServerTextKeys.YouCannotReceiveMoreGold)], ChatType.System);
                Enqueue(new S.CancelItemRental());

                ItemRentalPartner.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.PlayerUnableReceiveGold)], Name), ChatType.System);
                ItemRentalPartner.Enqueue(new S.CancelItemRental());
            ...
            ItemRentalPartner.ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.YouHaveRentedFromUntil)], item.FriendlyName, Name, item.RentalInformation.ExpiryDate), ChatType.System);

            GainGold(ItemRentalPartner.ItemRentalFeeAmount);
            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.ReceivedGoldForItemRental)], ItemRentalPartner.ItemRentalFeeAmount), ChatType.System);
        ...
        public void SealHero()
        {
            ...
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HeroCannotBeSealed)]), ChatType.Hint);
                return;
            }
        ...
        public void DeleteHero()
        {
            ...
            Info.Heroes[CurrentHeroIndex] = null;
            CurrentHero = null;
            ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HeroReleasedFromService)]), ChatType.Hint);
        }
        ...
        private bool AddHero(HeroInfo hero)
        {
            ...
            if (heroCount >= Info.MaximumHeroCount)
            {
                ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.YouCannotSummonMoreHeroes)]), ChatType.Hint);
                return false;
            }
            ...
            for (int i = 0; i < Info.Heroes.Length; i++)
            {
                ...
                {
                    ReceiveChat(string.Format(ServerTextMap[nameof(ServerTextKeys.HeroAddedToStorage)]), ChatType.Hint);
