    ...
    public class PlayerObject : HumanObject
    {
        ...
        public override void Process()
        {
            ...
            if (Account.HasExpandedStorage && Envir.Now > Account.ExpandedStorageExpiryDate)
            {
                ...
                Enqueue(new ServerPacket.ResizeStorage { Size = Account.Storage.Length, HasExpandedStorage = Account.HasExpandedStorage, ExpiryTime = Account.ExpandedStorageExpiryDate });
            }
        ...
        public override void LevelUp()
        {
            ...
            Enqueue(new ServerPacket.LevelChanged { Level = Level, Experience = Experience, MaxExperience = MaxExperience });
        ...
        public void CheckMapInfo(MapInfo mapInfo)
        {
            ...
            {
                Enqueue(new ServerPacket.WorldMapSetupInfo { Setup = Settings.WorldMapSetup, TeleportToNpcCost = Settings.TeleportToNpcCost });
        ...
        private void StartGameSuccess()
        {
            ...
            if (Info.Thrusting) Enqueue(new ServerPacket.SpellToggle { ObjectID = ObjectID, Spell = Spell.Thrusting, CanUse = true });
            if (Info.HalfMoon) Enqueue(new ServerPacket.SpellToggle { ObjectID = ObjectID, Spell = Spell.HalfMoon, CanUse = true });
            if (Info.CrossHalfMoon) Enqueue(new ServerPacket.SpellToggle { ObjectID = ObjectID, Spell = Spell.CrossHalfMoon, CanUse = true });
            if (Info.DoubleSlash) Enqueue(new ServerPacket.SpellToggle { ObjectID = ObjectID, Spell = Spell.DoubleSlash, CanUse = true });
                    ...
                    case "ADDSTORAGE":
                        {
                            ...
                            if (Account.Gold >= cost)
                            {
                                ...
                                Enqueue(new ServerPacket.ResizeStorage { Size = Account.ExpandStorage(), HasExpandedStorage = Account.HasExpandedStorage, ExpiryTime = Account.ExpandedStorageExpiryDate });
                            }
                            else
                            {
                                ReceiveChat(GameLanguage.LowGold, ChatType.System);
        ...
        {
            ServerPacket.TakeBackHeroItem p = new ServerPacket.TakeBackHeroItem { From = from, To = to, Success = false };
        ...
        {
            ServerPacket.TransferHeroItem p = new ServerPacket.TransferHeroItem { From = from, To = to, Success = false };
                ...
                case ItemType.Scroll:
                    ...
                    switch (item.Info.Shape)
                    {
                        ...
                        case 4: //RepairOil
                            ...
                            Enqueue(new ServerPacket.ItemRepaired { UniqueID = temp.UniqueID, MaxDura = temp.MaxDura, CurrentDura = temp.CurrentDura });
                            break;
                        case 5: //WarGodOil
                            temp = Info.Equipment[(int)EquipmentSlot.Weapon];
                            if (temp == null || temp.MaxDura == temp.CurrentDura)
        ...
        public void CombineItem(MirGridType grid, ulong fromID, ulong toID)
        {
            ...
            if (canRepair && array[indexTo] != null)
            {
                ...
                Enqueue(new ServerPacket.ItemRepaired { UniqueID = tempTo.UniqueID, MaxDura = tempTo.MaxDura, CurrentDura = tempTo.CurrentDura });
            }
            ...
            if (canSlotUpgrade && array[indexTo] != null)
            {
                ...
                Enqueue(new ServerPacket.ItemSlotSizeChanged { UniqueID = tempTo.UniqueID, SlotSize = tempTo.Slots.Length });
            }
            ...
            if (canSeal && array[indexTo] != null)
            {
                ...
                Enqueue(new ServerPacket.ItemSealChanged { UniqueID = tempTo.UniqueID, ExpiryDate = tempTo.SealedInfo.ExpiryDate });
            }
        ...
        public void RepairItem(ulong uniqueID, bool special = false)
        {
            ...
            for (int n = 0; n < CurrentMap.Npcs.Count; n++)
            {
                ...
                Enqueue(new ServerPacket.ItemRepaired { UniqueID = uniqueID, MaxDura = temp.MaxDura, CurrentDura = temp.CurrentDura });
                return;
            }
        }
        public void SendStorage()
        {
            ...
            Enqueue(new ServerPacket.UserStorage { Storage = Account.Storage }); // Should be no alter before being sent.
        }
        ...
        public void GetMarket(string name, ItemType type)
        {
            ...
            Enqueue(new ServerPacket.NpcMarket { Listings = clientListings, Pages = (Search.Count - 1) / 10 + 1, UserMode = UserMatch });
        ...
        public void MarketBuy(ulong auctionID, uint bidPrice = 0)
        {
            ...
            if (MarketPanelType == MarketPanelType.GameShop)
            {
                foreach (AuctionInfo auction in Search)
                {
                    ...
                    GainItem(item);
                    Enqueue(new ServerPacket.MarketSuccess { Message = string.Format("You bought {0} for {1:#,##0} Credit", auction.Item.FriendlyName, auction.Price) });
                ...
                for (int n = 0; n < CurrentMap.Npcs.Count; n++)
                {
                    ...
                    foreach (AuctionInfo auction in Search)
                    {
                        ...
                        if (auction.ItemType == MarketItemType.Consign)
                        {
                            ...
                            Enqueue(new ServerPacket.MarketSuccess { Message = string.Format("You bought {0} for {1:#,##0} Gold", auction.Item.FriendlyName, auction.Price) });
                            MarketSearch(MatchName, MatchType);
                        }
                        else
                        {
                            ...
                            Enqueue(new ServerPacket.MarketSuccess { Message = string.Format("You bid {1:#,##0} Gold for {0}", auction.Item.FriendlyName, auction.CurrentBid) });
                            MarketSearch(MatchName, MatchType);
                        }
        ...
        public void MarketSellNow(ulong auctionID)
        {
            ...
            for (int n = 0; n < CurrentMap.Npcs.Count; n++)
            {
                ...
                foreach (AuctionInfo auction in Account.Auctions)
                {
                    ...
                    GainGold(gold);
                    Enqueue(new ServerPacket.MarketSuccess { Message = string.Format("You sold {0} for {1:#,##0} Gold. \nEarnings: {2:#,##0} Gold.\nCommision: {3:#,##0} Gold.‎", auction.Item.FriendlyName, cost, gold, cost - gold) });
                    MarketSearch(MatchName, MatchType);
                    return;
                }
            ...
            foreach (UserItem item in Info.Inventory)
            {
                if (item != null)
                {
                    if (item.UniqueID == UniqueID)
                    {
                        ...
                        Enqueue(new ServerPacket.AwakeningNeedMaterials { Materials = materials, MaterialsCount = materialCount });
                        break;
                    }
                }
            }
        ...
        public void AwakeningEffect(bool isSuccess, bool[] isHit)
        {
            for (int i = 0; i < 5; i++)
            {
                Enqueue(new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = isHit[i] ? SpellEffect.AwakeningHit : SpellEffect.AwakeningMiss, EffectType = 0, DelayTime = (uint)(i * 500) });
                Broadcast(new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = isHit[i] ? SpellEffect.AwakeningHit : SpellEffect.AwakeningMiss, EffectType = 0, DelayTime = (uint)(i * 500) });
            }

            Enqueue(new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = isSuccess ? SpellEffect.AwakeningSuccess : SpellEffect.AwakeningFail, EffectType = 0, DelayTime = 2500 });
            Broadcast(new ServerPacket.ObjectEffect { ObjectID = ObjectID, Effect = isSuccess ? SpellEffect.AwakeningSuccess : SpellEffect.AwakeningFail, EffectType = 0, DelayTime = 2500 });
        }
                ...
                if (Info.Inventory[i] != null)
                {
                        ...
                        Info.Inventory[i].Info.Type == ItemType.Awakening)
                    {
                            ...
                            currentCount[0] > 0)
                        {
                            ...
                            {
                                Enqueue(new ServerPacket.DeleteItem { UniqueID = Info.Inventory[i].UniqueID, Count = Info.Inventory[i].Count });
                                currentCount[0] -= (byte)Info.Inventory[i].Count;
                                Info.Inventory[i] = null;
                            }
                            else if (Info.Inventory[i].Count > currentCount[0])
                            {
                                Enqueue(new ServerPacket.DeleteItem { UniqueID = Info.Inventory[i].UniqueID, Count = currentCount[0] });
                            ...
                            currentCount[1] > 0)
                        {
                            ...
                            {
                                Enqueue(new ServerPacket.DeleteItem { UniqueID = Info.Inventory[i].UniqueID, Count = currentCount[1] });
        ...
        public void GroupInvite(bool accept)
        {
            ...
            if (GroupInvitation.GroupMembers == null)
            {
                ...
                GroupInvitation.Enqueue(new ServerPacket.GroupMembersMap { PlayerName = GroupInvitation.Name, PlayerMap = GroupInvitation.CurrentMap.Info.Title });
                GroupInvitation.Enqueue(new ServerPacket.SendMemberLocation { MemberName = GroupInvitation.Name, MemberLocation = GroupInvitation.CurrentLocation });
            }
            ...
            for (int i = 0; i < GroupMembers.Count; i++)
            {
                ...
                member.Enqueue(new ServerPacket.ObjectHealth { ObjectID = ObjectID, Percent = PercentHealth, Expire = time });
                Enqueue(new ServerPacket.ObjectHealth { ObjectID = member.ObjectID, Percent = member.PercentHealth, Expire = time });

                for (int j = 0; j < member.Pets.Count; j++)
                {
                    MonsterObject pet = member.Pets[j];

                    Enqueue(new ServerPacket.ObjectHealth { ObjectID = pet.ObjectID, Percent = pet.PercentHealth, Expire = time });
                }
            }
        ...
        public void GroupMemberMapNameChanged()
        {
            ...
            {
                PlayerObject member = GroupMembers[i];
                member.Enqueue(new ServerPacket.GroupMembersMap { PlayerName = Name, PlayerMap = CurrentMap.Info.Title });
                Enqueue(new ServerPacket.GroupMembersMap { PlayerName = member.Name, PlayerMap = member.CurrentMap.Info.Title });
            }
                ...
                if (TradePair[p] != null)
                {
                    for (int t = 0; t < TradePair[p].Info.Trade.Length; t++)
                    {
                        ...
                        if(FreeSpace(TradePair[p].Info.Inventory) < 1)
                        {
                            ...
                            TradePair[p].Enqueue(new ServerPacket.DeleteItem { UniqueID = temp.UniqueID, Count = temp.Count });
        ...
        public bool CheckNeedQuestItem(UserItem item, bool gainItem = true)
        {
                ...
                Where(e => CanGainQuestItem(item)))
            {
                if (gainItem)
                {
                    ...
                    Enqueue(new ServerPacket.SendOutputMessage { Message = string.Format("You found {0}.", item.FriendlyName), Type = OutputMessageType.Quest });
        ...
        public void CheckNeedQuestKill(MonsterInfo mInfo)
        {
                    ...
                    Where(quest => quest.NeedKill(mInfo)))
            {
                ...
                Enqueue(new ServerPacket.SendOutputMessage { Message = string.Format("You killed {0}.", mInfo.GameName), Type = OutputMessageType.Quest });
        ...
        public void IntelligentCreatureSay(IntelligentCreatureType pType, string message)
        {
            ...
            for (int i = 0; i < Pets.Count; i++)
            {
                ...
                Enqueue(new ServerPacket.ObjectChat { ObjectID = Pets[i].ObjectID, Text = message, Type = ChatType.Normal });
                return;
            }
        }
        ...
        {
            ServerPacket.UpdateIntelligentCreatureList packet = new ServerPacket.UpdateIntelligentCreatureList
            {
        ...
        public void NpcDivorce()
        {
            ...
            if (player != null)
            {
                ...
                if (player.Info.Equipment[(int)EquipmentSlot.RingL] != null)
                    player.Enqueue(new ServerPacket.RefreshItem { Item = player.Info.Equipment[(int)EquipmentSlot.RingL] });
            }
        }
        ...
        public void ReplaceWeddingRing(ulong uniqueID)
        {
            ...
            Enqueue(new ServerPacket.EquipItem { Grid = MirGridType.Inventory, UniqueID = temp.UniqueID, To = (int)EquipmentSlot.RingL, Success = true });
        ...
        public void DivorceReply(bool accept)
        {
            ...
            if (DivorceProposal.Info.Equipment[(int)EquipmentSlot.RingL] != null)
            {
                ...
                DivorceProposal.Enqueue(new ServerPacket.RefreshItem { Item = DivorceProposal.Info.Equipment[(int)EquipmentSlot.RingL] });
            }
        ...
        public void GetRelationship(bool CheckOnline = true)
        {
            if (Info.Married == 0)
            {
                Enqueue(new ServerPacket.LoverUpdate { Name = "", Date = Info.MarriedDate, MapName = "", MarriedDays = 0 });
            }
            else
            {
                CharacterInfo Lover = Envir.GetCharacterInfo(Info.Married);
                ...
                if (player == null)
                    Enqueue(new ServerPacket.LoverUpdate { Name = Lover.Name, Date = Info.MarriedDate, MapName = "", MarriedDays = (short)(Envir.Now - Info.MarriedDate).TotalDays });
                else
                {
                    Enqueue(new ServerPacket.LoverUpdate { Name = Lover.Name, Date = Info.MarriedDate, MapName = player.CurrentMap.Info.Title, MarriedDays = (short)(Envir.Now - Info.MarriedDate).TotalDays });
                    if (CheckOnline)
                    {
                        player.GetRelationship(false);
        ...
        public void LogoutRelationship()
        {
            ...
            if (player != null)
            {
                player.Enqueue(new ServerPacket.LoverUpdate { Name = Info.Name, Date = player.Info.MarriedDate, MapName = "", MarriedDays = (short)(Envir.Now - Info.MarriedDate).TotalDays });
        ...
        public void GetMentor(bool CheckOnline = true)
        {
            ...
            else
            {
                ...
                Enqueue(new ServerPacket.MentorUpdate { Name = mentor.Name, Level = mentor.Level, Online = player != null, MenteeEXP = Info.MentorExp });
        ...
        public void LogoutMentor()
        {
            ...
            if (player != null)
            {
                player.Enqueue(new ServerPacket.MentorUpdate { Name = Info.Name, Level = Info.Level, Online = false, MenteeEXP = mentor.MentorExp });
                player.ReceiveChat(String.Format("{0} has gone offline.", Info.Name), ChatType.System);
            }
        }
        ...
        public void GameshopBuy(int GIndex, byte Quantity)
        {
            ...
            if (canAfford)
            {
                ...
                if (GoldCost != 0) Enqueue(new ServerPacket.LoseGold { Gold = GoldCost });
                if (CreditCost != 0) Enqueue(new ServerPacket.LoseCredit { Credit = CreditCost });
        ...
        {
            var packet = new ServerPacket.DepositRentalItem { From = from, To = to, Success = false };
        ...
        {
            var packet = new ServerPacket.RetrieveRentalItem { From = from, To = to, Success = false };
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                if (rentalPair[i].ItemRentalDepositedItem != null)
                {
                    ...
                    if (FreeSpace(rentalPair[i].Info.Inventory) < 1)
                    {
                        ...
                        rentalPair[i].Enqueue(new ServerPacket.DeleteItem { UniqueID = item.UniqueID, Count = item.Count });
        ...
        {
            ServerPacket.ItemRentalLock p = new ServerPacket.ItemRentalLock { Success = false, GoldLocked = false, ItemLocked = false };
        ...
        public void SummonHero()
        {
            ...
            Enqueue(new ServerPacket.UpdateHeroSpawnState { State = hero.Dead ? HeroSpawnState.Dead : HeroSpawnState.Summoned });
        }
        ...
        {
            ServerPacket.ManageHeroes p = new ServerPacket.ManageHeroes() { MaximumCount = Info.MaximumHeroCount, CurrentHero = CurrentHero?.ClientInformation };
        ...
        public void SendNpcGoods(ServerPacket.NpcGoods goods)
        {
            ...
            for (var i = 0; i < chunks.Count; i++)
            {
                ...
                Enqueue(new ServerPacket.NpcGoods { Progress = prog, List = chunks[i], Rate = goods.Rate, Type = goods.Type, HideAddedStats = goods.HideAddedStats });
            }
        }
    }
}
