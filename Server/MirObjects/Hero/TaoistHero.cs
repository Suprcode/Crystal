    ...
    public class TaoistHero : HeroObject {
        ...
        protected override void ProcessAttack() {
            ...
            if(CanUseMagic(magic) && poisonItem != null) {
                if((poisonItem.Info.Shape == 1 && !Target.PoisonList.Any(p => p.PType == PoisonType.Green)) ||
                   (poisonItem.Info.Shape == 2 && !Target.PoisonList.Any(p => p.PType == PoisonType.Red))) {
                    BeginMagic(magic.Spell, Direction, Target.ObjectID, Target.CurrentLocation);
                    return;
                }
            }
