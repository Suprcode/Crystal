...
namespace Server.Library.MirObjects.Player {
    public class Reporting {
        ...
        public void ItemSplit(UserItem item, UserItem newItem, MirGridType grid,
                              [CallerMemberName] string source = "") {
            string message =
                $"Item Split - {item.Info.Name} from x{item.Count + newItem.Count} to x{item.Count} in {grid} - Created new item ({newItem.UniqueID}) x {newItem.Count}";

            LogMessage(message, source);
        }

        public void ItemMerged(UserItem fromItem, UserItem toItem, int slotFrom, int slotTo, MirGridType gridFrom,
                               MirGridType gridTo, [CallerMemberName] string source = "") {
            string message =
                $"Item Merged - {fromItem.Info.Name} with {toItem.Info.Name} from {slotFrom} ({gridFrom}) to {slotTo} ({gridTo}) ({toItem.UniqueID})";

            LogMessage(message, source);
        }

        public void ItemCombined(UserItem fromItem, UserItem toItem, int slotFrom, int slotTo, MirGridType grid,
                                 [CallerMemberName] string source = "") {
            string message =
                $"Item Combined - {fromItem.Info.Name} with {toItem.Info.Name} from {slotFrom} to {slotTo} in {grid} ({toItem.UniqueID})";

            LogMessage(message, source);
        }

        public void ItemMoved(UserItem item, MirGridType from, MirGridType to, int slotFrom, int slotTo,
                              string info = "", [CallerMemberName] string source = "") {
            string message =
                $"Item Moved - {(item != null ? item.Info.Name : "Empty")} from {from}:{slotFrom} to {to}:{slotTo} ({item?.UniqueID}) {info}";
        ...
        public void ItemGSBought(GameShopItem item, uint amount, uint CreditCost, uint GoldCost,
                                 [CallerMemberName] string source = "") {
            string message =
                $"Purchased {item.Info.FriendlyName} x{amount} for {CreditCost} Credits and {GoldCost} Gold.";
        ...
        public void GoldChanged(uint amount, bool lost = true, string info = "",
                                [CallerMemberName] string source = "") {
        ...
        public void CreditChanged(uint amount, bool lost = true, string info = "",
                                  [CallerMemberName] string source = "") {
        ...
        public void ItemError(MirGridType from, MirGridType to, int slotFrom, int slotTo,
                              [CallerMemberName] string source = "") {
        ...
        private void LogMessage(string message, string source) {
            try {
                string logMessage = $"{_player.Name} - {source} : {message}";
        ...
        private void LogHeroMessage(string message, string source) {
            try {
                string logMessage = $"{_player.Name}[Hero] - {source} : {message}";
