...
namespace Server.Library.MirObjects.NPC {
    public class NpcSegment {
        ...
        public List<NpcChecks> CheckList = new();
        public List<NpcActions> ActList = new(), ElseActList = new();
        ...
        public NpcSegment(NpcPage page, List<string> say, List<string> buttons, List<string> elseSay,
                          List<string> elseButtons, List<string> gotoButtons) {
            Page = page;
        ...
        public string[] ParseArguments(string[] words) {
            ...
            for (int i = 0; i < words.Length; i++) {
                foreach(Match m in r.Matches(words[i].ToUpper())) {
                    ...
                    if(Page.Args.Count >= sequence + 1) {
                        words[i] = words[i].Replace(m.Groups[0].Value, Page.Args[sequence]);
                    }
                }
            }
        ...
        public void AddVariable(MapObject player, string key, string value) {
            ...
            for (int i = 0; i < player.NpcVar.Count; i++) {
                if(!String.Equals(player.NpcVar[i].Key, key, StringComparison.CurrentCultureIgnoreCase)) {
                    continue;
                }
        ...
        public string FindVariable(MapObject player, string key) {
            ...
            foreach(KeyValuePair<string, string> t in player.NpcVar) {
                if(String.Equals(t.Key, tempKey, StringComparison.CurrentCultureIgnoreCase)) {
                    return t.Value;
                }
            }
        ...
        public void ParseCheck(string line) {
            ...
            Regex regexFlag = new Regex(@"\[(.*?)\]");
            Regex regexQuote = new Regex("\"([^\"]*)\"");
            ...
            switch (parts[0].ToUpper()) {
                ...
                case "CHECKNAMELIST":
                    ...
                    string fileName = Path.Combine(Settings.NameListPath, listPath);
                ...
                case "CHECK":
                    ...
                    Match match = regexFlag.Match(parts[1]);
                    if(match.Success) {
                ...
                case "CHECKEXACTMON":
                    ...
                    CheckList.Add(new NpcChecks(CheckType.CheckExactMon, parts[1], parts[2], parts[3], parts[4],
                        tempString));
                    break;
        ...
        public void ParseAct(List<NpcActions> acts, string line) {
            ...
            switch (parts[0].ToUpper()) {
                ...
                case "CALL":
                    ...
                    NpcScript script = NpcScript.GetOrAdd(0, listPath, NpcScriptType.Called);
                ...
                case "LOCALMESSAGE":
                    Match match = regexQuote.Match(line);
                    if(match.Success) {
                ...
                case "CALC":
                    ...
                    if(match.Success) {
                        acts.Add(new NpcActions(ActionType.Calc, "%" + parts[1], parts[2], valueToStore,
                            parts[1].Insert(1, "-")));
                    }
        ...
        public List<string> ParseSay(PlayerObject player, List<string> speech) {
            for (int i = 0; i < speech.Count; i++) {
                string[] parts = speech[i].Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public string ReplaceValue(PlayerObject player, string param) {
            ...
            Regex varRegex = new Regex(@"(.*?)\(([A-Z][0-9])\)");
            Regex oneValRegex = new Regex(@"(.*?)\(((.*?))\)");
            Regex twoValRegex = new Regex(@"(.*?)\(((.*?),(.*?))\)");
            ...
            switch (innerMatch) {
                case "MONSTERCOUNT()":
                    ...
                    break;
                case "CONQUESTGUARD()":
                    string val1 = FindVariable(player, "%" + twoValMatch.Groups[3].Captures[0].Value.ToUpper());
                    string val2 = FindVariable(player, "%" + twoValMatch.Groups[4].Captures[0].Value.ToUpper());

                    int intVal1, intVal2;

                    if(int.TryParse(val1.Replace("%", ""), out intVal1) &&
                       int.TryParse(val2.Replace("%", ""), out intVal2)) {
                ...
                case "CONQUESTSCHEDULE()":
                    ...
                    if(int.TryParse(val1.Replace("%", ""), out intVal1)) {
                        ...
                        if(Conquest.GuildInfo.AttackerID == -1) {
                        ...
                        if(Envir.Guilds.FirstOrDefault(x => x.Guildindex == Conquest.GuildInfo.AttackerID) == null) {
                            ...
                            newValue = Envir.Guilds.FirstOrDefault(x => x.Guildindex == Conquest.GuildInfo.AttackerID)
                                .Name;
                        }
                    }
                ...
                case "ARMOUR":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Armour] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Armour].FriendlyName
                        : "No Armour";
                    break;
                case "WEAPON":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Weapon] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Weapon].FriendlyName
                        : "No Weapon";
                    break;
                case "RING_L":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.RingL] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.RingL].FriendlyName
                        : "No Ring";
                    break;
                case "RING_R":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.RingR] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.RingR].FriendlyName
                        : "No Ring";
                    break;
                case "BRACELET_L":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.BraceletL] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.BraceletL].FriendlyName
                        : "No Bracelet";
                    break;
                case "BRACELET_R":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.BraceletR] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.BraceletR].FriendlyName
                        : "No Bracelet";
                    break;
                case "NECKLACE":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Necklace] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Necklace].FriendlyName
                        : "No Necklace";
                    break;
                case "BELT":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Belt] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Belt].FriendlyName
                        : "No Belt";
                    break;
                case "BOOTS":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Boots] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Boots].FriendlyName
                        : "No Boots";
                    break;
                case "HELMET":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Helmet] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Helmet].FriendlyName
                        : "No Helmet";
                    break;
                case "AMULET":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Amulet] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Amulet].FriendlyName
                        : "No Amulet";
                    break;
                case "STONE":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Stone] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Stone].FriendlyName
                        : "No Stone";
                    break;
                case "TORCH":
                    newValue = player.Info.Equipment[(int)EquipmentSlot.Torch] != null
                        ? player.Info.Equipment[(int)EquipmentSlot.Torch].FriendlyName
                        : "No Torch";
                    break;
                ...
                case "GUILDNAME":
                    ...
                    break;
                case "ROLLRESULT":
                    newValue = player.NpcData.TryGetValue("NpcRollResult", out object _rollResult)
                ...
                case "MOUNTLOYALTY":
                    if(!player.Mount.HasMount) {
                        ...
                        newValue =
                            $"{player.Info.Equipment[(int)EquipmentSlot.Mount].CurrentDura} ({player.Info.Equipment[(int)EquipmentSlot.Mount].MaxDura})";
                    }
            ...
            for (int i = 0; i < CheckList.Count; i++) {
                ...
                switch (check.Type) {
                    case CheckType.CheckDay:
                        string day = Envir.Now.DayOfWeek.ToString().ToUpper();
                        string dayToCheck = param[0].ToUpper();
                    ...
                    case CheckType.CheckHour:
                        ...
                        int hour = Envir.Now.Hour;
                        uint hourToCheck = tempUint;
                    ...
                    case CheckType.CheckMinute:
                        ...
                        int minute = Envir.Now.Minute;
                        uint minuteToCheck = tempUint;
                    ...
                    case CheckType.CheckExactMon:
                        ...
                        failed = !Compare(param[1], Envir.Objects.Count(d => d.CurrentMap == map &&
                                                                             d.Race == ObjectType.Monster &&
                                                                             string.Equals(d.Name.Replace(" ", ""),
                                                                                 param[0],
        ...
        public bool Check(MonsterObject monster) {
            ...
            for (int i = 0; i < CheckList.Count; i++) {
                ...
                for (int j = 0; j < param.Count; j++) {
                    string[] parts = param[j].Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                ...
                switch (check.Type) {
                    ...
                    case CheckType.CheckRange:
                        int x, y, range;
                        if(!int.TryParse(param[0], out x) || !int.TryParse(param[1], out y) ||
                           !int.TryParse(param[2], out range)) {
                            failed = true;
                            break;
                        }
            ...
            for (int i = 0; i < CheckList.Count; i++) {
                ...
                switch (check.Type) {
                    ...
                    case CheckType.CheckItem:
                        ...
                        foreach(UserItem item in
                                player.Info.Inventory.Where(item => item != null && item.Info == info)) {
                            if(checkDura) {
                    ...
                    case CheckType.CheckGender:
                        ...
                        if(!Enum.TryParse(param[0], false, out gender)) {
                            failed = true;
                            break;
                        }
                    ...
                    case CheckType.CheckClass:
                        ...
                        if(!Enum.TryParse(param[0], true, out mirClass)) {
                            failed = true;
                            break;
                        }
                    ...
                    case CheckType.CheckNameList:
                        ...
                        string[] read = File.ReadAllLines(param[0]);
                    ...
                    case CheckType.Check:
                        ...
                        if(!uint.TryParse(param[0], out tempUint) || !uint.TryParse(param[1], out onCheck) ||
                           tempUint > Globals.FlagIndexCount) {
                            failed = true;
                            break;
                        }
                    ...
                    case CheckType.CheckPet:
                        ...
                        for (int c = player.Pets.Count - 1; c >= 0; c--) {
                            if(string.Compare(player.Pets[c].Info.Name, param[0], true) != 0) {
                                continue;
                            }
                    ...
                    case CheckType.HasBagSpace:
                        ...
                        for (int k = 0; k < player.Info.Inventory.Length; k++) {
                            if(player.Info.Inventory[k] == null) {
                                slotCount++;
                            }
                        }
                    ...
                    case CheckType.AffordGuard:
                        ...
                        try {
                            ...
                            ConquestGuildArcherInfo Archer =
                                Conquest.ArcherList.FirstOrDefault(g => g.Info.Index == tempInt2);
                    ...
                    case CheckType.AffordGate:
                        ...
                        try {
                            ...
                            ConquestGuildGateInfo Gate =
                                Conquest.GateList.FirstOrDefault(f => f.Info.Index == tempInt2);
                    ...
                    case CheckType.AffordWall:
                        ...
                        try {
                            ...
                            ConquestGuildWallInfo Wall =
                                Conquest.WallList.FirstOrDefault(h => h.Info.Index == tempInt2);
                    ...
                    case CheckType.CheckTimer: {
                        ...
                        try {
                            string globalTimerKey = "_-" + param[0];
            ...
            for (int i = 0; i < acts.Count; i++) {
                ...
                switch (act.Type) {
                    ...
                    case ActionType.GlobalMessage:
                        if(!Enum.TryParse(param[1], true, out chatType)) {
                            return;
                        }
                    ...
                    case ActionType.Mongen:
                        if(Param1 == null || Param2 == 0 || Param3 == 0) {
                            return;
                        }
            ...
            for (int i = 0; i < acts.Count; i++) {
                ...
                switch (act.Type) {
                    case ActionType.Move: {
                        ...
                        if(!int.TryParse(param[1], out int x)) {
                            return;
                        }

                        if(!int.TryParse(param[2], out int y)) {
                            return;
                        }
                    ...
                    case ActionType.InstanceMove: {
                        if(!int.TryParse(param[1], out int instanceId)) {
                            return;
                        }
                        ...
                        Map map = Envir.GetMapByNameAndInstance(param[0], instanceId);
                        if(map == null) {
                            return;
                        }
                    ...
                    case ActionType.GiveGold: {
                        if(!uint.TryParse(param[0], out uint gold)) {
                            return;
                        }
                    ...
                    case ActionType.GiveGuildGold: {
                        ...
                        player.MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                            { Type = 3, Amount = gold });
                    }
                        break;
                    ...
                    case ActionType.GiveCredit: {
                        if(!uint.TryParse(param[0], out uint credit)) {
                            return;
                        }
                    ...
                    case ActionType.GivePearls: {
                        if(!uint.TryParse(param[0], out uint pearls)) {
                            return;
                        }
                    ...
                    case ActionType.GiveItem: {
                        if(param.Count < 2 || !ushort.TryParse(param[1], out ushort count)) {
                            count = 1;
                        }
                        ...
                        if(info == null) {
                            MessageQueue.Enqueue(string.Format("Failed to get ItemInfo: {0}, Page: {1}", param[0],
                                Key));
                            break;
                        }
                        ...
                        while (count > 0) {
                            ...
                            if(item == null) {
                                MessageQueue.Enqueue(string.Format("Failed to create UserItem: {0}, Page: {1}",
                                    param[0], Key));
                                return;
                            }
                    ...
                    case ActionType.GivePet: {
                        ...
                        MonsterInfo monInfo = Envir.GetMonsterInfo(param[0]);
                        if(monInfo == null) {
                            return;
                        }
                    ...
                    case ActionType.DelNameList: {
                        string tempString = param[0];
                        File.WriteAllLines(tempString,
                            File.ReadLines(tempString).Where(l => l != player.Name).ToList());
                    }
                        break;
                    ...
                    case ActionType.DelGuildNameList: {
                        ...
                        string tempString = param[0];
                        File.WriteAllLines(tempString,
                            File.ReadLines(tempString).Where(l => l != player.MyGuild.Name).ToList());
                    }
                        break;
                    case ActionType.ClearNameList: {
                        string tempString = param[0];
                    ...
                    case ActionType.GiveHP: {
                        if(!int.TryParse(param[0], out int tempInt)) {
                            return;
                        }
                    ...
                    case ActionType.ChangeLevel: {
                        if(!ushort.TryParse(param[0], out ushort tempuShort)) {
                            return;
                        }
                    ...
                    case ActionType.ChangeClass: {
                        if(!Enum.TryParse(param[0], true, out MirClass mirClass)) {
                            return;
                        }
                    ...
                    case ActionType.GlobalMessage: {
                        if(!Enum.TryParse(param[1], true, out ChatType chatType)) {
                            return;
                        }

                        ServerPacket.Chat p = new ServerPacket.Chat { Message = param[0], Type = chatType };
                        Envir.Broadcast(p);
                    }
                        break;
                    ...
                    case ActionType.GiveSkill: {
                        ...
                        Spell skill;
                        if(!Enum.TryParse(param[0], true, out skill)) {
                            return;
                        }
                        ...
                        if(param.Count > 1) {
                            spellLevel = byte.TryParse(param[1], out spellLevel)
                    ...
                    case ActionType.RemoveSkill: {
                        if(!Enum.TryParse(param[0], true, out Spell skill)) {
                            return;
                        }
                        ...
                        for (int j = player.Info.Magics.Count - 1; j >= 0; j--) {
                    ...
                    case ActionType.Goto: {
                        DelayedAction action = new(DelayedType.Npc, -1, player.NpcObjectID, player.NpcScriptID,
                            "[" + param[0] + "]");
                    ...
                    case ActionType.Call: {
                        if(!int.TryParse(param[0], out int scriptID)) {
                            return;
                        }

                        DelayedAction action = new DelayedAction(DelayedType.Npc, -1, player.NpcObjectID, scriptID,
                            "[@MAIN]");
                        player.ActionList.Add(action);
                    }
                        break;
                    ...
                    case ActionType.Set: {
                        int flagIndex;
                        uint onCheck;
                        if(!int.TryParse(param[0], out flagIndex)) {
                            return;
                        }

                        if(!uint.TryParse(param[1], out onCheck)) {
                            return;
                        }
                        ...
                        bool flagIsOn = Convert.ToBoolean(onCheck);

                        player.Info.Flags[flagIndex] = flagIsOn;

                        for (int f = player.CurrentMap.Npcs.Count - 1; f >= 0; f--) {
                            if(Functions.InRange(player.CurrentMap.Npcs[f].CurrentLocation, player.CurrentLocation,
                                   Globals.DataRange)) {
                    ...
                    case ActionType.Param1: {
                        if(!int.TryParse(param[1], out int tempInt)) {
                            return;
                        }
                    ...
                    case ActionType.Mongen: {
                        ...
                        if(!byte.TryParse(param[1], out byte tempByte)) {
                            return;
                        }
                    ...
                    case ActionType.TimeRecall: {
                        string tempString = "";
                        if(!long.TryParse(param[0], out long tempLong)) {
                            return;
                        }
                        ...
                        DelayedAction action = new DelayedAction(DelayedType.Npc, Envir.Time + (tempLong * 1000),
                            player.NpcObjectID, player.NpcScriptID, tempString, tempMap, tempPoint);
                        player.ActionList.Add(action);
                    }
                        break;
                    ...
                    case ActionType.TimeRecallGroup: {
                        ...
                        for (int j = 0; j < player.GroupMembers.Count(); j++) {
                            PlayerObject groupMember = player.GroupMembers[j];

                            DelayedAction action = new DelayedAction(DelayedType.Npc, Envir.Time + (tempLong * 1000),
                                player.NpcObjectID, player.NpcScriptID, tempString, player.CurrentMap,
                    ...
                    case ActionType.DelayGoto: {
                        ...
                        DelayedAction action = new DelayedAction(DelayedType.Npc, Envir.Time + (tempLong * 1000),
                            player.NpcObjectID, player.NpcScriptID, "[" + param[1] + "]");
                        player.ActionList.Add(action);
                    }
                        break;
                    ...
                    case ActionType.MonClear: {
                        ...
                        Map map = Envir.GetMapByNameAndInstance(param[0], tempInt);
                        if(map == null) {
                            return;
                        }
                        ...
                        foreach(Cell cell in map.Cells) {
                            ...
                            for (int j = 0; j < cell.Objects.Count(); j++) {
                                ...
                                if(!string.IsNullOrEmpty(param[2]) &&
                                   string.Compare(param[2], ((MonsterObject)ob).Info.Name, true) != 0) {
                                    continue;
                                }
                    ...
                    case ActionType.Calc: {
                        ...
                        if(resultLeft && resultRight) {
                            try {
                                ...
                                MessageQueue.Enqueue(string.Format("Incorrect operator: {0}, Page: {1}", param[1],
                                    Key));
                            }
                        } else {
                    ...
                    case ActionType.GiveBuff: {
                        ...
                        player.AddBuff((BuffType)(byte)Enum.Parse(typeof(BuffType), param[0], true), player,
                            Settings.Second * duration, new Stats(), visible);
                    }
                        break;
                    ...
                    case ActionType.RefreshEffects: {
                        ...
                        ServerPacket.ObjectLevelEffects p = new ServerPacket.ObjectLevelEffects
                            { ObjectID = player.ObjectID, LevelEffects = player.LevelEffects };
                        player.Enqueue(p);
                        player.Broadcast(p);
                    }
                        break;
                    ...
                    case ActionType.GroupGoto: {
                        ...
                        for (int j = 0; j < player.GroupMembers.Count(); j++) {
                            DelayedAction action = new DelayedAction(DelayedType.Npc, Envir.Time, player.NpcObjectID,
                                player.NpcScriptID, "[" + param[0] + "]");
                    ...
                    case ActionType.EnterMap: {
                        if(!player.NpcData.TryGetValue("NpcMoveMap", out object _npcMoveMap) ||
                           !player.NpcData.TryGetValue("NpcMoveCoord", out object _npcMoveCoord)) {
                            return;
                        }
                    ...
                    case ActionType.ConquestGuard: {
                        ...
                        ConquestGuildArcherInfo conquestArcher =
                            conquest.ArcherList.FirstOrDefault(z => z.Index == tempInt);
                        if(conquestArcher == null) {
                            return;
                        }
                        ...
                        if(player.IsGM) {
                            ...
                            if(player.MyGuild == null || player.MyGuild.Gold < conquestArcher.GetRepairCost()) {
                                return;
                            }

                            player.MyGuild.Gold -= conquestArcher.GetRepairCost();
                            player.MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                                { Type = 2, Amount = conquestArcher.GetRepairCost() });
                    ...
                    case ActionType.ConquestGate: {
                        ...
                        if(player.IsGM) {
                            ...
                            if(player.MyGuild == null || player.MyGuild.Gold < conquestGate.GetRepairCost()) {
                                return;
                            }

                            player.MyGuild.Gold -= (uint)conquestGate.GetRepairCost();
                            player.MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                                { Type = 2, Amount = (uint)conquestGate.GetRepairCost() });
                    ...
                    case ActionType.ConquestWall: {
                        ...
                        if(player.IsGM) {
                            ...
                            if(player.MyGuild == null || player.MyGuild.Gold < conquestWall.GetRepairCost()) {
                                return;
                            }

                            player.MyGuild.Gold -= (uint)conquestWall.GetRepairCost();
                            player.MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                                { Type = 2, Amount = (uint)conquestWall.GetRepairCost() });
                    ...
                    case ActionType.ConquestSiege: {
                        ...
                        ConquestGuildSiegeInfo conquestSiege =
                            conquest.SiegeList.FirstOrDefault(z => z.Index == tempInt);
                        ...
                        if(player.IsGM) {
                            ...
                            if(player.MyGuild == null || player.MyGuild.Gold < conquestSiege.GetRepairCost()) {
                                return;
                            }

                            player.MyGuild.Gold -= (uint)conquestSiege.GetRepairCost();
                            player.MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                                { Type = 2, Amount = (uint)conquestSiege.GetRepairCost() });
                    ...
                    case ActionType.TakeConquestGold: {
                        ...
                        if(player.MyGuild != null && player.MyGuild.Guildindex == conquest.GuildInfo.Owner) {
                            ...
                            player.MyGuild.SendServerPacket(new ServerPacket.GuildStorageGoldChange()
                                { Type = 3, Amount = conquest.GuildInfo.GoldStorage });
                    ...
                    case ActionType.GetRandomText: {
                        ...
                        if(!File.Exists(randomTextPath)) {
                            MessageQueue.Enqueue(
                                string.Format("the randomTextFile:{0} does not exist.", randomTextPath));
                        } else {
                            string[] lines = File.ReadAllLines(randomTextPath);
                    ...
                    case ActionType.PlaySound: {
                        if(!int.TryParse(param[0], out int soundID)) {
                            return;
                        }
                    ...
                    case ActionType.SetTimer: {
                        if(!int.TryParse(param[1], out int seconds) || !byte.TryParse(param[2], out byte type)) {
                            return;
                        }
                    ...
                    case ActionType.UnequipItem: {
                        ...
                        for (int e = 0; e < player.Info.Equipment.Length; e++) {
                            UserItem item = player.Info.Equipment[e];

                            if(item == null) {
                                continue;
                            }

                            EquipmentSlot slotName = (EquipmentSlot)e;

                            if(!string.IsNullOrEmpty(slot) && slot.ToLower() != slotName.ToString().ToLower()) {
                                continue;
                            }

                            if(!player.CanRemoveItem(MirGridType.Inventory, item) || item.Cursed ||
                               item.WeddingRing != -1) {
                                continue;
                            }

                            for (int k = 0; k < player.Info.Inventory.Length; k++) {
                                UserItem freeSlot = player.Info.Inventory[k];
                        ...
                        ServerPacket.UserSlotsRefresh packet = new() {
                    ...
                    case ActionType.RollDie: {
                        ...
                        int result = Envir.Random.Next(1, 7);

                        ServerPacket.Roll p = new ServerPacket.Roll
                            { Type = 0, Page = param[0], AutoRoll = autoRoll, Result = result };
                    ...
                    case ActionType.RollYut: {
                        ...
                        ServerPacket.Roll p = new ServerPacket.Roll
                            { Type = 1, Page = param[0], AutoRoll = autoRoll, Result = result };
                    ...
                    case ActionType.Drop: {
                        string path = param[0];
                        List<DropInfo> drops = new List<DropInfo>();
                        DropInfo.Load(drops, "Npc", path, 0, false);

                        foreach(DropInfo drop in drops) {
                            DropRewardInfo reward = drop.AttemptDrop(player?.Stats[Stat.ItemDropRatePercent] ?? 0,
                                player?.Stats[Stat.GoldDropRatePercent] ?? 0);
                            ...
                            if(reward != null) {
                                ...
                                foreach(ItemInfo dropItem in reward.Items) {
                    ...
                    case ActionType.ConquestRepairAll: {
                        if(!player.IsGM) {
                            player.ReceiveChat($"You are not a GM and this command is not enabled for you.",
                                ChatType.System);
                            MessageQueue.Enqueue(
                                $"GM Command @CONQUESTREPAIRALL invoked by non-GM player: {player.Name}");
                            return;
                        }
                        ...
                        MessageQueue.Enqueue(
                            $"@CONQUESTREPAIRALL invoked by GM: {player.Name} on account index: {player.Info.AccountInfo.Index}");
                ...
                switch (act.Type) {
                    ...
                    */
                    ...
                    case ActionType.GiveBuff: {
                        ...
                        monster.AddBuff((BuffType)(byte)Enum.Parse(typeof(BuffType), param[0], true), monster,
                            Settings.Second * tempInt, new Stats(), visible);
                    }
                        break;
        ...
        private void Success(PlayerObject player) {
            ...
            List<string> parseSay = new List<String>(Say);
        ...
        private void Failed(PlayerObject player) {
            ...
            List<string> parseElseSay = new List<String>(ElseSay);
