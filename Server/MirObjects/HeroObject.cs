...
namespace Server.Library.MirObjects {
    public class HeroObject : HumanObject {
        ...
        public override bool CanMove => base.CanMove && !ActiveBlizzard && !ActiveReincarnation;

        public override bool CanWalk => base.CanWalk && !ActiveBlizzard && !ActiveReincarnation;
        public override bool CanRun => base.CanRun && !ActiveBlizzard && !ActiveReincarnation;

        public override bool CanAttack => base.CanAttack && !ActiveBlizzard && !ActiveReincarnation;

        protected override bool CanCast => base.CanCast && !ActiveBlizzard && !ActiveReincarnation;
        ...
        protected bool HasMagic(Spell spell) {
            return Info.Magics.Any(x => x.Spell == spell);
        }
        ...
        public override void UseItem(ulong id) {
            ServerPacket.UseItem p = new ServerPacket.UseItem
                { UniqueID = id, Grid = MirGridType.HeroInventory, Success = false };

            UserItem item = null;
            int index = -1;

            if(Owner.Hero != null && Owner.Hero.Dead) {
                return;
            }
                ...
                case ItemType.Scroll:
                    ...
                    switch (item.Info.Shape) {
                        ...
                        case 4: //RepairOil
                            ...
                            Owner.Enqueue(new ServerPacket.ItemRepaired
        ...
        public override void LevelUp() {
            ...
            Owner.Enqueue(new ServerPacket.HeroLevelChanged
        ...
        public override void BroadcastHealthChange() {
            ...
            if(Owner.GroupMembers != null) {
                for (int i = 0; i < Owner.GroupMembers.Count; i++) {
                    ...
                    if(member.CurrentMap != CurrentMap ||
        ...
        public byte PercentMana => (byte)(MP / (float)Stats[Stat.MP] * 100);
        ...
        public override void Process() {
            ...
            if(!Functions.InRange(CurrentLocation, Owner.CurrentLocation, Globals.DataRange) ||
        ...
        protected virtual void ProcessSearch() {
            ...
            if(Owner.Info.HeroBehaviour == HeroBehaviour.Follow || !Mount.CanAttack) {
                return;
            }
                        ...
                        for (int i = 0; i < cell.Objects.Count; i++) {
                            ...
                            switch (ob.Race) {
                                ...
                                case ObjectType.Hero:
                                    ...
                                    if(Owner.Info.HeroBehaviour == HeroBehaviour.CounterAttack && ob.Target != this &&
                                       ob.Target != Owner) {
                                        continue;
                                    }
        ...
        private void SendInfo() {
            GetItemInfo();
            ServerPacket.HeroInformation packet = new ServerPacket.HeroInformation {
                ObjectID = ObjectID,
                Name = Name,
                Class = Class,
                Gender = Gender,
