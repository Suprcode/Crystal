...
namespace Server.Library.MirObjects {
    public class IntelligentCreatureObject : MonsterObject {
        ...
        public string CustomName {
            get => CreatureInfo.CustomName;
            set => CreatureInfo.CustomName = value;
        }
        ...
        public IntelligentCreatureRules CreatureRules = new();

        public IntelligentCreatureItemFilter ItemFilter {
            get => CreatureInfo.Filter;
            set => CreatureInfo.Filter = value;
        }

        public IntelligentCreaturePickupMode CurrentPickupMode {
            get => CreatureInfo.petMode;
            set => CreatureInfo.petMode = value;
        }

        public List<MapObject> TargetList = new();
        public bool FillingTargetList = false;
        public bool DoTargetList = false;
        public bool TargetListTargetClean = false;

        public int Fullness {
            get => CreatureInfo.Fullness;
            set => CreatureInfo.Fullness = value;
        }
        ...
        public long BlackstoneTime {
            get => CreatureInfo.BlackstoneTime;
            set => CreatureInfo.BlackstoneTime = value;
        }
        ...
        public long MaintainfoodTime {
            get => CreatureInfo.MaintainFoodTime;
            set => CreatureInfo.MaintainFoodTime = value;
        }
        ...
        protected override bool CanMove => Envir.Time > MoveTime && Envir.Time > ActionTime;

        public override string Name {
            get => Master == null ? CustomName :
                Dead ? CustomName : string.Format("{0}_{1}'s Pet", CustomName, Master.Name);
            set => throw new NotSupportedException();
        }

        protected override bool CanAttack => !Dead && Envir.Time > AttackTime && Envir.Time > ActionTime;

        public override ObjectType Race => ObjectType.Creature;
        ...
        protected override void ProcessRoam() {
            ...
            if(Master != null) {
                ...
                } else if(Envir.Random.Next(100) >= 60) {
        ...
        protected override void ProcessTarget() {
            ...
            if(Target.CurrentLocation == CurrentLocation || (!CheckAndMoveTo(Target.CurrentLocation) &&
                                                             Functions.InRange(CurrentLocation, Target.CurrentLocation,
                                                                 1))) {
                Attack();
                ...
                for (int y = CurrentLocation.Y - d; y <= CurrentLocation.Y + d; y++) {
                    ...
                    for (int x = CurrentLocation.X - d;
                        ...
                        for (int i = 0; i < cell.Objects.Count; i++) {
                            ...
                            if(ob.Owner != null && ob.Owner != this && ob.Owner != Master &&
                               !IsMasterGroupMember(ob.Owner)) {
                                continue;
                            }
        ...
        private void ProcessTargetList() {
            ...
            if(targ == null) {
                ...
                if(targ.CurrentMap != CurrentMap) {
                    remove = true;
                }

                if(targ.Race != ObjectType.Item) {
                    remove = true;
                }

                if(targ.Owner != null && targ.Owner != this && targ.Owner != Master &&
                   !IsMasterGroupMember(targ.Owner)) {
                    remove = true;
                }
            }
                ...
                if(item.Item != null) {
                    ...
                    if(item.Item.Info.ShowGroupPickup && IsMasterGroupMember(Master)) {
                        for (int j = 0; j < Master.GroupMembers.Count; j++) {
                            Master.GroupMembers[j].ReceiveChat(Name + " Picked up: {" + item.Item.FriendlyName + "}",
                                ChatType.Hint);
                        }
                    }

                    if(item.Item.Info.Grade == ItemGrade.Mythical || item.Item.Info.Grade == ItemGrade.Legendary ||
                       item.Item.Info.Grade == ItemGrade.Heroic) {
                        Master.ReceiveChat("Pet Picked up: {" + item.Item.FriendlyName + "}", ChatType.Hint);
                        ((PlayerObject)Master).Enqueue(new ServerPacket.IntelligentCreaturePickup
                            { ObjectID = ObjectID });
                    }
        ...
        private bool CheckItemAgainstFilter(ItemType iType) {
            ...
            if(ItemFilter.PetPickupAll) {
                return true;
            }

            switch (iType) {
                case ItemType.Nothing
                    : // <---- im not sure if any item will ever hold this ItemType but better to prevent then cure
                    return false;
        ...
        public void ManualPickup(bool mousemode, Point atLocation) {
            ...
            if(!mousemode && CreatureRules.SemiAutoPickupEnabled &&
               CurrentPickupMode == IntelligentCreaturePickupMode.SemiAutomatic) {
        ...
        public void DecreaseFullness(int amount) {
            if(Fullness <= 0 || MaintainfoodTime > 0) {
                return;
            }
        ...
        private bool IsMasterGroupMember(MapObject player) {
            if(player.Race != ObjectType.Player || Master == null) {
                return false;
            }

            return ((PlayerObject)Master).GroupMembers != null &&
                   ((PlayerObject)Master).GroupMembers.Contains((PlayerObject)player);
        }
