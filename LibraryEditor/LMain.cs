...
using LibraryEditor.Graphics;
using Shared;
using Shared.Extensions;

namespace LibraryEditor {
    public partial class LMain : Form {
        private readonly Dictionary<int, int> _indexList = new();
        ...
        public LMain() {
            ...
            FrameAction.ValueType = typeof(MirAction);
            FrameAction.DataSource = Enum.GetValues(typeof(MirAction));
                    ...
                    try {
                        ...
                        Parallel.For(0, files.Length, options, i => {
                            if(Path.GetExtension(files[i]) == ".wtl") {
                                WTLLibrary WTLlib = new(files[i]);
                                WTLlib.ToMLibrary();
                            } else {
                                WeMadeLibrary WILlib = new(files[i]);
                                WILlib.ToMLibrary();
                            }
        ...
        public static Bitmap AddPaddingToBitmap(Bitmap originalBitmap, int padding) {
            ...
            Bitmap paddedBitmap = new(newWidth, newHeight);
        ...
        private void PreviewListView_SelectedIndexChanged(object sender, EventArgs e) {
            ...
            if(!ApplyOffsets) {
                newImage = new Bitmap(
                    Math.Max(_referenceImage?.Width ?? 0, Math.Max(image.Width, referenceImage?.Width ?? 0)),
                    Math.Max(_referenceImage?.Height ?? 0, Math.Max(image.Height, referenceImage?.Height ?? 0)));
                ...
                int maxWidth = Math.Max(image.Width, referenceImage?.Width ?? 0);
                int maxHeight = Math.Max(image.Height, referenceImage?.Height ?? 0);
                ...
                using(var g = Graphics.FromImage(newImage)) {
                    ...
                    g.DrawImage(image,
                        new Point(offsetX < 0 ? Math.Abs(offsetX) : 0, offsetY < 0 ? Math.Abs(offsetY) : 0));
                }

                if(_referenceImage != null) {
                    int newMaxWidth = Math.Max(_referenceImage.Width, newImage.Width + Math.Abs(_selectedImage.X));
                    int newMaxHeight = Math.Max(_referenceImage.Height, newImage.Height + Math.Abs(_selectedImage.Y));

                    Bitmap anotherNewBitmap = new Bitmap(newMaxWidth, newMaxHeight);
                    using(var g = Graphics.FromImage(anotherNewBitmap)) {
                        g.DrawImage(_referenceImage,
                            new Point(_selectedImage.X < 0 ? Math.Abs(_selectedImage.X) : 0,
                                _selectedImage.Y < 0 ? Math.Abs(_selectedImage.Y) : 0));
                        g.DrawImage(image,
                            new Point(_selectedImage.X > 0 ? _selectedImage.X : 0,
                                _selectedImage.Y > 0 ? _selectedImage.Y : 0));
                    }
        ...
        private void AddButton_Click(object sender, EventArgs e) {
            ...
            List<string> fileNames = new(ImportImageDialog.FileNames);
            ...
            for (int i = 0; i < fileNames.Count; i++) {
                ...
                fileName = Path.Combine(Path.GetDirectoryName(fileName), "Placements",
        ...
        private void OpenShadowLibraryAndImport(string filename) {
            ...
            if(_shadowLibrary != null) {
                _shadowLibrary.Close();
            }
            ...
            for (int i = 0; i < _library.Images.Count; i++) {
                MLibraryV2.MImage mImage = _library.GetMImage(i);
                if(mImage == null || mImage.Image == null) {
                    continue;
                }

                MLibraryV2.MImage shadowImage = _shadowLibrary.GetMImage(i);
                if(shadowImage == null || shadowImage.Image == null) {
                    continue;
                }

                int offSetX = -mImage.X + shadowImage.X;
                int offSetY = -mImage.Y + shadowImage.Y;

                int maxWidth = Math.Max(mImage.Width, shadowImage.Width + Math.Abs(offSetX));
                int maxHeight = Math.Max(mImage.Height, shadowImage.Height + Math.Abs(offSetY));

                Bitmap newBitmap = new Bitmap(maxWidth, maxHeight);
                using(var g = Graphics.FromImage(newBitmap)) {
                    g.DrawImage(mImage.Image,
                        new Point(offSetX < 0 ? Math.Abs(offSetX) : 0, offSetY < 0 ? Math.Abs(offSetY) : 0));
            ...
            try {
                Task.Factory.StartNew(() => {
                    ...
                    Parallel.For(0, OpenWeMadeDialog.FileNames.Length, options, i => {
                        string fileName = OpenWeMadeDialog.FileNames[i];
                        string ext = Path.GetExtension(fileName).ToUpper();
                        if(ext == ".WTL") {
                            WTLLibrary WTLlib = new(fileName);
                            WTLlib.ToMLibrary();
                        } else if(ext == ".LIB") {
                            MLibraryV1 v1Lib = new(fileName);
                            v1Lib.ToMLibrary();
                        } else {
                            WeMadeLibrary WILlib = new(fileName);
                            WILlib.ToMLibrary();
                        }
        ...
        private void copyToToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            MLibraryV2 tempLibrary = new(SaveLibraryDialog.FileName);
            ...
            for (int i = 0; i < copyList.Count; i++) {
                ...
                tempLibrary.AddImage(image.Image, image.MaskImage, image.X, image.Y,
                    checkboxRemoveBlackOnImport.Checked);
            }
        ...
        private void countBlanksToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            for (int i = 0; i < OpenLibraryDialog.FileNames.Length; i++) {
                MLibraryV2 library = new(OpenLibraryDialog.FileNames[i]);
        ...
        private void safeToolStripMenuItem_Click(object sender, EventArgs e) {
            if(MessageBox.Show("Are you sure you want to remove the blank images?",
                   "Remove Blanks", MessageBoxButtons.YesNo) != DialogResult.Yes) {
                return;
            }
        ...
        public static void ProcessDir(string sourceDir, int recursionLvl, string outputDir) {
            if(recursionLvl <= HowDeepToScan) {
                ...
                foreach(string fileName in fileEntries) {
                    ...
                    MLibraryV0 OldLibrary = new(fileName);
                    MLibraryV2 NewLibrary = new(outputDir + Path.GetFileName(fileName)) {
                        Images = new List<MLibraryV2.MImage>(), IndexList = new List<int>(),
                        Count = OldLibrary.Images.Count
                    };
                    ;
                    ...
                    for (int j = 0; j < OldLibrary.Images.Count; j++) {
                        ...
                        NewLibrary.Images[j] = new MLibraryV2.MImage(oldimage.FBytes, oldimage.Width, oldimage.Height)
                ...
                foreach(string subdir in subdirEntries) {
                    ...
                    if(Path.GetFileName(Path.GetFullPath(subdir).TrimEnd(Path.DirectorySeparatorChar)) ==
                       Path.GetFileName(Path.GetFullPath(outputDir).TrimEnd(Path.DirectorySeparatorChar))) {
                        continue;
                    }
                    ...
                    if((File.GetAttributes(subdir) &
                       ...
                       FileAttributes.ReparsePoint) {
                        ProcessDir(subdir, recursionLvl + 1,
                            outputDir + " \\" +
                            Path.GetFileName(Path.GetFullPath(subdir).TrimEnd(Path.DirectorySeparatorChar)) + "\\");
                    }
                }
            }
        }
        ...
        private void ExportButton_Click(object sender, EventArgs e) {
            ...
            for (int i = _col[0]; i < _col[0] + _col.Count; i++) {
                ...
                File.WriteAllLines(_folder + "/Placements/" + i.ToString() + ".txt",
        ...
        private void LMain_Resize(object sender, EventArgs e) {
            if(splitContainer1.SplitterDistance <= Height - 150) {
                return;
            }

            if(Height - 150 > 0) {
                splitContainer1.SplitterDistance = Height - 150;
            }
        }
        ...
        private Image ImageBoxZoom(Image image, Size size) {
            ...
            Bitmap _bmp = new(_originalImage, Convert.ToInt32(_originalImage.Width * size.Width),
                Convert.ToInt32(_originalImage.Height * size.Height));
        ...
        private void ZoomTrackBar_Scroll(object sender, EventArgs e) {
            ...
            if(ZoomTrackBar.Value > 0) {
                try {
                    ...
                    Bitmap _newBMP = new(_selectedImage.Width * ZoomTrackBar.Value,
                        _selectedImage.Height * ZoomTrackBar.Value);
        ...
        private void PreviewListView_VirtualItemsSelectionRangeChanged(object sender,
        ...
        private void buttonReplace_Click(object sender, EventArgs e) {
            ...
            Bitmap newBmp = new(ofd.FileName);
        ...
        private void previousImageToolStripMenuItem_Click(object sender, EventArgs e) {
            try {
                if(PreviewListView.Visible && PreviewListView.Items.Count > 0) {
                    ...
                    if(_selectedImage.Height == 1 && _selectedImage.Width == 1 &&
                       PreviewListView.SelectedIndices[0] != 0) {
                        previousImageToolStripMenuItem_Click(null, null);
                    }
                }
            } catch(Exception) {
                PreviewListView.SelectedIndices.Clear();
                PreviewListView.Items[PreviewListView.Items.Count - 1].Selected = true;
            }
        }
        ...
        private void autofillNpcFramesToolStripMenuItem_Click(object sender, EventArgs e) {
            if(FolderLibraryDialog.ShowDialog() != DialogResult.OK) {
                return;
            }

            string path = FolderLibraryDialog.SelectedPath;
            ...
            foreach(string file in files) {
                ...
                if(!int.TryParse(name, out int imageNumber)) {
                    continue;
                }
        ...
        private void UpdateFrameGridView() {
            ...
            foreach(MirAction action in _library.Frames.Keys) {
                Frame frame = _library.Frames[action];

                int rowIndex = frameGridView.Rows.Add();

                DataGridViewRow row = frameGridView.Rows[rowIndex];
        ...
        private void UpdateFrameGridData() {
            ...
            foreach(DataGridViewRow row in frameGridView.Rows) {
                ...
                MirAction action = (MirAction)row.Cells["FrameAction"].Value;

                if(_library.Frames.ContainsKey(action)) {
                    MessageBox.Show(
                        string.Format($"The action '{action}' exists more than once so will not be saved."));
                    continue;
                }

                Frame frame = new Frame(cells["FrameStart"].Value.ValueOrDefault<int>(),
        ...
        private void frameGridView_RowEnter(object sender, DataGridViewCellEventArgs e) {
            DataGridViewRow row = frameGridView.Rows[e.RowIndex];
        ...
        private void FrameAnimTimer_Tick(object sender, EventArgs e) {
            ...
            try {
                ...
                int drawFrame = _drawFrame.Start + (_drawFrame.OffSet * (byte)_currentDirection) + _currentFrame;
        ...
        private FrameSet GetFrameSetByImage(Monster image) {
            ...
            throw new NotImplementedException(
                "The method 'GetFrameSetByImage' must be updated before this function can be used");
