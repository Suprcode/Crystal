    ...
    public partial class GameShop : Form {
        ...
        private void LoadGameShopItems() {
            ...
            for (int i = 0; i < SMain.EditEnvir.GameShopList.Count; i++) {
                if(!ClassFilter_lb.Items.Contains(SMain.EditEnvir.GameShopList[i].Class)) {
                    ClassFilter_lb.Items.Add(SMain.EditEnvir.GameShopList[i].Class);
                }

                if(!CategoryFilter_lb.Items.Contains(SMain.EditEnvir.GameShopList[i].Category)) {
                    CategoryFilter_lb.Items.Add(SMain.EditEnvir.GameShopList[i].Category);
                }
        ...
        private void UpdateGameShopList() {
            GameShopListBox.Items.Clear();
            for (int i = 0; i < SMain.EditEnvir.GameShopList.Count; i++) {
                if(ClassFilter_lb.Text == "All Classes" ||
                   SMain.EditEnvir.GameShopList[i].Class == ClassFilter_lb.Text) {
                    if(SectionFilter_lb.Text == "All Items" ||
                       (SMain.EditEnvir.GameShopList[i].TopItem && SectionFilter_lb.Text == "Top Items") ||
                       (SMain.EditEnvir.GameShopList[i].Deal && SectionFilter_lb.Text == "Sale Items") ||
                       (SMain.EditEnvir.GameShopList[i].Date > Envir.Now.AddDays(-7) &&
                        SectionFilter_lb.Text == "New Items")) {
                        if(CategoryFilter_lb.Text == "All Categories" ||
                           SMain.EditEnvir.GameShopList[i].Category == CategoryFilter_lb.Text) {
                            GameShopListBox.Items.Add(SMain.EditEnvir.GameShopList[i]);
                        }
                    }
                }
        ...
        private void GPPrice_textbox_TextChanged(object sender, EventArgs e) {
            ...
            if(ActiveControl.Text != "") {
                GoldPrice_textbox.Text = (temp * Settings.CredxGold).ToString();
            }
        }
        ...
        private void ServerLog_button_Click(object sender, EventArgs e) {
            if(SMain.Envir.Running) {
                if(MessageBox.Show(
                       "Reseting purchase logs cannot be reverted and will set stock levels back to defaults, This will take effect instantly.",
                       "Remove Logs?", MessageBoxButtons.YesNo) != DialogResult.Yes) {
                    return;
                }
                ...
                if(MessageBox.Show(
                       "Reseting purchase logs cannot be reverted and will set stock levels back to defaults, This will take effect when you start the server",
                       "Remove Logs?", MessageBoxButtons.YesNo) != DialogResult.Yes) {
                    return;
                }
