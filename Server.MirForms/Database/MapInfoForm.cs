    ...
    public partial class MapInfoForm : Form {
        ...
        private void UpdateInterface() {
            ...
            if(mi.WeatherParticles != WeatherSetting.None) {
                for (int i = 0; i < lstParticles.Items.Count; i++) {
                    object item = lstParticles.Items[i];
                    if(item != null) {
            ...
            for (int i = 1; i < _selectedMapInfos.Count; i++) {
                ...
                if(MapIndexTextBox.Text != mi.Index.ToString()) {
                    MapIndexTextBox.Text = string.Empty;
                }
                ...
                if(MiniMapTextBox.Text != mi.MiniMap.ToString()) {
                    MiniMapTextBox.Text = string.Empty;
                }

                if(BigMapTextBox.Text != mi.BigMap.ToString()) {
                    BigMapTextBox.Text = string.Empty;
                }

                if(LightsComboBox.SelectedItem == null || (LightSetting)LightsComboBox.SelectedItem != mi.Light) {
                    LightsComboBox.SelectedItem = null;
                }

                if(MineComboBox.SelectedItem == null || MineComboBox.SelectedIndex != mi.MineIndex) {
                    MineComboBox.SelectedIndex = 1;
                }

                if(MusicTextBox.Text != mi.Music.ToString()) {
                    MusicTextBox.Text = string.Empty;
                }
                ...
                if(FireTextbox.Text != mi.FireDamage.ToString()) {
                    FireTextbox.Text = string.Empty;
                }
                ...
                if(LightningTextbox.Text != mi.LightningDamage.ToString()) {
                    LightningTextbox.Text = string.Empty;
                }

                if(MapDarkLighttextBox.Text != mi.MapDarkLight.ToString()) {
                    MapDarkLighttextBox.Text = string.Empty;
                }
        ...
        private void UpdateSafeZoneInterface() {
            ...
            if(SafeZoneInfoListBox.Items.Count != _info.SafeZones.Count) {
                SafeZoneInfoListBox.Items.Clear();
                for (int i = 0; i < _info.SafeZones.Count; i++) {
            ...
            for (int i = 1; i < _selectedSafeZoneInfos.Count; i++) {
                ...
                if(SizeTextBox.Text != info.Size.ToString()) {
        ...
        private void UpdateRespawnInterface() {
            ...
            for (int i = 1; i < _selectedRespawnInfos.Count; i++) {
                ...
                if(MonsterInfoComboBox.SelectedItem !=
                   Envir.MonsterInfoList.FirstOrDefault(x => x.Index == info.MonsterIndex)) {
                    MonsterInfoComboBox.SelectedItem = null;
                }
                ...
                if(CountTextBox.Text != info.Count.ToString()) {
                    CountTextBox.Text = string.Empty;
                }

                if(SpreadTextBox.Text != info.Spread.ToString()) {
                    SpreadTextBox.Text = string.Empty;
                }
                ...
                if(chkRespawnEnableTick.Checked != (info.RespawnTicks == 0)) {
                    ...
                    if(chkRespawnEnableTick.Checked) {
                        if(DelayTextBox.Text != info.RespawnTicks.ToString()) {
                            DelayTextBox.Text = string.Empty;
                        }
                        ...
                        if(DelayTextBox.Text != info.Delay.ToString()) {
                            DelayTextBox.Text = string.Empty;
                        }

                        chkrespawnsave.Enabled = false;
                        chkrespawnsave.Checked = false;
                        if(Randomtextbox.Text != info.RandomDelay.ToString()) {
                            Randomtextbox.Text = string.Empty;
                        }
                    }
                }

                if(DirectionTextBox.Text != info.Direction.ToString()) {
                    DirectionTextBox.Text = string.Empty;
                }
        ...
        private void UpdateMovementInterface() {
            ...
            ConquestComboBox.SelectedItem =
                Envir.ConquestInfoList.FirstOrDefault(x => x.Index == info.ConquestIndex)?.Name;
            if(ConquestComboBox.SelectedItem == null) {
                ConquestComboBox.SelectedIndex = 0;
            }
            ...
            for (int i = 1; i < _selectedMovementInfos.Count; i++) {
                ...
                if(DestMapComboBox.SelectedItem != Envir.MapInfoList.FirstOrDefault(x => x.Index == info.MapIndex)) {
                    DestMapComboBox.SelectedItem = null;
                }
                ...
                if(BigMapIconTextBox.Text != info.Icon.ToString()) {
                    BigMapIconTextBox.Text = string.Empty;
                }

                if(ConquestComboBox.SelectedItem !=
                   Envir.ConquestInfoList.FirstOrDefault(x => x.Index == info.ConquestIndex)) {
                    ConquestComboBox.SelectedItem = null;
                }
            }
        }
        ...
        private void UpdateMineZoneInterface() {
            ...
            if(MZListlistBox.Items.Count != _info.MineZones.Count) {
                MZListlistBox.Items.Clear();
                for (int i = 0; i < _info.MineZones.Count; i++) {
            ...
            for (int i = 1; i < _selectedMineZones.Count; i++) {
                ...
                if(MZSizetextBox.Text != info.Size.ToString()) {
                    MZSizetextBox.Text = string.Empty;
                }
            }
        }
        ...
        private void RefreshMapList() {
            ...
            for (int i = 0; i < MapInfoListBox.Items.Count; i++) {
                selected.Add(MapInfoListBox.GetSelected(i));
            }
        ...
        private void RefreshSafeZoneList() {
            ...
            for (int i = 0; i < SafeZoneInfoListBox.Items.Count; i++) {
                selected.Add(SafeZoneInfoListBox.GetSelected(i));
            }
        ...
        private void RefreshMovementList() {
            ...
            for (int i = 0; i < MovementInfoListBox.Items.Count; i++) {
                selected.Add(MovementInfoListBox.GetSelected(i));
            }
        ...
        private void RefreshMineZoneList() {
            ...
            for (int i = 0; i < MZListlistBox.Items.Count; i++) {
                selected.Add(MZListlistBox.GetSelected(i));
            }
        ...
        private void RemoveButton_Click(object sender, EventArgs e) {
            ...
            if(MessageBox.Show("Are you sure you want to remove the selected maps?", "Remove Maps?",
        ...
        private void LoadImage(ushort miniMapValue) {
            string imagePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Envir", "Previews", "Minimaps",
                miniMapValue + ".bmp");
        ...
        private void RemoveSZButton_Click(object sender, EventArgs e) {
            ...
            if(MessageBox.Show("Are you sure you want to remove the selected SafeZones?", "Remove SafeZones?",
                   MessageBoxButtons.YesNo) != DialogResult.Yes) {
                return;
            }
        ...
        private void RemoveRButton_Click(object sender, EventArgs e) {
            ...
            if(MessageBox.Show("Are you sure you want to remove the selected Respawns?", "Remove Respawns?",
                   MessageBoxButtons.YesNo) != DialogResult.Yes) {
                return;
            }
        ...
        private void DelayTextBox_TextChanged(object sender, EventArgs e) {
            ...
            for (int i = 0; i < _selectedRespawnInfos.Count; i++) {
                if(chkRespawnEnableTick.Checked) {
                    _selectedRespawnInfos[i].RespawnTicks =
                        Math.Max((ushort)1,
                            temp); //you can never have respawnticks set to 0 or it would bug the entire thing really
        ...
        private void RemoveMButton_Click(object sender, EventArgs e) {
            ...
            if(MessageBox.Show("Are you sure you want to remove the selected Movements?", "Remove Movements?",
                   MessageBoxButtons.YesNo) != DialogResult.Yes) {
                return;
            }
        ...
        private void MZDeletebutton_Click(object sender, EventArgs e) {
            ...
            if(MessageBox.Show("Are you sure you want to remove the selected MineZones?", "Remove MineZones?",
                   MessageBoxButtons.YesNo) != DialogResult.Yes) {
                return;
            }
        ...
        private String PrintMapAttributes(MapInfo map) {
            ...
            if(map.Fire) {
                textOut += " FIRE(" + map.FireDamage + ")";
            }

            if(map.Lightning) {
                textOut += " LIGHTNING(" + map.LightningDamage + ")";
            }
        ...
        private void ImportMonGenButton_Click(object sender, EventArgs e) {
            ...
            for (int i = 0; i < ConvertMonGenInfo.monGenList.Count; i++) {
                try {
                    int monsterIndex = Envir.MonsterInfoList.Find(a => a.Name == ConvertMonGenInfo.monGenList[i].Name)
                        .Index;
                    if(monsterIndex == -1) {
                        continue;
                    }

                    RespawnInfo respawnInfo = new() {
        ...
        private void ConquestComboBox_SelectedIndexChanged(object sender, EventArgs e) {
            ...
            for (int i = 0; i < _selectedMovementInfos.Count; i++) {
                ...
                if(cmb.SelectedIndex >= 0) {
                    ConquestInfo info =
                        Envir.ConquestInfoList.FirstOrDefault(x => x.Name == ConquestComboBox.SelectedItem.ToString());

                    if(info != null) {
                        conquestIndex = info.Index;
                    }

                    MovementInfo thisMovement = _info.Movements.FirstOrDefault(x =>
                        x.MapIndex == _selectedMovementInfos[i].MapIndex &&
