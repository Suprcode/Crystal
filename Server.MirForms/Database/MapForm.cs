...
namespace Server.Database {
    public static class ConvertMapInfo {
        ...
        public static void Start(Envir envirToUpdate) {
            ...
            for (int i = 0; i < lines.Length; i++) {
                ...
                {
                    lines[i] = Regex.Replace(lines[i], @"\s+", " "); // Clear white-space
                    lines[i] = lines[i].Replace(" ;", ";"); // Remove space before semi-colon

                    // Trim comment at the end of the line
                    if(lines[i].Contains(';')) {
                        lines[i] = lines[i].Substring(0, lines[i].IndexOf(";", StringComparison.Ordinal));
                    }

                    MapInfo newMapInfo = new() { Index = ++EditEnvir.MapIndex };

                    string[] a = lines[i].Split(']'); // Split map info into [0] = MapFile MapName 0 || [1] = Attributes
                    string[] b = a[0].Split(' ');

                    newMapInfo.FileName = b[0].TrimStart('['); // Assign MapFile from variable and trim leading '[' char
                    newMapInfo.Title =
                        b[1].Replace("*", " "); // Assign MapName from variable, replacing asterisk with space

                    List<string> mapAttributes = new(); // Group of all attributes associated with that map
                    ...
                    if(newMapInfo.Fire) {
                        ...
                        newMapInfo.FireDamage =
                            Convert.ToInt16(mapAttributes[index].TrimStart("FIRE(".ToCharArray()).TrimEnd(')'));
                    }
                    ...
                    if(newMapInfo.Lightning) {
                        ...
                        newMapInfo.LightningDamage =
                            Convert.ToInt16(mapAttributes[index].TrimStart("LIGHTNING(".ToCharArray()).TrimEnd(')'));
                    }
                    ...
                    if(newMapInfo.NoReconnect) {
                        ...
                        newMapInfo.NoReconnectMap =
                            mapAttributes[index].TrimStart("NORECONNECT(".ToCharArray()).TrimEnd(')');
                    }

                    if(mapAttributes.Any(x => x.StartsWith("MINIMAP(".ToUpper()))) {
                        int index = mapAttributes.FindIndex(x => x.StartsWith("MINIMAP(".ToUpper()));
                        newMapInfo.MiniMap =
                            Convert.ToUInt16(mapAttributes[index].TrimStart("MINIMAP(".ToCharArray()).TrimEnd(')'));
                    }

                    if(mapAttributes.Any(x => x.StartsWith("BIGMAP(".ToUpper()))) {
                        int index = mapAttributes.FindIndex(x => x.StartsWith("BIGMAP(".ToUpper()));
                        newMapInfo.BigMap =
                            Convert.ToUInt16(mapAttributes[index].TrimStart("BIGMAP(".ToCharArray()).TrimEnd(')'));
                    }

                    if(mapAttributes.Any(s => s.Contains("MINE(".ToUpper()))) {
                        int index = mapAttributes.FindIndex(x => x.StartsWith("MINE(".ToUpper()));
                        newMapInfo.MineIndex =
                            Convert.ToByte(mapAttributes[index].TrimStart("MINE(".ToCharArray()).TrimEnd(')'));
                    }

                    if(mapAttributes.Any(s => s.Contains("MAPLIGHT(".ToUpper()))) {
                        int index = mapAttributes.FindIndex(x => x.StartsWith("MAPLIGHT(".ToUpper()));
                        newMapInfo.MapDarkLight =
                            Convert.ToByte(mapAttributes[index].TrimStart("MAPLIGHT(".ToCharArray()).TrimEnd(')'));
                    }

                    if(mapAttributes.Any(s => s.Contains("MUSIC(".ToUpper()))) {
                        int index = mapAttributes.FindIndex(x => x.StartsWith("MUSIC(".ToUpper()));
                        newMapInfo.Music =
                            Convert.ToUInt16(mapAttributes[index].TrimStart("MUSIC(".ToCharArray()).TrimEnd(')'));
                    }

                    if(mapAttributes.Any(x =>
                           x.StartsWith("LIGHT(".ToUpper()))) // Check if there is a LIGHT attribute and get its value
                    {
                    ...
                    try {
                        if(lines[k].StartsWith(EditEnvir.MapInfoList[j].FileName + " ")) {
                            ...
                            if(lines[k].Contains("NEEDCONQUEST")) {
                                ...
                                int conqIndex =
                                    int.Parse(conq.Replace("NEEDCONQUEST(", "").Replace(")", "")); //get value
                            ...
                            lines[k] = Regex.Replace(lines[k], @"\s+", " "); // Clear whitespace
                            lines[k] = lines[k].Replace(" ;", ";"); // Remove space before semi-colon

                            // Trim comment at the end of the line
                            if(lines[k].Contains(';')) {
                                lines[k] = lines[k].Substring(0, lines[k].IndexOf(";", StringComparison.Ordinal));
                            }

                            string[] c = lines[k].Split(' ');
                            ...
                            int toMapIndex =
                                EditEnvir.MapInfoList.FindIndex(a =>
                                    a.FileName == c[3]); //check existing maps for the connection info
                            int toMap = -1;
                ...
                for (int k = 0; k < lines.Length; k++) {
                    ...
                    string[]
                        line = lines[k].Replace(")", "")
                            .Split(','); // STARTZONE(0,150,150,50) -> STARTZONE(0 || 150 || 150 || 50
                    string[] head = line[0].Split('('); // STARTZONE(0 -> STARTZONE || 0
                    try {
                        if(head[1] == EditEnvir.MapInfoList[j].FileName) {
                            SafeZoneInfo newSafeZone = new() {
                            ...
                            if(!EditEnvir.MapInfoList[j].SafeZones.Exists(sz =>
                                   sz.StartPoint == newSafeZone.StartPoint && sz.Location.X == newSafeZone.Location.X &&
                                   sz.Location.Y == newSafeZone.Location.Y)) {
    ...
    public static class ConvertNpcInfo {
        public static List<NpcInfo> NpcInfoList = new();
        ...
        public static void Start() {
            ...
            string[] NpcList = File.ReadAllLines(Path);
            ...
            for (int i = 0; i < NpcList.Length; i++) {
                ...
                string[] Line = Regex.Replace(NpcList[i], @"\s+", " ").Split(' ');
    ...
    public static class ConvertMonGenInfo {
        public static List<MonGenInfo> monGenList = new();
        ...
        public static void Start() {
            ...
            for (int i = 0; i < ofd.FileNames.Length; i++) {
                string[] MonGen = File.ReadAllLines(ofd.FileNames[i]);

                for (int j = 0; j < MonGen.Length; j++) {
                    if(MonGen[j].Contains(';')) {
                        MonGen[j] = MonGen[j].Substring(0, MonGen[j].IndexOf(";", StringComparison.Ordinal));
                    }

                    string[] Line = Regex.Replace(MonGen[j], @"\s+", " ").Split(',');
